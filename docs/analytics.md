...
This document is the source of truth for the analytics events AICortex ships
...
See [ACX-1122](https://github.com/aicortex/aicortex) for the design context.
...
Self-hosters should **never inherit a AICortex-issued `POSTHOG_API_KEY`** —
...
- **Person properties (`$set`)** — use for mutable cohort signals
  ...
  `setPersonProperties()` in `@aicortex/core/analytics`. Use
...
| `signup_source` | string | Opaque attribution bundle from the frontend cookie `aicortex_signup_source` (UTM + referrer). Empty when the cookie is absent. |
...
| `cli_version` | string | Version of the `aicortex` CLI that registered it. |
...
- Attribution is NOT a separate event; UTM + referrer origin are written
  to the `aicortex_signup_source` cookie on the first anonymous pageview
