...
This doc explains the failure mode that caused [ACX-963][acx-963] and the
matrix the daemon now follows when writing Codex's per-task `config.toml`.

[acx-963]: https://aicortex-api.copilothub.ai/issues/28c34ad2-102a-4f46-91ac-336ed78c5859
...
If you see `no such host` *inside a Codex session on macOS* but `curl https://aicortex-api.copilothub.ai` from a plain shell on the same machine works, you are hitting the Seatbelt bug below.
...
The daemon writes a *aicortex-managed* block into each task's
`$CODEX_HOME/config.toml`, delimited by `# BEGIN aicortex-managed` /
`# END aicortex-managed` markers. Anything outside the markers is left
...
`[permissions.aicortex]`, a bare `sandbox_mode = "..."` line would be parsed
as `permissions.aicortex.sandbox_mode` and Codex would silently ignore it.
...
```bash
# Is the AICortex API reachable at all?
curl -sSf https://aicortex-api.copilothub.ai/healthz
```

From inside a Codex session (after the daemon writes its config):

```bash
aicortex issue list --limit 1 --output json >/dev/null && echo OK
```
...
- **A. Domain-scoped `permissions` profile** (tight): when the upstream
  `network_access` fix is available, prefer writing a `permissions.aicortex`
  profile that allows only `aicortex-api.copilothub.ai` and
  `aicortex-static.copilothub.ai`. Keeps filesystem sandbox intact.
...
sed -n '/# BEGIN aicortex-managed/,/# END aicortex-managed/p' \
  ~/aicortex_workspaces/$WORKSPACE_ID/$TASK_SHORT/codex-home/config.toml
```
