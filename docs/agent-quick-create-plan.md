...
**核心 invariant**:AICortex 不教 runtime 怎么用 skill,只把文件摆到 runtime 已经会扫的位置。
...
- skill 内容跟 GitHub 同步,不需要 vendoring 进 aicortex 仓库
...
3. Daemon 构造 prompt:
   ...
   - 加上"调用 aicortex issue create CLI 命令"的指令

4. Agent 跑 LLM + tool calling:
   - LLM 输出形如 `aicortex issue create --title="..." --description="..."` 的命令
   - daemon 执行 CLI 命令,CLI 调 POST /api/issues 创建 issue
   - CLI 自动在请求里带上 AICORTEX_QUICK_CREATE_TASK_ID env(daemon/daemon.go:2081)
...
- **Daemon prompt builder**:`daemon/prompt.go` 加 `buildSkillFindPrompt`(类比 buildQuickCreatePrompt)
  - 喂给 agent 的 prompt 大致:
    ...
    输出:调用 `aicortex skill find --output-results '<JSON>'` 命令,
    JSON 形态为 [{name, description, source_url, reason}, ...]
    ```
- **CLI 命令**(新):`aicortex skill find --output-results <JSON>`
...
| `server/cmd/aicortex/cmd_skill.go` | 加 `find --output-results` 子命令 |
...
- **Daemon prompt builder**:`buildAgentCreatePrompt` 指挥 agent 三步走:
  ```
  1. 调用 `aicortex skill find --output-results ...` 选 skill
     (或直接看 curated 清单选)
  2. 基于选定 skill 写 instructions
  3. 调用 `aicortex agent create --name ... --instructions ... --skill-ids ...`
     创建 agent 并关联 skill
  ```
- **CLI 命令**(新):`aicortex agent create`
  - 后端 handler 已存在(handler/agent.go:CreateAgent),只需要绑 CLI(~50 行)
  - 创建时带 `AICORTEX_AI_DRAFT_TASK_ID` env,服务端用它做 origin 标记 + LinkTaskToAgent
...
| `server/cmd/aicortex/cmd_agent.go` | 加 `create` 子命令(handler 已有) |
...
### 4.3 [SOFT] 缺 `aicortex skill find` CLI

**影响范围**:Phase 2

**方案**:加一个 CLI 子命令,模仿 `aicortex skill import` 的实现(`server/cmd/aicortex/cmd_skill.go:55-60, 323-357`)。**注意**:这个命令不发 HTTP 请求,只是 LLM agent 用来"输出推荐结果"的 channel——它把 LLM 推荐的 JSON 写到 daemon 指定的临时文件,daemon 读完塞进 inbox notification。

**工作量**:小(~80 行)

### 4.4 [SOFT] 缺 `aicortex agent create` CLI

**影响范围**:Phase 3

**方案**:后端 handler 已有(`handler/agent.go:CreateAgent`),只需在 `server/cmd/aicortex/cmd_agent.go` 加 `create` 子命令。
...
AICortex 只有**单体 skill**(SKILL.md + skill_file),没有 plugin / bundle 概念。要接入得新写 plugin parser + 拆分逻辑,工作量大,而 skills.sh 已经覆盖了同一批高质量内容(skills.sh 后端就是 GitHub raw,绝大多数 skill 作者就在 GitHub 上,Anthropic plugin 体系里的 skill 通常也在作者的 GitHub repo 里有单体副本)。
...
- skill 教 agent 调什么?调 `npx skills`(装到本地,目标错)?调 AICortex API(那要写 tool channel,绕一大圈)
...
**判断**(详见之前讨论):
- ClawHub 服务的是 OpenClaw 平台(AICortex 同生态位竞品的内容生态)
...
| AICortex CLI 入口 | `server/cmd/aicortex/main.go:62-79` |
| Skill CLI 命令 | `server/cmd/aicortex/cmd_skill.go:17-96`(已有 import,无 find) |
| Agent CLI 命令 | `server/cmd/aicortex/cmd_agent.go:101-112`(已有 list/get,无 create) |
