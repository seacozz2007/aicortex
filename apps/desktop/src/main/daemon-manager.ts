...
const PREFS_PATH = join(homedir(), ".aicortex", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/aicortex/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".aicortex", "profiles", profile)
    : join(homedir(), ".aicortex");
}
...
// Sidecar file that records which AICortex user the cached PAT in config.json
...
//   ~/.aicortex/profiles/desktop-<host>/
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["aicortex.exe"] : ["aicortex"];
 ...
 *   to `apps/desktop/resources/bin/aicortex`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<AICortex.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "aicortex.exe" : "aicortex";
...
/**
 * Returns a usable `aicortex` binary path. Priority:
 ...
 *   5. `aicortex` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "AICortex Desktop" }),
  });
  if (!res.ok) {
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".aicortex");
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, AICORTEX_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "aicortex CLI is not installed" };
