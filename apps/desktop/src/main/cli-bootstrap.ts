...
// Desktop prefers the bundled `aicortex` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/aicortex/aicortex/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "aicortex.exe" : "aicortex";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `aicortex-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `aicortex` binary. If one is already present at
