...
// aicortex CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // aicortex://auth/callback?token=<jwt>
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      plugins: true,
      additionalArguments: [`--aicortex-locale=${systemLocale}`],
    },
  });
...
// single-worktree case still lands at "AICortex Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `AICortex Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "AICortex Canary";
...
if (is.dev) {
  ...
  // to "AICortex", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.aicortex.desktop.dev" : "ai.aicortex.desktop",
    );
