const RELEASE_ARCHIVE_PREFIX = "aicortex-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `aicortex-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `aicortex_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `aicortex_${os}_${mappedArch}.${ext}`;
