...
import {
  ...
  SidebarTrigger,
  useSidebar,
} from "@aicortex/ui/components/ui/sidebar";
import { ModalRegistry } from "@aicortex/views/modals/registry";
import { AppSidebar } from "@aicortex/views/layout";
...
import { WorkspaceSlugProvider, paths, useCurrentWorkspace } from "@aicortex/core/paths";
import { getCurrentSlug, subscribeToCurrentSlug } from "@aicortex/core/platform";
import { useDesktopUnreadBadge } from "@aicortex/views/platform";
...
function useInternalLinkHandler() {
  useEffect(() => {
    ...
    window.addEventListener("aicortex:navigate", handler);
    return () => window.removeEventListener("aicortex:navigate", handler);
  }, []);
}
  ...
  useEffect(() => {
    return window.desktopAPI.onInboxOpen(({ slug, issueKey }) => {
      ...
      window.dispatchEvent(
        new CustomEvent("aicortex:navigate", { detail: { path: inboxPath } }),
      );
    });
  }, []);
