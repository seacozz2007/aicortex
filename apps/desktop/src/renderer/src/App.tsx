...
import { CoreProvider } from "@aicortex/core/platform";
import { pickLocale } from "@aicortex/core/i18n";
import { useAuthStore } from "@aicortex/core/auth";
import { workspaceKeys, workspaceListOptions } from "@aicortex/core/workspace/queries";
import { api } from "@aicortex/core/api";
import { useHasOnboarded } from "@aicortex/core/paths";
import { ThemeProvider } from "@aicortex/ui/components/common/theme-provider";
import { AICortexIcon } from "@aicortex/ui/components/common/aicortex-icon";
import { Toaster } from "@aicortex/ui/components/ui/sonner";
...
import { RESOURCES } from "@aicortex/views/locales";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.aicortex config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (aicortex://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (aicortex://auth/callback?token=...).
  ...
  useEffect(() => {
    if (!user) return;
    const token = localStorage.getItem("aicortex_token");
    if (!token) return;
  ...
  if (isLoading || bootstrapping) {
    return (
      <div className="flex h-screen items-center justify-center">
        <AICortexIcon className="size-6 animate-pulse" />
      </div>
    );
  }
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          AICortex Desktop could not load <code>~/.aicortex/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
