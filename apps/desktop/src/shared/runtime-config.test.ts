...
describe("runtime config", () => {
  it("uses cloud defaults without a desktop.json file", () => {
    expect(DEFAULT_RUNTIME_CONFIG).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.aicortex.ai",
      wsUrl: "wss://api.aicortex.ai/ws",
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.aicortex.ai" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.aicortex.ai",
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/aicortex" })),
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    // public web host, not the api host. AICortex's convention exposes the
    // api at `api.<web-host>`, so stripping the leading label gives the
    // right web origin without a separate VITE_APP_URL.
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.aicortex.ai" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.aicortex.ai",
      wsUrl: "wss://api.test.aicortex.ai/ws",
      appUrl: "https://test.aicortex.ai",
    });
  });
  ...
  it("dev VITE_APP_URL still wins over apiUrl-derived value", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "https://api.test.aicortex.ai",
        appUrl: "https://staging.aicortex.ai",
      }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.aicortex.ai",
