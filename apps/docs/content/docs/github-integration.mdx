...
| **Webhook (background)** | On every `pull_request` event, AICortex upserts the PR row, scans the PR for issue identifiers, and (re)builds the link rows. Idempotent — replaying a delivery is a no-op. |
...
- **PRs in repos the App isn't installed on** — without the App, AICortex never receives the webhook.
- **Manually linking a PR to an issue** — there is no UI for this yet. If your team's convention puts identifiers in a place AICortex isn't reading, add them to the PR title or body.

## Disconnecting

In **Settings → Integrations** there is no installation list — you manage existing installations from GitHub directly:

- **From GitHub** — uninstall the AICortex GitHub App at `https://github.com/settings/installations` (personal) or `https://github.com/organizations/<org>/settings/installations` (org). AICortex receives the `installation.deleted` webhook and drops the row in real time; any open Settings tab updates without a refresh.
- **Disconnect from inside AICortex is admin-only** — the Settings card is hidden for non-admins.
...
- The GitHub App requests **read-only** access to pull requests and metadata. AICortex never pushes commits, comments, or status checks back to GitHub.

## Self-host setup

If you're running AICortex on AICortex Cloud, the integration is already configured — skip this section.
...
| Field | Value |
|---|---|
| **GitHub App name** | Anything recognizable, e.g. `AICortex` or `AICortex (staging)`. |
| **Homepage URL** | Your AICortex frontend, e.g. `https://aicortex.example.com`. |
| **Callback URL** | Leave blank — AICortex doesn't use OAuth user identity. |
| **Setup URL** | `https://<api-host>/api/github/setup`. **Check "Redirect on update"**. |
| **Webhook → Active** | Enabled. |
| **Webhook URL** | `https://<api-host>/api/webhooks/github`. |
| **Webhook secret** | Generate a long random string (e.g. `openssl rand -hex 32`). You'll paste the same value into AICortex's env in step 2. |
...
- The `/api/webhooks/github` endpoint returns **`503 github webhooks not configured`** — AICortex refuses to process events with no secret, rather than silently treating every signature as valid.
...
- **No CI / check state** — only the PR itself is mirrored. Build status, review comments, and reviewers are not surfaced in AICortex.
