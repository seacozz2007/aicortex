...
PR **关闭但没合并**——只更新 PR 卡片的状态为 `Closed`，issue 状态不变。"关闭但不合并"语义因团队而异，AICortex 不替用户做决定。
...
- **App 没安装的仓库里的 PR**——没 App，AICortex 收不到 webhook
- **手动把 PR 关联到 issue**——暂时没有这个 UI。如果你们的约定把编号放到 AICortex 不扫的地方，请改放到 PR 标题或正文里

## 断开连接

**Settings → Integrations** 里没有 installation 列表——现有 installation 直接到 GitHub 上管理：

- **从 GitHub 卸载** —— 个人在 `https://github.com/settings/installations`、组织在 `https://github.com/organizations/<org>/settings/installations` 卸载 AICortex App。AICortex 收到 `installation.deleted` webhook 后立刻删行；任何已打开的 Settings tab 实时更新，不用刷新
- **AICortex 这边的断开是 admin only** —— 卡片对非 admin 不显示连接操作
...
- GitHub App 申请的是 PR 和 Metadata 的 **只读** 权限。AICortex 从不向 GitHub 推 commit、评论或 status check

## Self-Host 配置

如果你在 AICortex Cloud 上，集成已经配好——跳过本节。
...
| 字段 | 值 |
|---|---|
| **GitHub App name** | 任何能辨识的名字，例如 `AICortex` 或 `AICortex (staging)` |
| **Homepage URL** | 你的 AICortex 前端，例如 `https://aicortex.example.com` |
...
- 顶部 **public link** 的尾部即 slug。`https://github.com/apps/aicortex-acme` → slug = `aicortex-acme`
- 你刚生成的 **webhook secret**（GitHub 之后不会再让你读取这个值——现在就保存好）

<Callout type="warning">
**Webhook secret ≠ Client secret。** App 设置页里两个字段紧挨着。**Webhook secret** 用于签 `pull_request` payload，这才是 AICortex 需要的那个；**Client secret** 是 OAuth 用的，和本集成无关。混淆这两个会得到「每条 webhook 都 `401 invalid signature`」的诡异症状。
</Callout>
...
- `/api/webhooks/github` 直接返回 **`503 github webhooks not configured`**——AICortex 在 secret 没配置时拒绝处理事件，不会出现「没 secret 也接受 webhook」的安全坑
...
- **不读 CI / check 状态**——只镜像 PR 本身，构建状态、reviewer 评论、reviewer 列表都没接进 AICortex
