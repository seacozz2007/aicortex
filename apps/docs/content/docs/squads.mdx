...
```bash
aicortex squad create --name "Frontend Team" --leader frontend-lead-agent
aicortex squad member add <squad-id> --member-id <agent-or-user-uuid> --type agent --role "Owns Tailwind / shadcn surface"
```

## How a squad-assigned issue runs

When a non-Backlog issue is assigned to a squad, AICortex immediately enqueues a `task` for the **leader agent** (not for every member). The flow then looks like this:

1. **Leader claims the task.** The agent runtime picks up the task on its next poll, same as any other agent assignment.
2. **Leader is briefed.** On claim, AICortex appends three sections to the leader's system prompt — see [What the leader sees on every turn](#what-the-leader-sees-on-every-turn) below.
...
4. **Leader records its evaluation** via `aicortex squad activity <issue-id> action --reason "..."`. This writes an entry to the issue's activity timeline so humans can see the leader actually evaluated the trigger.
...
- **Squad Instructions** — your custom guidance for this squad (set on the squad detail page or via `aicortex squad update --instructions`). Use this for routing rules ("send DB work to Alice, frontend to Bob"), escalation policies, or anything else the leader needs to know that isn't already in the issue.
...
**Archiving a squad** (`aicortex squad delete <id>`, or the Archive button on the detail page):
...
| `aicortex squad get <id>` | Show one squad's name, leader, description, instructions |
| `aicortex squad create --name "..." --leader <agent>` | Create a squad (owner / admin) |
| `aicortex squad update <id> [--name X] [--description X] [--instructions X] [--leader Y] [--avatar-url Z]` | Update one or more fields |
| `aicortex squad delete <id>` | Archive (soft-delete) — transfers assigned issues to the leader |
| `aicortex squad member list <id>` | List a squad's members |
| `aicortex squad member add <id> --member-id <uuid> --type agent\|member [--role "..."]` | Add a member (owner / admin) |
| `aicortex squad member remove <id> --member-id <uuid> --type agent\|member` | Remove a member (the leader cannot be removed — change leader first) |
| `aicortex squad activity <issue-id> <action\|no_action\|failed> --reason "..."` | Recorded by the leader agent at the end of every turn |

`--leader` accepts an agent name or UUID; for everything else, IDs come from `aicortex agent list --output json`, `aicortex workspace members --output json`, and `aicortex squad list --output json`.
