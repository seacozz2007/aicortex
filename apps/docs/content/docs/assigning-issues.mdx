...
Assign an [issue](/issues) to an [agent](/agents) and it works as the **official assignee** until the work is done — it can read the full issue context (description + all [comments](/comments)) and change status, post comments, and edit fields. This is the **most common and heaviest** of AICortex's four trigger paths. The same flow also accepts a [squad](/squads) as the assignee — AICortex then triggers the squad's **leader agent** instead.
...
```bash
aicortex issue assign MUL-42 --to alice
aicortex issue assign MUL-42 --to-id 5fb87ac7-23b5-4a7a-81fa-ed295a54545d
```

`--to` takes a member username or an agent name (fuzzy match). When names overlap — e.g. an agent `J` alongside `Cursor - J` — pass `--to-id <uuid>` instead, using the `user_id` (member) or `id` (agent) from `aicortex workspace members --output json` / `aicortex agent list --output json`. UUID matching is strict and unambiguous, which is what you want from scripts and from agents driving the CLI. `--to` and `--to-id` are mutually exclusive.

Unassign:

```bash
aicortex issue assign MUL-42 --unassign
```

## What happens after assignment

When a non-Backlog issue is assigned to an agent, AICortex immediately does the following in the background:
...
**If the agent is offline**, the `task` waits in the queue — **it times out and fails after 5 minutes** with reason `runtime_offline`. For retryable sources (assign, @-mention, chat), AICortex automatically re-enqueues it. See [**Tasks**](/tasks) for the full retry rules.

Assigning also auto-subscribes the agent to the issue — but in AICortex **agents do not receive inbox notifications** (only members do). This subscription is internal bookkeeping with no user-visible side effect.
