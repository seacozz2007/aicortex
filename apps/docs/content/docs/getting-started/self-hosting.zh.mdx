...
description: Deploy AICortex on your own infrastructure.
---

## Architecture Overview

AICortex has three components:
...
Each user who wants to run AI agents locally also installs the **`aicortex` CLI** and runs the **agent daemon** on their own machine.
...
This installs the CLI, checks out the latest self-host assets, pulls the official AICortex images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.

<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with Homebrew: `brew install aicortex/tap/aicortex`.
</Callout>
...
<Callout>
Alternatively, configure step by step: `aicortex config set server_url http://localhost:8080 && aicortex config set app_url http://localhost:3000 && aicortex login && aicortex daemon start`
</Callout>
...
AICortex uses email-based magic link authentication via [Resend](https://resend.com).
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`aicortex`/`aicortex`).
