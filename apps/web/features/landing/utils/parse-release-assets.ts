 ...
 *   aicortex-desktop-{version}-mac-{arch}.{dmg|zip}
 *   aicortex-desktop-{version}-windows-{arch}.exe
 *   aicortex-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^aicortex-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
