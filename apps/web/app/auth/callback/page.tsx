...
import { sanitizeNextUrl, useAuthStore } from "@aicortex/core/auth";
import { workspaceKeys } from "@aicortex/core/workspace/queries";
import { paths, resolvePostAuthDestination } from "@aicortex/core/paths";
...
import {
  ...
  CardDescription,
  CardContent,
} from "@aicortex/ui/components/ui/card";
    ...
    if (isDesktop) {
      ...
      api
        ...
        .then(({ token }) => {
          ...
          window.location.href = `aicortex://auth/callback?token=${encodeURIComponent(token)}`;
        })
        .catch((err) => {
      ...
      loginWithGoogle(code, redirectUri)
        .then(async (loggedInUser) => {
          ...
          //    login on app.aicortex.ai instead of clicking the email link,
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Opening AICortex</CardTitle>
            <CardDescription>
              You should see a prompt to open the AICortex desktop app. If
          ...
          <CardContent className="flex justify-center">
            <Button
              variant="outline"
              onClick={() => {
                window.location.href = `aicortex://auth/callback?token=${encodeURIComponent(desktopToken)}`;
              }}
            >
              Open AICortex Desktop
            </Button>
          </CardContent>
        </Card>
      </div>
