...
import enCommon from "@aicortex/views/locales/en/common.json";
import enAuth from "@aicortex/views/locales/en/auth.json";
import enSettings from "@aicortex/views/locales/en/settings.json";
...
vi.mock("@aicortex/core/auth", async () => {
  const actual =
    await vi.importActual<typeof import("@aicortex/core/auth")>(
      "@aicortex/core/auth",
    );
...
describe("LoginPage", () => {
  ...
  it("renders login form with email input and continue button", () => {
    ...
    expect(screen.getByText("Sign in to AICortex")).toBeInTheDocument();
  ...
  it("calls sendCode with email on submit", async () => {
    ...
    await user.type(screen.getByLabelText("Email"), "test@aicortex.ai");
    await user.click(screen.getByRole("button", { name: "Continue" }));

    await waitFor(() => {
      expect(mockSendCode).toHaveBeenCalledWith("test@aicortex.ai");
    });
  });
  ...
  // the aicortex:// deep link, not silently redirect to the workspace page.
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@aicortex.ai" };
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "aicortex://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open AICortex Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
