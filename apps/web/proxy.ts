...
import { matchLocale, LOCALE_COOKIE } from "@aicortex/core/i18n";
...
// Forward the resolved locale to RSC layouts via the `x-aicortex-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set("x-aicortex-locale", resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("aicortex_logged_in");
