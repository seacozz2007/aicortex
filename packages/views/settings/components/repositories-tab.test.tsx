...
const workspaceRef = vi.hoisted(() => ({
  current: {
    ...
    name: "Test Workspace",
    slug: "test-workspace",
    repos: [{ url: "https://github.com/aicortex/aicortex" }] as { url: string }[],
  },
}));
...
describe("RepositoriesTab — view/edit toggle", () => {
  beforeEach(() => {
    ...
    workspaceRef.current = {
      ...
      name: "Test Workspace",
      slug: "test-workspace",
      repos: [{ url: "https://github.com/aicortex/aicortex" }],
    };
  ...
  it("renders persisted repos in display mode (no input)", () => {
    ...
    expect(screen.getByText("https://github.com/aicortex/aicortex")).toBeTruthy();
  });
  ...
  it("clicking Edit reveals an input pre-filled with the URL", async () => {
    ...
    expect(input.value).toBe("https://github.com/aicortex/aicortex");
  });
  ...
  it("Save re-enables after editing, then returns to display mode + disabled on success", async () => {
    ...
    await user.type(input, "https://github.com/aicortex/edited");
  ...
  it("Cancel on a dirty edited row reverts the URL and exits edit mode", async () => {
    ...
    const input = screen.getByRole("textbox") as HTMLInputElement;
    await user.clear(input);
    await user.type(input, "https://github.com/aicortex/changed");
    expect(screen.getByRole("button", { name: /^Save$/ })).not.toBeDisabled();
  ...
  it("accepts scp-like shorthand without browser URL validation blocking submit", async () => {
    ...
    const input = screen.getByRole("textbox") as HTMLInputElement;
    await user.clear(input);
    await user.type(input, "git@github.com:aicortex/aicortex.git");
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "git@github.com:aicortex/aicortex.git" }],
      });
    });
  });
