...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.aicortex/${p}`),
  mockMembers: {
    current: [] as Array<{
      id: string;
      workspace_id: string;
...
vi.mock("@aicortex/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.aicortex/${p}`);
    mockMembers.current = [];
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.aicortex//ws-test/issues/issue-1");
