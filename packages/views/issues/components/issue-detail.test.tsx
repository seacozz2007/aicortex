...
import type { Issue, TimelineEntry } from "@aicortex/core/types";
import { I18nProvider } from "@aicortex/core/i18n/react";
...
vi.mock("@aicortex/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @aicortex/core/hooks.tsx). vi.mock("@aicortex/core/paths") only intercepts
...
// Mock @aicortex/core/workspace/hooks
vi.mock("@aicortex/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @aicortex/core/paths — after the URL-driven workspace refactor,
...
vi.mock("@aicortex/core/paths", async () => {
  const actual = await vi.importActual<typeof import("@aicortex/core/paths")>(
    "@aicortex/core/paths",
  );
  return {
    ...actual,
...
vi.mock("../../navigation", () => ({
  ...
  useNavigation: () => ({
    ...
    getShareableUrl: (p: string) => `https://app.aicortex.com${p}`,
  }),
...
// Mock issue config
vi.mock("@aicortex/core/issues/config", () => ({
...
vi.mock("@aicortex/core/issues/stores", () => ({
...
// Mock react-resizable-panels (used by @aicortex/ui/components/ui/resizable)
