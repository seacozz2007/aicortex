...
import { ApiError } from "@aicortex/core/api";
...
vi.mock("@aicortex/ui/components/ui/sidebar", () => ({
...
vi.mock("@aicortex/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@aicortex/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));

vi.mock("@aicortex/core/auth", () => ({
...
vi.mock("@aicortex/core/api", async (importOriginal) => ({ ...(await importOriginal<typeof import("@aicortex/core/api")>()), api: {} }));
vi.mock("@aicortex/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@aicortex/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@aicortex/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@aicortex/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@aicortex/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@aicortex/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@aicortex/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@aicortex/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@aicortex/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
