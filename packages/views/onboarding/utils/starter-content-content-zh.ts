...
export function buildWelcomeIssueText(
  ...
  const header = [
    `欢迎来到 AICortex！👋`,
    ``,
    `这是你工作区里的第一个 issue。下面你的智能体马上会回复——这就是 AICortex 里工作的方式：你写下你想做的事，智能体（或同事）接手并在评论里回复。`,
    ``,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        ...
        description: `${header}你好智能体，这是 ${name} 第一次用 AICortex。${name} 主要会让你做 **编码相关的工作**。\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 AICortex。${name} 希望你帮忙做 **规划与拆解工作**。\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 AICortex。${name} 会让你做 **调研和写作** —— 起草、摘要、分析。\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 AICortex。${name} 还在 **探索** AICortex 能做什么 —— 暂时没有具体目标。\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 AICortex。${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 AICortex。\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**每个 issue 右侧都有一个 Properties 面板**。从这里控制谁来做什么。AICortex 里的智能体被触发的条件是 issue 同时满足：`,
        ``,
        ...
        ``,
        `[关于把 issue 分配给智能体 →](https://aicortex.ai/docs/zh/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[关于创建智能体 →](https://aicortex.ai/docs/zh/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[关于自动化 →](https://aicortex.ai/docs/zh/autopilots)`,
      ].join("\n"),
    },
  ];
