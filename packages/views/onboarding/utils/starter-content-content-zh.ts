import type { QuestionnaireAnswers } from "@aicortex/core/onboarding";
import type { ImportStarterIssuePayload } from "@aicortex/core/api";
...
export const PROJECT = {
  title: "上手指南",
  description: "几件可以在 AICortex 里上手试一试的事，按你的节奏走。",
};
...
export function buildWelcomeIssueText(
  ...
  const header = [
    `欢迎来到 AICortex！👋`,
    ``,
    `这是你工作区里的第一个 issue。下面你的Agent马上会回复——这就是 AICortex 里工作的方式：你写下你想做的事，Agent（或同事）接手并在评论里回复。`,
    ``,
    `[了解 AICortex 是怎么运转的 →](https://aicortex.ai/docs/zh/how-aicortex-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **说明我们在 AICortex 里怎么协作**：`,
    `   - 把 issue 分配给你 **并** 把状态置为 **Todo** 才会触发你开工（Backlog 状态会让你暂停）`,
    `   - 在评论里 @你 适合丢一个快速问题`,
    `   - **Workspace Context**（在 Settings → General）是这个工作区里每个Agent都会读到的共享背景`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好Agent，这是 ${name} 第一次用 AICortex。${name} 主要会让你做 **编码相关的工作**。\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好Agent，这是 ${name} 第一次用 AICortex。${name} 希望你帮忙做 **规划与拆解工作**。\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好Agent，这是 ${name} 第一次用 AICortex。${name} 会让你做 **调研和写作** —— 起草、摘要、分析。\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好Agent，这是 ${name} 第一次用 AICortex。${name} 还在 **探索** AICortex 能做什么 —— 暂时没有具体目标。\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好Agent，这是 ${name} 第一次用 AICortex。${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 欢迎来到 AICortex —— 一起开工",
        description: `${header}你好Agent，这是 ${name} 第一次用 AICortex。\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      priority: "high",
      assign_to_self: true,
      title: "学会怎么在任意 issue 上触发你的Agent",
      description: [
        `**每个 issue 右侧都有一个 Properties 面板**。从这里控制谁来做什么。AICortex 里的Agent被触发的条件是 issue 同时满足：`,
        ``,
        `  Assignee = 你的Agent  AND  Status = Todo（不是 Backlog）`,
        ``,
        `**现在就试一下**：`,
        `1. 在侧边栏顶部点 **New Issue**（或按 \`C\`）`,
        `2. 标题写成类似"试运行：用 3 条 bullet 总结我们的产品"`,
        `3. 在右侧面板找到 **Assignee** → 点击 → 选你的Agent`,
        `4. 找到 **Status** → 点击 → 选 **Todo**`,
        `5. 滚动到 Activity —— Agent一开工就会出现一张 **Live card**`,
        ``,
        `**⚠️ 容易踩**：新建的 issue 默认是 Backlog 状态，Agent在 Backlog 是暂停的。第一次会弹一个提示——意思就是"翻到 Todo 才会开工"。`,
        ``,
        `**怎么算成功**：Live card 里出现Agent在思考的状态，Status 自动翻到 **In Progress**。`,
        ``,
        `[关于把 issue 分配给Agent →](https://aicortex.ai/docs/zh/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      priority: "high",
      assign_to_self: true,
      title: "和Agent聊天 —— 不需要建 issue",
      description: [
        `不是每个问题都值得开一个 issue。要快速来回对话，用 **Chat 面板**。`,
        ``,
        `**在哪**：看屏幕 **右下角** —— 有一个圆形按钮，上面是一个 **💬 对话气泡**。Agent在工作时按钮会脉动；有未读回复时会有红色小角标。`,
        ``,
        `**现在就试一下**：`,
        `1. 点 💬 按钮 → 一个面板从右侧滑入`,
        `2. 在 **输入框左下角** 点Agent头像 → 从下拉里选你的Agent`,
        `3. 输入一个简短问题："这个工作区里你能帮我做什么？"`,
        `4. 按 **Enter**`,
        ``,
        `**附赠技巧 —— 在评论里 @Agent**：在任何 issue 底部的评论框里输入 \`@\`，会弹出一个下拉，列出成员、Agent和其他 issue。选一个Agent → 写下问题 → 发送。被 @ 的Agent会在评论里回复。`,
        ``,
        `**怎么算成功**：Agent在几秒内通过 chat 面板（或评论里）回复。`,
        ``,
        `[关于聊天 →](https://aicortex.ai/docs/zh/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        `**Workspace Context** 是一段共享系统提示，这个工作区里每个Agent在执行任何 task 之前都会读它。这是让Agent回复更精准、最有杠杆的一件事。`,
        ``,
        `**在哪**：`,
        ...
        `点 **Save**。`,
        ``,
        `**怎么算成功**：你下次分给Agent一个 task，它会自动用上 context 里的信息，不需要你再解释一遍。`,
        ``,
        `[关于工作区 →](https://aicortex.ai/docs/zh/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        ...
        ``,
        `[关于成员与角色 →](https://aicortex.ai/docs/zh/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  if (q.role === "developer" || q.use_case === "coding") {
    tier2.push({
      ...
      title: "接入一个 Git 仓库",
      description: [
        `接入后，被分配 task 的Agent可以 clone、读取、提交对你仓库的修改。`,
        ``,
        `**在哪**：`,
        `1. 侧边栏 → **Settings**（⚙️）`,
  ...
  tier2.push({
    ...
    priority: "medium",
    assign_to_self: true,
    title: "再创建一个不同分工的Agent",
    description: [
      `跑一个分工明确的小型Agent团队，比一个万能选手更好用。一个写代码、一个做规划、一个写文 —— 各自有各自的指令。`,
      ``,
      `**说明**：没有什么强制把"编码Agent"锁死在编码上。指令本质就是 system prompt，随时可改。分开是为了让每个Agent的回复更聚焦。`,
      ``,
      `**在哪**：`,
      ...
      `4. 点 **Create**`,
      ``,
      `**怎么算成功**：新Agent出现在任意 issue 的 Assignee 下拉里，也出现在 Agents 页的左侧列表。`,
      ``,
      `[关于创建Agent →](https://aicortex.ai/docs/zh/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "打磨Agent的 Instructions",
      description: [
        `创建Agent只是开始。**Instructions tab** 才是塑造它行为的地方。`,
        ``,
        `**在哪**：`,
        `1. 侧边栏 → **Agents**（🤖）`,
        `2. 在左侧列表点你想调整的Agent`,
        ...
        ``,
        `Workspace Context 和Agent Instructions 是叠加的——每个 task 都会同时带上。Instructions 写这个Agent特有的，Context 写整个工作区都适用的。`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `当Agent在某个 issue 上工作时，**Activity** 区顶部会出现一张 **Live card**（滚动时会粘在视口顶部）。`,
        ``,
        `Live card 实时展示：`,
        `- Agent正在调用哪个工具（例如读文件、网页搜索）`,
        `- 流式输出的思考与中间结果`,
        ...
        ``,
        `[关于执行任务 →](https://aicortex.ai/docs/zh/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        `当有人——或者Agent—— @你 或者把 issue 分给你时，事件会落到你的 **Inbox**。`,
        ``,
        `**在哪**：`,
        ...
        ``,
        `[关于收件箱 →](https://aicortex.ai/docs/zh/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        `**Autopilot** 把一段 prompt 变成定时 task。每天/每周/每小时自动建一个 issue 并分给Agent。`,
        ``,
        `**在哪**：`,
        ...
        ``,
        `[关于定时任务 →](https://aicortex.ai/docs/zh/autopilots)`,
      ].join("\n"),
    },
  ];
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**为什么先做这个**：没有运行时 = Agent跑不了任何 task。Tier 1 之下的所有事情都等这个。`,
        ``,
        `**运行时**是守护进程（一个跑在你机器上的小后台进程）和一款 AI 编程工具——Claude Code、Codex 等等——的组合。装了多款工具就会出现多个运行时。运行时是真正执行Agent接到的 task 的那一层。`,
        ``,
        `**方案 A —— 桌面应用（macOS，Mac 推荐）**：`,
        `1. 去 [github.com/aicortex/aicortex/releases/latest](https://github.com/aicortex/aicortex/releases/latest) 下载 macOS 的 \`.dmg\``,
        `2. 安装并打开`,
        ...
        ``,
        `[关于守护进程与运行时 →](https://aicortex.ai/docs/zh/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        `4. 点 **Create**`,
        ``,
        `**说明**：Agent本质上就是 LLM + 指令 + 工作区访问权限。没有什么强制把"编码Agent"锁死在编码上 —— 同一个Agent可以做调研、写作、规划。保持灵活。`,
        ``,
        `**怎么算成功**：新Agent出现在任意 issue 的 Assignee 下拉里。`,
        ``,
        `[关于创建Agent →](https://aicortex.ai/docs/zh/agents-create)`,
      ].join("\n"),
    },
  ...
  const tier2: ImportStarterIssuePayload[] = [
    {
      ...
      priority: "medium",
      assign_to_self: true,
      title: "把第一个真实 task 分给Agent",
      description: [
        `**前置条件**：上面两条 task 都做完，你已经有运行时 + Agent。`,
        ``,
        `**AICortex 怎么触发Agent**：`,
        `- 把 issue 分给Agent`,
        `- 状态置为 **Todo**（不是 Backlog —— Backlog 会让Agent暂停）`,
        `- Agent自动接手`,
        ``,
        `**现在就试一下**：`,
        `1. 在侧边栏顶部点 **New Issue**（或按 \`C\`）`,
        ...
        `5. 看Agent在评论里回复，Activity 里出现 **Live card**`,
        ``,
        `**⚠️ 容易踩**：新 issue 默认是 **Backlog**。必须翻到 **Todo** 才会触发Agent。`,
        ``,
        `[关于把 issue 分配给Agent →](https://aicortex.ai/docs/zh/assigning-issues)`,
      ].join("\n"),
    },
  ...
  const tier3: ImportStarterIssuePayload[] = [
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "和Agent聊天 —— 创建之后再做",
      description: [
        `**前置条件**：你至少创建了一个Agent（Tier 1 #2）。`,
        ``,
        `不是每个问题都值得开一个 issue。要快速来回对话，用 **Chat 面板**。`,
        ``,
        ...
        `**试一下**：`,
        `1. 点 💬 按钮 → 一个面板从右侧滑入`,
        `2. 在输入框左下角，从下拉里选一个Agent`,
        `3. 输入问题 → 按 **Enter**`,
        ``,
        `**附赠技巧**：在任意 issue 的评论框里输入 \`@\` 可以提及Agent或成员。`,
        ``,
        `[关于聊天 →](https://aicortex.ai/docs/zh/chat)`,
      ].join("\n"),
    },
  ...
  tier3.push(
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "塑造Agent的 Instructions（创建之后再做）",
      description: [
        `**前置条件**：你至少有一个Agent。`,
        ``,
        `创建Agent只是开始。**Instructions tab** 才是塑造它行为的地方。`,
    ...
    },
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "实时观看Agent工作（分配 task 之后再做）",
      description: [
        `**了解性 task** —— 现在不用做什么，知道有这个东西就行。`,
        ``,
        `当Agent在某个 issue 上工作时，**Activity** 区顶部会出现一张 **Live card**（滚动时会粘在视口顶部）。`,
        ``,
        `Live card 实时展示Agent正在调用哪个工具、流式思考、当前状态。执行结束后，下方的 **Task Run History** 列出每一次运行 —— 点 **View transcript** 可以打开完整时间线。`,
        ``,
        `[关于执行任务 →](https://aicortex.ai/docs/zh/tasks)`,
      ].join("\n"),
    },
    {
      ...
      priority: "low",
      assign_to_self: true,
      title: "搭一个 Autopilot（有了Agent之后再做）",
      description: [
        `**前置条件**：你至少有一个Agent。`,
        ``,
        `**Autopilot** 把一段 prompt 变成定时 task。每天/每周/每小时自动建一个 issue 并分给Agent。`,
