...
import {
  ...
  InputOTPGroup,
  InputOTPSlot,
} from "@aicortex/ui/components/ui/input-otp";
...
import type { User } from "@aicortex/core/types";
    ...
    api
      ...
      .catch(() => {
        ...
        api
          ...
          .catch(() => {
            api.setToken(null);
            localStorage.removeItem("aicortex_token");
          });
      });
  }, [cliCallback]);
    ...
    async (value: string) => {
      ...
      try {
        if (cliCallback) {
          ...
          localStorage.setItem("aicortex_token", token);
          api.setToken(token);
          onTokenObtained?.();
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      if (authSourceRef.current === "localStorage") {
        ...
        const stored = localStorage.getItem("aicortex_token");
