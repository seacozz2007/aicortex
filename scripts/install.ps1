# AICortex installer for Windows — one command to get started.
#
# Install CLI (default): connects to aicortex.ai
#   irm https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.ps1 | iex
#
# Self-host: starts a local AICortex server + installs CLI + configures
#   $env:AICORTEX_MODE="local"; irm https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/aicortex/aicortex.git"
$RepoWebUrl    = "https://github.com/aicortex/aicortex"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".aicortex\server"
$InstallDir    = if ($env:AICORTEX_INSTALL_DIR) { $env:AICORTEX_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/aicortex/aicortex/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:AICORTEX_SELFHOST_REF) {
        return $env:AICORTEX_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = aicortex version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing AICortex CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "AICortex requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/aicortex/aicortex/releases/download/$latest/aicortex-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "aicortex-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "aicortex.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/aicortex/aicortex/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "aicortex.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "aicortex-cli-$version-windows-$arch.zip"
        $legacyAsset = "aicortex_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "aicortex.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".aicortex\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "aicortex.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "aicortex.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "aicortex.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "aicortex.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "AICortex CLI installed to $binDir\aicortex.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "aicortex") {
        ...
        if ($isUpToDate) {
            Write-Ok "AICortex CLI is up to date ($currentVer)"
            return
        }

        Write-Info "AICortex CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "AICortex CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "aicortex")) {
        Write-Fail "CLI installed but 'aicortex' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. AICortex self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:AICORTEX_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up AICortex server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning AICortex repository..."
    ...
    Write-Info "Pulling official AICortex images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting AICortex services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "AICortex server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  AICortex - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] AICortex CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     aicortex setup               " -NoNewline; Write-Host "# Connect to AICortex Cloud (aicortex.ai)" -ForegroundColor DarkGray
    Write-Host "     aicortex setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:AICORTEX_MODE="with-server"; irm https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  AICortex - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] AICortex server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     aicortex setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:AICORTEX_MODE="stop"; irm https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping AICortex services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No AICortex installation found at $InstallDir"
    }

    if (Test-CommandExists "aicortex") {
        try {
            aicortex daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:AICORTEX_MODE) { $env:AICORTEX_MODE.ToLower() } else { "default" }
