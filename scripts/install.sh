#!/usr/bin/env bash
# AICortex installer — installs the CLI and optionally provisions a self-host server.
#
...
#
# After installation, run `aicortex setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/aicortex/aicortex.git"
REPO_WEB_URL="https://github.com/aicortex/aicortex"  # without .git, for GitHub web APIs
INSTALL_DIR="${AICORTEX_INSTALL_DIR:-$HOME/.aicortex/server}"
BREW_PACKAGE="aicortex/tap/aicortex"
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). AICortex supports macOS, Linux, and Windows." ;;
  esac
...
# ---------------------------------------------------------------------------
install_cli_brew() {
  info "Installing AICortex CLI via Homebrew..."
  if ! brew tap aicortex/tap 2>/dev/null; then
  ...
  if ! brew install "$BREW_PACKAGE" 2>/dev/null; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      ok "AICortex CLI already installed via Homebrew"
    else
      fail "Failed to install aicortex via Homebrew."
    fi
  else
    ok "AICortex CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing AICortex CLI from GitHub Releases..."
  ...
  local url="https://github.com/aicortex/aicortex/releases/download/${latest}/aicortex-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/aicortex.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/aicortex.tar.gz" -C "$tmp_dir" aicortex

  # Try /usr/local/bin first, fall back to ~/.local/bin
  local bin_dir="/usr/local/bin"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/aicortex" "$bin_dir/aicortex"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/aicortex" "$bin_dir/aicortex"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/aicortex" "$bin_dir/aicortex"
    chmod +x "$bin_dir/aicortex"
  ...
  rm -rf "$tmp_dir"
  ok "AICortex CLI installed to $bin_dir/aicortex"
}
...
get_selfhost_ref() {
  if [ -n "${AICORTEX_SELFHOST_REF:-}" ]; then
    printf '%s' "$AICORTEX_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading AICortex CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "AICortex CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "AICortex CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists aicortex; then
    local current_ver
    # `aicortex version` outputs "aicortex v0.1.13 (commit: abc1234)" — extract just the version
    current_ver=$(aicortex version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "AICortex CLI is up to date ($current_ver)"
      return 0
    fi

    info "AICortex CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(aicortex version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ok "AICortex CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists aicortex; then
    fail "CLI installed but 'aicortex' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. AICortex self-hosting requires Docker and Docker Compose.
...
# ---------------------------------------------------------------------------
setup_server() {
  info "Setting up AICortex server..."
  local server_ref
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning AICortex repository..."
  ...
  info "Pulling official AICortex images..."
  pull_official_selfhost_images
  info "Starting AICortex services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  AICortex — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ AICortex CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}aicortex setup${RESET}                # Connect to AICortex Cloud (aicortex.ai)\n"
  printf "     ${CYAN}aicortex setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  AICortex — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ AICortex server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}aicortex setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  printf "\n"
  info "Stopping AICortex services..."
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No AICortex installation found at $INSTALL_DIR"
  fi

  if command_exists aicortex; then
    aicortex daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the AICortex CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "After installation, run 'aicortex setup' to configure your environment."
        exit 0
        ;;
