# Graph Report - aicortex  (2026-06-12)

## Corpus Check
- 2517 files · ~4,102,112 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 39264 nodes · 75247 edges · 2246 communities (2133 shown, 113 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 3559 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6dcdb5a7`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_API Error Handling & Client Setup|API Error Handling & Client Setup]]
- [[_COMMUNITY_Virtual Office Presence System|Virtual Office Presence System]]
- [[_COMMUNITY_Desktop App Shell & Window Management|Desktop App Shell & Window Management]]
- [[_COMMUNITY_API Client & CRUD Operations|API Client & CRUD Operations]]
- [[_COMMUNITY_Workspace Presence & Agent Detail|Workspace Presence & Agent Detail]]
- [[_COMMUNITY_Issue Actions & Dropdown Menus|Issue Actions & Dropdown Menus]]
- [[_COMMUNITY_Issue Context Menus & Primitives|Issue Context Menus & Primitives]]
- [[_COMMUNITY_File Upload & Emoji Picker|File Upload & Emoji Picker]]
- [[_COMMUNITY_Visibility & Permission Labels|Visibility & Permission Labels]]
- [[_COMMUNITY_Cross-Workspace Test Fixtures|Cross-Workspace Test Fixtures]]
- [[_COMMUNITY_Agent & Autopilot Creation|Agent & Autopilot Creation]]
- [[_COMMUNITY_Issue URL Sync & Reply Input|Issue URL Sync & Reply Input]]
- [[_COMMUNITY_Agent Presence & Run Counts|Agent Presence & Run Counts]]
- [[_COMMUNITY_Agent UI Section & Filters|Agent UI Section & Filters]]
- [[_COMMUNITY_Task Status & Agent Activity|Task Status & Agent Activity]]
- [[_COMMUNITY_Comments & Timeline|Comments & Timeline]]
- [[_COMMUNITY_Analytics & API Instance|Analytics & API Instance]]
- [[_COMMUNITY_Utility & Regex Tests|Utility & Regex Tests]]
- [[_COMMUNITY_Module Exports (Agents)|Module Exports (Agents)]]
- [[_COMMUNITY_Agent Query Key Factories|Agent Query Key Factories]]
- [[_COMMUNITY_File Upload Handlers|File Upload Handlers]]
- [[_COMMUNITY_OpenClaw Runtime Args|OpenClaw Runtime Args]]
- [[_COMMUNITY_Onboarding & Starter Content|Onboarding & Starter Content]]
- [[_COMMUNITY_Codex Runtime & Sandbox|Codex Runtime & Sandbox]]
- [[_COMMUNITY_Private Agent Permission Tests|Private Agent Permission Tests]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 139|Community 139]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 312|Community 312]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 329|Community 329]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 346|Community 346]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 359|Community 359]]
- [[_COMMUNITY_Community 360|Community 360]]
- [[_COMMUNITY_Community 361|Community 361]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 363|Community 363]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 371|Community 371]]
- [[_COMMUNITY_Community 372|Community 372]]
- [[_COMMUNITY_Community 373|Community 373]]
- [[_COMMUNITY_Community 374|Community 374]]
- [[_COMMUNITY_Community 375|Community 375]]
- [[_COMMUNITY_Community 376|Community 376]]
- [[_COMMUNITY_Community 377|Community 377]]
- [[_COMMUNITY_Community 378|Community 378]]
- [[_COMMUNITY_Community 379|Community 379]]
- [[_COMMUNITY_Community 380|Community 380]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]
- [[_COMMUNITY_Community 383|Community 383]]
- [[_COMMUNITY_Community 384|Community 384]]
- [[_COMMUNITY_Community 385|Community 385]]
- [[_COMMUNITY_Community 386|Community 386]]
- [[_COMMUNITY_Community 387|Community 387]]
- [[_COMMUNITY_Community 388|Community 388]]
- [[_COMMUNITY_Community 389|Community 389]]
- [[_COMMUNITY_Community 390|Community 390]]
- [[_COMMUNITY_Community 391|Community 391]]
- [[_COMMUNITY_Community 392|Community 392]]
- [[_COMMUNITY_Community 393|Community 393]]
- [[_COMMUNITY_Community 394|Community 394]]
- [[_COMMUNITY_Community 395|Community 395]]
- [[_COMMUNITY_Community 396|Community 396]]
- [[_COMMUNITY_Community 397|Community 397]]
- [[_COMMUNITY_Community 398|Community 398]]
- [[_COMMUNITY_Community 399|Community 399]]
- [[_COMMUNITY_Community 400|Community 400]]
- [[_COMMUNITY_Community 401|Community 401]]
- [[_COMMUNITY_Community 402|Community 402]]
- [[_COMMUNITY_Community 403|Community 403]]
- [[_COMMUNITY_Community 404|Community 404]]
- [[_COMMUNITY_Community 405|Community 405]]
- [[_COMMUNITY_Community 406|Community 406]]
- [[_COMMUNITY_Community 407|Community 407]]
- [[_COMMUNITY_Community 408|Community 408]]
- [[_COMMUNITY_Community 409|Community 409]]
- [[_COMMUNITY_Community 410|Community 410]]
- [[_COMMUNITY_Community 411|Community 411]]
- [[_COMMUNITY_Community 412|Community 412]]
- [[_COMMUNITY_Community 413|Community 413]]
- [[_COMMUNITY_Community 414|Community 414]]
- [[_COMMUNITY_Community 415|Community 415]]
- [[_COMMUNITY_Community 416|Community 416]]
- [[_COMMUNITY_Community 417|Community 417]]
- [[_COMMUNITY_Community 418|Community 418]]
- [[_COMMUNITY_Community 419|Community 419]]
- [[_COMMUNITY_Community 420|Community 420]]
- [[_COMMUNITY_Community 421|Community 421]]
- [[_COMMUNITY_Community 422|Community 422]]
- [[_COMMUNITY_Community 423|Community 423]]
- [[_COMMUNITY_Community 424|Community 424]]
- [[_COMMUNITY_Community 425|Community 425]]
- [[_COMMUNITY_Community 426|Community 426]]
- [[_COMMUNITY_Community 427|Community 427]]
- [[_COMMUNITY_Community 428|Community 428]]
- [[_COMMUNITY_Community 429|Community 429]]
- [[_COMMUNITY_Community 430|Community 430]]
- [[_COMMUNITY_Community 431|Community 431]]
- [[_COMMUNITY_Community 432|Community 432]]
- [[_COMMUNITY_Community 433|Community 433]]
- [[_COMMUNITY_Community 434|Community 434]]
- [[_COMMUNITY_Community 435|Community 435]]
- [[_COMMUNITY_Community 436|Community 436]]
- [[_COMMUNITY_Community 437|Community 437]]
- [[_COMMUNITY_Community 438|Community 438]]
- [[_COMMUNITY_Community 439|Community 439]]
- [[_COMMUNITY_Community 440|Community 440]]
- [[_COMMUNITY_Community 441|Community 441]]
- [[_COMMUNITY_Community 442|Community 442]]
- [[_COMMUNITY_Community 443|Community 443]]
- [[_COMMUNITY_Community 444|Community 444]]
- [[_COMMUNITY_Community 445|Community 445]]
- [[_COMMUNITY_Community 446|Community 446]]
- [[_COMMUNITY_Community 447|Community 447]]
- [[_COMMUNITY_Community 448|Community 448]]
- [[_COMMUNITY_Community 449|Community 449]]
- [[_COMMUNITY_Community 450|Community 450]]
- [[_COMMUNITY_Community 451|Community 451]]
- [[_COMMUNITY_Community 452|Community 452]]
- [[_COMMUNITY_Community 453|Community 453]]
- [[_COMMUNITY_Community 454|Community 454]]
- [[_COMMUNITY_Community 455|Community 455]]
- [[_COMMUNITY_Community 456|Community 456]]
- [[_COMMUNITY_Community 457|Community 457]]
- [[_COMMUNITY_Community 458|Community 458]]
- [[_COMMUNITY_Community 459|Community 459]]
- [[_COMMUNITY_Community 460|Community 460]]
- [[_COMMUNITY_Community 461|Community 461]]
- [[_COMMUNITY_Community 462|Community 462]]
- [[_COMMUNITY_Community 463|Community 463]]
- [[_COMMUNITY_Community 464|Community 464]]
- [[_COMMUNITY_Community 465|Community 465]]
- [[_COMMUNITY_Community 466|Community 466]]
- [[_COMMUNITY_Community 467|Community 467]]
- [[_COMMUNITY_Community 468|Community 468]]
- [[_COMMUNITY_Community 469|Community 469]]
- [[_COMMUNITY_Community 470|Community 470]]
- [[_COMMUNITY_Community 471|Community 471]]
- [[_COMMUNITY_Community 472|Community 472]]
- [[_COMMUNITY_Community 473|Community 473]]
- [[_COMMUNITY_Community 474|Community 474]]
- [[_COMMUNITY_Community 475|Community 475]]
- [[_COMMUNITY_Community 476|Community 476]]
- [[_COMMUNITY_Community 477|Community 477]]
- [[_COMMUNITY_Community 478|Community 478]]
- [[_COMMUNITY_Community 479|Community 479]]
- [[_COMMUNITY_Community 480|Community 480]]
- [[_COMMUNITY_Community 481|Community 481]]
- [[_COMMUNITY_Community 482|Community 482]]
- [[_COMMUNITY_Community 483|Community 483]]
- [[_COMMUNITY_Community 484|Community 484]]
- [[_COMMUNITY_Community 485|Community 485]]
- [[_COMMUNITY_Community 486|Community 486]]
- [[_COMMUNITY_Community 487|Community 487]]
- [[_COMMUNITY_Community 488|Community 488]]
- [[_COMMUNITY_Community 489|Community 489]]
- [[_COMMUNITY_Community 490|Community 490]]
- [[_COMMUNITY_Community 491|Community 491]]
- [[_COMMUNITY_Community 492|Community 492]]
- [[_COMMUNITY_Community 493|Community 493]]
- [[_COMMUNITY_Community 494|Community 494]]
- [[_COMMUNITY_Community 495|Community 495]]
- [[_COMMUNITY_Community 496|Community 496]]
- [[_COMMUNITY_Community 497|Community 497]]
- [[_COMMUNITY_Community 498|Community 498]]
- [[_COMMUNITY_Community 499|Community 499]]
- [[_COMMUNITY_Community 500|Community 500]]
- [[_COMMUNITY_Community 501|Community 501]]
- [[_COMMUNITY_Community 502|Community 502]]
- [[_COMMUNITY_Community 503|Community 503]]
- [[_COMMUNITY_Community 504|Community 504]]
- [[_COMMUNITY_Community 505|Community 505]]
- [[_COMMUNITY_Community 506|Community 506]]
- [[_COMMUNITY_Community 507|Community 507]]
- [[_COMMUNITY_Community 508|Community 508]]
- [[_COMMUNITY_Community 509|Community 509]]
- [[_COMMUNITY_Community 510|Community 510]]
- [[_COMMUNITY_Community 511|Community 511]]
- [[_COMMUNITY_Community 512|Community 512]]
- [[_COMMUNITY_Community 513|Community 513]]
- [[_COMMUNITY_Community 514|Community 514]]
- [[_COMMUNITY_Community 515|Community 515]]
- [[_COMMUNITY_Community 516|Community 516]]
- [[_COMMUNITY_Community 517|Community 517]]
- [[_COMMUNITY_Community 518|Community 518]]
- [[_COMMUNITY_Community 519|Community 519]]
- [[_COMMUNITY_Community 520|Community 520]]
- [[_COMMUNITY_Community 521|Community 521]]
- [[_COMMUNITY_Community 522|Community 522]]
- [[_COMMUNITY_Community 523|Community 523]]
- [[_COMMUNITY_Community 524|Community 524]]
- [[_COMMUNITY_Community 525|Community 525]]
- [[_COMMUNITY_Community 526|Community 526]]
- [[_COMMUNITY_Community 527|Community 527]]
- [[_COMMUNITY_Community 528|Community 528]]
- [[_COMMUNITY_Community 529|Community 529]]
- [[_COMMUNITY_Community 530|Community 530]]
- [[_COMMUNITY_Community 531|Community 531]]
- [[_COMMUNITY_Community 532|Community 532]]
- [[_COMMUNITY_Community 533|Community 533]]
- [[_COMMUNITY_Community 534|Community 534]]
- [[_COMMUNITY_Community 535|Community 535]]
- [[_COMMUNITY_Community 536|Community 536]]
- [[_COMMUNITY_Community 537|Community 537]]
- [[_COMMUNITY_Community 538|Community 538]]
- [[_COMMUNITY_Community 539|Community 539]]
- [[_COMMUNITY_Community 540|Community 540]]
- [[_COMMUNITY_Community 541|Community 541]]
- [[_COMMUNITY_Community 542|Community 542]]
- [[_COMMUNITY_Community 543|Community 543]]
- [[_COMMUNITY_Community 544|Community 544]]
- [[_COMMUNITY_Community 545|Community 545]]
- [[_COMMUNITY_Community 546|Community 546]]
- [[_COMMUNITY_Community 547|Community 547]]
- [[_COMMUNITY_Community 548|Community 548]]
- [[_COMMUNITY_Community 549|Community 549]]
- [[_COMMUNITY_Community 550|Community 550]]
- [[_COMMUNITY_Community 551|Community 551]]
- [[_COMMUNITY_Community 552|Community 552]]
- [[_COMMUNITY_Community 553|Community 553]]
- [[_COMMUNITY_Community 554|Community 554]]
- [[_COMMUNITY_Community 555|Community 555]]
- [[_COMMUNITY_Community 556|Community 556]]
- [[_COMMUNITY_Community 557|Community 557]]
- [[_COMMUNITY_Community 558|Community 558]]
- [[_COMMUNITY_Community 559|Community 559]]
- [[_COMMUNITY_Community 560|Community 560]]
- [[_COMMUNITY_Community 561|Community 561]]
- [[_COMMUNITY_Community 562|Community 562]]
- [[_COMMUNITY_Community 563|Community 563]]
- [[_COMMUNITY_Community 564|Community 564]]
- [[_COMMUNITY_Community 565|Community 565]]
- [[_COMMUNITY_Community 566|Community 566]]
- [[_COMMUNITY_Community 567|Community 567]]
- [[_COMMUNITY_Community 568|Community 568]]
- [[_COMMUNITY_Community 569|Community 569]]
- [[_COMMUNITY_Community 570|Community 570]]
- [[_COMMUNITY_Community 571|Community 571]]
- [[_COMMUNITY_Community 572|Community 572]]
- [[_COMMUNITY_Community 573|Community 573]]
- [[_COMMUNITY_Community 574|Community 574]]
- [[_COMMUNITY_Community 575|Community 575]]
- [[_COMMUNITY_Community 576|Community 576]]
- [[_COMMUNITY_Community 577|Community 577]]
- [[_COMMUNITY_Community 578|Community 578]]
- [[_COMMUNITY_Community 579|Community 579]]
- [[_COMMUNITY_Community 580|Community 580]]
- [[_COMMUNITY_Community 581|Community 581]]
- [[_COMMUNITY_Community 582|Community 582]]
- [[_COMMUNITY_Community 583|Community 583]]
- [[_COMMUNITY_Community 584|Community 584]]
- [[_COMMUNITY_Community 585|Community 585]]
- [[_COMMUNITY_Community 586|Community 586]]
- [[_COMMUNITY_Community 587|Community 587]]
- [[_COMMUNITY_Community 588|Community 588]]
- [[_COMMUNITY_Community 589|Community 589]]
- [[_COMMUNITY_Community 590|Community 590]]
- [[_COMMUNITY_Community 591|Community 591]]
- [[_COMMUNITY_Community 592|Community 592]]
- [[_COMMUNITY_Community 593|Community 593]]
- [[_COMMUNITY_Community 594|Community 594]]
- [[_COMMUNITY_Community 595|Community 595]]
- [[_COMMUNITY_Community 596|Community 596]]
- [[_COMMUNITY_Community 597|Community 597]]
- [[_COMMUNITY_Community 598|Community 598]]
- [[_COMMUNITY_Community 599|Community 599]]
- [[_COMMUNITY_Community 600|Community 600]]
- [[_COMMUNITY_Community 601|Community 601]]
- [[_COMMUNITY_Community 602|Community 602]]
- [[_COMMUNITY_Community 603|Community 603]]
- [[_COMMUNITY_Community 604|Community 604]]
- [[_COMMUNITY_Community 605|Community 605]]
- [[_COMMUNITY_Community 606|Community 606]]
- [[_COMMUNITY_Community 607|Community 607]]
- [[_COMMUNITY_Community 608|Community 608]]
- [[_COMMUNITY_Community 610|Community 610]]
- [[_COMMUNITY_Community 611|Community 611]]
- [[_COMMUNITY_Community 612|Community 612]]
- [[_COMMUNITY_Community 613|Community 613]]
- [[_COMMUNITY_Community 614|Community 614]]
- [[_COMMUNITY_Community 615|Community 615]]
- [[_COMMUNITY_Community 616|Community 616]]
- [[_COMMUNITY_Community 617|Community 617]]
- [[_COMMUNITY_Community 618|Community 618]]
- [[_COMMUNITY_Community 619|Community 619]]
- [[_COMMUNITY_Community 620|Community 620]]
- [[_COMMUNITY_Community 621|Community 621]]
- [[_COMMUNITY_Community 622|Community 622]]
- [[_COMMUNITY_Community 623|Community 623]]
- [[_COMMUNITY_Community 624|Community 624]]
- [[_COMMUNITY_Community 625|Community 625]]
- [[_COMMUNITY_Community 626|Community 626]]
- [[_COMMUNITY_Community 627|Community 627]]
- [[_COMMUNITY_Community 628|Community 628]]
- [[_COMMUNITY_Community 629|Community 629]]
- [[_COMMUNITY_Community 630|Community 630]]
- [[_COMMUNITY_Community 631|Community 631]]
- [[_COMMUNITY_Community 632|Community 632]]
- [[_COMMUNITY_Community 633|Community 633]]
- [[_COMMUNITY_Community 634|Community 634]]
- [[_COMMUNITY_Community 635|Community 635]]
- [[_COMMUNITY_Community 636|Community 636]]
- [[_COMMUNITY_Community 637|Community 637]]
- [[_COMMUNITY_Community 638|Community 638]]
- [[_COMMUNITY_Community 639|Community 639]]
- [[_COMMUNITY_Community 640|Community 640]]
- [[_COMMUNITY_Community 641|Community 641]]
- [[_COMMUNITY_Community 642|Community 642]]
- [[_COMMUNITY_Community 643|Community 643]]
- [[_COMMUNITY_Community 644|Community 644]]
- [[_COMMUNITY_Community 645|Community 645]]
- [[_COMMUNITY_Community 646|Community 646]]
- [[_COMMUNITY_Community 647|Community 647]]
- [[_COMMUNITY_Community 648|Community 648]]
- [[_COMMUNITY_Community 649|Community 649]]
- [[_COMMUNITY_Community 650|Community 650]]
- [[_COMMUNITY_Community 651|Community 651]]
- [[_COMMUNITY_Community 652|Community 652]]
- [[_COMMUNITY_Community 653|Community 653]]
- [[_COMMUNITY_Community 654|Community 654]]
- [[_COMMUNITY_Community 655|Community 655]]
- [[_COMMUNITY_Community 656|Community 656]]
- [[_COMMUNITY_Community 657|Community 657]]
- [[_COMMUNITY_Community 658|Community 658]]
- [[_COMMUNITY_Community 659|Community 659]]
- [[_COMMUNITY_Community 660|Community 660]]
- [[_COMMUNITY_Community 661|Community 661]]
- [[_COMMUNITY_Community 662|Community 662]]
- [[_COMMUNITY_Community 663|Community 663]]
- [[_COMMUNITY_Community 664|Community 664]]
- [[_COMMUNITY_Community 665|Community 665]]
- [[_COMMUNITY_Community 666|Community 666]]
- [[_COMMUNITY_Community 667|Community 667]]
- [[_COMMUNITY_Community 668|Community 668]]
- [[_COMMUNITY_Community 669|Community 669]]
- [[_COMMUNITY_Community 670|Community 670]]
- [[_COMMUNITY_Community 671|Community 671]]
- [[_COMMUNITY_Community 672|Community 672]]
- [[_COMMUNITY_Community 673|Community 673]]
- [[_COMMUNITY_Community 674|Community 674]]
- [[_COMMUNITY_Community 675|Community 675]]
- [[_COMMUNITY_Community 676|Community 676]]
- [[_COMMUNITY_Community 677|Community 677]]
- [[_COMMUNITY_Community 678|Community 678]]
- [[_COMMUNITY_Community 679|Community 679]]
- [[_COMMUNITY_Community 680|Community 680]]
- [[_COMMUNITY_Community 681|Community 681]]
- [[_COMMUNITY_Community 682|Community 682]]
- [[_COMMUNITY_Community 683|Community 683]]
- [[_COMMUNITY_Community 684|Community 684]]
- [[_COMMUNITY_Community 685|Community 685]]
- [[_COMMUNITY_Community 686|Community 686]]
- [[_COMMUNITY_Community 687|Community 687]]
- [[_COMMUNITY_Community 688|Community 688]]
- [[_COMMUNITY_Community 689|Community 689]]
- [[_COMMUNITY_Community 690|Community 690]]
- [[_COMMUNITY_Community 691|Community 691]]
- [[_COMMUNITY_Community 692|Community 692]]
- [[_COMMUNITY_Community 693|Community 693]]
- [[_COMMUNITY_Community 694|Community 694]]
- [[_COMMUNITY_Community 695|Community 695]]
- [[_COMMUNITY_Community 696|Community 696]]
- [[_COMMUNITY_Community 697|Community 697]]
- [[_COMMUNITY_Community 698|Community 698]]
- [[_COMMUNITY_Community 699|Community 699]]
- [[_COMMUNITY_Community 700|Community 700]]
- [[_COMMUNITY_Community 701|Community 701]]
- [[_COMMUNITY_Community 702|Community 702]]
- [[_COMMUNITY_Community 703|Community 703]]
- [[_COMMUNITY_Community 704|Community 704]]
- [[_COMMUNITY_Community 705|Community 705]]
- [[_COMMUNITY_Community 706|Community 706]]
- [[_COMMUNITY_Community 707|Community 707]]
- [[_COMMUNITY_Community 708|Community 708]]
- [[_COMMUNITY_Community 709|Community 709]]
- [[_COMMUNITY_Community 710|Community 710]]
- [[_COMMUNITY_Community 711|Community 711]]
- [[_COMMUNITY_Community 712|Community 712]]
- [[_COMMUNITY_Community 713|Community 713]]
- [[_COMMUNITY_Community 714|Community 714]]
- [[_COMMUNITY_Community 715|Community 715]]
- [[_COMMUNITY_Community 716|Community 716]]
- [[_COMMUNITY_Community 717|Community 717]]
- [[_COMMUNITY_Community 718|Community 718]]
- [[_COMMUNITY_Community 719|Community 719]]
- [[_COMMUNITY_Community 720|Community 720]]
- [[_COMMUNITY_Community 721|Community 721]]
- [[_COMMUNITY_Community 722|Community 722]]
- [[_COMMUNITY_Community 723|Community 723]]
- [[_COMMUNITY_Community 724|Community 724]]
- [[_COMMUNITY_Community 725|Community 725]]
- [[_COMMUNITY_Community 726|Community 726]]
- [[_COMMUNITY_Community 727|Community 727]]
- [[_COMMUNITY_Community 728|Community 728]]
- [[_COMMUNITY_Community 729|Community 729]]
- [[_COMMUNITY_Community 730|Community 730]]
- [[_COMMUNITY_Community 731|Community 731]]
- [[_COMMUNITY_Community 732|Community 732]]
- [[_COMMUNITY_Community 733|Community 733]]
- [[_COMMUNITY_Community 734|Community 734]]
- [[_COMMUNITY_Community 735|Community 735]]
- [[_COMMUNITY_Community 736|Community 736]]
- [[_COMMUNITY_Community 737|Community 737]]
- [[_COMMUNITY_Community 738|Community 738]]
- [[_COMMUNITY_Community 739|Community 739]]
- [[_COMMUNITY_Community 740|Community 740]]
- [[_COMMUNITY_Community 741|Community 741]]
- [[_COMMUNITY_Community 742|Community 742]]
- [[_COMMUNITY_Community 743|Community 743]]
- [[_COMMUNITY_Community 744|Community 744]]
- [[_COMMUNITY_Community 745|Community 745]]
- [[_COMMUNITY_Community 746|Community 746]]
- [[_COMMUNITY_Community 747|Community 747]]
- [[_COMMUNITY_Community 748|Community 748]]
- [[_COMMUNITY_Community 749|Community 749]]
- [[_COMMUNITY_Community 750|Community 750]]
- [[_COMMUNITY_Community 751|Community 751]]
- [[_COMMUNITY_Community 752|Community 752]]
- [[_COMMUNITY_Community 753|Community 753]]
- [[_COMMUNITY_Community 754|Community 754]]
- [[_COMMUNITY_Community 755|Community 755]]
- [[_COMMUNITY_Community 756|Community 756]]
- [[_COMMUNITY_Community 757|Community 757]]
- [[_COMMUNITY_Community 758|Community 758]]
- [[_COMMUNITY_Community 759|Community 759]]
- [[_COMMUNITY_Community 760|Community 760]]
- [[_COMMUNITY_Community 761|Community 761]]
- [[_COMMUNITY_Community 762|Community 762]]
- [[_COMMUNITY_Community 763|Community 763]]
- [[_COMMUNITY_Community 764|Community 764]]
- [[_COMMUNITY_Community 765|Community 765]]
- [[_COMMUNITY_Community 766|Community 766]]
- [[_COMMUNITY_Community 767|Community 767]]
- [[_COMMUNITY_Community 768|Community 768]]
- [[_COMMUNITY_Community 769|Community 769]]
- [[_COMMUNITY_Community 770|Community 770]]
- [[_COMMUNITY_Community 771|Community 771]]
- [[_COMMUNITY_Community 772|Community 772]]
- [[_COMMUNITY_Community 773|Community 773]]
- [[_COMMUNITY_Community 774|Community 774]]
- [[_COMMUNITY_Community 775|Community 775]]
- [[_COMMUNITY_Community 776|Community 776]]
- [[_COMMUNITY_Community 777|Community 777]]
- [[_COMMUNITY_Community 778|Community 778]]
- [[_COMMUNITY_Community 779|Community 779]]
- [[_COMMUNITY_Community 780|Community 780]]
- [[_COMMUNITY_Community 781|Community 781]]
- [[_COMMUNITY_Community 782|Community 782]]
- [[_COMMUNITY_Community 783|Community 783]]
- [[_COMMUNITY_Community 784|Community 784]]
- [[_COMMUNITY_Community 785|Community 785]]
- [[_COMMUNITY_Community 786|Community 786]]
- [[_COMMUNITY_Community 787|Community 787]]
- [[_COMMUNITY_Community 788|Community 788]]
- [[_COMMUNITY_Community 789|Community 789]]
- [[_COMMUNITY_Community 790|Community 790]]
- [[_COMMUNITY_Community 791|Community 791]]
- [[_COMMUNITY_Community 792|Community 792]]
- [[_COMMUNITY_Community 793|Community 793]]
- [[_COMMUNITY_Community 794|Community 794]]
- [[_COMMUNITY_Community 795|Community 795]]
- [[_COMMUNITY_Community 796|Community 796]]
- [[_COMMUNITY_Community 797|Community 797]]
- [[_COMMUNITY_Community 798|Community 798]]
- [[_COMMUNITY_Community 799|Community 799]]
- [[_COMMUNITY_Community 800|Community 800]]
- [[_COMMUNITY_Community 801|Community 801]]
- [[_COMMUNITY_Community 802|Community 802]]
- [[_COMMUNITY_Community 803|Community 803]]
- [[_COMMUNITY_Community 804|Community 804]]
- [[_COMMUNITY_Community 805|Community 805]]
- [[_COMMUNITY_Community 806|Community 806]]
- [[_COMMUNITY_Community 807|Community 807]]
- [[_COMMUNITY_Community 808|Community 808]]
- [[_COMMUNITY_Community 809|Community 809]]
- [[_COMMUNITY_Community 810|Community 810]]
- [[_COMMUNITY_Community 811|Community 811]]
- [[_COMMUNITY_Community 812|Community 812]]
- [[_COMMUNITY_Community 813|Community 813]]
- [[_COMMUNITY_Community 814|Community 814]]
- [[_COMMUNITY_Community 815|Community 815]]
- [[_COMMUNITY_Community 816|Community 816]]
- [[_COMMUNITY_Community 817|Community 817]]
- [[_COMMUNITY_Community 818|Community 818]]
- [[_COMMUNITY_Community 819|Community 819]]
- [[_COMMUNITY_Community 820|Community 820]]
- [[_COMMUNITY_Community 821|Community 821]]
- [[_COMMUNITY_Community 822|Community 822]]
- [[_COMMUNITY_Community 823|Community 823]]
- [[_COMMUNITY_Community 824|Community 824]]
- [[_COMMUNITY_Community 825|Community 825]]
- [[_COMMUNITY_Community 826|Community 826]]
- [[_COMMUNITY_Community 827|Community 827]]
- [[_COMMUNITY_Community 828|Community 828]]
- [[_COMMUNITY_Community 829|Community 829]]
- [[_COMMUNITY_Community 830|Community 830]]
- [[_COMMUNITY_Community 831|Community 831]]
- [[_COMMUNITY_Community 832|Community 832]]
- [[_COMMUNITY_Community 833|Community 833]]
- [[_COMMUNITY_Community 834|Community 834]]
- [[_COMMUNITY_Community 835|Community 835]]
- [[_COMMUNITY_Community 836|Community 836]]
- [[_COMMUNITY_Community 837|Community 837]]
- [[_COMMUNITY_Community 838|Community 838]]
- [[_COMMUNITY_Community 839|Community 839]]
- [[_COMMUNITY_Community 840|Community 840]]
- [[_COMMUNITY_Community 841|Community 841]]
- [[_COMMUNITY_Community 842|Community 842]]
- [[_COMMUNITY_Community 843|Community 843]]
- [[_COMMUNITY_Community 844|Community 844]]
- [[_COMMUNITY_Community 845|Community 845]]
- [[_COMMUNITY_Community 846|Community 846]]
- [[_COMMUNITY_Community 847|Community 847]]
- [[_COMMUNITY_Community 848|Community 848]]
- [[_COMMUNITY_Community 849|Community 849]]
- [[_COMMUNITY_Community 850|Community 850]]
- [[_COMMUNITY_Community 851|Community 851]]
- [[_COMMUNITY_Community 852|Community 852]]
- [[_COMMUNITY_Community 853|Community 853]]
- [[_COMMUNITY_Community 854|Community 854]]
- [[_COMMUNITY_Community 855|Community 855]]
- [[_COMMUNITY_Community 856|Community 856]]
- [[_COMMUNITY_Community 857|Community 857]]
- [[_COMMUNITY_Community 858|Community 858]]
- [[_COMMUNITY_Community 859|Community 859]]
- [[_COMMUNITY_Community 860|Community 860]]
- [[_COMMUNITY_Community 861|Community 861]]
- [[_COMMUNITY_Community 862|Community 862]]
- [[_COMMUNITY_Community 863|Community 863]]
- [[_COMMUNITY_Community 864|Community 864]]
- [[_COMMUNITY_Community 865|Community 865]]
- [[_COMMUNITY_Community 866|Community 866]]
- [[_COMMUNITY_Community 867|Community 867]]
- [[_COMMUNITY_Community 868|Community 868]]
- [[_COMMUNITY_Community 869|Community 869]]
- [[_COMMUNITY_Community 870|Community 870]]
- [[_COMMUNITY_Community 871|Community 871]]
- [[_COMMUNITY_Community 872|Community 872]]
- [[_COMMUNITY_Community 873|Community 873]]
- [[_COMMUNITY_Community 874|Community 874]]
- [[_COMMUNITY_Community 875|Community 875]]
- [[_COMMUNITY_Community 876|Community 876]]
- [[_COMMUNITY_Community 877|Community 877]]
- [[_COMMUNITY_Community 878|Community 878]]
- [[_COMMUNITY_Community 879|Community 879]]
- [[_COMMUNITY_Community 880|Community 880]]
- [[_COMMUNITY_Community 881|Community 881]]
- [[_COMMUNITY_Community 882|Community 882]]
- [[_COMMUNITY_Community 883|Community 883]]
- [[_COMMUNITY_Community 884|Community 884]]
- [[_COMMUNITY_Community 885|Community 885]]
- [[_COMMUNITY_Community 886|Community 886]]
- [[_COMMUNITY_Community 887|Community 887]]
- [[_COMMUNITY_Community 888|Community 888]]
- [[_COMMUNITY_Community 889|Community 889]]
- [[_COMMUNITY_Community 890|Community 890]]
- [[_COMMUNITY_Community 891|Community 891]]
- [[_COMMUNITY_Community 892|Community 892]]
- [[_COMMUNITY_Community 893|Community 893]]
- [[_COMMUNITY_Community 894|Community 894]]
- [[_COMMUNITY_Community 895|Community 895]]
- [[_COMMUNITY_Community 911|Community 911]]
- [[_COMMUNITY_Community 912|Community 912]]
- [[_COMMUNITY_Community 913|Community 913]]
- [[_COMMUNITY_Community 914|Community 914]]
- [[_COMMUNITY_Community 915|Community 915]]
- [[_COMMUNITY_Community 916|Community 916]]
- [[_COMMUNITY_Community 917|Community 917]]
- [[_COMMUNITY_Community 918|Community 918]]
- [[_COMMUNITY_Community 919|Community 919]]
- [[_COMMUNITY_Community 920|Community 920]]
- [[_COMMUNITY_Community 921|Community 921]]
- [[_COMMUNITY_Community 922|Community 922]]
- [[_COMMUNITY_Community 923|Community 923]]
- [[_COMMUNITY_Community 924|Community 924]]
- [[_COMMUNITY_Community 925|Community 925]]
- [[_COMMUNITY_Community 926|Community 926]]
- [[_COMMUNITY_Community 927|Community 927]]
- [[_COMMUNITY_Community 928|Community 928]]
- [[_COMMUNITY_Community 929|Community 929]]
- [[_COMMUNITY_Community 930|Community 930]]
- [[_COMMUNITY_Community 931|Community 931]]
- [[_COMMUNITY_Community 932|Community 932]]
- [[_COMMUNITY_Community 933|Community 933]]
- [[_COMMUNITY_Community 934|Community 934]]
- [[_COMMUNITY_Community 935|Community 935]]
- [[_COMMUNITY_Community 936|Community 936]]
- [[_COMMUNITY_Community 937|Community 937]]
- [[_COMMUNITY_Community 938|Community 938]]
- [[_COMMUNITY_Community 939|Community 939]]
- [[_COMMUNITY_Community 940|Community 940]]
- [[_COMMUNITY_Community 941|Community 941]]
- [[_COMMUNITY_Community 942|Community 942]]
- [[_COMMUNITY_Community 943|Community 943]]
- [[_COMMUNITY_Community 944|Community 944]]
- [[_COMMUNITY_Community 945|Community 945]]
- [[_COMMUNITY_Community 946|Community 946]]
- [[_COMMUNITY_Community 947|Community 947]]
- [[_COMMUNITY_Community 948|Community 948]]
- [[_COMMUNITY_Community 949|Community 949]]
- [[_COMMUNITY_Community 950|Community 950]]
- [[_COMMUNITY_Community 951|Community 951]]
- [[_COMMUNITY_Community 952|Community 952]]
- [[_COMMUNITY_Community 953|Community 953]]
- [[_COMMUNITY_Community 954|Community 954]]
- [[_COMMUNITY_Community 955|Community 955]]
- [[_COMMUNITY_Community 956|Community 956]]
- [[_COMMUNITY_Community 957|Community 957]]
- [[_COMMUNITY_Community 958|Community 958]]
- [[_COMMUNITY_Community 959|Community 959]]
- [[_COMMUNITY_Community 960|Community 960]]
- [[_COMMUNITY_Community 961|Community 961]]
- [[_COMMUNITY_Community 962|Community 962]]
- [[_COMMUNITY_Community 963|Community 963]]
- [[_COMMUNITY_Community 964|Community 964]]
- [[_COMMUNITY_Community 965|Community 965]]
- [[_COMMUNITY_Community 966|Community 966]]
- [[_COMMUNITY_Community 967|Community 967]]
- [[_COMMUNITY_Community 968|Community 968]]
- [[_COMMUNITY_Community 969|Community 969]]
- [[_COMMUNITY_Community 970|Community 970]]
- [[_COMMUNITY_Community 971|Community 971]]
- [[_COMMUNITY_Community 972|Community 972]]
- [[_COMMUNITY_Community 973|Community 973]]
- [[_COMMUNITY_Community 974|Community 974]]
- [[_COMMUNITY_Community 975|Community 975]]
- [[_COMMUNITY_Community 976|Community 976]]
- [[_COMMUNITY_Community 977|Community 977]]
- [[_COMMUNITY_Community 978|Community 978]]
- [[_COMMUNITY_Community 979|Community 979]]
- [[_COMMUNITY_Community 980|Community 980]]
- [[_COMMUNITY_Community 981|Community 981]]
- [[_COMMUNITY_Community 982|Community 982]]
- [[_COMMUNITY_Community 983|Community 983]]
- [[_COMMUNITY_Community 984|Community 984]]
- [[_COMMUNITY_Community 985|Community 985]]
- [[_COMMUNITY_Community 986|Community 986]]
- [[_COMMUNITY_Community 987|Community 987]]
- [[_COMMUNITY_Community 988|Community 988]]
- [[_COMMUNITY_Community 989|Community 989]]
- [[_COMMUNITY_Community 990|Community 990]]
- [[_COMMUNITY_Community 991|Community 991]]
- [[_COMMUNITY_Community 992|Community 992]]
- [[_COMMUNITY_Community 993|Community 993]]
- [[_COMMUNITY_Community 994|Community 994]]
- [[_COMMUNITY_Community 995|Community 995]]
- [[_COMMUNITY_Community 996|Community 996]]
- [[_COMMUNITY_Community 997|Community 997]]
- [[_COMMUNITY_Community 998|Community 998]]
- [[_COMMUNITY_Community 999|Community 999]]
- [[_COMMUNITY_Community 1000|Community 1000]]
- [[_COMMUNITY_Community 1001|Community 1001]]
- [[_COMMUNITY_Community 1002|Community 1002]]
- [[_COMMUNITY_Community 1003|Community 1003]]
- [[_COMMUNITY_Community 1004|Community 1004]]
- [[_COMMUNITY_Community 1005|Community 1005]]
- [[_COMMUNITY_Community 1006|Community 1006]]
- [[_COMMUNITY_Community 1007|Community 1007]]
- [[_COMMUNITY_Community 1008|Community 1008]]
- [[_COMMUNITY_Community 1009|Community 1009]]
- [[_COMMUNITY_Community 1010|Community 1010]]
- [[_COMMUNITY_Community 1011|Community 1011]]
- [[_COMMUNITY_Community 1012|Community 1012]]
- [[_COMMUNITY_Community 1013|Community 1013]]
- [[_COMMUNITY_Community 1014|Community 1014]]
- [[_COMMUNITY_Community 1015|Community 1015]]
- [[_COMMUNITY_Community 1016|Community 1016]]
- [[_COMMUNITY_Community 1017|Community 1017]]
- [[_COMMUNITY_Community 1018|Community 1018]]
- [[_COMMUNITY_Community 1019|Community 1019]]
- [[_COMMUNITY_Community 1020|Community 1020]]
- [[_COMMUNITY_Community 1021|Community 1021]]
- [[_COMMUNITY_Community 1022|Community 1022]]
- [[_COMMUNITY_Community 1023|Community 1023]]
- [[_COMMUNITY_Community 1024|Community 1024]]
- [[_COMMUNITY_Community 1025|Community 1025]]
- [[_COMMUNITY_Community 1026|Community 1026]]
- [[_COMMUNITY_Community 1027|Community 1027]]
- [[_COMMUNITY_Community 1028|Community 1028]]
- [[_COMMUNITY_Community 1029|Community 1029]]
- [[_COMMUNITY_Community 1030|Community 1030]]
- [[_COMMUNITY_Community 1031|Community 1031]]
- [[_COMMUNITY_Community 1032|Community 1032]]
- [[_COMMUNITY_Community 1033|Community 1033]]
- [[_COMMUNITY_Community 1034|Community 1034]]
- [[_COMMUNITY_Community 1035|Community 1035]]
- [[_COMMUNITY_Community 1036|Community 1036]]
- [[_COMMUNITY_Community 1037|Community 1037]]
- [[_COMMUNITY_Community 1038|Community 1038]]
- [[_COMMUNITY_Community 1039|Community 1039]]
- [[_COMMUNITY_Community 1040|Community 1040]]
- [[_COMMUNITY_Community 1041|Community 1041]]
- [[_COMMUNITY_Community 1042|Community 1042]]
- [[_COMMUNITY_Community 1043|Community 1043]]
- [[_COMMUNITY_Community 1044|Community 1044]]
- [[_COMMUNITY_Community 1045|Community 1045]]
- [[_COMMUNITY_Community 1046|Community 1046]]
- [[_COMMUNITY_Community 1047|Community 1047]]
- [[_COMMUNITY_Community 1048|Community 1048]]
- [[_COMMUNITY_Community 1049|Community 1049]]
- [[_COMMUNITY_Community 1050|Community 1050]]
- [[_COMMUNITY_Community 1051|Community 1051]]
- [[_COMMUNITY_Community 1052|Community 1052]]
- [[_COMMUNITY_Community 1053|Community 1053]]
- [[_COMMUNITY_Community 1054|Community 1054]]
- [[_COMMUNITY_Community 1055|Community 1055]]
- [[_COMMUNITY_Community 1056|Community 1056]]
- [[_COMMUNITY_Community 1057|Community 1057]]
- [[_COMMUNITY_Community 1058|Community 1058]]
- [[_COMMUNITY_Community 1059|Community 1059]]
- [[_COMMUNITY_Community 1060|Community 1060]]
- [[_COMMUNITY_Community 1061|Community 1061]]
- [[_COMMUNITY_Community 1062|Community 1062]]
- [[_COMMUNITY_Community 1063|Community 1063]]
- [[_COMMUNITY_Community 1064|Community 1064]]
- [[_COMMUNITY_Community 1065|Community 1065]]
- [[_COMMUNITY_Community 1066|Community 1066]]
- [[_COMMUNITY_Community 1067|Community 1067]]
- [[_COMMUNITY_Community 1068|Community 1068]]
- [[_COMMUNITY_Community 1069|Community 1069]]
- [[_COMMUNITY_Community 1070|Community 1070]]
- [[_COMMUNITY_Community 1071|Community 1071]]
- [[_COMMUNITY_Community 1072|Community 1072]]
- [[_COMMUNITY_Community 1073|Community 1073]]
- [[_COMMUNITY_Community 1074|Community 1074]]
- [[_COMMUNITY_Community 1075|Community 1075]]
- [[_COMMUNITY_Community 1076|Community 1076]]
- [[_COMMUNITY_Community 1077|Community 1077]]
- [[_COMMUNITY_Community 1078|Community 1078]]
- [[_COMMUNITY_Community 1079|Community 1079]]
- [[_COMMUNITY_Community 1080|Community 1080]]
- [[_COMMUNITY_Community 1081|Community 1081]]
- [[_COMMUNITY_Community 1082|Community 1082]]
- [[_COMMUNITY_Community 1083|Community 1083]]
- [[_COMMUNITY_Community 1084|Community 1084]]
- [[_COMMUNITY_Community 1085|Community 1085]]
- [[_COMMUNITY_Community 1086|Community 1086]]
- [[_COMMUNITY_Community 1087|Community 1087]]
- [[_COMMUNITY_Community 1088|Community 1088]]
- [[_COMMUNITY_Community 1089|Community 1089]]
- [[_COMMUNITY_Community 1090|Community 1090]]
- [[_COMMUNITY_Community 1091|Community 1091]]
- [[_COMMUNITY_Community 1092|Community 1092]]
- [[_COMMUNITY_Community 1093|Community 1093]]
- [[_COMMUNITY_Community 1094|Community 1094]]
- [[_COMMUNITY_Community 1095|Community 1095]]
- [[_COMMUNITY_Community 1096|Community 1096]]
- [[_COMMUNITY_Community 1097|Community 1097]]
- [[_COMMUNITY_Community 1098|Community 1098]]
- [[_COMMUNITY_Community 1099|Community 1099]]
- [[_COMMUNITY_Community 1100|Community 1100]]
- [[_COMMUNITY_Community 1101|Community 1101]]
- [[_COMMUNITY_Community 1102|Community 1102]]
- [[_COMMUNITY_Community 1103|Community 1103]]
- [[_COMMUNITY_Community 1104|Community 1104]]
- [[_COMMUNITY_Community 1105|Community 1105]]
- [[_COMMUNITY_Community 1106|Community 1106]]
- [[_COMMUNITY_Community 1107|Community 1107]]
- [[_COMMUNITY_Community 1108|Community 1108]]
- [[_COMMUNITY_Community 1109|Community 1109]]
- [[_COMMUNITY_Community 1110|Community 1110]]
- [[_COMMUNITY_Community 1111|Community 1111]]
- [[_COMMUNITY_Community 1112|Community 1112]]
- [[_COMMUNITY_Community 1113|Community 1113]]
- [[_COMMUNITY_Community 1114|Community 1114]]
- [[_COMMUNITY_Community 1115|Community 1115]]
- [[_COMMUNITY_Community 1116|Community 1116]]
- [[_COMMUNITY_Community 1117|Community 1117]]
- [[_COMMUNITY_Community 1118|Community 1118]]
- [[_COMMUNITY_Community 1119|Community 1119]]
- [[_COMMUNITY_Community 1120|Community 1120]]
- [[_COMMUNITY_Community 1121|Community 1121]]
- [[_COMMUNITY_Community 1122|Community 1122]]
- [[_COMMUNITY_Community 1123|Community 1123]]
- [[_COMMUNITY_Community 1124|Community 1124]]
- [[_COMMUNITY_Community 1125|Community 1125]]
- [[_COMMUNITY_Community 1126|Community 1126]]
- [[_COMMUNITY_Community 1127|Community 1127]]
- [[_COMMUNITY_Community 1128|Community 1128]]
- [[_COMMUNITY_Community 1129|Community 1129]]
- [[_COMMUNITY_Community 1130|Community 1130]]
- [[_COMMUNITY_Community 1131|Community 1131]]
- [[_COMMUNITY_Community 1132|Community 1132]]
- [[_COMMUNITY_Community 1133|Community 1133]]
- [[_COMMUNITY_Community 1134|Community 1134]]
- [[_COMMUNITY_Community 1135|Community 1135]]
- [[_COMMUNITY_Community 1136|Community 1136]]
- [[_COMMUNITY_Community 1137|Community 1137]]
- [[_COMMUNITY_Community 1138|Community 1138]]
- [[_COMMUNITY_Community 1139|Community 1139]]
- [[_COMMUNITY_Community 1140|Community 1140]]
- [[_COMMUNITY_Community 1141|Community 1141]]
- [[_COMMUNITY_Community 1142|Community 1142]]
- [[_COMMUNITY_Community 1143|Community 1143]]
- [[_COMMUNITY_Community 1144|Community 1144]]
- [[_COMMUNITY_Community 1145|Community 1145]]
- [[_COMMUNITY_Community 1146|Community 1146]]
- [[_COMMUNITY_Community 1147|Community 1147]]
- [[_COMMUNITY_Community 1148|Community 1148]]
- [[_COMMUNITY_Community 1149|Community 1149]]
- [[_COMMUNITY_Community 1150|Community 1150]]
- [[_COMMUNITY_Community 1151|Community 1151]]
- [[_COMMUNITY_Community 1152|Community 1152]]
- [[_COMMUNITY_Community 1153|Community 1153]]
- [[_COMMUNITY_Community 1154|Community 1154]]
- [[_COMMUNITY_Community 1155|Community 1155]]
- [[_COMMUNITY_Community 1156|Community 1156]]
- [[_COMMUNITY_Community 1157|Community 1157]]
- [[_COMMUNITY_Community 1158|Community 1158]]
- [[_COMMUNITY_Community 1159|Community 1159]]
- [[_COMMUNITY_Community 1160|Community 1160]]
- [[_COMMUNITY_Community 1161|Community 1161]]
- [[_COMMUNITY_Community 1162|Community 1162]]
- [[_COMMUNITY_Community 1163|Community 1163]]
- [[_COMMUNITY_Community 1164|Community 1164]]
- [[_COMMUNITY_Community 1165|Community 1165]]
- [[_COMMUNITY_Community 1166|Community 1166]]
- [[_COMMUNITY_Community 1167|Community 1167]]
- [[_COMMUNITY_Community 1168|Community 1168]]
- [[_COMMUNITY_Community 1169|Community 1169]]
- [[_COMMUNITY_Community 1170|Community 1170]]
- [[_COMMUNITY_Community 1171|Community 1171]]
- [[_COMMUNITY_Community 1172|Community 1172]]
- [[_COMMUNITY_Community 1173|Community 1173]]
- [[_COMMUNITY_Community 1174|Community 1174]]
- [[_COMMUNITY_Community 1175|Community 1175]]
- [[_COMMUNITY_Community 1176|Community 1176]]
- [[_COMMUNITY_Community 1177|Community 1177]]
- [[_COMMUNITY_Community 1178|Community 1178]]
- [[_COMMUNITY_Community 1179|Community 1179]]
- [[_COMMUNITY_Community 1180|Community 1180]]
- [[_COMMUNITY_Community 1181|Community 1181]]
- [[_COMMUNITY_Community 1182|Community 1182]]
- [[_COMMUNITY_Community 1183|Community 1183]]
- [[_COMMUNITY_Community 1184|Community 1184]]
- [[_COMMUNITY_Community 1185|Community 1185]]
- [[_COMMUNITY_Community 1186|Community 1186]]
- [[_COMMUNITY_Community 1187|Community 1187]]
- [[_COMMUNITY_Community 1188|Community 1188]]
- [[_COMMUNITY_Community 1189|Community 1189]]
- [[_COMMUNITY_Community 1190|Community 1190]]
- [[_COMMUNITY_Community 1191|Community 1191]]
- [[_COMMUNITY_Community 1192|Community 1192]]
- [[_COMMUNITY_Community 1193|Community 1193]]
- [[_COMMUNITY_Community 1194|Community 1194]]
- [[_COMMUNITY_Community 1195|Community 1195]]
- [[_COMMUNITY_Community 1196|Community 1196]]
- [[_COMMUNITY_Community 1197|Community 1197]]
- [[_COMMUNITY_Community 1198|Community 1198]]
- [[_COMMUNITY_Community 1199|Community 1199]]
- [[_COMMUNITY_Community 1200|Community 1200]]
- [[_COMMUNITY_Community 1201|Community 1201]]
- [[_COMMUNITY_Community 1202|Community 1202]]
- [[_COMMUNITY_Community 1203|Community 1203]]
- [[_COMMUNITY_Community 1204|Community 1204]]
- [[_COMMUNITY_Community 1205|Community 1205]]
- [[_COMMUNITY_Community 1206|Community 1206]]
- [[_COMMUNITY_Community 1207|Community 1207]]
- [[_COMMUNITY_Community 1208|Community 1208]]
- [[_COMMUNITY_Community 1209|Community 1209]]
- [[_COMMUNITY_Community 1210|Community 1210]]
- [[_COMMUNITY_Community 1211|Community 1211]]
- [[_COMMUNITY_Community 1212|Community 1212]]
- [[_COMMUNITY_Community 1213|Community 1213]]
- [[_COMMUNITY_Community 1214|Community 1214]]
- [[_COMMUNITY_Community 1215|Community 1215]]
- [[_COMMUNITY_Community 1216|Community 1216]]
- [[_COMMUNITY_Community 1217|Community 1217]]
- [[_COMMUNITY_Community 1218|Community 1218]]
- [[_COMMUNITY_Community 1219|Community 1219]]
- [[_COMMUNITY_Community 1220|Community 1220]]
- [[_COMMUNITY_Community 1221|Community 1221]]
- [[_COMMUNITY_Community 1222|Community 1222]]
- [[_COMMUNITY_Community 1223|Community 1223]]
- [[_COMMUNITY_Community 1224|Community 1224]]
- [[_COMMUNITY_Community 1225|Community 1225]]
- [[_COMMUNITY_Community 1226|Community 1226]]
- [[_COMMUNITY_Community 1227|Community 1227]]
- [[_COMMUNITY_Community 1228|Community 1228]]
- [[_COMMUNITY_Community 1229|Community 1229]]
- [[_COMMUNITY_Community 1230|Community 1230]]
- [[_COMMUNITY_Community 1231|Community 1231]]
- [[_COMMUNITY_Community 1232|Community 1232]]
- [[_COMMUNITY_Community 1233|Community 1233]]
- [[_COMMUNITY_Community 1234|Community 1234]]
- [[_COMMUNITY_Community 1235|Community 1235]]
- [[_COMMUNITY_Community 1236|Community 1236]]
- [[_COMMUNITY_Community 1237|Community 1237]]
- [[_COMMUNITY_Community 1238|Community 1238]]
- [[_COMMUNITY_Community 1239|Community 1239]]
- [[_COMMUNITY_Community 1240|Community 1240]]
- [[_COMMUNITY_Community 1241|Community 1241]]
- [[_COMMUNITY_Community 1242|Community 1242]]
- [[_COMMUNITY_Community 1243|Community 1243]]
- [[_COMMUNITY_Community 1244|Community 1244]]
- [[_COMMUNITY_Community 1245|Community 1245]]
- [[_COMMUNITY_Community 1246|Community 1246]]
- [[_COMMUNITY_Community 1247|Community 1247]]
- [[_COMMUNITY_Community 1248|Community 1248]]
- [[_COMMUNITY_Community 1249|Community 1249]]
- [[_COMMUNITY_Community 1250|Community 1250]]
- [[_COMMUNITY_Community 1251|Community 1251]]
- [[_COMMUNITY_Community 1252|Community 1252]]
- [[_COMMUNITY_Community 1253|Community 1253]]
- [[_COMMUNITY_Community 1254|Community 1254]]
- [[_COMMUNITY_Community 1255|Community 1255]]
- [[_COMMUNITY_Community 1256|Community 1256]]
- [[_COMMUNITY_Community 1257|Community 1257]]
- [[_COMMUNITY_Community 1258|Community 1258]]
- [[_COMMUNITY_Community 1259|Community 1259]]
- [[_COMMUNITY_Community 1260|Community 1260]]
- [[_COMMUNITY_Community 1261|Community 1261]]
- [[_COMMUNITY_Community 1262|Community 1262]]
- [[_COMMUNITY_Community 1263|Community 1263]]
- [[_COMMUNITY_Community 1264|Community 1264]]
- [[_COMMUNITY_Community 1265|Community 1265]]
- [[_COMMUNITY_Community 1266|Community 1266]]
- [[_COMMUNITY_Community 1267|Community 1267]]
- [[_COMMUNITY_Community 1268|Community 1268]]
- [[_COMMUNITY_Community 1269|Community 1269]]
- [[_COMMUNITY_Community 1270|Community 1270]]
- [[_COMMUNITY_Community 1271|Community 1271]]
- [[_COMMUNITY_Community 1272|Community 1272]]
- [[_COMMUNITY_Community 1273|Community 1273]]
- [[_COMMUNITY_Community 1274|Community 1274]]
- [[_COMMUNITY_Community 1275|Community 1275]]
- [[_COMMUNITY_Community 1276|Community 1276]]
- [[_COMMUNITY_Community 1277|Community 1277]]
- [[_COMMUNITY_Community 1278|Community 1278]]
- [[_COMMUNITY_Community 1279|Community 1279]]
- [[_COMMUNITY_Community 1280|Community 1280]]
- [[_COMMUNITY_Community 1281|Community 1281]]
- [[_COMMUNITY_Community 1282|Community 1282]]
- [[_COMMUNITY_Community 1283|Community 1283]]
- [[_COMMUNITY_Community 1284|Community 1284]]
- [[_COMMUNITY_Community 1285|Community 1285]]
- [[_COMMUNITY_Community 1286|Community 1286]]
- [[_COMMUNITY_Community 1287|Community 1287]]
- [[_COMMUNITY_Community 1288|Community 1288]]
- [[_COMMUNITY_Community 1289|Community 1289]]
- [[_COMMUNITY_Community 1290|Community 1290]]
- [[_COMMUNITY_Community 1291|Community 1291]]
- [[_COMMUNITY_Community 1292|Community 1292]]
- [[_COMMUNITY_Community 1293|Community 1293]]
- [[_COMMUNITY_Community 1294|Community 1294]]
- [[_COMMUNITY_Community 1295|Community 1295]]
- [[_COMMUNITY_Community 1296|Community 1296]]
- [[_COMMUNITY_Community 1297|Community 1297]]
- [[_COMMUNITY_Community 1298|Community 1298]]
- [[_COMMUNITY_Community 1299|Community 1299]]
- [[_COMMUNITY_Community 1300|Community 1300]]
- [[_COMMUNITY_Community 1301|Community 1301]]
- [[_COMMUNITY_Community 1302|Community 1302]]
- [[_COMMUNITY_Community 1303|Community 1303]]
- [[_COMMUNITY_Community 1304|Community 1304]]
- [[_COMMUNITY_Community 1305|Community 1305]]
- [[_COMMUNITY_Community 1306|Community 1306]]
- [[_COMMUNITY_Community 1307|Community 1307]]
- [[_COMMUNITY_Community 1308|Community 1308]]
- [[_COMMUNITY_Community 1309|Community 1309]]
- [[_COMMUNITY_Community 1310|Community 1310]]
- [[_COMMUNITY_Community 1311|Community 1311]]
- [[_COMMUNITY_Community 1312|Community 1312]]
- [[_COMMUNITY_Community 1313|Community 1313]]
- [[_COMMUNITY_Community 1314|Community 1314]]
- [[_COMMUNITY_Community 1315|Community 1315]]
- [[_COMMUNITY_Community 1316|Community 1316]]
- [[_COMMUNITY_Community 1317|Community 1317]]
- [[_COMMUNITY_Community 1318|Community 1318]]
- [[_COMMUNITY_Community 1319|Community 1319]]
- [[_COMMUNITY_Community 1320|Community 1320]]
- [[_COMMUNITY_Community 1321|Community 1321]]
- [[_COMMUNITY_Community 1322|Community 1322]]
- [[_COMMUNITY_Community 1323|Community 1323]]
- [[_COMMUNITY_Community 1324|Community 1324]]
- [[_COMMUNITY_Community 1325|Community 1325]]
- [[_COMMUNITY_Community 1326|Community 1326]]
- [[_COMMUNITY_Community 1327|Community 1327]]
- [[_COMMUNITY_Community 1328|Community 1328]]
- [[_COMMUNITY_Community 1329|Community 1329]]
- [[_COMMUNITY_Community 1330|Community 1330]]
- [[_COMMUNITY_Community 1331|Community 1331]]
- [[_COMMUNITY_Community 1332|Community 1332]]
- [[_COMMUNITY_Community 1333|Community 1333]]
- [[_COMMUNITY_Community 1334|Community 1334]]
- [[_COMMUNITY_Community 1335|Community 1335]]
- [[_COMMUNITY_Community 1336|Community 1336]]
- [[_COMMUNITY_Community 1337|Community 1337]]
- [[_COMMUNITY_Community 1338|Community 1338]]
- [[_COMMUNITY_Community 1339|Community 1339]]
- [[_COMMUNITY_Community 1340|Community 1340]]
- [[_COMMUNITY_Community 1341|Community 1341]]
- [[_COMMUNITY_Community 1342|Community 1342]]
- [[_COMMUNITY_Community 1343|Community 1343]]
- [[_COMMUNITY_Community 1344|Community 1344]]
- [[_COMMUNITY_Community 1345|Community 1345]]
- [[_COMMUNITY_Community 1346|Community 1346]]
- [[_COMMUNITY_Community 1347|Community 1347]]
- [[_COMMUNITY_Community 1348|Community 1348]]
- [[_COMMUNITY_Community 1349|Community 1349]]
- [[_COMMUNITY_Community 1350|Community 1350]]
- [[_COMMUNITY_Community 1351|Community 1351]]
- [[_COMMUNITY_Community 1352|Community 1352]]
- [[_COMMUNITY_Community 1353|Community 1353]]
- [[_COMMUNITY_Community 1354|Community 1354]]
- [[_COMMUNITY_Community 1355|Community 1355]]
- [[_COMMUNITY_Community 1356|Community 1356]]
- [[_COMMUNITY_Community 1357|Community 1357]]
- [[_COMMUNITY_Community 1358|Community 1358]]
- [[_COMMUNITY_Community 1359|Community 1359]]
- [[_COMMUNITY_Community 1360|Community 1360]]
- [[_COMMUNITY_Community 1361|Community 1361]]
- [[_COMMUNITY_Community 1362|Community 1362]]
- [[_COMMUNITY_Community 1363|Community 1363]]
- [[_COMMUNITY_Community 1364|Community 1364]]
- [[_COMMUNITY_Community 1365|Community 1365]]
- [[_COMMUNITY_Community 1366|Community 1366]]
- [[_COMMUNITY_Community 1367|Community 1367]]
- [[_COMMUNITY_Community 1368|Community 1368]]
- [[_COMMUNITY_Community 1369|Community 1369]]
- [[_COMMUNITY_Community 1370|Community 1370]]
- [[_COMMUNITY_Community 1371|Community 1371]]
- [[_COMMUNITY_Community 1372|Community 1372]]
- [[_COMMUNITY_Community 1373|Community 1373]]
- [[_COMMUNITY_Community 1374|Community 1374]]
- [[_COMMUNITY_Community 1375|Community 1375]]
- [[_COMMUNITY_Community 1376|Community 1376]]
- [[_COMMUNITY_Community 1377|Community 1377]]
- [[_COMMUNITY_Community 1378|Community 1378]]
- [[_COMMUNITY_Community 1379|Community 1379]]
- [[_COMMUNITY_Community 1380|Community 1380]]
- [[_COMMUNITY_Community 1381|Community 1381]]
- [[_COMMUNITY_Community 1382|Community 1382]]
- [[_COMMUNITY_Community 1383|Community 1383]]
- [[_COMMUNITY_Community 1384|Community 1384]]
- [[_COMMUNITY_Community 1385|Community 1385]]
- [[_COMMUNITY_Community 1386|Community 1386]]
- [[_COMMUNITY_Community 1387|Community 1387]]
- [[_COMMUNITY_Community 1388|Community 1388]]
- [[_COMMUNITY_Community 1389|Community 1389]]
- [[_COMMUNITY_Community 1390|Community 1390]]
- [[_COMMUNITY_Community 1391|Community 1391]]
- [[_COMMUNITY_Community 1392|Community 1392]]
- [[_COMMUNITY_Community 1393|Community 1393]]
- [[_COMMUNITY_Community 1394|Community 1394]]
- [[_COMMUNITY_Community 1395|Community 1395]]
- [[_COMMUNITY_Community 1396|Community 1396]]
- [[_COMMUNITY_Community 1397|Community 1397]]
- [[_COMMUNITY_Community 1398|Community 1398]]
- [[_COMMUNITY_Community 1399|Community 1399]]
- [[_COMMUNITY_Community 1400|Community 1400]]
- [[_COMMUNITY_Community 1401|Community 1401]]
- [[_COMMUNITY_Community 1402|Community 1402]]
- [[_COMMUNITY_Community 1403|Community 1403]]
- [[_COMMUNITY_Community 1404|Community 1404]]
- [[_COMMUNITY_Community 1405|Community 1405]]
- [[_COMMUNITY_Community 1406|Community 1406]]
- [[_COMMUNITY_Community 1407|Community 1407]]
- [[_COMMUNITY_Community 1408|Community 1408]]
- [[_COMMUNITY_Community 1409|Community 1409]]
- [[_COMMUNITY_Community 1410|Community 1410]]
- [[_COMMUNITY_Community 1411|Community 1411]]
- [[_COMMUNITY_Community 1412|Community 1412]]
- [[_COMMUNITY_Community 1413|Community 1413]]
- [[_COMMUNITY_Community 1414|Community 1414]]
- [[_COMMUNITY_Community 1415|Community 1415]]
- [[_COMMUNITY_Community 1416|Community 1416]]
- [[_COMMUNITY_Community 1417|Community 1417]]
- [[_COMMUNITY_Community 1418|Community 1418]]
- [[_COMMUNITY_Community 1419|Community 1419]]
- [[_COMMUNITY_Community 1420|Community 1420]]
- [[_COMMUNITY_Community 1421|Community 1421]]
- [[_COMMUNITY_Community 1422|Community 1422]]
- [[_COMMUNITY_Community 1423|Community 1423]]
- [[_COMMUNITY_Community 1424|Community 1424]]
- [[_COMMUNITY_Community 1425|Community 1425]]
- [[_COMMUNITY_Community 1426|Community 1426]]
- [[_COMMUNITY_Community 1427|Community 1427]]
- [[_COMMUNITY_Community 1428|Community 1428]]
- [[_COMMUNITY_Community 1429|Community 1429]]
- [[_COMMUNITY_Community 1430|Community 1430]]
- [[_COMMUNITY_Community 1431|Community 1431]]
- [[_COMMUNITY_Community 1432|Community 1432]]
- [[_COMMUNITY_Community 1433|Community 1433]]
- [[_COMMUNITY_Community 1434|Community 1434]]
- [[_COMMUNITY_Community 1435|Community 1435]]
- [[_COMMUNITY_Community 1436|Community 1436]]
- [[_COMMUNITY_Community 1437|Community 1437]]
- [[_COMMUNITY_Community 1438|Community 1438]]
- [[_COMMUNITY_Community 1439|Community 1439]]
- [[_COMMUNITY_Community 1440|Community 1440]]
- [[_COMMUNITY_Community 1441|Community 1441]]
- [[_COMMUNITY_Community 1442|Community 1442]]
- [[_COMMUNITY_Community 1443|Community 1443]]
- [[_COMMUNITY_Community 1444|Community 1444]]
- [[_COMMUNITY_Community 1445|Community 1445]]
- [[_COMMUNITY_Community 1446|Community 1446]]
- [[_COMMUNITY_Community 1447|Community 1447]]
- [[_COMMUNITY_Community 1448|Community 1448]]
- [[_COMMUNITY_Community 1449|Community 1449]]
- [[_COMMUNITY_Community 1450|Community 1450]]
- [[_COMMUNITY_Community 1451|Community 1451]]
- [[_COMMUNITY_Community 1452|Community 1452]]
- [[_COMMUNITY_Community 1453|Community 1453]]
- [[_COMMUNITY_Community 1454|Community 1454]]
- [[_COMMUNITY_Community 1455|Community 1455]]
- [[_COMMUNITY_Community 1456|Community 1456]]
- [[_COMMUNITY_Community 1457|Community 1457]]
- [[_COMMUNITY_Community 1458|Community 1458]]
- [[_COMMUNITY_Community 1459|Community 1459]]
- [[_COMMUNITY_Community 1460|Community 1460]]
- [[_COMMUNITY_Community 1461|Community 1461]]
- [[_COMMUNITY_Community 1462|Community 1462]]
- [[_COMMUNITY_Community 1463|Community 1463]]
- [[_COMMUNITY_Community 1464|Community 1464]]
- [[_COMMUNITY_Community 1465|Community 1465]]
- [[_COMMUNITY_Community 1466|Community 1466]]
- [[_COMMUNITY_Community 1467|Community 1467]]
- [[_COMMUNITY_Community 1468|Community 1468]]
- [[_COMMUNITY_Community 1469|Community 1469]]
- [[_COMMUNITY_Community 1470|Community 1470]]
- [[_COMMUNITY_Community 1471|Community 1471]]
- [[_COMMUNITY_Community 1472|Community 1472]]
- [[_COMMUNITY_Community 1473|Community 1473]]
- [[_COMMUNITY_Community 1474|Community 1474]]
- [[_COMMUNITY_Community 1475|Community 1475]]
- [[_COMMUNITY_Community 1476|Community 1476]]
- [[_COMMUNITY_Community 1477|Community 1477]]
- [[_COMMUNITY_Community 1478|Community 1478]]
- [[_COMMUNITY_Community 1479|Community 1479]]
- [[_COMMUNITY_Community 1480|Community 1480]]
- [[_COMMUNITY_Community 1481|Community 1481]]
- [[_COMMUNITY_Community 1482|Community 1482]]
- [[_COMMUNITY_Community 1483|Community 1483]]
- [[_COMMUNITY_Community 1484|Community 1484]]
- [[_COMMUNITY_Community 1485|Community 1485]]
- [[_COMMUNITY_Community 1486|Community 1486]]
- [[_COMMUNITY_Community 1487|Community 1487]]
- [[_COMMUNITY_Community 1488|Community 1488]]
- [[_COMMUNITY_Community 1489|Community 1489]]
- [[_COMMUNITY_Community 1490|Community 1490]]
- [[_COMMUNITY_Community 1491|Community 1491]]
- [[_COMMUNITY_Community 1492|Community 1492]]
- [[_COMMUNITY_Community 1493|Community 1493]]
- [[_COMMUNITY_Community 1494|Community 1494]]
- [[_COMMUNITY_Community 1495|Community 1495]]
- [[_COMMUNITY_Community 1496|Community 1496]]
- [[_COMMUNITY_Community 1497|Community 1497]]
- [[_COMMUNITY_Community 1498|Community 1498]]
- [[_COMMUNITY_Community 1499|Community 1499]]
- [[_COMMUNITY_Community 1500|Community 1500]]
- [[_COMMUNITY_Community 1501|Community 1501]]
- [[_COMMUNITY_Community 1502|Community 1502]]
- [[_COMMUNITY_Community 1503|Community 1503]]
- [[_COMMUNITY_Community 1504|Community 1504]]
- [[_COMMUNITY_Community 1505|Community 1505]]
- [[_COMMUNITY_Community 1506|Community 1506]]
- [[_COMMUNITY_Community 1507|Community 1507]]
- [[_COMMUNITY_Community 1508|Community 1508]]
- [[_COMMUNITY_Community 1509|Community 1509]]
- [[_COMMUNITY_Community 1510|Community 1510]]
- [[_COMMUNITY_Community 1511|Community 1511]]
- [[_COMMUNITY_Community 1512|Community 1512]]
- [[_COMMUNITY_Community 1513|Community 1513]]
- [[_COMMUNITY_Community 1514|Community 1514]]
- [[_COMMUNITY_Community 1515|Community 1515]]
- [[_COMMUNITY_Community 1516|Community 1516]]
- [[_COMMUNITY_Community 1517|Community 1517]]
- [[_COMMUNITY_Community 1518|Community 1518]]
- [[_COMMUNITY_Community 1519|Community 1519]]
- [[_COMMUNITY_Community 1520|Community 1520]]
- [[_COMMUNITY_Community 1521|Community 1521]]
- [[_COMMUNITY_Community 1522|Community 1522]]
- [[_COMMUNITY_Community 1523|Community 1523]]
- [[_COMMUNITY_Community 1524|Community 1524]]
- [[_COMMUNITY_Community 1525|Community 1525]]
- [[_COMMUNITY_Community 1526|Community 1526]]
- [[_COMMUNITY_Community 1527|Community 1527]]
- [[_COMMUNITY_Community 1528|Community 1528]]
- [[_COMMUNITY_Community 1529|Community 1529]]
- [[_COMMUNITY_Community 1530|Community 1530]]
- [[_COMMUNITY_Community 1531|Community 1531]]
- [[_COMMUNITY_Community 1532|Community 1532]]
- [[_COMMUNITY_Community 1533|Community 1533]]
- [[_COMMUNITY_Community 1534|Community 1534]]
- [[_COMMUNITY_Community 1535|Community 1535]]
- [[_COMMUNITY_Community 1536|Community 1536]]
- [[_COMMUNITY_Community 1537|Community 1537]]
- [[_COMMUNITY_Community 1538|Community 1538]]
- [[_COMMUNITY_Community 1539|Community 1539]]
- [[_COMMUNITY_Community 1540|Community 1540]]
- [[_COMMUNITY_Community 1541|Community 1541]]
- [[_COMMUNITY_Community 1542|Community 1542]]
- [[_COMMUNITY_Community 1543|Community 1543]]
- [[_COMMUNITY_Community 1544|Community 1544]]
- [[_COMMUNITY_Community 1545|Community 1545]]
- [[_COMMUNITY_Community 1546|Community 1546]]
- [[_COMMUNITY_Community 1547|Community 1547]]
- [[_COMMUNITY_Community 1548|Community 1548]]
- [[_COMMUNITY_Community 1549|Community 1549]]
- [[_COMMUNITY_Community 1550|Community 1550]]
- [[_COMMUNITY_Community 1551|Community 1551]]
- [[_COMMUNITY_Community 1552|Community 1552]]
- [[_COMMUNITY_Community 1553|Community 1553]]
- [[_COMMUNITY_Community 1554|Community 1554]]
- [[_COMMUNITY_Community 1555|Community 1555]]
- [[_COMMUNITY_Community 1556|Community 1556]]
- [[_COMMUNITY_Community 1557|Community 1557]]
- [[_COMMUNITY_Community 1558|Community 1558]]
- [[_COMMUNITY_Community 1559|Community 1559]]
- [[_COMMUNITY_Community 1560|Community 1560]]
- [[_COMMUNITY_Community 1561|Community 1561]]
- [[_COMMUNITY_Community 1562|Community 1562]]
- [[_COMMUNITY_Community 1563|Community 1563]]
- [[_COMMUNITY_Community 1564|Community 1564]]
- [[_COMMUNITY_Community 1565|Community 1565]]
- [[_COMMUNITY_Community 1566|Community 1566]]
- [[_COMMUNITY_Community 1567|Community 1567]]
- [[_COMMUNITY_Community 1568|Community 1568]]
- [[_COMMUNITY_Community 1569|Community 1569]]
- [[_COMMUNITY_Community 1570|Community 1570]]
- [[_COMMUNITY_Community 1571|Community 1571]]
- [[_COMMUNITY_Community 1572|Community 1572]]
- [[_COMMUNITY_Community 1573|Community 1573]]
- [[_COMMUNITY_Community 1574|Community 1574]]
- [[_COMMUNITY_Community 1575|Community 1575]]
- [[_COMMUNITY_Community 1576|Community 1576]]
- [[_COMMUNITY_Community 1577|Community 1577]]
- [[_COMMUNITY_Community 1578|Community 1578]]
- [[_COMMUNITY_Community 1579|Community 1579]]
- [[_COMMUNITY_Community 1580|Community 1580]]
- [[_COMMUNITY_Community 1581|Community 1581]]
- [[_COMMUNITY_Community 1582|Community 1582]]
- [[_COMMUNITY_Community 1583|Community 1583]]
- [[_COMMUNITY_Community 1584|Community 1584]]
- [[_COMMUNITY_Community 1585|Community 1585]]
- [[_COMMUNITY_Community 1586|Community 1586]]
- [[_COMMUNITY_Community 1587|Community 1587]]
- [[_COMMUNITY_Community 1588|Community 1588]]
- [[_COMMUNITY_Community 1589|Community 1589]]
- [[_COMMUNITY_Community 1590|Community 1590]]
- [[_COMMUNITY_Community 1591|Community 1591]]
- [[_COMMUNITY_Community 1592|Community 1592]]
- [[_COMMUNITY_Community 1593|Community 1593]]
- [[_COMMUNITY_Community 1594|Community 1594]]
- [[_COMMUNITY_Community 1595|Community 1595]]
- [[_COMMUNITY_Community 1596|Community 1596]]
- [[_COMMUNITY_Community 1597|Community 1597]]
- [[_COMMUNITY_Community 1598|Community 1598]]
- [[_COMMUNITY_Community 1599|Community 1599]]
- [[_COMMUNITY_Community 1600|Community 1600]]
- [[_COMMUNITY_Community 1601|Community 1601]]
- [[_COMMUNITY_Community 1602|Community 1602]]
- [[_COMMUNITY_Community 1603|Community 1603]]
- [[_COMMUNITY_Community 1604|Community 1604]]
- [[_COMMUNITY_Community 1605|Community 1605]]
- [[_COMMUNITY_Community 1606|Community 1606]]
- [[_COMMUNITY_Community 1607|Community 1607]]
- [[_COMMUNITY_Community 1608|Community 1608]]
- [[_COMMUNITY_Community 1609|Community 1609]]
- [[_COMMUNITY_Community 1610|Community 1610]]
- [[_COMMUNITY_Community 1611|Community 1611]]
- [[_COMMUNITY_Community 1612|Community 1612]]
- [[_COMMUNITY_Community 1613|Community 1613]]
- [[_COMMUNITY_Community 1614|Community 1614]]
- [[_COMMUNITY_Community 1615|Community 1615]]
- [[_COMMUNITY_Community 1616|Community 1616]]
- [[_COMMUNITY_Community 1617|Community 1617]]
- [[_COMMUNITY_Community 1618|Community 1618]]
- [[_COMMUNITY_Community 1619|Community 1619]]
- [[_COMMUNITY_Community 1620|Community 1620]]
- [[_COMMUNITY_Community 1621|Community 1621]]
- [[_COMMUNITY_Community 1622|Community 1622]]
- [[_COMMUNITY_Community 1623|Community 1623]]
- [[_COMMUNITY_Community 1624|Community 1624]]
- [[_COMMUNITY_Community 1625|Community 1625]]
- [[_COMMUNITY_Community 1626|Community 1626]]
- [[_COMMUNITY_Community 1627|Community 1627]]
- [[_COMMUNITY_Community 1628|Community 1628]]
- [[_COMMUNITY_Community 1629|Community 1629]]
- [[_COMMUNITY_Community 1630|Community 1630]]
- [[_COMMUNITY_Community 1631|Community 1631]]
- [[_COMMUNITY_Community 1632|Community 1632]]
- [[_COMMUNITY_Community 1633|Community 1633]]
- [[_COMMUNITY_Community 1634|Community 1634]]
- [[_COMMUNITY_Community 1635|Community 1635]]
- [[_COMMUNITY_Community 1636|Community 1636]]
- [[_COMMUNITY_Community 1637|Community 1637]]
- [[_COMMUNITY_Community 1638|Community 1638]]
- [[_COMMUNITY_Community 1639|Community 1639]]
- [[_COMMUNITY_Community 1640|Community 1640]]
- [[_COMMUNITY_Community 1641|Community 1641]]
- [[_COMMUNITY_Community 1642|Community 1642]]
- [[_COMMUNITY_Community 1643|Community 1643]]
- [[_COMMUNITY_Community 1644|Community 1644]]
- [[_COMMUNITY_Community 1645|Community 1645]]
- [[_COMMUNITY_Community 1646|Community 1646]]
- [[_COMMUNITY_Community 1647|Community 1647]]
- [[_COMMUNITY_Community 1648|Community 1648]]
- [[_COMMUNITY_Community 1649|Community 1649]]
- [[_COMMUNITY_Community 1650|Community 1650]]
- [[_COMMUNITY_Community 1651|Community 1651]]
- [[_COMMUNITY_Community 1652|Community 1652]]
- [[_COMMUNITY_Community 1653|Community 1653]]
- [[_COMMUNITY_Community 1654|Community 1654]]
- [[_COMMUNITY_Community 1655|Community 1655]]
- [[_COMMUNITY_Community 1656|Community 1656]]
- [[_COMMUNITY_Community 1657|Community 1657]]
- [[_COMMUNITY_Community 1658|Community 1658]]
- [[_COMMUNITY_Community 1659|Community 1659]]
- [[_COMMUNITY_Community 1660|Community 1660]]
- [[_COMMUNITY_Community 1661|Community 1661]]
- [[_COMMUNITY_Community 1662|Community 1662]]
- [[_COMMUNITY_Community 1663|Community 1663]]
- [[_COMMUNITY_Community 1664|Community 1664]]
- [[_COMMUNITY_Community 1665|Community 1665]]
- [[_COMMUNITY_Community 1666|Community 1666]]
- [[_COMMUNITY_Community 1667|Community 1667]]
- [[_COMMUNITY_Community 1668|Community 1668]]
- [[_COMMUNITY_Community 1669|Community 1669]]
- [[_COMMUNITY_Community 1670|Community 1670]]
- [[_COMMUNITY_Community 1671|Community 1671]]
- [[_COMMUNITY_Community 1672|Community 1672]]
- [[_COMMUNITY_Community 1673|Community 1673]]
- [[_COMMUNITY_Community 1674|Community 1674]]
- [[_COMMUNITY_Community 1675|Community 1675]]
- [[_COMMUNITY_Community 1676|Community 1676]]
- [[_COMMUNITY_Community 1677|Community 1677]]
- [[_COMMUNITY_Community 1678|Community 1678]]
- [[_COMMUNITY_Community 1679|Community 1679]]
- [[_COMMUNITY_Community 1680|Community 1680]]
- [[_COMMUNITY_Community 1681|Community 1681]]
- [[_COMMUNITY_Community 1682|Community 1682]]
- [[_COMMUNITY_Community 1683|Community 1683]]
- [[_COMMUNITY_Community 1684|Community 1684]]
- [[_COMMUNITY_Community 1685|Community 1685]]
- [[_COMMUNITY_Community 1686|Community 1686]]
- [[_COMMUNITY_Community 1687|Community 1687]]
- [[_COMMUNITY_Community 1688|Community 1688]]
- [[_COMMUNITY_Community 1689|Community 1689]]
- [[_COMMUNITY_Community 1691|Community 1691]]
- [[_COMMUNITY_Community 1692|Community 1692]]
- [[_COMMUNITY_Community 1693|Community 1693]]
- [[_COMMUNITY_Community 1694|Community 1694]]
- [[_COMMUNITY_Community 1695|Community 1695]]
- [[_COMMUNITY_Community 1696|Community 1696]]
- [[_COMMUNITY_Community 1697|Community 1697]]
- [[_COMMUNITY_Community 1698|Community 1698]]
- [[_COMMUNITY_Community 1699|Community 1699]]
- [[_COMMUNITY_Community 1700|Community 1700]]
- [[_COMMUNITY_Community 1701|Community 1701]]
- [[_COMMUNITY_Community 1702|Community 1702]]
- [[_COMMUNITY_Community 1703|Community 1703]]
- [[_COMMUNITY_Community 1704|Community 1704]]
- [[_COMMUNITY_Community 1705|Community 1705]]
- [[_COMMUNITY_Community 1706|Community 1706]]
- [[_COMMUNITY_Community 1707|Community 1707]]
- [[_COMMUNITY_Community 1708|Community 1708]]
- [[_COMMUNITY_Community 1709|Community 1709]]
- [[_COMMUNITY_Community 1710|Community 1710]]
- [[_COMMUNITY_Community 1711|Community 1711]]
- [[_COMMUNITY_Community 1712|Community 1712]]
- [[_COMMUNITY_Community 1713|Community 1713]]
- [[_COMMUNITY_Community 1714|Community 1714]]
- [[_COMMUNITY_Community 1715|Community 1715]]
- [[_COMMUNITY_Community 1716|Community 1716]]
- [[_COMMUNITY_Community 1717|Community 1717]]
- [[_COMMUNITY_Community 1718|Community 1718]]
- [[_COMMUNITY_Community 1719|Community 1719]]
- [[_COMMUNITY_Community 1720|Community 1720]]
- [[_COMMUNITY_Community 1721|Community 1721]]
- [[_COMMUNITY_Community 1722|Community 1722]]
- [[_COMMUNITY_Community 1723|Community 1723]]
- [[_COMMUNITY_Community 1724|Community 1724]]
- [[_COMMUNITY_Community 1725|Community 1725]]
- [[_COMMUNITY_Community 1726|Community 1726]]
- [[_COMMUNITY_Community 1727|Community 1727]]
- [[_COMMUNITY_Community 1728|Community 1728]]
- [[_COMMUNITY_Community 1729|Community 1729]]
- [[_COMMUNITY_Community 1730|Community 1730]]
- [[_COMMUNITY_Community 1731|Community 1731]]
- [[_COMMUNITY_Community 1732|Community 1732]]
- [[_COMMUNITY_Community 1733|Community 1733]]
- [[_COMMUNITY_Community 1734|Community 1734]]
- [[_COMMUNITY_Community 1735|Community 1735]]
- [[_COMMUNITY_Community 1736|Community 1736]]
- [[_COMMUNITY_Community 1737|Community 1737]]
- [[_COMMUNITY_Community 1738|Community 1738]]
- [[_COMMUNITY_Community 1739|Community 1739]]
- [[_COMMUNITY_Community 1740|Community 1740]]
- [[_COMMUNITY_Community 1741|Community 1741]]
- [[_COMMUNITY_Community 1742|Community 1742]]
- [[_COMMUNITY_Community 1743|Community 1743]]
- [[_COMMUNITY_Community 1744|Community 1744]]
- [[_COMMUNITY_Community 1745|Community 1745]]
- [[_COMMUNITY_Community 1746|Community 1746]]
- [[_COMMUNITY_Community 1747|Community 1747]]
- [[_COMMUNITY_Community 1748|Community 1748]]
- [[_COMMUNITY_Community 1749|Community 1749]]
- [[_COMMUNITY_Community 1750|Community 1750]]
- [[_COMMUNITY_Community 1751|Community 1751]]
- [[_COMMUNITY_Community 1752|Community 1752]]
- [[_COMMUNITY_Community 1753|Community 1753]]
- [[_COMMUNITY_Community 1754|Community 1754]]
- [[_COMMUNITY_Community 1755|Community 1755]]
- [[_COMMUNITY_Community 1756|Community 1756]]
- [[_COMMUNITY_Community 1757|Community 1757]]
- [[_COMMUNITY_Community 1758|Community 1758]]
- [[_COMMUNITY_Community 1759|Community 1759]]
- [[_COMMUNITY_Community 1760|Community 1760]]
- [[_COMMUNITY_Community 1761|Community 1761]]
- [[_COMMUNITY_Community 1762|Community 1762]]
- [[_COMMUNITY_Community 1763|Community 1763]]
- [[_COMMUNITY_Community 1764|Community 1764]]
- [[_COMMUNITY_Community 1765|Community 1765]]
- [[_COMMUNITY_Community 1766|Community 1766]]
- [[_COMMUNITY_Community 1767|Community 1767]]
- [[_COMMUNITY_Community 1768|Community 1768]]
- [[_COMMUNITY_Community 1769|Community 1769]]
- [[_COMMUNITY_Community 1770|Community 1770]]
- [[_COMMUNITY_Community 1771|Community 1771]]
- [[_COMMUNITY_Community 1772|Community 1772]]
- [[_COMMUNITY_Community 1773|Community 1773]]
- [[_COMMUNITY_Community 1774|Community 1774]]
- [[_COMMUNITY_Community 1775|Community 1775]]
- [[_COMMUNITY_Community 1776|Community 1776]]
- [[_COMMUNITY_Community 1777|Community 1777]]
- [[_COMMUNITY_Community 1778|Community 1778]]
- [[_COMMUNITY_Community 1779|Community 1779]]
- [[_COMMUNITY_Community 1780|Community 1780]]
- [[_COMMUNITY_Community 1781|Community 1781]]
- [[_COMMUNITY_Community 1782|Community 1782]]
- [[_COMMUNITY_Community 1783|Community 1783]]
- [[_COMMUNITY_Community 1784|Community 1784]]
- [[_COMMUNITY_Community 1785|Community 1785]]
- [[_COMMUNITY_Community 1786|Community 1786]]
- [[_COMMUNITY_Community 1787|Community 1787]]
- [[_COMMUNITY_Community 1788|Community 1788]]
- [[_COMMUNITY_Community 1789|Community 1789]]
- [[_COMMUNITY_Community 1790|Community 1790]]
- [[_COMMUNITY_Community 1791|Community 1791]]
- [[_COMMUNITY_Community 1792|Community 1792]]
- [[_COMMUNITY_Community 1793|Community 1793]]
- [[_COMMUNITY_Community 1794|Community 1794]]
- [[_COMMUNITY_Community 1795|Community 1795]]
- [[_COMMUNITY_Community 1796|Community 1796]]
- [[_COMMUNITY_Community 1797|Community 1797]]
- [[_COMMUNITY_Community 1798|Community 1798]]
- [[_COMMUNITY_Community 1799|Community 1799]]
- [[_COMMUNITY_Community 1800|Community 1800]]
- [[_COMMUNITY_Community 1801|Community 1801]]
- [[_COMMUNITY_Community 1802|Community 1802]]
- [[_COMMUNITY_Community 1803|Community 1803]]
- [[_COMMUNITY_Community 1804|Community 1804]]
- [[_COMMUNITY_Community 1805|Community 1805]]
- [[_COMMUNITY_Community 1806|Community 1806]]
- [[_COMMUNITY_Community 1807|Community 1807]]
- [[_COMMUNITY_Community 1808|Community 1808]]
- [[_COMMUNITY_Community 1809|Community 1809]]
- [[_COMMUNITY_Community 1810|Community 1810]]
- [[_COMMUNITY_Community 1811|Community 1811]]
- [[_COMMUNITY_Community 1812|Community 1812]]
- [[_COMMUNITY_Community 1813|Community 1813]]
- [[_COMMUNITY_Community 1814|Community 1814]]
- [[_COMMUNITY_Community 1815|Community 1815]]
- [[_COMMUNITY_Community 1816|Community 1816]]
- [[_COMMUNITY_Community 1817|Community 1817]]
- [[_COMMUNITY_Community 1818|Community 1818]]
- [[_COMMUNITY_Community 1819|Community 1819]]
- [[_COMMUNITY_Community 1820|Community 1820]]
- [[_COMMUNITY_Community 1821|Community 1821]]
- [[_COMMUNITY_Community 1822|Community 1822]]
- [[_COMMUNITY_Community 1823|Community 1823]]
- [[_COMMUNITY_Community 1824|Community 1824]]
- [[_COMMUNITY_Community 1825|Community 1825]]
- [[_COMMUNITY_Community 1826|Community 1826]]
- [[_COMMUNITY_Community 1827|Community 1827]]
- [[_COMMUNITY_Community 1828|Community 1828]]
- [[_COMMUNITY_Community 1829|Community 1829]]
- [[_COMMUNITY_Community 1830|Community 1830]]
- [[_COMMUNITY_Community 1831|Community 1831]]
- [[_COMMUNITY_Community 1832|Community 1832]]
- [[_COMMUNITY_Community 1833|Community 1833]]
- [[_COMMUNITY_Community 1834|Community 1834]]
- [[_COMMUNITY_Community 1835|Community 1835]]
- [[_COMMUNITY_Community 1836|Community 1836]]
- [[_COMMUNITY_Community 1837|Community 1837]]
- [[_COMMUNITY_Community 1838|Community 1838]]
- [[_COMMUNITY_Community 1839|Community 1839]]
- [[_COMMUNITY_Community 1840|Community 1840]]
- [[_COMMUNITY_Community 1841|Community 1841]]
- [[_COMMUNITY_Community 1842|Community 1842]]
- [[_COMMUNITY_Community 1843|Community 1843]]
- [[_COMMUNITY_Community 1844|Community 1844]]
- [[_COMMUNITY_Community 1845|Community 1845]]
- [[_COMMUNITY_Community 1846|Community 1846]]
- [[_COMMUNITY_Community 1847|Community 1847]]
- [[_COMMUNITY_Community 1848|Community 1848]]
- [[_COMMUNITY_Community 1849|Community 1849]]
- [[_COMMUNITY_Community 1850|Community 1850]]
- [[_COMMUNITY_Community 1851|Community 1851]]
- [[_COMMUNITY_Community 1852|Community 1852]]
- [[_COMMUNITY_Community 1853|Community 1853]]
- [[_COMMUNITY_Community 1854|Community 1854]]
- [[_COMMUNITY_Community 1855|Community 1855]]
- [[_COMMUNITY_Community 1856|Community 1856]]
- [[_COMMUNITY_Community 1857|Community 1857]]
- [[_COMMUNITY_Community 1858|Community 1858]]
- [[_COMMUNITY_Community 1859|Community 1859]]
- [[_COMMUNITY_Community 1860|Community 1860]]
- [[_COMMUNITY_Community 1861|Community 1861]]
- [[_COMMUNITY_Community 1862|Community 1862]]
- [[_COMMUNITY_Community 1863|Community 1863]]
- [[_COMMUNITY_Community 1864|Community 1864]]
- [[_COMMUNITY_Community 1865|Community 1865]]
- [[_COMMUNITY_Community 1866|Community 1866]]
- [[_COMMUNITY_Community 1867|Community 1867]]
- [[_COMMUNITY_Community 1868|Community 1868]]
- [[_COMMUNITY_Community 1869|Community 1869]]
- [[_COMMUNITY_Community 1870|Community 1870]]
- [[_COMMUNITY_Community 1871|Community 1871]]
- [[_COMMUNITY_Community 1872|Community 1872]]
- [[_COMMUNITY_Community 1873|Community 1873]]
- [[_COMMUNITY_Community 1874|Community 1874]]
- [[_COMMUNITY_Community 1875|Community 1875]]
- [[_COMMUNITY_Community 1876|Community 1876]]
- [[_COMMUNITY_Community 1877|Community 1877]]
- [[_COMMUNITY_Community 1878|Community 1878]]
- [[_COMMUNITY_Community 1879|Community 1879]]
- [[_COMMUNITY_Community 1880|Community 1880]]
- [[_COMMUNITY_Community 1881|Community 1881]]
- [[_COMMUNITY_Community 1882|Community 1882]]
- [[_COMMUNITY_Community 1883|Community 1883]]
- [[_COMMUNITY_Community 1884|Community 1884]]
- [[_COMMUNITY_Community 1885|Community 1885]]
- [[_COMMUNITY_Community 1886|Community 1886]]
- [[_COMMUNITY_Community 1887|Community 1887]]
- [[_COMMUNITY_Community 1888|Community 1888]]
- [[_COMMUNITY_Community 1889|Community 1889]]
- [[_COMMUNITY_Community 1890|Community 1890]]
- [[_COMMUNITY_Community 1891|Community 1891]]
- [[_COMMUNITY_Community 1892|Community 1892]]
- [[_COMMUNITY_Community 1893|Community 1893]]
- [[_COMMUNITY_Community 1894|Community 1894]]
- [[_COMMUNITY_Community 1895|Community 1895]]
- [[_COMMUNITY_Community 1896|Community 1896]]
- [[_COMMUNITY_Community 1897|Community 1897]]
- [[_COMMUNITY_Community 1898|Community 1898]]
- [[_COMMUNITY_Community 1899|Community 1899]]
- [[_COMMUNITY_Community 1900|Community 1900]]
- [[_COMMUNITY_Community 1901|Community 1901]]
- [[_COMMUNITY_Community 1902|Community 1902]]
- [[_COMMUNITY_Community 1903|Community 1903]]
- [[_COMMUNITY_Community 1904|Community 1904]]
- [[_COMMUNITY_Community 1905|Community 1905]]
- [[_COMMUNITY_Community 1906|Community 1906]]
- [[_COMMUNITY_Community 1907|Community 1907]]
- [[_COMMUNITY_Community 1908|Community 1908]]
- [[_COMMUNITY_Community 1909|Community 1909]]
- [[_COMMUNITY_Community 1910|Community 1910]]
- [[_COMMUNITY_Community 1911|Community 1911]]
- [[_COMMUNITY_Community 1912|Community 1912]]
- [[_COMMUNITY_Community 1913|Community 1913]]
- [[_COMMUNITY_Community 1914|Community 1914]]
- [[_COMMUNITY_Community 1915|Community 1915]]
- [[_COMMUNITY_Community 1916|Community 1916]]
- [[_COMMUNITY_Community 1917|Community 1917]]
- [[_COMMUNITY_Community 1918|Community 1918]]
- [[_COMMUNITY_Community 1919|Community 1919]]
- [[_COMMUNITY_Community 1920|Community 1920]]
- [[_COMMUNITY_Community 1921|Community 1921]]
- [[_COMMUNITY_Community 1922|Community 1922]]
- [[_COMMUNITY_Community 1923|Community 1923]]
- [[_COMMUNITY_Community 1924|Community 1924]]
- [[_COMMUNITY_Community 1925|Community 1925]]
- [[_COMMUNITY_Community 1926|Community 1926]]
- [[_COMMUNITY_Community 1927|Community 1927]]
- [[_COMMUNITY_Community 1928|Community 1928]]
- [[_COMMUNITY_Community 1929|Community 1929]]
- [[_COMMUNITY_Community 1930|Community 1930]]
- [[_COMMUNITY_Community 1931|Community 1931]]
- [[_COMMUNITY_Community 1932|Community 1932]]
- [[_COMMUNITY_Community 1933|Community 1933]]
- [[_COMMUNITY_Community 1934|Community 1934]]
- [[_COMMUNITY_Community 1935|Community 1935]]
- [[_COMMUNITY_Community 1936|Community 1936]]
- [[_COMMUNITY_Community 1937|Community 1937]]
- [[_COMMUNITY_Community 1938|Community 1938]]
- [[_COMMUNITY_Community 1939|Community 1939]]
- [[_COMMUNITY_Community 1940|Community 1940]]
- [[_COMMUNITY_Community 1941|Community 1941]]
- [[_COMMUNITY_Community 1942|Community 1942]]
- [[_COMMUNITY_Community 1943|Community 1943]]
- [[_COMMUNITY_Community 1944|Community 1944]]
- [[_COMMUNITY_Community 1945|Community 1945]]
- [[_COMMUNITY_Community 1946|Community 1946]]
- [[_COMMUNITY_Community 1947|Community 1947]]
- [[_COMMUNITY_Community 1948|Community 1948]]
- [[_COMMUNITY_Community 1949|Community 1949]]
- [[_COMMUNITY_Community 1950|Community 1950]]
- [[_COMMUNITY_Community 1951|Community 1951]]
- [[_COMMUNITY_Community 1952|Community 1952]]
- [[_COMMUNITY_Community 1953|Community 1953]]
- [[_COMMUNITY_Community 1954|Community 1954]]
- [[_COMMUNITY_Community 1955|Community 1955]]
- [[_COMMUNITY_Community 1956|Community 1956]]
- [[_COMMUNITY_Community 1957|Community 1957]]
- [[_COMMUNITY_Community 1958|Community 1958]]
- [[_COMMUNITY_Community 1959|Community 1959]]
- [[_COMMUNITY_Community 1960|Community 1960]]
- [[_COMMUNITY_Community 1961|Community 1961]]
- [[_COMMUNITY_Community 1962|Community 1962]]
- [[_COMMUNITY_Community 1963|Community 1963]]
- [[_COMMUNITY_Community 1964|Community 1964]]
- [[_COMMUNITY_Community 1965|Community 1965]]
- [[_COMMUNITY_Community 1966|Community 1966]]
- [[_COMMUNITY_Community 1967|Community 1967]]
- [[_COMMUNITY_Community 1968|Community 1968]]
- [[_COMMUNITY_Community 1969|Community 1969]]
- [[_COMMUNITY_Community 1970|Community 1970]]
- [[_COMMUNITY_Community 1971|Community 1971]]
- [[_COMMUNITY_Community 1972|Community 1972]]
- [[_COMMUNITY_Community 1973|Community 1973]]
- [[_COMMUNITY_Community 1974|Community 1974]]
- [[_COMMUNITY_Community 1975|Community 1975]]
- [[_COMMUNITY_Community 1976|Community 1976]]
- [[_COMMUNITY_Community 1977|Community 1977]]
- [[_COMMUNITY_Community 1978|Community 1978]]
- [[_COMMUNITY_Community 1979|Community 1979]]
- [[_COMMUNITY_Community 1980|Community 1980]]
- [[_COMMUNITY_Community 1981|Community 1981]]
- [[_COMMUNITY_Community 1982|Community 1982]]
- [[_COMMUNITY_Community 1983|Community 1983]]
- [[_COMMUNITY_Community 1984|Community 1984]]
- [[_COMMUNITY_Community 1985|Community 1985]]
- [[_COMMUNITY_Community 1986|Community 1986]]
- [[_COMMUNITY_Community 1987|Community 1987]]
- [[_COMMUNITY_Community 1988|Community 1988]]
- [[_COMMUNITY_Community 1989|Community 1989]]
- [[_COMMUNITY_Community 1990|Community 1990]]
- [[_COMMUNITY_Community 1991|Community 1991]]
- [[_COMMUNITY_Community 1992|Community 1992]]
- [[_COMMUNITY_Community 1993|Community 1993]]
- [[_COMMUNITY_Community 1994|Community 1994]]
- [[_COMMUNITY_Community 1995|Community 1995]]
- [[_COMMUNITY_Community 1996|Community 1996]]
- [[_COMMUNITY_Community 1997|Community 1997]]
- [[_COMMUNITY_Community 1998|Community 1998]]
- [[_COMMUNITY_Community 1999|Community 1999]]
- [[_COMMUNITY_Community 2000|Community 2000]]
- [[_COMMUNITY_Community 2001|Community 2001]]
- [[_COMMUNITY_Community 2002|Community 2002]]
- [[_COMMUNITY_Community 2003|Community 2003]]
- [[_COMMUNITY_Community 2004|Community 2004]]
- [[_COMMUNITY_Community 2005|Community 2005]]
- [[_COMMUNITY_Community 2006|Community 2006]]
- [[_COMMUNITY_Community 2007|Community 2007]]
- [[_COMMUNITY_Community 2008|Community 2008]]
- [[_COMMUNITY_Community 2009|Community 2009]]
- [[_COMMUNITY_Community 2010|Community 2010]]
- [[_COMMUNITY_Community 2011|Community 2011]]
- [[_COMMUNITY_Community 2012|Community 2012]]
- [[_COMMUNITY_Community 2013|Community 2013]]
- [[_COMMUNITY_Community 2014|Community 2014]]
- [[_COMMUNITY_Community 2015|Community 2015]]
- [[_COMMUNITY_Community 2016|Community 2016]]
- [[_COMMUNITY_Community 2017|Community 2017]]
- [[_COMMUNITY_Community 2018|Community 2018]]
- [[_COMMUNITY_Community 2019|Community 2019]]
- [[_COMMUNITY_Community 2020|Community 2020]]
- [[_COMMUNITY_Community 2021|Community 2021]]
- [[_COMMUNITY_Community 2022|Community 2022]]
- [[_COMMUNITY_Community 2023|Community 2023]]
- [[_COMMUNITY_Community 2024|Community 2024]]
- [[_COMMUNITY_Community 2025|Community 2025]]
- [[_COMMUNITY_Community 2026|Community 2026]]
- [[_COMMUNITY_Community 2027|Community 2027]]
- [[_COMMUNITY_Community 2028|Community 2028]]
- [[_COMMUNITY_Community 2029|Community 2029]]
- [[_COMMUNITY_Community 2030|Community 2030]]
- [[_COMMUNITY_Community 2031|Community 2031]]
- [[_COMMUNITY_Community 2032|Community 2032]]
- [[_COMMUNITY_Community 2033|Community 2033]]
- [[_COMMUNITY_Community 2034|Community 2034]]
- [[_COMMUNITY_Community 2035|Community 2035]]
- [[_COMMUNITY_Community 2036|Community 2036]]
- [[_COMMUNITY_Community 2037|Community 2037]]
- [[_COMMUNITY_Community 2038|Community 2038]]
- [[_COMMUNITY_Community 2039|Community 2039]]
- [[_COMMUNITY_Community 2040|Community 2040]]
- [[_COMMUNITY_Community 2041|Community 2041]]
- [[_COMMUNITY_Community 2042|Community 2042]]
- [[_COMMUNITY_Community 2043|Community 2043]]
- [[_COMMUNITY_Community 2044|Community 2044]]
- [[_COMMUNITY_Community 2045|Community 2045]]
- [[_COMMUNITY_Community 2046|Community 2046]]
- [[_COMMUNITY_Community 2047|Community 2047]]
- [[_COMMUNITY_Community 2048|Community 2048]]
- [[_COMMUNITY_Community 2049|Community 2049]]
- [[_COMMUNITY_Community 2050|Community 2050]]
- [[_COMMUNITY_Community 2051|Community 2051]]
- [[_COMMUNITY_Community 2052|Community 2052]]
- [[_COMMUNITY_Community 2053|Community 2053]]
- [[_COMMUNITY_Community 2054|Community 2054]]
- [[_COMMUNITY_Community 2055|Community 2055]]
- [[_COMMUNITY_Community 2056|Community 2056]]
- [[_COMMUNITY_Community 2057|Community 2057]]
- [[_COMMUNITY_Community 2058|Community 2058]]
- [[_COMMUNITY_Community 2059|Community 2059]]
- [[_COMMUNITY_Community 2060|Community 2060]]
- [[_COMMUNITY_Community 2061|Community 2061]]
- [[_COMMUNITY_Community 2062|Community 2062]]
- [[_COMMUNITY_Community 2063|Community 2063]]
- [[_COMMUNITY_Community 2064|Community 2064]]
- [[_COMMUNITY_Community 2065|Community 2065]]
- [[_COMMUNITY_Community 2066|Community 2066]]
- [[_COMMUNITY_Community 2067|Community 2067]]
- [[_COMMUNITY_Community 2068|Community 2068]]
- [[_COMMUNITY_Community 2069|Community 2069]]
- [[_COMMUNITY_Community 2070|Community 2070]]
- [[_COMMUNITY_Community 2071|Community 2071]]
- [[_COMMUNITY_Community 2072|Community 2072]]
- [[_COMMUNITY_Community 2073|Community 2073]]
- [[_COMMUNITY_Community 2074|Community 2074]]
- [[_COMMUNITY_Community 2075|Community 2075]]
- [[_COMMUNITY_Community 2076|Community 2076]]
- [[_COMMUNITY_Community 2077|Community 2077]]
- [[_COMMUNITY_Community 2078|Community 2078]]
- [[_COMMUNITY_Community 2079|Community 2079]]
- [[_COMMUNITY_Community 2080|Community 2080]]
- [[_COMMUNITY_Community 2081|Community 2081]]
- [[_COMMUNITY_Community 2082|Community 2082]]
- [[_COMMUNITY_Community 2083|Community 2083]]
- [[_COMMUNITY_Community 2084|Community 2084]]
- [[_COMMUNITY_Community 2085|Community 2085]]
- [[_COMMUNITY_Community 2086|Community 2086]]
- [[_COMMUNITY_Community 2087|Community 2087]]
- [[_COMMUNITY_Community 2088|Community 2088]]
- [[_COMMUNITY_Community 2089|Community 2089]]
- [[_COMMUNITY_Community 2090|Community 2090]]
- [[_COMMUNITY_Community 2091|Community 2091]]
- [[_COMMUNITY_Community 2092|Community 2092]]
- [[_COMMUNITY_Community 2093|Community 2093]]
- [[_COMMUNITY_Community 2094|Community 2094]]
- [[_COMMUNITY_Community 2095|Community 2095]]
- [[_COMMUNITY_Community 2096|Community 2096]]
- [[_COMMUNITY_Community 2097|Community 2097]]
- [[_COMMUNITY_Community 2098|Community 2098]]
- [[_COMMUNITY_Community 2099|Community 2099]]
- [[_COMMUNITY_Community 2100|Community 2100]]
- [[_COMMUNITY_Community 2101|Community 2101]]
- [[_COMMUNITY_Community 2102|Community 2102]]
- [[_COMMUNITY_Community 2103|Community 2103]]
- [[_COMMUNITY_Community 2104|Community 2104]]
- [[_COMMUNITY_Community 2105|Community 2105]]
- [[_COMMUNITY_Community 2106|Community 2106]]
- [[_COMMUNITY_Community 2107|Community 2107]]
- [[_COMMUNITY_Community 2108|Community 2108]]
- [[_COMMUNITY_Community 2109|Community 2109]]
- [[_COMMUNITY_Community 2110|Community 2110]]
- [[_COMMUNITY_Community 2111|Community 2111]]
- [[_COMMUNITY_Community 2112|Community 2112]]
- [[_COMMUNITY_Community 2113|Community 2113]]
- [[_COMMUNITY_Community 2114|Community 2114]]
- [[_COMMUNITY_Community 2115|Community 2115]]
- [[_COMMUNITY_Community 2116|Community 2116]]
- [[_COMMUNITY_Community 2117|Community 2117]]
- [[_COMMUNITY_Community 2118|Community 2118]]
- [[_COMMUNITY_Community 2119|Community 2119]]
- [[_COMMUNITY_Community 2120|Community 2120]]
- [[_COMMUNITY_Community 2121|Community 2121]]
- [[_COMMUNITY_Community 2122|Community 2122]]
- [[_COMMUNITY_Community 2123|Community 2123]]
- [[_COMMUNITY_Community 2144|Community 2144]]
- [[_COMMUNITY_Community 2145|Community 2145]]
- [[_COMMUNITY_Community 2146|Community 2146]]
- [[_COMMUNITY_Community 2147|Community 2147]]
- [[_COMMUNITY_Community 2148|Community 2148]]
- [[_COMMUNITY_Community 2149|Community 2149]]
- [[_COMMUNITY_Community 2150|Community 2150]]
- [[_COMMUNITY_Community 2151|Community 2151]]
- [[_COMMUNITY_Community 2153|Community 2153]]
- [[_COMMUNITY_Community 2154|Community 2154]]
- [[_COMMUNITY_Community 2155|Community 2155]]
- [[_COMMUNITY_Community 2156|Community 2156]]
- [[_COMMUNITY_Community 2157|Community 2157]]
- [[_COMMUNITY_Community 2158|Community 2158]]
- [[_COMMUNITY_Community 2159|Community 2159]]
- [[_COMMUNITY_Community 2160|Community 2160]]
- [[_COMMUNITY_Community 2223|Community 2223]]
- [[_COMMUNITY_Community 2224|Community 2224]]
- [[_COMMUNITY_Community 2225|Community 2225]]
- [[_COMMUNITY_Community 2226|Community 2226]]
- [[_COMMUNITY_Community 2227|Community 2227]]
- [[_COMMUNITY_Community 2228|Community 2228]]
- [[_COMMUNITY_Community 2229|Community 2229]]
- [[_COMMUNITY_Community 2230|Community 2230]]
- [[_COMMUNITY_Community 2231|Community 2231]]
- [[_COMMUNITY_Community 2233|Community 2233]]
- [[_COMMUNITY_Community 2234|Community 2234]]
- [[_COMMUNITY_Community 2238|Community 2238]]
- [[_COMMUNITY_Community 2240|Community 2240]]
- [[_COMMUNITY_Community 2241|Community 2241]]
- [[_COMMUNITY_Community 2242|Community 2242]]
- [[_COMMUNITY_Community 2244|Community 2244]]
- [[_COMMUNITY_Community 2245|Community 2245]]
- [[_COMMUNITY_Community 2246|Community 2246]]
- [[_COMMUNITY_Community 2249|Community 2249]]
- [[_COMMUNITY_Community 2250|Community 2250]]
- [[_COMMUNITY_Community 2251|Community 2251]]
- [[_COMMUNITY_Community 2254|Community 2254]]

## God Nodes (most connected - your core abstractions)
1. `cn()` - 640 edges
2. `useWorkspaceId()` - 344 edges
3. `ApiClient` - 244 edges
4. `newRequest()` - 217 edges
5. `Button()` - 214 edges
6. `api` - 194 edges
7. `useAuthStore` - 181 edges
8. `useWorkspacePaths()` - 139 edges
9. `useNavigation()` - 128 edges
10. `parseUUIDOrBadRequest()` - 127 edges

## Surprising Connections (you probably didn't know these)
- `Generated Sofa Sprite (Background Removed, Beige Fabric + Navy Accent)` --references--> `Virtual Office Sofa/Couch (Isometric, Beige/Navy Two-Seater)`  [EXTRACTED]
  generated_images/sprite-sofa-nobg.png → PRD/agent-virtual-office.md
- `Generated Sofa Sprite (1024x1024 RGB PNG, Gray BG, Dark Navy Blue Two-Seater Couch)` --references--> `Virtual Office Sofa/Couch (Isometric, Beige/Navy Two-Seater)`  [EXTRACTED]
  generated_images/sprite-sofa.png → PRD/agent-virtual-office.md
- `Generated Whiteboard Sprite (1024x1024 RGB PNG, Gray BG, Colorful Sticky Notes + Diagrams, Wheeled Stand)` --references--> `Virtual Office Whiteboard (Isometric, Metal Frame + Sticky Notes + Diagrams on Wheeled Stand)`  [EXTRACTED]
  generated_images/sprite-whiteboard.png → PRD/demo.html
- `Generated Whiteboard Sprite (Background Removed, Transparent BG)` --references--> `Virtual Office Whiteboard (Isometric, Metal Frame + Sticky Notes + Diagrams on Wheeled Stand)`  [EXTRACTED]
  generated_images/sprite-whiteboard-nobg.png → PRD/demo.html
- `withGuard()` --calls--> `fn`  [INFERRED]
  apps/desktop/src/main/daemon-manager.ts → packages/core/platform/workspace-storage.test.ts

## Import Cycles
- 1-file cycle: `packages/views/common/actor-avatar.tsx -> packages/views/common/actor-avatar.tsx`
- 2-file cycle: `packages/views/common/actor-avatar.tsx -> packages/views/members/member-profile-card.tsx -> packages/views/common/actor-avatar.tsx`
- 3-file cycle: `packages/views/issues/components/index.ts -> packages/views/issues/components/issues-page.tsx -> packages/views/issues/components/issues-header.tsx -> packages/views/issues/components/index.ts`

## Hyperedges (group relationships)
- **Office Furniture Sprites Collection** — desk_furniture_sprite, sofa_furniture_sprite, whiteboard_furniture_sprite, coffee_prop_sprite, assets_sprites_sprite_plant_png [INFERRED 0.85]
- **Agent Animation State Machine Frames** — walking_animation, typing_animation, resting_animation, meeting_animation, offline_animation [EXTRACTED 1.00]
- **Issue Status Workflow** — status_backlog, status_todo, status_in_progress, status_in_review, status_done, status_blocked, status_cancelled [EXTRACTED 1.00]
- **Agent Activation Methods** — concept_issue_assignee, concept_agent_mention [EXTRACTED 1.00]
- **Workspace Roles Hierarchy** — role_owner, role_admin, role_member [EXTRACTED 1.00]
- **Project Properties** — concept_project_lead, concept_project_status, concept_project_priority, concept_project_progress, concept_project_resource, concept_project_pin [EXTRACTED 1.00]
- **Squad System — Named Agent Group with Leader Routing** — squad_concept, squad_leader, squad_members, squad_assignment_flow, squad_operating_protocol, squad_roster, squad_instructions, squad_trigger_rules, squad_archive, squad_at_mention, squad_reassignment, squad_permissions, squad_evaluation_cli [EXTRACTED 1.00]
- **Task System — Agent Execution Unit with State Machine and Retry** — task_concept, task_state_machine, task_queued_state, task_dispatched_state, task_running_state, task_completed_state, task_failed_state, task_cancelled_state, task_timeout, task_retry_policy, task_retryable_reasons, task_non_retryable_reasons, task_autopilot_no_retry, task_manual_rerun, task_automatic_retry, task_session_resumption, task_issue_status_rollback, task_trigger_sources [EXTRACTED 1.00]
- **Workspace System — Self-Contained Collaboration Space** — workspace_concept, workspace_creation, workspace_slug, workspace_issue_prefix, workspace_issue_numbers, issue_number_sequence, workspace_deletion, workspace_ownership_transfer [EXTRACTED 1.00]
- **Troubleshooting Guide — Top 7 Self-Hosting Issues** — trouble_daemon_connect, trouble_tasks_stuck_queued, trouble_websocket, trouble_emails, trouble_dev_verification_code, trouble_port_conflict, trouble_log_locations, trouble_daemon_connect_causes, trouble_tasks_stuck_causes, trouble_websocket_causes, trouble_emails_causes, AICORTEX_SERVER_URL, FRONTEND_ORIGIN, RESEND_API_KEY, AICORTEX_DEV_VERIFICATION_CODE, APP_ENV [EXTRACTED 1.00]
- **Self-Hosting Configuration Variables** — AICORTEX_SERVER_URL, FRONTEND_ORIGIN, RESEND_API_KEY, AICORTEX_DEV_VERIFICATION_CODE, APP_ENV [EXTRACTED 1.00]
- **Generated Virtual Office Furniture Sprite Collection (Chunk 21)** — genimg_sprite_coffee_png, genimg_sprite_coffee_nobg_png, genimg_sprite_desk_png, genimg_sprite_desk_nobg_png, genimg_sprite_desk_black_png, genimg_sprite_plant_png, genimg_sprite_plant_nobg_png, genimg_sprite_sofa_nobg_png [EXTRACTED 1.00]
- **Background Removal Post-Processing Pipeline** — genimg_sprite_coffee_nobg_png, genimg_sprite_desk_nobg_png, genimg_sprite_plant_nobg_png, genimg_sprite_sofa_nobg_png [EXTRACTED 1.00]
- **Virtual Office Furniture Sprites (Coffee, Desk, Plant, Sofa)** — virtual_office_coffee_machine_concept, virtual_office_desk_concept, virtual_office_plant_concept, virtual_office_sofa_concept [EXTRACTED 1.00]
- **Generated Virtual Office Furniture Sprite Collection (Chunk 22 - Sofa Original + Whiteboard)** — genimg_sprite_sofa_png, genimg_sprite_whiteboard_png, genimg_sprite_whiteboard_nobg_png [EXTRACTED 1.00]
- **Background Removal Post-Processing Pipeline (Whiteboard)** — genimg_sprite_whiteboard_nobg_png [EXTRACTED 1.00]
- **Virtual Office Furniture Entities (Sofa + Whiteboard Concepts)** — virtual_office_sofa_concept, virtual_office_whiteboard_concept [EXTRACTED 1.00]
- **Sofa Sprite Generation Lineage (Original + Nobg Variant)** — genimg_sprite_sofa_png, genimg_sprite_sofa_nobg_png [EXTRACTED 1.00]
- **Whiteboard Sprite Generation Lineage (Original + Background Removed)** — genimg_sprite_whiteboard_png, genimg_sprite_whiteboard_nobg_png [EXTRACTED 1.00]

## Communities (2246 total, 113 thin omitted)

### Community 0 - "API Error Handling & Client Setup"
Cohesion: 0.04
Nodes (45): workspace, confirm_action, confirm_cancel, context_label, context_placeholder, danger_zone, delete_button, delete_description (+37 more)

### Community 1 - "Virtual Office Presence System"
Cohesion: 0.04
Nodes (91): useCreateAutopilot(), useCreateAutopilotTrigger(), useDeleteAutopilot(), useDeleteAutopilotTrigger(), useTriggerAutopilot(), useUpdateAutopilot(), useUpdateAutopilotTrigger(), Page() (+83 more)

### Community 2 - "Desktop App Shell & Window Management"
Cohesion: 0.02
Nodes (162): useWorkspacePresencePrefetch(), api, useAuthStore, CliCallbackConfig, GoogleAuthConfig, LoginPage(), LoginPageProps, redirectToCliCallback() (+154 more)

### Community 3 - "API Client & CRUD Operations"
Cohesion: 0.05
Nodes (46): DesignCommentQueuePanel(), DesignCommentToolbar(), MODE_TOOLS, ZOOM_STEPS, ColorSwatch(), DesignElementInspector(), DesignViewport, StageOverlayBounds (+38 more)

### Community 4 - "Workspace Presence & Agent Detail"
Cohesion: 0.02
Nodes (8): ApiClient, parseWithFallback(), CreateEditChatShareModalProps, AgentTask, ChatSession, ChatShareLink, DesignSession, User

### Community 5 - "Issue Actions & Dropdown Menus"
Cohesion: 0.09
Nodes (91): assertAutopilotDuplicateRunSkipped(), assertAutopilotNotFailureMonitorCandidate(), assertJSONEqual(), cleanupHandlerTestFixture(), createHandlerTestAgent(), createHandlerTestTaskForAgent(), createVerificationCodeForTest(), fetchAgentMcpConfig() (+83 more)

### Community 6 - "Issue Context Menus & Primitives"
Cohesion: 0.16
Nodes (27): ContextBadge(), DaemonPanel(), DaemonPanelProps, DisplayItem, EmptyState(), FilterChip(), GroupRows(), highlight() (+19 more)

### Community 7 - "File Upload & Emoji Picker"
Cohesion: 0.03
Nodes (191): MenuPrimitives, ActorIssuesPanel(), ActorIssuesSkeleton(), SCOPE_VALUES, TaskActorType, PropRow(), BatchActionToolbar(), assigneeGroupId() (+183 more)

### Community 8 - "Visibility & Permission Labels"
Cohesion: 0.48
Nodes (5): mockAgents, mockAuthValue, mockMembers, mockUser, mockWorkspace

### Community 9 - "Cross-Workspace Test Fixtures"
Cohesion: 0.04
Nodes (108): FileUploadButton(), FileUploadButtonProps, PillButton(), SubmitButton(), AvatarPickerProps, ChatInputProps, logger, AttachmentList() (+100 more)

### Community 10 - "Agent & Autopilot Creation"
Cohesion: 0.13
Nodes (32): IssueActionsContextMenu(), IssueActionsContextMenuProps, IssueActionsDropdown(), IssueActionsDropdownProps, contextPrimitives, dropdownPrimitives, IssueActionsMenuItems(), IssueActionsMenuItemsProps (+24 more)

### Community 11 - "Issue URL Sync & Reply Input"
Cohesion: 0.02
Nodes (175): taskStatusConfig, agentTasksOptions(), AgentActivity, summarizeActivityWindow(), useWorkspaceActivityMap(), CapabilityBanner(), getCopy(), Reason (+167 more)

### Community 12 - "Agent Presence & Run Counts"
Cohesion: 0.02
Nodes (222): ApiError, ChatShareManagementPage(), CreateEditChatShareModal(), logger, useCreateChatShareLink(), useDeleteChatShareLink(), useUpdateChatShareLink(), chatShareKeys (+214 more)

### Community 13 - "Agent UI Section & Filters"
Cohesion: 0.03
Nodes (168): NotFound(), EmojiPicker, QUICK_EMOJIS, QuickEmojiPicker(), QuickEmojiPickerProps, AgentDetailInspector(), AvatarEditor(), DescriptionEditor() (+160 more)

### Community 14 - "Task Status & Agent Activity"
Cohesion: 0.04
Nodes (108): AuthState, collectThreadReplies(), githubKeys, useIssueReactions(), useIssueSubscribers(), commentToTimelineEntry(), cacheUpdates, first (+100 more)

### Community 15 - "Comments & Timeline"
Cohesion: 0.08
Nodes (66): inAgentExecutionContext(), init(), newAPIClient(), normalizeAPIBaseURL(), parseCustomArgs(), parseCustomEnv(), requireWorkspaceID(), resolveCustomEnv() (+58 more)

### Community 16 - "Analytics & API Instance"
Cohesion: 0.03
Nodes (149): useAgentPresenceDetail(), useWorkspaceAgentAvailability(), WorkspaceAgentAvailability, apiErrorStatus(), isChatSessionNotFound(), EnsureChatSessionOptions, invalidateStaleChatSession(), useEnsureChatSession() (+141 more)

### Community 17 - "Utility & Regex Tests"
Cohesion: 0.02
Nodes (110): exports, ./agents, ./agents/derive-presence, ./agents/queries, ./agents/stores, ./agents/use-agent-presence, ./agents/visibility-label, ./analytics (+102 more)

### Community 18 - "Module Exports (Agents)"
Cohesion: 0.02
Nodes (123): link, makeAgent(), makeTask(), mockAgents, mockIssue, mockPresence, mockSnapshot, renderCard() (+115 more)

### Community 19 - "Agent Query Key Factories"
Cohesion: 0.06
Nodes (85): buildOpenclawArgs(), checkOpenclawVersion(), compareOpenclawVersion(), customArgsContains(), openclawInt64(), openclawInt64FirstOf(), parseOpenclawUsage(), parseOpenclawVersion() (+77 more)

### Community 20 - "File Upload Handlers"
Cohesion: 0.05
Nodes (99): agentActivityKeys, agentRunCountsKeys, agentTasksKeys, agentTaskSnapshotKeys, agentTemplateDetailOptions(), agentTemplateKeys, agentTemplateListOptions(), setApiInstance() (+91 more)

### Community 21 - "OpenClaw Runtime Args"
Cohesion: 0.10
Nodes (85): readFile(), codexSandboxPolicyFor(), Prepare(), ReadGCMeta(), discardLogger(), TestCleanupPreservesLogs(), TestCodexSandboxPolicyFor(), TestEnsureCodexSandboxConfigCreatesDefaultLinux() (+77 more)

### Community 22 - "Onboarding & Starter Content"
Cohesion: 0.17
Nodes (17): parseUUIDOrBadRequest(), commentMentionsAnyone(), squadMemberToResponse(), squadToResponse(), SquadMemberResponse, SquadResponse, Handler, Request (+9 more)

### Community 23 - "Codex Runtime & Sandbox"
Cohesion: 0.21
Nodes (8): TestStderrTailForwardsAndCapturesTail(), fakeStdin, newStderrTail(), withAgentStderr(), stderrTail, Mutex, Mutex, Writer

### Community 24 - "Private Agent Permission Tests"
Cohesion: 0.18
Nodes (32): ParseCommand(), errTest, noopEventBus, NewPortPool(), contains(), TestFormatEnvStatus_Failed(), TestFormatEnvStatus_NoPort(), TestFormatEnvStatus_Ready() (+24 more)

### Community 25 - "Community 25"
Cohesion: 0.08
Nodes (61): AgentSpriteData, Bubble, buildPath(), createSprite(), randomIdleTimer(), SpriteState, tickSprite(), transitionSprite() (+53 more)

### Community 26 - "Community 26"
Cohesion: 0.07
Nodes (46): AICortexMark(), baseOptions, externalLinkText(), GitHubMark(), sitemap(), DocsSettings(), Byline(), NumberedCard() (+38 more)

### Community 27 - "Community 27"
Cohesion: 0.11
Nodes (31): VISIBILITY_DESCRIPTION, VISIBILITY_LABEL, VISIBILITY_TOOLTIP, visibilityLabel(), LabelsPanel(), VisibilityBadge(), Filter, FilterButton() (+23 more)

### Community 28 - "Community 28"
Cohesion: 0.04
Nodes (62): Backend, Browser, chatACPPool, canonicalTimezoneName(), convertProjectResourcesForEnv(), convertReposForEnv(), convertSkillsForEnv(), defaultArgsForProvider() (+54 more)

### Community 29 - "Community 29"
Cohesion: 0.10
Nodes (54): bufferWriter, extractACPSessionID(), newACPProviderErrorSniffer(), resolveResumedSessionID(), fakeHermesACPRateLimitScript(), fakeHermesACPTransientRetryScript(), TestBuildHermesSessionParamsIncludesModel(), TestBuildHermesSessionParamsOmitsEmptyModel() (+46 more)

### Community 30 - "Community 30"
Cohesion: 0.06
Nodes (52): blockedArgMode, buildClaudeInput(), buildEnv(), detectCLIVersion(), extractVersionLine(), filterCustomArgs(), isFilteredChildEnvKey(), mergeEnv() (+44 more)

### Community 31 - "Community 31"
Cohesion: 0.02
Nodes (116): account, click_avatar_hint, name_label, save, saving, section_profile, toast_avatar_failed, toast_avatar_updated (+108 more)

### Community 32 - "Community 32"
Cohesion: 0.13
Nodes (25): contains(), generateCode(), isDevVerificationCode(), isProductionEnv(), isSixDigitCode(), signupSourceFromRequest(), userToResponse(), GoogleLoginRequest (+17 more)

### Community 33 - "Community 33"
Cohesion: 0.11
Nodes (60): acpDiscoveryProvider, Model, modelCacheEntry, cachedDiscovery(), claudeStaticModels(), codexStaticModels(), copilotStaticModels(), cursorStaticModels() (+52 more)

### Community 34 - "Community 34"
Cohesion: 0.03
Nodes (113): agentRunCounts30dOptions(), ActorAvatarHoverCardShell(), ActorAvatarProfileLink(), AgentAvatarHoverCard(), AgentHoverCardVariant, AgentStatusDot(), MemberAvatarHoverCard(), EmojiPicker() (+105 more)

### Community 35 - "Community 35"
Cohesion: 0.08
Nodes (64): scopeCall, Response, T, Context, M, Pool, Response, T (+56 more)

### Community 36 - "Community 36"
Cohesion: 0.10
Nodes (65): blockingBackend, NewClient(), mergeUsage(), newWorkspaceState(), createDaemonTestRepo(), newRepoReadyTestDaemon(), newTestDaemon(), TestBuildPromptAutopilotRunOnly() (+57 more)

### Community 37 - "Community 37"
Cohesion: 0.04
Nodes (56): detail, activity_section, add_property_action, add_sub_issue_aria, add_sub_issue_tooltip, add_sub_issues, agents_group, archive_tooltip (+48 more)

### Community 38 - "Community 38"
Cohesion: 0.11
Nodes (60): claimRuntimeGuardTask, claimTaskForRuntimeGuard(), createRuntimeGuardAgent(), createRuntimeGuardRuntime(), newDaemonTokenRequest(), setHandlerTestWorkspaceRepos(), setupForeignWorkspaceFixture(), TestCancelTask_CrossWorkspace_Returns404() (+52 more)

### Community 39 - "Community 39"
Cohesion: 0.16
Nodes (13): ClientIdentity, DaemonHeartbeatAckPayload, designSystemRef, ParseDesignSystemResource(), taskToResponse(), isNotFound(), RawMessage, AgentRuntime (+5 more)

### Community 40 - "Community 40"
Cohesion: 0.09
Nodes (35): chatSessionFromAllListRow(), chatSessionFromListRow(), chatSessionToResponse(), chatShareLinkToResponse(), generateToken(), ChatShareLinkResponse, CreateChatShareLinkRequest, ctxWorkspaceID() (+27 more)

### Community 41 - "Community 41"
Cohesion: 0.04
Nodes (56): dependencies, @aicortex/core, @aicortex/ui, @aicortex/views, @base-ui/react, class-variance-authority, clsx, cmdk (+48 more)

### Community 42 - "Community 42"
Cohesion: 0.04
Nodes (56): detail, activity_section, add_property_action, add_sub_issue_aria, add_sub_issue_tooltip, add_sub_issues, agents_group, archive_tooltip (+48 more)

### Community 43 - "Community 43"
Cohesion: 0.08
Nodes (48): detectOutboundIP(), init(), openBrowser(), resolveAppURL(), resolveCallbackBinding(), resolveToken(), runAuthLogin(), runAuthLoginBrowser() (+40 more)

### Community 44 - "Community 44"
Cohesion: 0.03
Nodes (160): AgentOverviewPane(), AgentOverviewPaneProps, DetailTab, detailTabs, TAB_LABEL_KEY, TabContent(), AgentRowActions(), AgentRowActionsProps (+152 more)

### Community 45 - "Community 45"
Cohesion: 0.16
Nodes (25): CustomPricingDialog(), ChartLegend(), computeTotals(), CostByBlock(), CostByList(), DailyBreakdownTable(), DailyMetric, DailyTab() (+17 more)

### Community 46 - "Community 46"
Cohesion: 0.12
Nodes (63): ActivityLog, Agent, AgentRuntime, AgentSkill, AgentTaskQueue, Attachment, Autopilot, AutopilotRun (+55 more)

### Community 47 - "Community 47"
Cohesion: 0.09
Nodes (44): BatchDeleteIssuesRequest, BatchUpdateIssuesRequest, CreateIssueRequest, groupedIssueRow, GroupedIssuesResponse, uuidSliceToStrings(), assigneeGroupID(), duplicateIssueMessage() (+36 more)

### Community 48 - "Community 48"
Cohesion: 0.07
Nodes (39): geistMono, HTML_LANG, inter, isSupportedLocale(), metadata, RootLayout(), sourceSerif, viewport (+31 more)

### Community 49 - "Community 49"
Cohesion: 0.19
Nodes (13): redisImportEnvelope, RedisLocalSkillImportStore, RedisLocalSkillListStore, localSkillImportKey(), localSkillImportPendingKey(), localSkillListKey(), localSkillListPendingKey(), RuntimeLocalSkillSummary (+5 more)

### Community 50 - "Community 50"
Cohesion: 0.20
Nodes (20): createTabRouter(), createId(), defaultPathFor(), defaultTabFor(), extractWorkspaceSlug(), findTabLocation(), getActiveTab(), makeTab() (+12 more)

### Community 51 - "Community 51"
Cohesion: 0.05
Nodes (49): commands, copy_identifier, copy_issue_link, current_theme_aria, new_issue, new_project, switch_to_dark, switch_to_light (+41 more)

### Community 52 - "Community 52"
Cohesion: 0.12
Nodes (51): ActiveProfile, bootstrapCli(), bundledCliPath(), clearToken(), configWriteChain, DEFAULT_PREFS, deriveProfileName(), desktopSpawnEnv() (+43 more)

### Community 53 - "Community 53"
Cohesion: 0.04
Nodes (56): PreferencesTab(), I18nWrapper(), mockPersist, mockReload, mockToastWarning, mockUpdateMe, TEST_RESOURCES, useAuthStore (+48 more)

### Community 54 - "Community 54"
Cohesion: 0.08
Nodes (57): eventResult, ioErrReader, extractToolOutput(), opencodeWindowsPackageCandidates(), resolveOpenCodeNativeFromShim(), equalStringSlice(), fakeStat(), TestExtractToolOutputNil() (+49 more)

### Community 55 - "Community 55"
Cohesion: 0.13
Nodes (30): defaultRenderMention(), MarkdownProps, MemoizedMarkdown, out, parser, CodeBlock(), CodeBlockProps, InlineCode() (+22 more)

### Community 56 - "Community 56"
Cohesion: 0.04
Nodes (51): accepted, redirecting, title, batch, empty_continue, empty_hint, empty_title, error_generic (+43 more)

### Community 57 - "Community 57"
Cohesion: 0.08
Nodes (28): Level, Init(), isTerminal(), NewLogger(), parseLevel(), TestDBCollectorExposesPoolStats(), TestHTTPMiddlewareSkipsHealthProbePaths(), TestHTTPMiddlewareUsesRoutePatternLabels() (+20 more)

### Community 58 - "Community 58"
Cohesion: 0.10
Nodes (43): ghInstallationPayload, ghPullRequestPayload, coalesce(), derivePRState(), extractIdentifiers(), fetchInstallationAccount(), githubAppSlug(), githubInstallationToResponse() (+35 more)

### Community 59 - "Community 59"
Cohesion: 0.08
Nodes (30): ArchiveAgentParams, ArchiveAgentsByRuntimeParams, CancelAgentTasksByIssueAndAgentParams, CompleteAgentTaskParams, CreateAgentParams, CreateAgentTaskParams, CreateQuickCreateTaskParams, ExpireStaleQueuedTasksParams (+22 more)

### Community 60 - "Community 60"
Cohesion: 0.04
Nodes (47): dependencies, @aicortex/core, @aicortex/ui, @base-ui/react, cmdk, @dnd-kit/core, @dnd-kit/sortable, @dnd-kit/utilities (+39 more)

### Community 61 - "Community 61"
Cohesion: 0.04
Nodes (48): exports, ./agents, ./auth, ./autopilots/components, ./chat, ./chat-share/public, ./common/actor-avatar, ./common/markdown (+40 more)

### Community 62 - "Community 62"
Cohesion: 0.05
Nodes (44): accepted, redirecting, title, batch, empty_continue, empty_hint, empty_title, error_generic (+36 more)

### Community 63 - "Community 63"
Cohesion: 0.05
Nodes (44): step_workspace, create_failed_toast, create_new_subtitle, create_new_title, cta_create_named, cta_create_workspace, cta_creating, cta_open (+36 more)

### Community 64 - "Community 64"
Cohesion: 0.14
Nodes (34): Cache, bareDirName(), bareHeadBranch(), ensureRemoteTrackingLayout(), excludeFromGit(), getRemoteDefaultBranch(), gitCloneBare(), gitEnv() (+26 more)

### Community 65 - "Community 65"
Cohesion: 0.05
Nodes (44): step_workspace, create_failed_toast, create_new_subtitle, create_new_title, cta_create_named, cta_create_workspace, cta_creating, cta_open (+36 more)

### Community 66 - "Community 66"
Cohesion: 0.22
Nodes (24): buildClaudeArgs(), mustMarshal(), TestBuildClaudeArgsBlocksMcpConfig(), TestBuildClaudeArgsExtraArgsBeforeCustomArgsAndFiltersBoth(), TestBuildClaudeArgsFiltersBlockedCustomArgs(), TestBuildClaudeArgsIncludesStrictMCPConfig(), TestBuildClaudeArgsPassesThroughCustomArgs(), TestBuildClaudeInputEncodesUserMessage() (+16 more)

### Community 67 - "Community 67"
Cohesion: 0.18
Nodes (39): assigneeKinds, assigneeMatch, issueDisplayKey(), loadActorDisplayLookup(), resolveIssueRef(), activeDuplicateIssueCreateMessage(), ambiguousAssigneeError(), formatAssignee() (+31 more)

### Community 68 - "Community 68"
Cohesion: 0.07
Nodes (23): after, authedFetch(), form, pngBytes, sendBody, uploaded, UploadRow, userId (+15 more)

### Community 69 - "Community 69"
Cohesion: 0.14
Nodes (39): canAssignAgentToIssue(), canChangeMemberRole(), canDeleteComment(), canDeleteRuntime(), canDeleteSkill(), canDeleteWorkspace(), canEditAgent(), canEditComment() (+31 more)

### Community 70 - "Community 70"
Cohesion: 0.06
Nodes (54): CommandHandler, EmailService, Config, dbExecutor, ctxMember(), int8ToPtr(), New(), parseUUID() (+46 more)

### Community 71 - "Community 71"
Cohesion: 0.20
Nodes (14): API Response Compatibility Rules, Testing Infrastructure (Vitest, Playwright, Go test), Backend UUID Parsing Convention, Development PostgreSQL Docker Compose, Self-Host Docker Compose Stack, Production vs Development Mode Comparison, Go Backend (Chi Router), Next.js Frontend (App Router) (+6 more)

### Community 72 - "Community 72"
Cohesion: 0.06
Nodes (35): ArtifactRelayHandler, client, ClientIdentity, HeartbeatHandler, Hub, mustMarshalRaw(), NewHub(), taskAvailableFrame() (+27 more)

### Community 73 - "Community 73"
Cohesion: 0.05
Nodes (41): add_member_dialog, cancel, description, label_member, label_optional, label_role, placeholder_role_inline, title (+33 more)

### Community 74 - "Community 74"
Cohesion: 0.10
Nodes (28): AttachLabelRequest, CreateLabelRequest, issueReactionToResponse(), IssueReactionResponse, labelsToResponse(), labelToResponse(), normalizeColor(), validateLabelName() (+20 more)

### Community 75 - "Community 75"
Cohesion: 0.40
Nodes (3): ChatSharePublicView(), ChatSharePublicViewProps, ChatSharePublicInfo

### Community 76 - "Community 76"
Cohesion: 0.05
Nodes (41): add_member_dialog, cancel, description, label_member, label_optional, label_role, placeholder_role_inline, title (+33 more)

### Community 77 - "Community 77"
Cohesion: 0.05
Nodes (41): usage, cost_by_caption_agent_one, cost_by_caption_agent_other, cost_by_caption_model_one, cost_by_caption_model_other, cost_by_tab_agent, cost_by_tab_model, cost_by_title_agent (+33 more)

### Community 78 - "Community 78"
Cohesion: 0.14
Nodes (48): drainRPCScript(), newTestCodexClient(), splitLines(), TestBuildCodexArgsExtraArgsBeforeCustomArgsAndFiltersBoth(), TestCodexCloseAllPending(), TestCodexExecuteSurfacesStderrWhenChildExitsEarly(), TestCodexHandleInvalidJSON(), TestCodexHandleResponseError() (+40 more)

### Community 79 - "Community 79"
Cohesion: 0.09
Nodes (29): AgentSessionResumeFields, needsTerminalBootstrap(), resolveAgentResumeId(), appendTerminalDetectBuffer(), extractAgentSessionIdFromTerminalOutput(), lastMatch(), normalizeProvider(), ChatTerminalPanel() (+21 more)

### Community 80 - "Community 80"
Cohesion: 0.16
Nodes (34): newFlagTestCmd(), newIssueCreateTestCmd(), pipeStdin(), TestActorDisplayLookupLazyLoads(), TestFetchAutopilotCandidatesPaginates(), TestFormatAssignee(), TestIssueSubscriberList(), TestIssueSubscriberMutationBody() (+26 more)

### Community 81 - "Community 81"
Cohesion: 0.03
Nodes (181): ApiClientIdentity, ApiClientOptions, ImportStarterContentResponse, ImportStarterWelcomeIssueTemplate, LoginResponse, PreviewTooLargeError, PreviewUnsupportedError, AgentTemplateSchema (+173 more)

### Community 82 - "Community 82"
Cohesion: 0.04
Nodes (48): avg_duration, cancel_failed_toast, cancel_task_aria, cancel_task_tooltip, cancelling_tooltip, dispatched_prefix, empty_30d, empty_now (+40 more)

### Community 83 - "Community 83"
Cohesion: 0.05
Nodes (40): blurb, instructions, label, blurb, instructions, label, blurb, instructions (+32 more)

### Community 84 - "Community 84"
Cohesion: 0.16
Nodes (40): CoreProperties, AgentCreated(), AgentTaskCancelled(), AgentTaskCompleted(), AgentTaskDispatched(), agentTaskEvent(), AgentTaskFailed(), AgentTaskQueued() (+32 more)

### Community 85 - "Community 85"
Cohesion: 0.12
Nodes (34): AgentSkillSummary, agentToResponse(), canViewAgentEnv(), computeTaskKind(), decodeJSONBodyWithRawFields(), TestComputeTaskKind(), redactEnv(), redactMcpConfig() (+26 more)

### Community 86 - "Community 86"
Cohesion: 0.07
Nodes (32): cli, authorize, authorizing, description, different_account, title, common, back (+24 more)

### Community 87 - "Community 87"
Cohesion: 0.06
Nodes (38): daily, metric_cost, metric_tasks, metric_time, metric_tokens, no_data, title_cost, title_tasks (+30 more)

### Community 88 - "Community 88"
Cohesion: 0.05
Nodes (39): detail, all_runtimes, delete_aria, delete_button, delete_tooltip, diagnostics_cli, diagnostics_timezone, diagnostics_title (+31 more)

### Community 89 - "Community 89"
Cohesion: 0.06
Nodes (38): daily, metric_cost, metric_tasks, metric_time, metric_tokens, no_data, title_cost, title_tasks (+30 more)

### Community 90 - "Community 90"
Cohesion: 0.06
Nodes (38): commands, copy_identifier, copy_issue_link, current_theme_aria, new_issue, new_project, switch_to_dark, switch_to_light (+30 more)

### Community 91 - "Community 91"
Cohesion: 0.05
Nodes (38): description, label, description, label, members, cannot_demote_last_owner, cannot_demote_last_owner_title, change_role (+30 more)

### Community 92 - "Community 92"
Cohesion: 0.09
Nodes (63): clawhubFileEntry, clawhubVersionDetail, githubTreeEntry, AgentSkillSummary, clawhubFileEntry, clawhubLatestVersion, clawhubSkill, clawhubVersionDetail (+55 more)

### Community 93 - "Community 93"
Cohesion: 0.06
Nodes (35): step_platform, cli_action, cli_dialog_connect, cli_dialog_description, cli_dialog_pick_hint, cli_dialog_title, cli_subtitle, cli_title (+27 more)

### Community 94 - "Community 94"
Cohesion: 0.04
Nodes (46): avg_duration, cancel_failed_toast, cancel_task_aria, cancel_task_tooltip, cancelling_tooltip, dispatched_prefix, empty_30d, empty_now (+38 more)

### Community 95 - "Community 95"
Cohesion: 0.05
Nodes (38): usage, cost_by_caption_agent_other, cost_by_caption_model_other, cost_by_tab_agent, cost_by_tab_model, cost_by_title_agent, cost_by_title_model, daily_breakdown_toggle (+30 more)

### Community 96 - "Community 96"
Cohesion: 0.15
Nodes (9): codexClient, Config, Context, ExecOptions, Mutex, pendingRPC, RawMessage, Session (+1 more)

### Community 97 - "Community 97"
Cohesion: 0.19
Nodes (45): addEmptyCommit(), cachedRepoHasFile(), createTestRepo(), createTestRepoAt(), currentBranchName(), gitConfigGet(), gitHead(), gitInfoExclude() (+37 more)

### Community 98 - "Community 98"
Cohesion: 0.12
Nodes (17): Client, Context, ReadCloser, T, looksLikeS3Hostname(), NewS3StorageFromEnv(), TestLooksLikeS3Hostname(), TestS3StorageKeyFromURL_CustomEndpointPreservesNestedKey() (+9 more)

### Community 99 - "Community 99"
Cohesion: 0.07
Nodes (75): buildCopilotArgs(), handleCopilotEvent(), newCopilotEventState(), parseCopilotEvent(), simulateCopilotEventLoop(), simulateCopilotEventLoopWithModel(), TestBuildCopilotArgsBaseline(), TestBuildCopilotArgsBlocksResumeAndACP() (+67 more)

### Community 100 - "Community 100"
Cohesion: 0.13
Nodes (28): captureDownloadInitiated(), captureDownloadPageViewed(), DownloadDetectPayload, DownloadInitiatedPayload, DownloadIntentSource, setPersonPropertiesOnce(), captureFeedbackOpened(), FeedbackOpenedSource (+20 more)

### Community 101 - "Community 101"
Cohesion: 0.11
Nodes (22): acpProviderErrorSniffer, buildHermesSessionParams(), extractACPToolCallText(), hermesToolNameFromTitle(), normalizeACPUpdate(), normalizeACPUpdateType(), parseToolArgsJSON(), promoteACPResultOnProviderError() (+14 more)

### Community 102 - "Community 102"
Cohesion: 0.06
Nodes (36): detail, all_runtimes, delete_aria, delete_button, delete_tooltip, diagnostics_cli, diagnostics_timezone, diagnostics_title (+28 more)

### Community 103 - "Community 103"
Cohesion: 0.06
Nodes (37): Agent, Agent Triggers, aicortex agent create, Agent Creator, Agent Creation Form, Archive, Assign Issue, Autopilot (Routines) (+29 more)

### Community 104 - "Community 104"
Cohesion: 0.05
Nodes (46): board, add_issue_tooltip, empty_column, empty_grouping, hidden_columns_label, hide_column, show_column, card (+38 more)

### Community 105 - "Community 105"
Cohesion: 0.07
Nodes (31): cli_install, copy_aria, intro, step1_label, step2_label, common, back, continue (+23 more)

### Community 106 - "Community 106"
Cohesion: 0.12
Nodes (28): CreateRuntimeLocalSkillImportRequest, InMemoryLocalSkillImportStore, InMemoryLocalSkillListStore, LocalSkillImportStore, LocalSkillListStore, reportedRuntimeLocalSkill, applyLocalSkillImportTimeout(), applyLocalSkillListTimeout() (+20 more)

### Community 107 - "Community 107"
Cohesion: 0.16
Nodes (15): AboutPage(), metadata, AboutPageClient(), FAQSection(), HowItWorksSection(), LandingFooter(), LandingHeader(), OpenSourceSection() (+7 more)

### Community 108 - "Community 108"
Cohesion: 0.06
Nodes (39): create_form, errors, name_label, name_placeholder, submit, submitting, url_label, url_placeholder (+31 more)

### Community 109 - "Community 109"
Cohesion: 0.06
Nodes (34): errors, move_failed, header, ascending, card_properties, descending, display_settings, filter_button (+26 more)

### Community 110 - "Community 110"
Cohesion: 0.11
Nodes (34): Agent Pixel Character Sprite (16x24px), Agent Character Sprite Sheet (Walking/Typing/Resting/Meeting/Offline), Coffee Cup / Break Room Decoration Sprite, Desk/Workstation Furniture Sprite with Blinking Screen, Office Plant Decoration Sprite, Sofa/Couch Furniture Sprite for Break Room, Meeting Room Whiteboard Sprite (2 Animation Frames), Coffee Cup Prop / Break Room Decoration (+26 more)

### Community 111 - "Community 111"
Cohesion: 0.20
Nodes (17): NewPATCache(), patCacheKey(), newRedisTestClient(), TestNewPATCache_NilRedisReturnsNil(), TestPATCache_NilSafe(), TestPATCache_Set_RespectsClampedTTL(), TestPATCache_SetGetInvalidate(), TestPATCache_TTL() (+9 more)

### Community 112 - "Community 112"
Cohesion: 0.18
Nodes (15): EventBus, BuildingStep, CloningStep, EventBus, InstallingStep, MigratingStep, Provisioner, isValidTransition() (+7 more)

### Community 113 - "Community 113"
Cohesion: 0.03
Nodes (76): add_child, description, title, toast_failed, toast_success, backlog_hint, toast_status_failed, common (+68 more)

### Community 114 - "Community 114"
Cohesion: 0.06
Nodes (34): step_platform, cli_action, cli_dialog_connect, cli_dialog_description, cli_dialog_pick_hint, cli_dialog_title, cli_subtitle, cli_title (+26 more)

### Community 115 - "Community 115"
Cohesion: 0.05
Nodes (51): archived, active_link, title, availability, all, offline, online, unstable (+43 more)

### Community 116 - "Community 116"
Cohesion: 0.06
Nodes (33): step_runtime, dialog_cancel, dialog_close, dialog_description, dialog_title, empty_headline, empty_lede_prefix, empty_lede_suffix (+25 more)

### Community 117 - "Community 117"
Cohesion: 0.09
Nodes (40): TestBuildCursorArgsNoChatSubcommand(), TestCursorACPBlockedArgs(), buildCursorArgs(), cursorErrorText(), normalizeCursorStreamLine(), TestBuildCursorArgs(), TestBuildCursorArgsCustomArgs(), TestBuildCursorArgsIgnoresSystemPromptAndMaxTurns() (+32 more)

### Community 118 - "Community 118"
Cohesion: 0.04
Nodes (54): archived, active_link, title, availability, all, offline, online, unstable (+46 more)

### Community 119 - "Community 119"
Cohesion: 0.05
Nodes (50): agent_live, cancel_failed, fallback_name, is_queued, is_working, queued_elapsed_prefix, stop_button, stop_tooltip (+42 more)

### Community 120 - "Community 120"
Cohesion: 0.11
Nodes (19): card1_actor_initial, card1_actor_name, card1_body_prefix, card1_body_suffix, card1_mention_content, card1_mention_research, card2_actor_name, card2_body (+11 more)

### Community 121 - "Community 121"
Cohesion: 0.09
Nodes (27): RefreshCloudFrontCookies(), Router, T, Bus, Client, Context, ForumAutoState, HeartbeatScheduler (+19 more)

### Community 122 - "Community 122"
Cohesion: 0.09
Nodes (25): DESIGN_EXAMPLES, DESIGN_INTENT_CHIPS, DESIGN_SUBCATEGORIES, DesignExample, designExampleBrief(), designExamplePreviewHtml(), designExamplePreviewPath(), DesignIntentChip (+17 more)

### Community 123 - "Community 123"
Cohesion: 0.06
Nodes (32): step_runtime, dialog_cancel, dialog_close, dialog_description, dialog_title, empty_headline, empty_lede_prefix, empty_lede_suffix (+24 more)

### Community 124 - "Community 124"
Cohesion: 0.18
Nodes (23): CommentBubble(), MeetingDetailPage(), ParticipantBadge(), EmptyState(), MeetingCard(), MeetingListPage(), MeetingSection(), STATUS_ORDER (+15 more)

### Community 125 - "Community 125"
Cohesion: 0.25
Nodes (22): runAgentAvatar(), freshAgentUpdateCmd(), TestAgentAvatarCaseInsensitiveExtension(), TestAgentAvatarHappyPath(), TestAgentAvatarMissingAgent(), TestAgentAvatarMissingFileFlag(), TestAgentAvatarNonexistentFile(), TestAgentAvatarOversizedFile() (+14 more)

### Community 126 - "Community 126"
Cohesion: 0.06
Nodes (31): dialog, auto_run_hint, cancel, close, collapse, create, creating, description_placeholder (+23 more)

### Community 127 - "Community 127"
Cohesion: 0.06
Nodes (31): create_issue, add_subissue, agent_breadcrumb, create_another, description_placeholder, manual_breadcrumb, more_options_aria, parent_with_id (+23 more)

### Community 128 - "Community 128"
Cohesion: 0.26
Nodes (25): NewForumAutoState(), newTestConfig(), TestCanReply_AntiSelfLoop(), TestCanReply_Cooldown(), TestConcurrentSafety(), TestDefaultAutoChatterConfig(), TestIsLastReplier(), TestIsNewPost() (+17 more)

### Community 129 - "Community 129"
Cohesion: 0.12
Nodes (20): EmptyClaimCache, AgentSkillData, AgentTaskQueue, ChatMessage, ChatSession, Client, Comment, Context (+12 more)

### Community 130 - "Community 130"
Cohesion: 0.06
Nodes (31): dialog, auto_run_hint, cancel, close, collapse, create, creating, description_placeholder (+23 more)

### Community 131 - "Community 131"
Cohesion: 0.06
Nodes (34): create_issue, add_subissue, agent_breadcrumb, create_another, description_placeholder, manual_breadcrumb, more_options_aria, parent_with_id (+26 more)

### Community 132 - "Community 132"
Cohesion: 0.07
Nodes (29): devDependencies, pg, @playwright/test, turbo, @types/node, @types/pg, typescript, name (+21 more)

### Community 133 - "Community 133"
Cohesion: 0.09
Nodes (23): AutopilotRunGCStatus, ChatSessionGCStatus, Client, isRuntimeNotFoundError(), isTaskNotFoundError(), isWorkspaceNotFoundError(), normalizeGOOS(), IssueGCStatus (+15 more)

### Community 134 - "Community 134"
Cohesion: 0.19
Nodes (32): handleRuntimeGoneFixture, multiProviderRegisterFixture, freshDaemon(), newHandleRuntimeGoneFixture(), newMultiProviderRegisterFixture(), stubAgentVersion(), TestHandleRuntimeGone_BackoffOnFailure(), TestHandleRuntimeGone_CoalescesConcurrentCallers() (+24 more)

### Community 135 - "Community 135"
Cohesion: 0.14
Nodes (30): concept-autopilot, concept-project, concept-project-lead, concept-project-pin, concept-project-resources, conventions-chinese-punctuation, conventions-chinese-style, conventions-code-comments (+22 more)

### Community 136 - "Community 136"
Cohesion: 0.08
Nodes (29): dependencies, @aicortex/ui, fumadocs-core, fumadocs-mdx, fumadocs-ui, lucide-react, mermaid, next (+21 more)

### Community 137 - "Community 137"
Cohesion: 0.17
Nodes (15): isPendingTaskInFlight(), mergePendingTaskOnEnqueue(), Props, UnicodeSpinner(), pickStageKeys(), Props, Stage, StageKey (+7 more)

### Community 138 - "Community 138"
Cohesion: 0.12
Nodes (15): Agent Identity, AICortex Agent Runtime, Attachments, Available Commands, Important: Always Use the `aicortex` CLI, Mentions, Output, Project Context (+7 more)

### Community 139 - "Community 139"
Cohesion: 0.07
Nodes (30): connect, back, cancel, create_agent, description, security_body, security_body_suffix, security_label (+22 more)

### Community 140 - "Community 140"
Cohesion: 0.10
Nodes (30): AdvanceTriggerNextRunParams, ClaimDueScheduleTriggersRow, CreateAutopilotParams, CreateAutopilotRunParams, CreateAutopilotTaskParams, CreateAutopilotTriggerParams, GetAutopilotInWorkspaceParams, ListAutopilotRunsParams (+22 more)

### Community 141 - "Community 141"
Cohesion: 0.07
Nodes (30): dependencies, @base-ui/react, class-variance-authority, clsx, cmdk, date-fns, embla-carousel-react, emoji-mart (+22 more)

### Community 142 - "Community 142"
Cohesion: 0.07
Nodes (30): create_squad, cancel, description, description_label, description_placeholder, group_members, group_my_agents, group_workspace_agents (+22 more)

### Community 143 - "Community 143"
Cohesion: 0.07
Nodes (30): connect, back, cancel, create_agent, description, security_body, security_body_suffix, security_label (+22 more)

### Community 144 - "Community 144"
Cohesion: 0.04
Nodes (45): workspace, confirm_action, confirm_cancel, context_label, context_placeholder, danger_zone, delete_button, delete_description (+37 more)

### Community 145 - "Community 145"
Cohesion: 0.12
Nodes (23): getAppVersion(), installContextMenu(), describeScheme(), downloadURLSafely(), getHttpProtocol(), isSafeExternalHttpUrl(), openExternalSafely(), BUNDLED_ICON_PATH (+15 more)

### Community 146 - "Community 146"
Cohesion: 0.11
Nodes (35): getCustomPricing(), aggregateByDate(), aggregateCostByAgent(), aggregateCostByHour(), aggregateCostByModel(), ARCH_LABEL, canonicalCandidates(), collectUnmappedModels() (+27 more)

### Community 147 - "Community 147"
Cohesion: 0.12
Nodes (44): ResolveUnderRoot(), TestResolveUnderRoot(), ReportIssueArtifacts(), ScanHTMLArtifacts(), mustWrite(), TestScanHTMLArtifacts(), ScanCandidate, createTaskDir() (+36 more)

### Community 148 - "Community 148"
Cohesion: 0.27
Nodes (24): NewBatchedHeartbeatScheduler(), TestBatchedHeartbeatScheduler_CoalescesAndFlushes(), TestBatchedHeartbeatScheduler_FlushIgnoresEmpty(), TestBatchedHeartbeatScheduler_OfflineFallsBackSync(), TestBatchedHeartbeatScheduler_RaceToOfflineSelfHeals(), TestBatchedHeartbeatScheduler_StopDrains(), TestBatchedHeartbeatScheduler_StopFlushesLateSchedule(), TestPassthroughHeartbeatScheduler_TouchAndRaceRecovery() (+16 more)

### Community 149 - "Community 149"
Cohesion: 0.14
Nodes (29): copyFile(), copyFileIfExists(), ensureDirSymlink(), ensureSymlink(), exposeSharedCodexPluginCache(), assertDirLinkTarget(), TestCopyFile(), TestCreateDirLink() (+21 more)

### Community 150 - "Community 150"
Cohesion: 0.17
Nodes (32): buildPerTaskOpenclawConfig(), execOpenclawCLI(), isOpenclawKeyMissing(), openclawActiveConfigPath(), openclawResolvedAgentsList(), prepareOpenclawConfig(), rewriteAgentsListWorkspaces(), installOpenclawStub() (+24 more)

### Community 151 - "Community 151"
Cohesion: 0.17
Nodes (20): AgentCostRow, AgentDashboardRow, aggregateAgentTokens(), aggregateDailyCost(), aggregateDailyTasks(), aggregateDailyTime(), aggregateDailyTokens(), computeDailyTotals() (+12 more)

### Community 152 - "Community 152"
Cohesion: 0.11
Nodes (23): CancelAgentTasksByRuntimeOrAgentParams, DeleteStaleOfflineRuntimesRow, FindLegacyRuntimesByDaemonIDParams, ForceOfflineRuntimesByIDsRow, GetAgentRuntimeForWorkspaceParams, ListAgentRuntimesByOwnerParams, MarkRuntimesOfflineByIDsParams, MarkRuntimesOfflineByIDsRow (+15 more)

### Community 153 - "Community 153"
Cohesion: 0.07
Nodes (29): comment, cancel_action, cancel_edit, collapse_tooltip, copied_toast, copy_action, delete_action, delete_desc (+21 more)

### Community 154 - "Community 154"
Cohesion: 0.15
Nodes (25): buildCodexArgs(), bytesContainsStr(), codexInt64(), codexSessionRoot(), describeCodexItemProgressActivity(), describeCodexSemanticActivity(), extractNestedString(), isCodexItemProgressActivity() (+17 more)

### Community 155 - "Community 155"
Cohesion: 0.07
Nodes (47): Claude Code, Claude Code, Codex, Cursor Agent, Agent Execution Model, Agent as First-Class Citizen, Gemini CLI, Gemini CLI (+39 more)

### Community 156 - "Community 156"
Cohesion: 0.11
Nodes (39): init(), runUpdate(), GitHubRelease, GitHubReleaseAsset, extractBinaryFromTarGz(), extractBinaryFromZip(), FetchLatestRelease(), fetchReleaseByTag() (+31 more)

### Community 157 - "Community 157"
Cohesion: 0.20
Nodes (26): AgentEntry, Config, ArtifactPatternsFromEnv(), buildLoginShellResolveScript(), isSafeAgentName(), LoadConfig(), NormalizeServerBaseURL(), patternsFromEnv() (+18 more)

### Community 158 - "Community 158"
Cohesion: 0.03
Nodes (57): page, empty, title, remove_label, card, agents_section, detail_link, more_agents_one (+49 more)

### Community 159 - "Community 159"
Cohesion: 0.14
Nodes (20): button, emailInput, getOTPInput(), I18nWrapper(), mockApiGetMe, mockApiIssueCliToken, mockApiListWorkspaces, mockApiSetToken (+12 more)

### Community 160 - "Community 160"
Cohesion: 0.26
Nodes (13): buildTunnelPreviewURL(), ChatWebPanel(), WebPreviewMode, DesignPreviewSourcePanel(), ModeToggle(), buildTunnelPreviewURL(), RuntimeTunnelPanel(), useRuntimeTunnelFeature() (+5 more)

### Community 161 - "Community 161"
Cohesion: 0.13
Nodes (29): TestSendChatMessage_InvalidAttachmentIDs(), TestSendChatMessage_LinksAttachments(), TestUpdateChatSession_RejectsBlank(), TestUpdateChatSession_RenamesTitle(), withChatTestWorkspaceCtx(), createHandlerTestChatSession(), newPreviewRequest(), seedPreviewAttachment() (+21 more)

### Community 162 - "Community 162"
Cohesion: 0.18
Nodes (26): ARCH_FLAGS, builderArgsForTarget(), bundleCliScript, deriveVersion(), desktopRoot, envWithLocalBins(), expandPlatformShorthand(), formatTarget() (+18 more)

### Community 163 - "Community 163"
Cohesion: 0.16
Nodes (18): Config, ConfigFromEnv(), IsLoopbackAddr(), TestIsLoopbackAddr(), Options, Client, Duration, T (+10 more)

### Community 164 - "Community 164"
Cohesion: 0.17
Nodes (21): CreateProjectResourceRequest, designSystemRef, githubRepoRef, localPathRef, isRemoteResourcePath(), isValidGitRepoURL(), parseUUIDLoose(), projectResourceToResponse() (+13 more)

### Community 165 - "Community 165"
Cohesion: 0.04
Nodes (47): 1.1 Open Design 是什么, 1.2 aicortex 现状, 1.3 TopNav 与 AppSidebar 的关系（现状）, 1. 背景, 2.1 设计目标, 2.2 推荐结构（方案 A，默认）, 2.3 二级菜单内容, 2.4 AppSidebar 映射（Desktop） (+39 more)

### Community 166 - "Community 166"
Cohesion: 0.07
Nodes (28): questionnaire, answered_progress, eyebrow, headline, learn_more, q1_other_placeholder, q1_question, q1_solo (+20 more)

### Community 167 - "Community 167"
Cohesion: 0.07
Nodes (33): cli_install, copy_aria, intro, step1_label, step2_label, cloud_waitlist, email_label, email_placeholder (+25 more)

### Community 168 - "Community 168"
Cohesion: 0.07
Nodes (28): members, cannot_demote_last_owner, cannot_demote_last_owner_title, change_role, confirm_action, confirm_cancel, invite_button, invite_email_placeholder (+20 more)

### Community 169 - "Community 169"
Cohesion: 0.08
Nodes (27): aicortex auth logout, aicortex auth status, aicortex config, AICortex 守护进程, aicortex daemon logs, aicortex daemon start, aicortex daemon status, aicortex daemon stop (+19 more)

### Community 170 - "Community 170"
Cohesion: 0.07
Nodes (27): row_actions, agent_archived_toast, agent_restored_toast, archive, archive_dialog_cancel, archive_dialog_confirm, archive_dialog_description, archive_dialog_title (+19 more)

### Community 171 - "Community 171"
Cohesion: 0.26
Nodes (16): AUDIO_EXTS, BASENAME_LANGUAGE_MAP, baseOf(), EXT_LANGUAGE_MAP, extensionToLanguage(), extOf(), getPreviewKind(), isPreviewable() (+8 more)

### Community 172 - "Community 172"
Cohesion: 0.35
Nodes (30): Bus, ListInboxItemsRow, Queries, T, addTestSubscriber(), cleanupInboxForIssue(), countInboxByTypeForRecipient(), createTestSubIssue() (+22 more)

### Community 173 - "Community 173"
Cohesion: 0.14
Nodes (16): NewQuerierAdapter(), parseUUID(), toPgtypeInt4(), toPgtypeText(), toPgtypeTimestamptz(), Querier, querierAdapter, Context (+8 more)

### Community 174 - "Community 174"
Cohesion: 0.14
Nodes (21): autopilotToResponse(), computeNextRun(), runToResponse(), triggerToResponse(), AutopilotResponse, AutopilotRunResponse, AutopilotTriggerResponse, CreateAutopilotRequest (+13 more)

### Community 175 - "Community 175"
Cohesion: 0.16
Nodes (16): captureItem, capturePayload, TestEmailDomain(), TestNoopClient(), TestPostHogClient_Batching(), TestPostHogClient_DropsWhenFull(), NewPostHogClient(), PostHogClient (+8 more)

### Community 176 - "Community 176"
Cohesion: 0.19
Nodes (20): DailyCostChart(), DailyTasksChart(), DailyTasksData, DailyTimeChart(), DailyTimeData, DailyTokensChart(), HourlyActivityChart(), HourlyCostPoint (+12 more)

### Community 177 - "Community 177"
Cohesion: 0.08
Nodes (25): compilerOptions, allowJs, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, incremental, isolatedModules (+17 more)

### Community 178 - "Community 178"
Cohesion: 0.15
Nodes (25): allAssignees, Assignee, AutonomousVisual(), buildFeatures(), buildHeatmapCells(), DailyCostBars(), FeaturesSection(), formatTokens() (+17 more)

### Community 179 - "Community 179"
Cohesion: 0.17
Nodes (17): dictionaryFactories, LocaleContext, LocaleContextValue, LocaleProvider(), createEnDict(), FeatureSection, FooterGroup, LandingDict (+9 more)

### Community 180 - "Community 180"
Cohesion: 0.08
Nodes (26): activity, activity_count_one, activity_count_other, assigned_to, changed_assignee, coalesced_badge, created, description_updated (+18 more)

### Community 181 - "Community 181"
Cohesion: 0.25
Nodes (30): containsString(), equalStrings(), importedFilePaths(), newGitHubFixtureClient(), TestDetectImportSource_RecognizesGitHub(), TestFetchFromGitHub_BlobURLImportsSpecificSkill(), TestFetchFromGitHub_FallsBackOnAPIBlocked(), TestFetchFromGitHub_OversizedSupportingFileFailsImport() (+22 more)

### Community 182 - "Community 182"
Cohesion: 0.09
Nodes (24): fab, default, running, unread_one, unread_other, input, placeholder_archived, placeholder_default (+16 more)

### Community 183 - "Community 183"
Cohesion: 0.16
Nodes (8): NodesKey(), RedisRelay, scopeConsumer, scopeKey, Context, DaemonRuntimeDeliverer, Mutex, WaitGroup

### Community 184 - "Community 184"
Cohesion: 0.24
Nodes (12): isUniqueViolation(), sanitizeNullBytes(), skillFileToResponse(), skillSummaryToResponse(), validateFilePath(), Handler, Request, ResponseWriter (+4 more)

### Community 185 - "Community 185"
Cohesion: 0.09
Nodes (37): DaemonHeartbeatPendingLocalSkillImport, DaemonHeartbeatPendingLocalSkills, DaemonHeartbeatPendingModelList, DaemonHeartbeatPendingUpdate, ArtifactListEntry, ArtifactRequestPayload, ArtifactResponsePayload, ChatDonePayload (+29 more)

### Community 186 - "Community 186"
Cohesion: 0.08
Nodes (25): compilerOptions, allowJs, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, incremental, isolatedModules (+17 more)

### Community 187 - "Community 187"
Cohesion: 0.08
Nodes (26): activity, activity_count_one, activity_count_other, assigned_to, changed_assignee, coalesced_badge, created, description_updated (+18 more)

### Community 188 - "Community 188"
Cohesion: 0.08
Nodes (26): copy_tooltip, description, done, title, 30, 365, 90, never (+18 more)

### Community 189 - "Community 189"
Cohesion: 0.08
Nodes (27): AICortex Server, Assignee, @all Mention, @-mention, Backlog, GitHub Webhook Curl Probe, GitHub App, GitHub Disconnect (+19 more)

### Community 190 - "Community 190"
Cohesion: 0.22
Nodes (21): desktopConfigPath(), errorMessage(), isMissingFileError(), loadRuntimeConfig(), configPath, DEFAULT_RUNTIME_CONFIG, deriveAppUrl(), deriveDevAppUrl() (+13 more)

### Community 191 - "Community 191"
Cohesion: 0.24
Nodes (28): resolveProfile(), buildDaemonStartArgs(), checkDaemonHealthOnPort(), daemonDirForProfile(), daemonLogPathForProfile(), daemonPIDPathForProfile(), emptyDash(), flagString() (+20 more)

### Community 192 - "Community 192"
Cohesion: 0.13
Nodes (40): actorDisplayLookupState, actorDisplayLookup, actorDisplayLookupState, ambiguousIDPrefixError(), compactUUID(), displayID(), fetchAutopilotCandidates(), fetchIssueCandidates() (+32 more)

### Community 193 - "Community 193"
Cohesion: 0.05
Nodes (37): change_aria, remove_aria, select_image_toast, upload_aria, upload_failed_toast, create_dialog, avatar, cancel (+29 more)

### Community 194 - "Community 194"
Cohesion: 0.12
Nodes (16): description, label, description, label, description, label, agent_activity, assignments (+8 more)

### Community 195 - "Community 195"
Cohesion: 0.08
Nodes (25): body, title, detail, add_file_aria, add_file_tooltip, all_skills, conflict_banner, delete_aria (+17 more)

### Community 196 - "Community 196"
Cohesion: 0.08
Nodes (39): InMemoryModelListStore, ModelEntry, ModelListRequest, ModelListStatus, ModelListStore, redisModelListEnvelope, RedisModelListStore, applyModelListTimeout() (+31 more)

### Community 197 - "Community 197"
Cohesion: 0.15
Nodes (22): CancelFunc, Client, Context, Client, T, T, emptyClaimKey(), emptyClaimVersion() (+14 more)

### Community 198 - "Community 198"
Cohesion: 0.14
Nodes (19): ActiveDuplicateError, AgentTaskQueue, Autopilot, AutopilotRun, Bus, Context, Issue, Queries (+11 more)

### Community 199 - "Community 199"
Cohesion: 0.33
Nodes (17): AgentTaskQueue, Bus, Context, LivenessStore, Pool, Queries, SelectStaleOnlineRuntimesRow, TaskService (+9 more)

### Community 200 - "Community 200"
Cohesion: 0.05
Nodes (37): change_aria, remove_aria, select_image_toast, upload_aria, upload_failed_toast, create_dialog, avatar, cancel (+29 more)

### Community 201 - "Community 201"
Cohesion: 0.08
Nodes (25): row_actions, agent_archived_toast, agent_restored_toast, archive, archive_dialog_cancel, archive_dialog_confirm, archive_dialog_description, archive_dialog_title (+17 more)

### Community 202 - "Community 202"
Cohesion: 0.12
Nodes (17): artifacts, empty_dir, empty_sources, hint, loading_files, loading_sources, preview_frame_title, title (+9 more)

### Community 203 - "Community 203"
Cohesion: 0.08
Nodes (25): body, title, detail, add_file_aria, add_file_tooltip, all_skills, conflict_banner, delete_aria (+17 more)

### Community 204 - "Community 204"
Cohesion: 0.08
Nodes (24): create_form, errors, name_label, name_placeholder, submit, submitting, url_label, url_placeholder (+16 more)

### Community 205 - "Community 205"
Cohesion: 0.18
Nodes (24): concept-clawhavoc-incident, concept-local-skill, concept-mcp, concept-session-resumption, concept-skill, concept-skill-attachment, concept-skill-injection-path, concept-skill-vs-mcp (+16 more)

### Community 206 - "Community 206"
Cohesion: 0.07
Nodes (31): add_trigger_dialog, label_field, label_placeholder, submit, submitting, title, toast_add_failed, toast_added (+23 more)

### Community 207 - "Community 207"
Cohesion: 0.08
Nodes (24): execution_log, cancel_failed, cancel_task_aria, cancel_task_tooltip, hide_past, retry_failed, retry_task_aria, retry_task_tooltip (+16 more)

### Community 208 - "Community 208"
Cohesion: 0.13
Nodes (21): all, button, desc, firstMatch(), getSubmitButton(), lastMatch(), makeAgent(), makeMember() (+13 more)

### Community 209 - "Community 209"
Cohesion: 0.08
Nodes (24): description, title, body, highlight, title, fallback, retry, title (+16 more)

### Community 210 - "Community 210"
Cohesion: 0.08
Nodes (24): runtime_import, choose_runtime, ignored_files_hint, import_button, importing, into_workspace, load_failed, must_be_online (+16 more)

### Community 211 - "Community 211"
Cohesion: 0.08
Nodes (38): InMemoryUpdateStore, redisUpdateEnvelope, RedisUpdateStore, applyUpdateTimeout(), NewInMemoryUpdateStore(), NewRedisUpdateStore(), TestRedisUpdateStore_CreateGetComplete(), TestRedisUpdateStore_EnvelopePersistsRunStartedAt() (+30 more)

### Community 212 - "Community 212"
Cohesion: 0.04
Nodes (47): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+39 more)

### Community 213 - "Community 213"
Cohesion: 0.08
Nodes (24): hint, title, hint, title, no_query, title, try_widening, with_query (+16 more)

### Community 214 - "Community 214"
Cohesion: 0.27
Nodes (24): codexMultiAgentEnabled(), ensureCodexMultiAgentConfig(), hasRootFeaturesTable(), injectManagedBlockIntoFeaturesTable(), renderAICortexMultiAgentBlock(), stripUserMultiAgentDirectives(), parseTOML(), requireMultiAgentDisabled() (+16 more)

### Community 215 - "Community 215"
Cohesion: 0.10
Nodes (23): agent_picker, filter_placeholder, select_agent, execution_mode, create_issue, run_only, relative_date, days_ago (+15 more)

### Community 216 - "Community 216"
Cohesion: 0.08
Nodes (24): execution_log, cancel_failed, cancel_task_aria, cancel_task_tooltip, hide_past, retry_failed, retry_task_aria, retry_task_tooltip (+16 more)

### Community 217 - "Community 217"
Cohesion: 0.08
Nodes (24): create_project, agents_group, description_placeholder, icon_tooltip, lead, lead_placeholder, members_group, no_lead (+16 more)

### Community 218 - "Community 218"
Cohesion: 0.08
Nodes (24): hint, title, hint, title, no_query, title, try_widening, with_query (+16 more)

### Community 219 - "Community 219"
Cohesion: 0.08
Nodes (24): description, title, body, highlight, title, fallback, retry, title (+16 more)

### Community 220 - "Community 220"
Cohesion: 0.08
Nodes (24): runtime_import, choose_runtime, ignored_files_hint, import_button, importing, into_workspace, load_failed, must_be_online (+16 more)

### Community 221 - "Community 221"
Cohesion: 0.17
Nodes (15): LivenessStore, noopLivenessStore, RedisLivenessStore, NewNoopLivenessStore(), NewRedisLivenessStore(), runtimeLivenessKey(), TestNoopLivenessStore_AlwaysUnavailable(), TestRedisLivenessStore_BatchEmptyInput() (+7 more)

### Community 222 - "Community 222"
Cohesion: 0.13
Nodes (25): browserTimezone(), COMMON_TIMEZONES, IntlWithSupportedValues, supportedTimezones(), timezoneOptions(), TimezoneSelect(), DailyMetric, DailyTrendBlock() (+17 more)

### Community 223 - "Community 223"
Cohesion: 0.08
Nodes (23): ringBuffer, newRingBuffer(), NewTerminalManager(), TestHandleAttachOpensMissingSession(), TestPTYStartsDefaultShell(), TestPTYStartsPowerShellWithArgs(), TerminalManager, TerminalSession (+15 more)

### Community 224 - "Community 224"
Cohesion: 0.25
Nodes (23): collectLocalSkillFiles(), enumerateLocalSkills(), isIgnoredLocalSkillEntry(), listRuntimeLocalSkills(), loadRuntimeLocalSkillBundle(), localSkillRootForProvider(), normalizeLocalSkillKey(), parseLocalSkillFrontmatter() (+15 more)

### Community 225 - "Community 225"
Cohesion: 0.15
Nodes (15): createBtn, disabledRow, makeRuntime(), makeTemplate(), members, mine, navigationStub, { onCreate } (+7 more)

### Community 226 - "Community 226"
Cohesion: 0.17
Nodes (13): createBlurShortcutExtension(), FileCardExtension, createEditorExtensions(), EditorExtensionsOptions, ImageExtension, LinkExtension, lowlight, createMarkdownCopyExtension() (+5 more)

### Community 227 - "Community 227"
Cohesion: 0.18
Nodes (16): createMarkdownPasteExtension(), codeBlock, expectLiteralPaste(), FakeClipboard, fakePasteEvent(), findFirst(), handled, json (+8 more)

### Community 228 - "Community 228"
Cohesion: 0.09
Nodes (32): composeOpenclawIncludeRoots(), classifyPoisonedError(), classifyPoisonedOutput(), TestClassifyPoisonedError(), TestClassifyPoisonedOutput(), ensureWorkdirAccessible(), isRemoteWorkdirPath(), normalizeWorkdirPath() (+24 more)

### Community 229 - "Community 229"
Cohesion: 0.15
Nodes (23): DualWriteBroadcaster, envelope, deliverEnvelope(), envelopeFromXMessage(), envelopeRedisValues(), HeartbeatKey(), injectEventID(), NewDualWriteBroadcaster() (+15 more)

### Community 230 - "Community 230"
Cohesion: 0.32
Nodes (23): install_cli(), install.sh script, add_to_path(), check_docker(), checkout_server_ref(), command_exists(), detect_os(), fail() (+15 more)

### Community 231 - "Community 231"
Cohesion: 0.18
Nodes (23): init(), InputMap(), TestInputMap(), TestInputMapNil(), TestNoFalsePositivesOnNormalText(), TestRedactAWSAccessKey(), TestRedactAWSSecretKey(), TestRedactBearerToken() (+15 more)

### Community 232 - "Community 232"
Cohesion: 0.10
Nodes (23): cancel, confirm, delete, loading, missionControl, activity_14d, agents_header, all (+15 more)

### Community 233 - "Community 233"
Cohesion: 0.05
Nodes (40): blurb, instructions, label, blurb, instructions, label, blurb, instructions (+32 more)

### Community 234 - "Community 234"
Cohesion: 0.13
Nodes (21): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+13 more)

### Community 235 - "Community 235"
Cohesion: 0.31
Nodes (14): buildAutopilotPrompt(), buildChatPrompt(), buildCommentPrompt(), BuildPrompt(), buildQuickCreatePrompt(), TestBuildPromptNonSquadLeaderNoRule(), TestBuildPromptSquadLeaderNoActionForAgentTrigger(), TestBuildPromptSquadLeaderNoActionForMemberTrigger() (+6 more)

### Community 236 - "Community 236"
Cohesion: 0.09
Nodes (22): inspector, avatar_updated_toast, avatar_upload_failed_toast, cancel, change_avatar_aria, description_placeholder, edit_description_title, no_description_placeholder (+14 more)

### Community 237 - "Community 237"
Cohesion: 0.09
Nodes (22): pickers, concurrency_range, concurrency_tooltip, model_clear, model_clear_title, model_custom_tooltip, model_custom_use, model_default (+14 more)

### Community 238 - "Community 238"
Cohesion: 0.09
Nodes (22): detail, activate_aria, add_trigger, delete_button, edit, field_agent, field_output_mode, field_prompt (+14 more)

### Community 239 - "Community 239"
Cohesion: 0.09
Nodes (22): comment, cancel_action, cancel_edit, collapse_tooltip, copied_toast, copy_action, delete_action, delete_desc (+14 more)

### Community 240 - "Community 240"
Cohesion: 0.09
Nodes (22): labels_panel, add_action, cancel_aria, color_label, create_failed, delete_aria, delete_dialog_cancel, delete_dialog_confirm (+14 more)

### Community 241 - "Community 241"
Cohesion: 0.18
Nodes (19): Int8, Text, Timestamptz, UUID, T, Int8ToPtr(), MustParseUUID(), ParseUUID() (+11 more)

### Community 242 - "Community 242"
Cohesion: 0.14
Nodes (30): GenerateDaemonToken(), GeneratePATToken(), HashToken(), JWTSecret(), MapClaims, Auth(), extractToken(), authMiddleware() (+22 more)

### Community 243 - "Community 243"
Cohesion: 0.08
Nodes (27): Client, Hub, authenticateToken(), checkOrigin(), firstMessageAuth(), HandleWebSocket(), init(), loadAllowedOrigins() (+19 more)

### Community 244 - "Community 244"
Cohesion: 0.15
Nodes (19): cachedReadiness, Pointer, readinessChecks, readinessResponse, cachedReadiness, Context, Duration, Mutex (+11 more)

### Community 245 - "Community 245"
Cohesion: 0.22
Nodes (13): commentToResponse(), shouldInheritParentMentions(), CommentResponse, CreateCommentRequest, Mention, AttachmentResponse, Comment, Context (+5 more)

### Community 246 - "Community 246"
Cohesion: 0.14
Nodes (26): DetectVersion(), LaunchHeader(), New(), TestDetectVersionFailsForMissingBinary(), TestLaunchHeaderCoversAllSupportedBackends(), TestLaunchHeaderReturnsEmptyForUnknownType(), TestNewDefaultsLogger(), TestNewRejectsUnknownType() (+18 more)

### Community 247 - "Community 247"
Cohesion: 0.04
Nodes (45): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+37 more)

### Community 248 - "Community 248"
Cohesion: 0.09
Nodes (22): inspector, avatar_updated_toast, avatar_upload_failed_toast, cancel, change_avatar_aria, description_placeholder, edit_description_title, no_description_placeholder (+14 more)

### Community 249 - "Community 249"
Cohesion: 0.09
Nodes (22): pickers, concurrency_range, concurrency_tooltip, model_clear, model_clear_title, model_custom_tooltip, model_custom_use, model_default (+14 more)

### Community 250 - "Community 250"
Cohesion: 0.09
Nodes (22): detail, activate_aria, add_trigger, delete_button, edit, field_agent, field_output_mode, field_prompt (+14 more)

### Community 251 - "Community 251"
Cohesion: 0.09
Nodes (22): labels_panel, add_action, cancel_aria, color_label, create_failed, delete_aria, delete_dialog_cancel, delete_dialog_confirm (+14 more)

### Community 252 - "Community 252"
Cohesion: 0.04
Nodes (45): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+37 more)

### Community 253 - "Community 253"
Cohesion: 0.17
Nodes (20): binaryNameForPlatform(), binName, date, destBinary, destDir, exists(), hasGo(), here (+12 more)

### Community 254 - "Community 254"
Cohesion: 0.11
Nodes (21): Issue (self-contained unit of work), Issue Comment Thread, Issue Deletion (irreversible, clears all associated data), Issue Number (workspace-unique prefix-digits), Issue Priority (none/urgent/high/medium/low), Issue Status (backlog/todo/in_progress/in_review/done/blocked/cancelled), Project (container for related issues), Project Deletion (issues are unlinked, not deleted) (+13 more)

### Community 255 - "Community 255"
Cohesion: 0.15
Nodes (18): EmptyState(), ForumPage(), ForumSkeleton(), agentColor(), agentInitial(), PostCard(), PostCardProps, PROVIDER_COLORS (+10 more)

### Community 256 - "Community 256"
Cohesion: 0.11
Nodes (31): buildPatternSet(), buildTaskUsage(), ratio(), ScanDiskUsage(), ShortID(), sortedKeys(), taskSize(), mustWriteMeta() (+23 more)

### Community 257 - "Community 257"
Cohesion: 0.10
Nodes (21): detail, agent_archived_toast, agent_restored_toast, agent_updated_toast, archive_dialog_cancel, archive_dialog_confirm, archive_dialog_description, archive_dialog_title (+13 more)

### Community 258 - "Community 258"
Cohesion: 0.10
Nodes (21): display, ascending_title, card_assignee, card_child_progress, card_description, card_due_date, card_labels, card_priority (+13 more)

### Community 259 - "Community 259"
Cohesion: 0.10
Nodes (21): filters, agents_group, issue_count_one, issue_count_other, members_group, no_assignee, no_labels, no_project (+13 more)

### Community 260 - "Community 260"
Cohesion: 0.14
Nodes (13): sanitizeTabPath(), activeTab, closedTabId, closingTab, createTabRouterMock, dispose, id1, id2 (+5 more)

### Community 261 - "Community 261"
Cohesion: 0.07
Nodes (28): questionnaire, answered_progress, eyebrow, headline, learn_more, q1_other_placeholder, q1_question, q1_solo (+20 more)

### Community 262 - "Community 262"
Cohesion: 0.06
Nodes (33): shell, cli_back_to_chat, cli_launch, cli_launch_disabled_explore, cli_launch_disabled_provider, cli_launch_disabled_runtime, cli_launch_label, cli_resume_drift_hint (+25 more)

### Community 263 - "Community 263"
Cohesion: 0.03
Nodes (107): buildPresenceMap(), deriveAgentAvailability(), deriveAgentPresenceDetail(), DerivePresenceInput, deriveWorkload(), deriveWorkloadDetail(), a, agentA (+99 more)

### Community 264 - "Community 264"
Cohesion: 0.34
Nodes (20): Add-ToUserPath(), Checkout-ServerRef(), Convert-ToCliArch(), Get-InstalledCliVersion(), Get-LatestVersion(), Get-SelfHostRef(), Get-WindowsCliArch(), Install-CliBinary() (+12 more)

### Community 265 - "Community 265"
Cohesion: 0.13
Nodes (28): buildGeminiArgs(), buildGeminiEnv(), envLookup(), TestBuildGeminiArgsBaseline(), TestBuildGeminiArgsFiltersBlockedCustomArgs(), TestBuildGeminiArgsOmitsModelWhenEmpty(), TestBuildGeminiArgsPassesThroughCustomArgs(), TestBuildGeminiArgsWithModel() (+20 more)

### Community 266 - "Community 266"
Cohesion: 0.09
Nodes (28): chatShareSessionKey, ChatShareWSHub, ChatShareWSOutgoing, NewChatSharePublicHandler(), parsePublicUUID(), resolveShareLinkStatus(), statusMessage(), NewChatShareWSHub() (+20 more)

### Community 267 - "Community 267"
Cohesion: 0.14
Nodes (15): CreatePreviewEnvironmentParams, GetPreviewEnvironmentByPRParams, UpdatePreviewEnvironmentCommitParams, UpdatePreviewEnvironmentDbNameParams, UpdatePreviewEnvironmentErrorParams, UpdatePreviewEnvironmentPortParams, UpdatePreviewEnvironmentStatusParams, Context (+7 more)

### Community 268 - "Community 268"
Cohesion: 0.10
Nodes (21): detail, agent_archived_toast, agent_restored_toast, agent_updated_toast, archive_dialog_cancel, archive_dialog_confirm, archive_dialog_description, archive_dialog_title (+13 more)

### Community 269 - "Community 269"
Cohesion: 0.10
Nodes (21): display, ascending_title, card_assignee, card_child_progress, card_description, card_due_date, card_labels, card_priority (+13 more)

### Community 270 - "Community 270"
Cohesion: 0.10
Nodes (21): agents_group, created_by, error_agent_unavailable_fallback, error_daemon_version, error_unknown, no_agents, pick_an_agent, prompt_placeholder (+13 more)

### Community 271 - "Community 271"
Cohesion: 0.11
Nodes (19): Tasks (English), 一个任务会经过哪些状态, 下一步, 任务能接着上次的上下文继续吗, 任务超时会怎样, 哪些失败会自动重试，哪些不会, 失败的任务对 issue 状态有什么影响, 手动重跑和自动重试的区别 (+11 more)

### Community 272 - "Community 272"
Cohesion: 0.10
Nodes (19): dependsOn, inputs, outputs, cache, persistent, cache, persistent, globalEnv (+11 more)

### Community 273 - "Community 273"
Cohesion: 0.05
Nodes (43): Agent Daemon, Assign Issue, Associating Issues with Projects, Authentication, Browser Login, Change Status, Change Status, Check Status (+35 more)

### Community 274 - "Community 274"
Cohesion: 0.10
Nodes (31): ChatContent(), DesignQuestionsPanel(), answerDisplay(), buildInitialState(), canonicalizeValue(), QuestionFormView(), QuestionFormViewProps, findLatestPendingQuestionForm() (+23 more)

### Community 275 - "Community 275"
Cohesion: 0.05
Nodes (43): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+35 more)

### Community 276 - "Community 276"
Cohesion: 0.10
Nodes (21): window, active_group, another_running, another_unread, archived_group_one, archived_group_other, chats, expand_tooltip (+13 more)

### Community 277 - "Community 277"
Cohesion: 0.07
Nodes (33): chip, fallback_label, delete_dialog, cancel, confirm, description, title, lead (+25 more)

### Community 278 - "Community 278"
Cohesion: 0.12
Nodes (23): CreateChatMessageParams, CreateChatSessionParams, CreateChatTaskParams, GetChatSessionInWorkspaceParams, GetLastChatTaskSessionRow, GetPendingChatTaskRow, ListAllChatSessionsByCreatorParams, ListAllChatSessionsByCreatorRow (+15 more)

### Community 279 - "Community 279"
Cohesion: 0.14
Nodes (19): createTestQueryClient(), editor, issuePromise, mockApiObj, mockAuthUser, mockIssue, mockRecordVisit, mockTimeline (+11 more)

### Community 280 - "Community 280"
Cohesion: 0.09
Nodes (22): detail, breadcrumb_fallback, copy_link, delete_action, description_placeholder, design_studio_action, dev_studio_action, empty_issues_hint (+14 more)

### Community 281 - "Community 281"
Cohesion: 0.22
Nodes (26): buildSquadLeaderBriefing(), buildSquadRoster(), formatMention(), formatRosterRow(), renderMemberRow(), addAgentMember(), addHumanMember(), claimAndDecodeAgent() (+18 more)

### Community 282 - "Community 282"
Cohesion: 0.21
Nodes (23): boundedBuffer, isSoftNotFound(), isTransientDaemonUnavailable(), RequestLogger(), requireLogLevel(), runRequestLogger(), TestIsSoftNotFound(), TestIsTransientDaemonUnavailable() (+15 more)

### Community 283 - "Community 283"
Cohesion: 0.21
Nodes (18): buildExpandedMermaidDocument(), buildSandboxedMermaidDocument(), getMermaid(), getMermaidLayout(), getMermaidThemeVariables(), getSandboxCssVariables(), hashChart(), MermaidAPI (+10 more)

### Community 284 - "Community 284"
Cohesion: 0.17
Nodes (6): mockQuerier, Context, PreviewEnvironment, PreviewPortPool, ProvisionState, Time

### Community 285 - "Community 285"
Cohesion: 0.18
Nodes (17): ContentEditor, editor, mockClearPrompt, mockProjectsQuery, mockQuickCreateIssue, mockQuickCreateStore, mockSetKeepOpen, mockSetLastActor (+9 more)

### Community 286 - "Community 286"
Cohesion: 0.15
Nodes (17): AddAgentSkillParams, CreateSkillParams, GetSkillByWorkspaceAndNameParams, GetSkillInWorkspaceParams, ListAgentSkillsByWorkspaceRow, ListAgentSkillSummariesRow, ListSkillSummariesByWorkspaceRow, RemoveAgentSkillParams (+9 more)

### Community 287 - "Community 287"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+33 more)

### Community 288 - "Community 288"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+33 more)

### Community 289 - "Community 289"
Cohesion: 0.10
Nodes (20): filters, agents_group, issue_count_other, members_group, no_assignee, no_labels, no_project, no_results (+12 more)

### Community 290 - "Community 290"
Cohesion: 0.09
Nodes (25): backlog_hint, toast_status_failed, common, back, cancel, close, collapse_tooltip, expand_tooltip (+17 more)

### Community 291 - "Community 291"
Cohesion: 0.28
Nodes (20): IssueResponse, Bus, Queries, Queries, T, addSubscriber(), extractIssueFields(), registerSubscriberListeners() (+12 more)

### Community 292 - "Community 292"
Cohesion: 0.11
Nodes (18): computedHash, source, sourceType, computedHash, source, sourceType, skills, frontend-design (+10 more)

### Community 293 - "Community 293"
Cohesion: 0.11
Nodes (19): dependencies, @aicortex/core, @aicortex/ui, @aicortex/views, @dnd-kit/core, @dnd-kit/modifiers, @dnd-kit/sortable, @dnd-kit/utilities (+11 more)

### Community 294 - "Community 294"
Cohesion: 0.11
Nodes (19): devDependencies, @aicortex/tsconfig, electron, electron-builder, @electron-toolkit/tsconfig, electron-vite, jsdom, react (+11 more)

### Community 295 - "Community 295"
Cohesion: 0.18
Nodes (17): appInfo, DaemonAPI, DaemonPrefs, DaemonStatus, DesktopAPI, UpdaterAPI, Window, daemonAPI (+9 more)

### Community 296 - "Community 296"
Cohesion: 0.12
Nodes (19): Backend (Go), Database (PostgreSQL 17 + pgvector), Frontend (Next.js 16), Self-Hosting, Agent Binary/Model Overrides, Dev Verification Code, Google OAuth, Email Auth (Resend) (+11 more)

### Community 297 - "Community 297"
Cohesion: 0.11
Nodes (19): add_action, add_dialog_cancel, add_dialog_confirm_default, add_dialog_confirm_one, add_dialog_confirm_other, add_dialog_description, add_dialog_empty, add_dialog_empty_partial (+11 more)

### Community 298 - "Community 298"
Cohesion: 0.11
Nodes (19): transcript, clear_filters, copied, copy_all, copy_filtered, dialog_title, events_filtered, events_one (+11 more)

### Community 299 - "Community 299"
Cohesion: 0.11
Nodes (19): summary, title, summary, title, summary, title, summary, title (+11 more)

### Community 300 - "Community 300"
Cohesion: 0.11
Nodes (19): actions, add_sub_issue, assignee, copy_link, copy_workdir_path, create_sub_issue, delete_issue, due_clear (+11 more)

### Community 301 - "Community 301"
Cohesion: 0.11
Nodes (19): agents_group, members_group, search_placeholder, squads_group, trigger_unassigned, clear_action, trigger_label, create_action (+11 more)

### Community 302 - "Community 302"
Cohesion: 0.03
Nodes (67): agents, create, more, work, help, changelog, docs, feedback (+59 more)

### Community 303 - "Community 303"
Cohesion: 0.11
Nodes (19): buildTimeline, completedFetch, Deferred, EventHandler, fireEvent(), makeTask(), mockApi, mountFetch (+11 more)

### Community 304 - "Community 304"
Cohesion: 0.13
Nodes (28): Context, ReadCloser, Reader, Request, ResponseWriter, T, isUnder(), NewLocalStorageFromEnv() (+20 more)

### Community 305 - "Community 305"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Interaction & Motion, 8. Responsive Behavior (+33 more)

### Community 306 - "Community 306"
Cohesion: 0.06
Nodes (27): ParseOptions, client, entry, fallback, opts, out, schema, stubFetchJson() (+19 more)

### Community 307 - "Community 307"
Cohesion: 0.17
Nodes (9): APIClient, AttachmentResponse, NewAPIClient(), normalizeGOOS(), HTTPError, Client, Context, Header (+1 more)

### Community 308 - "Community 308"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Interaction & Motion, 8. Responsive Behavior (+33 more)

### Community 309 - "Community 309"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+33 more)

### Community 310 - "Community 310"
Cohesion: 0.14
Nodes (25): buildPiArgs(), decodePiMessage(), decodePiResult(), decodePiString(), ensurePiSessionFile(), newPiSessionPath(), piSessionDir(), splitPiModel() (+17 more)

### Community 311 - "Community 311"
Cohesion: 0.14
Nodes (25): Float8, ChildIssueProgressRow, CountCreatedIssueAssigneesParams, CountCreatedIssueAssigneesRow, CountIssuesParams, CreateIssueParams, CreateIssueWithOriginParams, FindActiveDuplicateIssueParams (+17 more)

### Community 312 - "Community 312"
Cohesion: 0.15
Nodes (24): GetIssueUsageSummaryRow, GetWorkspaceUsageByDayParams, GetWorkspaceUsageByDayRow, GetWorkspaceUsageSummaryParams, GetWorkspaceUsageSummaryRow, ListDashboardAgentRunTimeParams, ListDashboardAgentRunTimeRow, ListDashboardRunTimeDailyParams (+16 more)

### Community 313 - "Community 313"
Cohesion: 0.11
Nodes (18): compilerOptions, declaration, declarationMap, esModuleInterop, forceConsistentCasingInFileNames, isolatedModules, module, moduleResolution (+10 more)

### Community 314 - "Community 314"
Cohesion: 0.11
Nodes (19): summary, title, summary, title, summary, title, summary, title (+11 more)

### Community 315 - "Community 315"
Cohesion: 0.10
Nodes (20): days_hours, hours_minutes, less_than_minute, minutes, fri, mon, sat, sun (+12 more)

### Community 316 - "Community 316"
Cohesion: 0.10
Nodes (20): window, active_group, another_running, another_unread, archived_group_other, chats, expand_tooltip, minimize_tooltip (+12 more)

### Community 317 - "Community 317"
Cohesion: 0.11
Nodes (19): actions, add_sub_issue, assignee, copy_link, copy_workdir_path, create_sub_issue, delete_issue, due_clear (+11 more)

### Community 318 - "Community 318"
Cohesion: 0.11
Nodes (19): agents_group, members_group, search_placeholder, squads_group, trigger_unassigned, clear_action, trigger_label, create_action (+11 more)

### Community 319 - "Community 319"
Cohesion: 0.08
Nodes (26): agents, create, more, work, nav, agents, autopilots, chat (+18 more)

### Community 320 - "Community 320"
Cohesion: 0.11
Nodes (19): card1_actor_initial, card1_actor_name, card1_body_prefix, card1_body_suffix, card1_mention_content, card1_mention_research, card2_actor_name, card2_body (+11 more)

### Community 321 - "Community 321"
Cohesion: 0.09
Nodes (24): AICORTEX_DEV_VERIFICATION_CODE — Fixed Local Test Code Env Var, AICORTEX_SERVER_URL — Daemon Server Connection URL, APP_ENV — Application Environment Variable (development/production), FRONTEND_ORIGIN — CORS Origin Configuration for Self-Hosted Frontend, RESEND_API_KEY — Resend Email Service API Key, Troubleshooting (English), WebSocket 连不上, 任务一直卡在 queued (+16 more)

### Community 322 - "Community 322"
Cohesion: 0.07
Nodes (59): captureDownloadIntent(), setPersonProperties(), ImportStarterContentPayload, ImportStarterIssuePayload, OptionCard(), OtherOptionCard(), RadioMark(), StepHeader() (+51 more)

### Community 323 - "Community 323"
Cohesion: 0.11
Nodes (19): days_hours, hours_minutes, less_than_minute, minutes, custom, daily, hourly, no_days (+11 more)

### Community 324 - "Community 324"
Cohesion: 0.11
Nodes (18): batch, assignee, cancel, delete, delete_dialog_desc_one, delete_dialog_desc_other, delete_dialog_title_one, delete_dialog_title_other (+10 more)

### Community 325 - "Community 325"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+33 more)

### Community 326 - "Community 326"
Cohesion: 0.24
Nodes (23): SelectAutopilotsExceedingFailureThresholdRow, autopilotEventPayload(), defaultFailureMonitorConfig(), emitAutopilotPausedNotifications(), envDurationNonNegative(), envDurationOrZero(), envDurationPositive(), envFailureMonitorConfig() (+15 more)

### Community 327 - "Community 327"
Cohesion: 0.05
Nodes (41): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+33 more)

### Community 328 - "Community 328"
Cohesion: 0.08
Nodes (26): help, changelog, docs, feedback, trigger, topnav, general, integrations (+18 more)

### Community 329 - "Community 329"
Cohesion: 0.13
Nodes (12): DefaultShardedStreamRelayConfig(), NewShardedStreamRelay(), ShardedStreamKey(), ShardedStreamRelay, ShardedStreamRelayConfig, Client, Context, DaemonRuntimeDeliverer (+4 more)

### Community 330 - "Community 330"
Cohesion: 0.13
Nodes (26): CreateProjectResourceRequestPayload, CreateProjectRequest, CreateProjectResourceRequestPayload, buildSearchQuery(), escapeLike(), buildProjectSearchQuery(), projectToResponse(), ProjectResponse (+18 more)

### Community 331 - "Community 331"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 332 - "Community 332"
Cohesion: 0.14
Nodes (17): aliases, components, hooks, lib, ui, utils, iconLibrary, rsc (+9 more)

### Community 333 - "Community 333"
Cohesion: 0.12
Nodes (18): Agent Event System, Agent Forum Demo, Agent Hover Tooltip, Named AI Agents, Agent Personality System, Agent State Machine, Forum Auto Simulation Engine, Forum Post Feed (+10 more)

### Community 334 - "Community 334"
Cohesion: 0.11
Nodes (18): add_action, add_dialog_cancel, add_dialog_confirm_default, add_dialog_confirm_other, add_dialog_description, add_dialog_empty, add_dialog_empty_partial, add_dialog_no_match (+10 more)

### Community 335 - "Community 335"
Cohesion: 0.27
Nodes (13): binaryName(), downloadToFile(), ensureManagedCli(), extractArchive(), fetchChecksums(), installFresh(), managedCliPath(), run() (+5 more)

### Community 336 - "Community 336"
Cohesion: 0.17
Nodes (17): architecture-agent-daemon, architecture-backend-structure, architecture-data-flow-rest, architecture-data-flow-ws, architecture-electron-desktop, architecture-go-backend, architecture-internal-packages-pattern, architecture-multi-tenancy (+9 more)

### Community 337 - "Community 337"
Cohesion: 0.18
Nodes (17): Claude Code, Codex, Gemini CLI, Hermes, OpenClaw, OpenCode, Pi, AICortex CLI (+9 more)

### Community 338 - "Community 338"
Cohesion: 0.16
Nodes (14): jitterDuration(), marshalRaw(), signalTaskWakeup(), sleepWithContextOrRuntimeChange(), taskWakeupURL(), TestTaskWakeupURL(), TestWSHeartbeatFreshnessSuppressesHTTP(), Conn (+6 more)

### Community 339 - "Community 339"
Cohesion: 0.04
Nodes (58): detail, archive, edit_advanced, empty, original_input, select_prompt, errors, archive_all_failed (+50 more)

### Community 340 - "Community 340"
Cohesion: 0.05
Nodes (40): author, name, compat, agentSkills, assets, skills, description, description_i18n (+32 more)

### Community 341 - "Community 341"
Cohesion: 0.12
Nodes (17): agents_label, agents_meta, autopilot_label, autopilot_meta, inbox_label, inbox_meta, issues_label, issues_meta (+9 more)

### Community 342 - "Community 342"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 343 - "Community 343"
Cohesion: 0.08
Nodes (29): AgentSkillFileData, TestAutopilotDispatchSkipsWhenRuntimeOffline(), TestAutopilotRunOnlyTaskTerminalEventsUpdateRun(), T, T, Agent, Bus, Hub (+21 more)

### Community 344 - "Community 344"
Cohesion: 0.15
Nodes (16): dependencies, @eslint/js, eslint-plugin-react, eslint-plugin-react-hooks, @next/eslint-plugin-next, typescript-eslint, exports, ./base (+8 more)

### Community 345 - "Community 345"
Cohesion: 0.12
Nodes (17): completed, failed, pending, running, timeout, update, action, available (+9 more)

### Community 346 - "Community 346"
Cohesion: 0.23
Nodes (8): AttachmentResponse, isTextPreviewable(), Attachment, Context, Handler, Request, ResponseWriter, UUID

### Community 347 - "Community 347"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 348 - "Community 348"
Cohesion: 0.16
Nodes (17): AddSquadMemberParams, ArchiveSquadParams, CreateSquadParams, GetSquadByAssigneeParams, GetSquadInWorkspaceParams, IsSquadMemberParams, ListSquadsByMemberParams, RemoveSquadMemberParams (+9 more)

### Community 349 - "Community 349"
Cohesion: 0.12
Nodes (17): transcript, clear_filters, copied, copy_all, copy_filtered, dialog_title, events_filtered, events_other (+9 more)

### Community 350 - "Community 350"
Cohesion: 0.12
Nodes (17): types, agent_blocked, agent_completed, assignee_changed, due_date_changed, issue_assigned, mentioned, new_comment (+9 more)

### Community 351 - "Community 351"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 352 - "Community 352"
Cohesion: 0.12
Nodes (17): agents_label, agents_meta, autopilot_label, autopilot_meta, inbox_label, inbox_meta, issues_label, issues_meta (+9 more)

### Community 353 - "Community 353"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 354 - "Community 354"
Cohesion: 0.12
Nodes (17): repositories, add, cancel_aria, delete_aria, description, description_placeholder, edit_aria, empty (+9 more)

### Community 355 - "Community 355"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Responsive Behavior, 8. Accessibility & States (+32 more)

### Community 356 - "Community 356"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Responsive Behavior, 8. Responsive Behavior (Extended) (+32 more)

### Community 357 - "Community 357"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 358 - "Community 358"
Cohesion: 0.12
Nodes (16): table, by_creator, lock_tooltip, name, no_description, source, source_clawhub, source_github (+8 more)

### Community 359 - "Community 359"
Cohesion: 0.14
Nodes (22): daemonSysProcAttr(), isAccessDeniedSpawnErr(), notifyShutdownContext(), tailLogFile(), GitHubPRWebhookPayload, RepoInfo, extractIssueIdentifiers(), itoa() (+14 more)

### Community 360 - "Community 360"
Cohesion: 0.15
Nodes (20): GetIssueByNumberParams, ExpandIssueIdentifiers(), findSkipRegions(), inSkipRegion(), isInsideMarkdownLink(), makeUUID(), TestExpandIssueIdentifiers(), uuidToString() (+12 more)

### Community 361 - "Community 361"
Cohesion: 0.47
Nodes (11): revocationFixture, assertRevoked(), setupRevocationFixture(), TestCreateWorkspace_RejectsReservedSlug(), TestDeleteMember_CancelsTasksFromAgentReassignment(), TestDeleteMember_NoRuntimes_DeletesMember(), TestDeleteMember_RevokesTargetRuntimes(), TestDeleteWorkspace_OwnerSucceeds() (+3 more)

### Community 362 - "Community 362"
Cohesion: 0.22
Nodes (9): dependencies, @formatjs/intl-localematcher, i18next, posthog-js, react-i18next, @tanstack/react-query, @tanstack/react-query-devtools, zod (+1 more)

### Community 363 - "Community 363"
Cohesion: 0.12
Nodes (16): list, cli_managed_badge, cli_update_available_aria, cli_update_available_tooltip, col_agents, col_cli, col_cost, col_health (+8 more)

### Community 364 - "Community 364"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 365 - "Community 365"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 366 - "Community 366"
Cohesion: 0.09
Nodes (22): detail, breadcrumb_fallback, copy_link, delete_action, description_placeholder, design_studio_action, dev_studio_action, empty_issues_hint (+14 more)

### Community 367 - "Community 367"
Cohesion: 0.05
Nodes (40): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+32 more)

### Community 368 - "Community 368"
Cohesion: 0.12
Nodes (16): description, label, description, label, description, label, agent_activity, assignments (+8 more)

### Community 369 - "Community 369"
Cohesion: 0.12
Nodes (16): table, by_creator, lock_tooltip, name, no_description, source, source_clawhub, source_github (+8 more)

### Community 370 - "Community 370"
Cohesion: 0.13
Nodes (15): scripts, brand-dev-electron, build, bundle-cli, dev, dev:staging, lint, package (+7 more)

### Community 371 - "Community 371"
Cohesion: 0.22
Nodes (13): SendEmailRequest, Client, T, buildInvitationParams(), NewEmailService(), sanitizeSubjectField(), TestBuildInvitationParams_EscapesHTMLInBody(), TestBuildInvitationParams_SubjectNotHTMLEscaped() (+5 more)

### Community 372 - "Community 372"
Cohesion: 0.24
Nodes (13): ChangelogPage(), metadata, anchorId(), ChangeList(), ChangelogPageClient(), fullDateLabel(), groupByMonth(), MonthGroup (+5 more)

### Community 373 - "Community 373"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 374 - "Community 374"
Cohesion: 0.13
Nodes (15): sidebar, created, created_by, files, id, metadata, origin, permissions (+7 more)

### Community 375 - "Community 375"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 376 - "Community 376"
Cohesion: 0.14
Nodes (20): agentActivity30dOptions(), ActivityBucket, ActivityWindowSummary, buildActivityMap(), deriveAgentActivity(), EMPTY, EMPTY_SUMMARY, startOfDay() (+12 more)

### Community 377 - "Community 377"
Cohesion: 0.23
Nodes (13): formatEnvStatus(), NewCommandHandler(), statusEmoji(), CommandHandler, CommandType, PreviewCommand, prInfo, Context (+5 more)

### Community 378 - "Community 378"
Cohesion: 0.15
Nodes (14): att, audio, buttons, code, dialog, frame, {
  getAttachmentTextContentMock,
  downloadMock,
  FakePreviewTooLargeError,
  FakePreviewUnsupportedError,
}, iframe (+6 more)

### Community 379 - "Community 379"
Cohesion: 0.13
Nodes (15): sessions, cancel, closed_group, create, creating, empty_prompt, list_title, new_action (+7 more)

### Community 380 - "Community 380"
Cohesion: 0.22
Nodes (20): connectWS(), makeTestToken(), newTestHub(), TestCheckOrigin(), TestHandleWebSocket_ClientIdentityFromQuery(), TestHub_Broadcast(), TestHub_BroadcastToMultipleClients(), TestHub_ClientDisconnect() (+12 more)

### Community 381 - "Community 381"
Cohesion: 0.08
Nodes (24): create_project, agents_group, description_placeholder, icon_tooltip, lead, lead_placeholder, members_group, no_lead (+16 more)

### Community 382 - "Community 382"
Cohesion: 0.25
Nodes (20): handleTaskActivity(), publishActivityEvent(), registerActivityListeners(), cleanupActivities(), listActivitiesForIssue(), TestActivityIssueCreated(), TestActivityIssueUpdated_AssigneeChanged(), TestActivityIssueUpdated_NoChangeFlags() (+12 more)

### Community 383 - "Community 383"
Cohesion: 0.26
Nodes (16): Context, Pool, T, envInt32(), logPoolConfig(), newDBPool(), poolParamsFromURL(), runDBStatsLogger() (+8 more)

### Community 384 - "Community 384"
Cohesion: 0.35
Nodes (18): Bus, Context, Event, InboxItem, Queries, mention, archiveStaleTaskFailedInbox(), inboxItemToResponse() (+10 more)

### Community 385 - "Community 385"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 386 - "Community 386"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 387 - "Community 387"
Cohesion: 0.09
Nodes (23): fab, default, running, unread_other, input, placeholder_archived, placeholder_default, placeholder_named (+15 more)

### Community 388 - "Community 388"
Cohesion: 0.18
Nodes (10): ForceOfflineRuntimesByIDsRow, revocationResult, logRevocation(), revocationResult, Agent, AgentRuntime, AgentTaskQueue, Context (+2 more)

### Community 389 - "Community 389"
Cohesion: 0.13
Nodes (15): integrations, connect_disabled_tooltip, connect_github, connect_opening, github_description_done, github_description_prefix, github_description_suffix, github_identifier_example (+7 more)

### Community 390 - "Community 390"
Cohesion: 0.13
Nodes (15): sidebar, created, created_by, files, id, metadata, origin, permissions (+7 more)

### Community 391 - "Community 391"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 392 - "Community 392"
Cohesion: 0.15
Nodes (13): Squad Archive — Soft Delete with Auto-reassignment to Leader, Squad Assignment Flow — Leader Triggered on Non-Backlog Issue Assignment, Squad @-mention — Trigger Leader Without Changing Assignee or Status, Squad Concept — Named Group of Agents and Members with a Leader, Squad Evaluation CLI — aicortex squad activity Command for Leader Logging, Squad Instructions — Custom Routing Rules Written by Squad Admin, Squad Leader — Must Be an Agent, Routes Work to Members, Squad Members — Agents or Human Members with Role Descriptions (+5 more)

### Community 393 - "Community 393"
Cohesion: 0.26
Nodes (11): CreateDesignSessionRequest, designSessionFromListRow(), designSessionFromRow(), DesignSessionResponse, ListDesignChatSessionsByProjectRow, ChatSession, Context, Handler (+3 more)

### Community 394 - "Community 394"
Cohesion: 0.10
Nodes (21): agents_group, created_by, error_agent_unavailable_fallback, error_daemon_version, error_unknown, no_agents, pick_an_agent, prompt_placeholder (+13 more)

### Community 395 - "Community 395"
Cohesion: 0.05
Nodes (39): 1.1 当前现状, 1.2 三阶段方案, 1.3 不在范围内, 1. 背景与目标, 2.1 Skill 是什么, 2.2 AICortex 的 Skill 数据模型, 2.3 Skill 流转链路(数据库 → runtime), 2.4 Template = Instructions + Skill 引用 (+31 more)

### Community 396 - "Community 396"
Cohesion: 0.36
Nodes (17): newAutoUpdateTestDaemon(), TestAutoUpdateLoop_EarlyExits(), TestTryAutoUpdate_DefersWhenClaimInFlightAtBarrier(), TestTryAutoUpdate_DoesNotRestartOnUpgradeFailure(), TestTryAutoUpdate_HoldsBarrierAcrossRestart(), TestTryAutoUpdate_ReleasesBarrierOnUpgradeFailure(), TestTryAutoUpdate_RunsUpgradeAndRestartsOnNewer(), TestTryAutoUpdate_SkipsWhenFetchFails() (+9 more)

### Community 397 - "Community 397"
Cohesion: 0.12
Nodes (16): add, save, unsaved_changes, input_placeholder, intro, launch_mode_args_placeholder, launch_mode_prefix, remove_aria (+8 more)

### Community 398 - "Community 398"
Cohesion: 0.29
Nodes (13): Carousel(), CarouselApi, CarouselContent(), CarouselContext, CarouselContextProps, CarouselItem(), CarouselNext(), CarouselOptions (+5 more)

### Community 399 - "Community 399"
Cohesion: 0.12
Nodes (16): message_list, copied_toast, copy_action, copy_failed_toast, failed_after, process_steps_one, process_steps_other, queued_follow_up_one (+8 more)

### Community 400 - "Community 400"
Cohesion: 0.16
Nodes (6): applyTheme(), buildPresenterHTML(), cycleTheme(), fromHash(), go(), openPresenterWindow()

### Community 401 - "Community 401"
Cohesion: 0.22
Nodes (5): agent-spritesheet, furniture-spritesheet, game-assets, office-environment, virtual-office-game

### Community 402 - "Community 402"
Cohesion: 0.16
Nodes (16): CreateGitHubInstallationParams, DeleteGitHubInstallationByInstallationIDRow, DeleteGitHubInstallationParams, GetGitHubPullRequestParams, GetSiblingPullRequestStateCountsForIssueParams, GetSiblingPullRequestStateCountsForIssueRow, LinkIssueToPullRequestParams, UnlinkIssueFromPullRequestParams (+8 more)

### Community 403 - "Community 403"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 404 - "Community 404"
Cohesion: 0.28
Nodes (11): Load(), loadFromFS(), TestLoad_RealTemplates(), TestLoadFromFS_DuplicateSlug(), TestLoadFromFS_Invalid(), TestLoadFromFS_PromptOnlyTemplate(), TestLoadFromFS_Valid(), validate() (+3 more)

### Community 405 - "Community 405"
Cohesion: 0.19
Nodes (14): DesignGenerationPreview(), STEP_LABEL_KEYS, activeStepId(), buildDesignGenerationPreviewModel(), deriveDesignGenerationSteps(), generationDetailLabel(), GenerationPreviewModel, GenerationPreviewStep (+6 more)

### Community 406 - "Community 406"
Cohesion: 0.18
Nodes (14): blockingLookupRepoCache, assertActiveTaskCount(), newBlockingLookupRepoCache(), TestHealthHandlerActiveTaskCountTracksCounter(), TestHealthHandlerReportsCLIVersionAndActiveTaskCount(), TestHealthHandlerRespondsWhileTaskRepoLookupWaits(), TestShutdownHandlerPostCancelsDaemonContext(), TestShutdownHandlerRejectsNonPost() (+6 more)

### Community 407 - "Community 407"
Cohesion: 0.14
Nodes (14): hint, title, page, empty, last_run_empty, new_autopilot, start_blank, table (+6 more)

### Community 408 - "Community 408"
Cohesion: 0.31
Nodes (12): AgentInteractiveCliSpec, appendResumeArgs(), buildAgentInteractiveCliLaunch(), buildAgentInteractiveCliLaunchSteps(), buildAgentInteractiveCliShellLaunch, buildShellCdCommand(), INTERACTIVE_CLI_BY_PROVIDER, INTERACTIVE_CLI_SPECS (+4 more)

### Community 409 - "Community 409"
Cohesion: 0.20
Nodes (13): backlogIssue, mockAuthState, mockCreatePinMutate, mockDeletePinMutate, mockIssue, mockOpenModal, mockUpdateMutate, pinListRef (+5 more)

### Community 410 - "Community 410"
Cohesion: 0.21
Nodes (8): Metrics, loadOrInitCounter(), snapshotCounters(), Bool, Int64, Map, RWMutex, Value

### Community 411 - "Community 411"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 412 - "Community 412"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Dark Mode, 8. Responsive Behavior (+31 more)

### Community 413 - "Community 413"
Cohesion: 0.18
Nodes (15): CreateAttachmentParams, DeleteAttachmentParams, GetAttachmentParams, LinkAttachmentsToChatMessageParams, LinkAttachmentsToCommentParams, LinkAttachmentsToIssueParams, ListAttachmentsByChatMessageIDsParams, ListAttachmentsByChatMessageParams (+7 more)

### Community 414 - "Community 414"
Cohesion: 0.16
Nodes (18): ArchiveAllInboxParams, ArchiveAllReadInboxParams, ArchiveCompletedInboxParams, ArchiveInboxByIssueAndTypeParams, ArchiveInboxByIssueAndTypeRow, ArchiveInboxByIssueParams, CountUnreadInboxParams, CreateInboxItemParams (+10 more)

### Community 415 - "Community 415"
Cohesion: 0.14
Nodes (13): finishReason, stopReason, meta, aborted, completion, durationMs, finalAssistantRawText, finalAssistantVisibleText (+5 more)

### Community 416 - "Community 416"
Cohesion: 0.14
Nodes (14): systemPromptReport, mode, sandboxed, bootstrapMaxChars, bootstrapTotalMaxChars, generatedAt, injectedWorkspaceFiles, model (+6 more)

### Community 417 - "Community 417"
Cohesion: 0.21
Nodes (16): BackdropGradient(), DownloadHero(), HeroContent, HeroDict, HeroTracking, PrimaryCta(), Props, resolveContent() (+8 more)

### Community 418 - "Community 418"
Cohesion: 0.22
Nodes (13): CreateDevChatSessionParams, ListDevChatSessionsByCreatorParams, ListDevChatSessionsByCreatorRow, ListDevChatSessionsByProjectParams, ListDevChatSessionsByProjectRow, ListDevChatSessionsByWorkspaceRow, SetWorkspaceDefaultDevAgentParams, ChatSession (+5 more)

### Community 419 - "Community 419"
Cohesion: 0.27
Nodes (17): advanceStageDescription(), appendToSummary(), hasMeetingLabel(), init(), meetingDescription(), meetingLabel(), parseStages(), runMeetingAdvance() (+9 more)

### Community 420 - "Community 420"
Cohesion: 0.14
Nodes (14): hint, title, page, empty, last_run_empty, new_autopilot, start_blank, table (+6 more)

### Community 421 - "Community 421"
Cohesion: 0.14
Nodes (14): batch, assignee, cancel, delete, delete_dialog_desc_other, delete_dialog_title_other, delete_dialog_warning, delete_failed (+6 more)

### Community 422 - "Community 422"
Cohesion: 0.26
Nodes (13): ClearAuthCookies(), cookieDomain(), generateCSRFToken(), isSecureCookie(), SetAuthCookies(), TestCookieDomain(), TestIsSecureCookie(), TestSetAuthCookies_HTTPSelfHost() (+5 more)

### Community 423 - "Community 423"
Cohesion: 0.30
Nodes (15): localSkillReportDaemon(), TestReportLocalSkillImportResult_RetriesOn500AndEventuallySucceeds(), TestReportLocalSkillListResult_DoesNotRetryOn4xx(), TestReportLocalSkillListResult_RetriesOn500AndEventuallySucceeds(), TestReportLocalSkillResult_AbortsOnContextCancel(), TestReportLocalSkillResult_GivesUpAfterAllAttemptsFail(), TestReportLocalSkillResult_SendsCorrectPath(), withFastLocalSkillReportBackoffs() (+7 more)

### Community 424 - "Community 424"
Cohesion: 0.12
Nodes (17): completed, failed, pending, running, timeout, update, action, available (+9 more)

### Community 425 - "Community 425"
Cohesion: 0.18
Nodes (13): actor_issues, empty, scope, description, label, title, description, label (+5 more)

### Community 426 - "Community 426"
Cohesion: 0.15
Nodes (13): desc, title, method, desc, title, chooser, manual, runtime (+5 more)

### Community 427 - "Community 427"
Cohesion: 0.15
Nodes (13): description, label, description, label, scopes, all, mine, unused (+5 more)

### Community 428 - "Community 428"
Cohesion: 0.17
Nodes (20): AgentResponse, fetchSkillFromURL(), fetchTemplateSkillsParallel(), init(), templateToDetail(), templateToSummary(), AgentTemplateResponse, AgentTemplateSkillResponse (+12 more)

### Community 429 - "Community 429"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 430 - "Community 430"
Cohesion: 0.24
Nodes (12): Context, Duration, SelectStaleOnlineRuntimesRow, T, UUID, fakeLiveness, candidateRow(), makeUUIDForFilter() (+4 more)

### Community 431 - "Community 431"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 432 - "Community 432"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 433 - "Community 433"
Cohesion: 0.27
Nodes (9): extractTrailingFields(), LEVEL_NORMALIZE, LogLevel, ParsedLogLine, parseLogLine(), a, b, r (+1 more)

### Community 434 - "Community 434"
Cohesion: 0.14
Nodes (18): extractCopyText(), splitTimeline(), f, f1, f2, items, message(), out (+10 more)

### Community 435 - "Community 435"
Cohesion: 0.15
Nodes (13): cancel, description, empty, field_cache_read, field_cache_write, field_input, field_output, open_button (+5 more)

### Community 436 - "Community 436"
Cohesion: 0.30
Nodes (10): semver, CheckMinCLIVersion(), CheckMinVersion(), parseSemver(), TestCheckMinCLIVersion(), TestCheckMinVersion(), TestExtractVersionLine(), TestParseSemver() (+2 more)

### Community 437 - "Community 437"
Cohesion: 0.18
Nodes (15): CountCommentsParams, CreateCommentParams, GetCommentInWorkspaceParams, HasAgentCommentedSinceParams, HasAgentRepliedInThreadParams, ListCommentsForIssueParams, ListCommentsSinceForIssueParams, ResolveCommentParams (+7 more)

### Community 438 - "Community 438"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 439 - "Community 439"
Cohesion: 0.15
Nodes (13): devDependencies, @aicortex/tsconfig, eslint-plugin-i18next, jsdom, rehype-sanitize, @testing-library/jest-dom, @testing-library/react, @testing-library/user-event (+5 more)

### Community 440 - "Community 440"
Cohesion: 0.18
Nodes (13): actor_issues, empty, scope, description, label, title, description, label (+5 more)

### Community 441 - "Community 441"
Cohesion: 0.15
Nodes (13): welcome, continue_on_web, download_desktop, headline_emphasis, headline_line1, headline_line2, illustration_caption, lede (+5 more)

### Community 442 - "Community 442"
Cohesion: 0.05
Nodes (39): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+31 more)

### Community 443 - "Community 443"
Cohesion: 0.05
Nodes (38): 10. 反模式清单, 11. 检查清单, 1. 设计哲学, 2.1 中性色阶梯, 2.2 语义色, 2.3 暗色模式, 2. 颜色体系, 3.1 字体家族 (+30 more)

### Community 444 - "Community 444"
Cohesion: 0.09
Nodes (21): delete_workspace_dialog, cancel, confirm, deleting, description, title, type_to_confirm_prefix, type_to_confirm_suffix (+13 more)

### Community 445 - "Community 445"
Cohesion: 0.15
Nodes (13): desc, title, method, desc, title, chooser, manual, runtime (+5 more)

### Community 446 - "Community 446"
Cohesion: 0.15
Nodes (13): description, label, description, label, scopes, all, mine, unused (+5 more)

### Community 447 - "Community 447"
Cohesion: 0.26
Nodes (11): author, email, name, description, homepage, license, main, name (+3 more)

### Community 448 - "Community 448"
Cohesion: 0.17
Nodes (12): CLI, CLI Agent Commands, CLI Auth and Setup Commands, CLI Authentication, CLI Autopilot Commands, CLI Daemon and Runtime Commands, CLI Issue Commands, CLI Miscellaneous Commands (+4 more)

### Community 449 - "Community 449"
Cohesion: 0.18
Nodes (11): BatchedHeartbeatScheduler, NewPassthroughHeartbeatScheduler(), HeartbeatScheduler, PassthroughHeartbeatScheduler, AgentRuntime, Context, Duration, Mutex (+3 more)

### Community 450 - "Community 450"
Cohesion: 0.16
Nodes (14): code_block, copy_code, file_card, uploading, link_hover, copy_failed, copy_link, link_copied (+6 more)

### Community 451 - "Community 451"
Cohesion: 0.17
Nodes (12): url, cancel, fallback_error, import, importing, importing_clawhub, importing_github, importing_skills_sh (+4 more)

### Community 452 - "Community 452"
Cohesion: 0.17
Nodes (12): manual, cancel, description_label, description_placeholder, fallback_error, name_conflict_hint, name_hint, name_label (+4 more)

### Community 453 - "Community 453"
Cohesion: 0.29
Nodes (7): method_card, manual_desc, manual_title, runtime_desc, runtime_title, url_desc, url_title

### Community 454 - "Community 454"
Cohesion: 0.05
Nodes (38): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+30 more)

### Community 455 - "Community 455"
Cohesion: 0.14
Nodes (5): ForumAutoState, ThreadState, AutoChatterConfig, Mutex, Time

### Community 456 - "Community 456"
Cohesion: 0.06
Nodes (33): shell, cli_back_to_chat, cli_launch, cli_launch_disabled_explore, cli_launch_disabled_provider, cli_launch_disabled_runtime, cli_launch_label, cli_resume_drift_hint (+25 more)

### Community 457 - "Community 457"
Cohesion: 0.22
Nodes (13): completeOnboardingRequest, dismissStarterContentRequest, importIssueSpec, importStarterContentRequest, importStarterContentResponse, joinCloudWaitlistRequest, strOrNullText(), patchOnboardingRequest (+5 more)

### Community 458 - "Community 458"
Cohesion: 0.05
Nodes (38): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+30 more)

### Community 459 - "Community 459"
Cohesion: 0.33
Nodes (14): issueNoAssignee(), issueWithAgentAssignee(), TestCommentMentionsOthersButNotAssignee(), TestCommentMentionsOthersButNotAssignee_NoAssignee(), TestIsReplyToMemberThread(), TestOnCommentTriggerDecision(), TestShouldInheritParentMentions(), TestShouldInheritParentMentions_AgentReviewDelegationDoesNotLeak() (+6 more)

### Community 460 - "Community 460"
Cohesion: 0.19
Nodes (15): Context, AgentTaskQueue, ChatSession, Context, Issue, T, UUID, dbScopeAuthorizer (+7 more)

### Community 461 - "Community 461"
Cohesion: 0.18
Nodes (18): AgentData, AgentData, AgentEntry, ChatAttachmentMeta, DesignExampleBundle, ProjectResourceData, RepoData, Runtime (+10 more)

### Community 462 - "Community 462"
Cohesion: 0.19
Nodes (13): activityToEntry(), AssigneeFrequencyEntry, RecentActivityResponse, TimelineEntry, timelinePaginatedResponse, ActivityLog, AttachmentResponse, Comment (+5 more)

### Community 463 - "Community 463"
Cohesion: 0.05
Nodes (38): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+30 more)

### Community 464 - "Community 464"
Cohesion: 0.34
Nodes (17): countSkillFiles(), countSkillsByName(), createRuntimeLocalSkillTestMember(), createRuntimeLocalSkillTestRuntime(), newRequestAsUser(), ptr(), TestCleanOptionalString(), TestGetLocalSkillImportRequest_RequiresRuntimeOwner() (+9 more)

### Community 465 - "Community 465"
Cohesion: 0.17
Nodes (12): exports, ./components/common/*, ./components/ui/*, ./hooks/*, ./i18n-types, ./lib/data-table, ./lib/utils, ./markdown (+4 more)

### Community 466 - "Community 466"
Cohesion: 0.03
Nodes (99): identify(), resetAnalytics(), getApi(), setSchemaLogger(), AuthStoreInstance, registerAuthStore(), AuthStoreOptions, createAuthStore() (+91 more)

### Community 467 - "Community 467"
Cohesion: 0.12
Nodes (16): list, cli_managed_badge, cli_update_available_aria, cli_update_available_tooltip, col_agents, col_cli, col_cost, col_health (+8 more)

### Community 468 - "Community 468"
Cohesion: 0.23
Nodes (12): cancel, confirm, deleting, description, title, delete_dialog, trigger_row, delete_dialog (+4 more)

### Community 469 - "Community 469"
Cohesion: 0.14
Nodes (14): message_list, copied_toast, copy_action, copy_failed_toast, failed_after, process_steps_other, queued_follow_up_one, queued_follow_up_other (+6 more)

### Community 470 - "Community 470"
Cohesion: 0.28
Nodes (14): EnsureDaemonID(), filterLegacyIDs(), LegacyDaemonIDs(), LegacyDaemonUUIDs(), promoteProfileDaemonID(), TestEnsureDaemonID_Persists(), TestEnsureDaemonID_PromotesPreChangeProfileFile(), TestEnsureDaemonID_RegeneratesCorruptFile() (+6 more)

### Community 471 - "Community 471"
Cohesion: 0.17
Nodes (12): url, cancel, fallback_error, import, importing, importing_clawhub, importing_github, importing_skills_sh (+4 more)

### Community 472 - "Community 472"
Cohesion: 0.17
Nodes (12): manual, cancel, description_label, description_placeholder, fallback_error, name_conflict_hint, name_hint, name_label (+4 more)

### Community 473 - "Community 473"
Cohesion: 0.29
Nodes (7): method_card, manual_desc, manual_title, runtime_desc, runtime_title, url_desc, url_title

### Community 474 - "Community 474"
Cohesion: 0.32
Nodes (7): INBOX_GROUP_KEYS, InboxGroupKey, NotificationsTab(), useUpdateNotificationPreferences(), notificationPreferenceKeys, notificationPreferenceOptions(), NotificationPreferences

### Community 475 - "Community 475"
Cohesion: 0.36
Nodes (11): apps/web/public/images/feature-bg.jpg, apps/web/public/images/feature-bg-2.jpg, apps/web/public/images/feature-bg-3.jpg, apps/web/public/images/feature-bg-4.jpg, apps/web/public/images/landing-bg.jpg, apps/web/public/images/landing-hero.png, FeaturesSection (features-section.tsx), LandingHero (landing-hero.tsx) (+3 more)

### Community 476 - "Community 476"
Cohesion: 0.05
Nodes (37): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+29 more)

### Community 477 - "Community 477"
Cohesion: 0.18
Nodes (11): Autopilot, Autopilot Failure Behavior, Autopilot Trigger Types, Chat, Autopilot Execution Modes, Autopilot Manual Trigger, Autopilot Run History, Autopilot Schedule Trigger (+3 more)

### Community 478 - "Community 478"
Cohesion: 0.16
Nodes (19): autoCommitAndPush(), ensureGitExclude(), runGit(), detectDefaultBranch(), extractLocalPath(), isDesignStudioTask(), isGitRepo(), pinnedWorkdirPath() (+11 more)

### Community 479 - "Community 479"
Cohesion: 0.20
Nodes (13): durationFromEnv(), envOrDefault(), intFromEnv(), parseFlexDuration(), sleepWithContext(), sleepWithContextOrWakeup(), TestParseFlexDuration(), TestParseFlexDuration_Invalid() (+5 more)

### Community 480 - "Community 480"
Cohesion: 0.23
Nodes (14): ActivityHeatmap(), DAY_LABELS, fmtDate(), fmtMoney(), getHeatmapColor(), Insight(), Insights, InsightsRow() (+6 more)

### Community 481 - "Community 481"
Cohesion: 0.13
Nodes (15): agent_label, agent_placeholder, allow_new_sessions_description, allow_new_sessions_label, create_success, expires_label, guide_message_label, guide_message_placeholder (+7 more)

### Community 482 - "Community 482"
Cohesion: 0.28
Nodes (12): AllPlatforms(), Arch, Format, isFallbackNeeded(), Platform, Props, Row(), RowProps (+4 more)

### Community 483 - "Community 483"
Cohesion: 0.23
Nodes (12): cancel, confirm, deleting, description, title, delete_dialog, trigger_row, delete_dialog (+4 more)

### Community 484 - "Community 484"
Cohesion: 0.23
Nodes (10): CreatePinRequest, pinnedItemToResponse(), PinnedItemResponse, ReorderItem, ReorderPinsRequest, ReorderItem, Handler, PinnedItem (+2 more)

### Community 485 - "Community 485"
Cohesion: 0.18
Nodes (11): agent_live, cancel_failed, fallback_name, is_queued, is_working, queued_elapsed_prefix, stop_button, stop_tooltip (+3 more)

### Community 486 - "Community 486"
Cohesion: 0.05
Nodes (37): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+29 more)

### Community 487 - "Community 487"
Cohesion: 0.05
Nodes (38): hyperframes, live_artifact, hub, attachments_soon, back_to_project, brand_name, chip_hints, clear_active_example (+30 more)

### Community 488 - "Community 488"
Cohesion: 0.08
Nodes (30): DesignExamplePreview(), CREATE_CHIP_IDS, DesignStudioHub(), INTENT_ICONS, MORE_CHIP_IDS, SUBCATEGORY_ICONS, CatalogItem(), DesignSystemPicker() (+22 more)

### Community 489 - "Community 489"
Cohesion: 0.05
Nodes (37): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+29 more)

### Community 490 - "Community 490"
Cohesion: 0.05
Nodes (37): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+29 more)

### Community 491 - "Community 491"
Cohesion: 0.63
Nodes (11): runningSquadLeaderTaskFixture, completeRunningTask(), countAgentCommentsForIssue(), newRunningSquadLeaderTaskFixture(), recordSquadLeaderEvaluationForTask(), recordSquadLeaderEvaluationForTaskWithHeader(), TestCompleteTask_SquadLeaderActionStillSynthesizesComment(), TestCompleteTask_SquadLeaderNoActionCanonicalizesTaskID() (+3 more)

### Community 492 - "Community 492"
Cohesion: 0.28
Nodes (11): detectGitRepo(), excludeFromGit(), fetchOrigin(), getRemoteDefaultBranch(), removeGitWorktree(), repoNameFromURL(), runGitWorktreeAdd(), sanitizeName() (+3 more)

### Community 493 - "Community 493"
Cohesion: 0.30
Nodes (10): ClientMetadata(), ClientMetadataFromContext(), SetClientMetadata(), TestClientMetadataExtractsHeaders(), TestClientMetadataMissingHeadersReturnEmpty(), TestSetClientMetadataAttachesValues(), clientMetadataKey, Context (+2 more)

### Community 494 - "Community 494"
Cohesion: 0.23
Nodes (15): GetRuntimeTaskHourlyActivityParams, GetRuntimeTaskHourlyActivityRow, GetRuntimeUsageByHourParams, GetRuntimeUsageByHourRow, ListRuntimeUsageByAgentParams, ListRuntimeUsageByAgentRow, ListRuntimeUsageDailyParams, ListRuntimeUsageDailyRow (+7 more)

### Community 495 - "Community 495"
Cohesion: 0.05
Nodes (37): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+29 more)

### Community 496 - "Community 496"
Cohesion: 0.21
Nodes (10): compilerOptions, outDir, paths, rootDir, exclude, extends, include, @/components/ui/* (+2 more)

### Community 497 - "Community 497"
Cohesion: 0.15
Nodes (13): page, export, import, learn_more, list_load_failed, list_load_failed_default, new_agent, of_total (+5 more)

### Community 498 - "Community 498"
Cohesion: 0.18
Nodes (11): tabs, activity, custom_args, discard_confirm, discard_dialog_description, discard_dialog_title, discard_keep, environment (+3 more)

### Community 499 - "Community 499"
Cohesion: 0.18
Nodes (11): empty_state, chat_with_name, first_time_actions, first_time_intro, first_time_pillars, first_time_pillars_suffix, first_time_title, returning_subtitle (+3 more)

### Community 500 - "Community 500"
Cohesion: 0.18
Nodes (11): feedback, github_hint_link, github_hint_prefix, placeholder, send, sending, title, toast_failed (+3 more)

### Community 501 - "Community 501"
Cohesion: 0.18
Nodes (11): runtime_aside, add_more_body, add_more_title, good_eyebrow, learn_more, swap_body, swap_title, what_eyebrow (+3 more)

### Community 502 - "Community 502"
Cohesion: 0.18
Nodes (11): labs, co_authored_by_description_prefix, co_authored_by_description_suffix, co_authored_by_label, forum_description, forum_label, pinned_workdir_description, pinned_workdir_label (+3 more)

### Community 503 - "Community 503"
Cohesion: 0.18
Nodes (11): chinese, english, sync_failed, title, preferences, language, theme, dark (+3 more)

### Community 504 - "Community 504"
Cohesion: 0.20
Nodes (10): aicortex issue, aicortex issue assign, aicortex issue comment, aicortex issue create, aicortex issue get, aicortex issue list, aicortex issue runs, aicortex issue status (+2 more)

### Community 505 - "Community 505"
Cohesion: 0.25
Nodes (10): applyTemplates(), commandsInGroup(), exactArgs(), formatCommandList(), init(), initHelp(), init(), main() (+2 more)

### Community 506 - "Community 506"
Cohesion: 0.22
Nodes (9): compilerOptions, baseUrl, composite, jsx, noImplicitAny, paths, extends, include (+1 more)

### Community 507 - "Community 507"
Cohesion: 0.05
Nodes (38): hyperframes, live_artifact, hub, attachments_soon, back_to_project, brand_name, chip_hints, clear_active_example (+30 more)

### Community 508 - "Community 508"
Cohesion: 0.13
Nodes (35): exampleBundle, exampleManifest, exampleManifestEntry, bootstrapHtmlPptDeck(), candidateSearchRoots(), copyDir(), copyFile(), designResourcesRoot() (+27 more)

### Community 509 - "Community 509"
Cohesion: 0.05
Nodes (36): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+28 more)

### Community 510 - "Community 510"
Cohesion: 0.20
Nodes (10): backlog_hint, description, dont_show_again, keep_in_backlog, move_to_todo, row_backlog_hint, row_backlog_label, row_todo_hint (+2 more)

### Community 511 - "Community 511"
Cohesion: 0.20
Nodes (10): resolve, bar_authors_more_one, bar_authors_more_other, bar_one, bar_other, collapse, resolve_action, resolve_failed (+2 more)

### Community 512 - "Community 512"
Cohesion: 0.20
Nodes (10): add, cancel, errors, placeholder, add_file, absolute, double_dot, empty (+2 more)

### Community 513 - "Community 513"
Cohesion: 0.20
Nodes (10): subline, by_creator, origin_clawhub, origin_github, origin_runtime_named, origin_runtime_provider, origin_runtime_unknown, origin_skills_sh (+2 more)

### Community 514 - "Community 514"
Cohesion: 0.14
Nodes (13): public, agent_default_name, disconnected, loading, new_session, reconnecting, send_tooltip, status_disabled (+5 more)

### Community 515 - "Community 515"
Cohesion: 0.05
Nodes (36): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+28 more)

### Community 516 - "Community 516"
Cohesion: 0.10
Nodes (30): CreateMemberRequest, CreateWorkspaceRequest, countOwners(), workspaceIDFromURL(), invitationToResponse(), InvitationResponse, MemberResponse, MemberWithUserResponse (+22 more)

### Community 517 - "Community 517"
Cohesion: 0.15
Nodes (12): Workspaces (English), Issue 编号, 下一步, 创建工作区, 删除工作区, Issue Number Sequence — Sequential Within Workspace, Never Reclaimed on Delete, Workspace — Self-Contained Collaboration Space in AICortex, Workspace Creation — Name, Slug, Issue Prefix (+4 more)

### Community 518 - "Community 518"
Cohesion: 0.47
Nodes (12): createIssueForTimeline(), fetchTimeline(), fetchTimelineWrapped(), seedTimelineEntries(), TestListTimeline_EmptyIssue(), TestListTimeline_LegacyWrappedShape_AroundFillsTargetIndex(), TestListTimeline_LegacyWrappedShape_OnPaginationParams(), TestListTimeline_MergesCommentsAndActivities() (+4 more)

### Community 519 - "Community 519"
Cohesion: 0.11
Nodes (33): chatMessage, chatRequest, chatResponse, ConfigFromEnv(), envOrDefault(), NewClientFromEnv(), NewOpenAIClient(), TestConfigFromEnv() (+25 more)

### Community 520 - "Community 520"
Cohesion: 0.09
Nodes (22): management, copied, copy_link, create, delete, delete_confirm, description, edit (+14 more)

### Community 521 - "Community 521"
Cohesion: 0.38
Nodes (14): NewRedisLocalSkillImportStore(), NewRedisLocalSkillListStore(), newRedisTestClient(), TestRedisLocalSkillImportStore_PopPendingAcrossInstances(), TestRedisLocalSkillImportStore_PreservesCreatorID(), TestRedisLocalSkillListStore_CreateGetComplete(), TestRedisLocalSkillListStore_PendingTimeout(), TestRedisLocalSkillListStore_PerRuntimeIsolation() (+6 more)

### Community 522 - "Community 522"
Cohesion: 0.36
Nodes (8): BuildCommentReplyInstructions(), TestBuildCommentReplyInstructionsCodexLinux(), TestBuildCommentReplyInstructionsEmptyWhenNoTrigger(), TestBuildCommentReplyInstructionsNonCodexLinux(), TestBuildCommentReplyInstructionsWindowsUsesContentFile(), TestInjectRuntimeConfigCommentTriggerUsesHelper(), TestInjectRuntimeConfigWindowsCommentTriggerHasNoStdin(), T

### Community 523 - "Community 523"
Cohesion: 0.38
Nodes (10): codexDarwinNetworkAccessFixed(), codexUpgradeHint(), ensureCodexSandboxConfig(), parseCodexSemver(), renderAICortexManagedBlock(), stripLegacySandboxDirectives(), upsertAICortexManagedBlock(), codexSandboxPolicy (+2 more)

### Community 524 - "Community 524"
Cohesion: 0.21
Nodes (11): localFirstPublisher, attachRealtimeTestClient(), TestNewRedisRelayWithClientsSeparatesBlockingReadPool(), TestRedisRelayStopPreventsNewConsumers(), TestShardedStreamRelayConfigDefaults(), TestShardedStreamRelayDeliverMessageUsesEnvelopeScope(), TestShardedStreamRelayShardForScopeIsStableAndBounded(), Client (+3 more)

### Community 525 - "Community 525"
Cohesion: 0.05
Nodes (36): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+28 more)

### Community 526 - "Community 526"
Cohesion: 0.19
Nodes (16): ForumPost, ForumReaction, ForumReply, AddForumReactionParams, CreateForumPostParams, CreateForumReplyParams, ListForumPostsParams, ListForumPostsRow (+8 more)

### Community 527 - "Community 527"
Cohesion: 0.05
Nodes (36): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+28 more)

### Community 528 - "Community 528"
Cohesion: 0.05
Nodes (36): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+28 more)

### Community 529 - "Community 529"
Cohesion: 0.23
Nodes (11): CreateInvitationParams, ExpireStalePendingInvitationsParams, GetPendingInvitationByEmailParams, ListPendingInvitationsByWorkspaceRow, ListPendingInvitationsForUserParams, ListPendingInvitationsForUserRow, Context, Queries (+3 more)

### Community 530 - "Community 530"
Cohesion: 0.19
Nodes (15): AttachLabelToIssueParams, CreateLabelParams, DeleteLabelParams, DetachLabelFromIssueParams, GetLabelParams, ListLabelsByIssueParams, ListLabelsForIssuesParams, ListLabelsForIssuesRow (+7 more)

### Community 531 - "Community 531"
Cohesion: 0.06
Nodes (35): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+27 more)

### Community 532 - "Community 532"
Cohesion: 0.18
Nodes (12): Rows, AgentTaskQueue, CommandTag, Context, Row, T, UUID, mockDBTX (+4 more)

### Community 533 - "Community 533"
Cohesion: 0.20
Nodes (10): skills_section, collapse, label, list_empty_default, list_empty_multi, list_loading, list_no_match, placeholder (+2 more)

### Community 534 - "Community 534"
Cohesion: 0.20
Nodes (10): context_anchor, aria_start, aria_stop, card_tooltip_issue, card_tooltip_issue_with_subtitle, card_tooltip_project, tooltip_disabled, tooltip_off (+2 more)

### Community 535 - "Community 535"
Cohesion: 0.16
Nodes (14): code_block, copy_code, file_card, uploading, link_hover, copy_failed, copy_link, link_copied (+6 more)

### Community 536 - "Community 536"
Cohesion: 0.06
Nodes (35): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+27 more)

### Community 537 - "Community 537"
Cohesion: 0.20
Nodes (10): backlog_hint, description, dont_show_again, keep_in_backlog, move_to_todo, row_backlog_hint, row_backlog_label, row_todo_hint (+2 more)

### Community 538 - "Community 538"
Cohesion: 0.13
Nodes (15): sidebar, configure_group, create_workspace, invitation_decline, invitation_join, invitation_workspace_fallback, log_out, new_issue (+7 more)

### Community 539 - "Community 539"
Cohesion: 0.20
Nodes (10): description, label, description, label, roles, description, label, admin (+2 more)

### Community 540 - "Community 540"
Cohesion: 0.18
Nodes (11): tabs, chat-share, general, integrations, labs, members, notifications, preferences (+3 more)

### Community 541 - "Community 541"
Cohesion: 0.20
Nodes (10): account, click_avatar_hint, name_label, save, saving, section_profile, toast_avatar_failed, toast_avatar_updated (+2 more)

### Community 542 - "Community 542"
Cohesion: 0.20
Nodes (10): add, cancel, errors, placeholder, add_file, absolute, double_dot, empty (+2 more)

### Community 543 - "Community 543"
Cohesion: 0.20
Nodes (10): subline, by_creator, origin_clawhub, origin_github, origin_runtime_named, origin_runtime_provider, origin_runtime_unknown, origin_skills_sh (+2 more)

### Community 544 - "Community 544"
Cohesion: 0.22
Nodes (4): { rerender }, reset(), state, useTabStore

### Community 545 - "Community 545"
Cohesion: 0.36
Nodes (9): cfBase64Encode(), loadKeyFromSecretsManager(), loadPrivateKey(), NewCloudFrontSignerFromEnv(), parseRSAPrivateKey(), CloudFrontSigner, Cookie, PrivateKey (+1 more)

### Community 546 - "Community 546"
Cohesion: 0.25
Nodes (9): Agent @-mention in Comment, Squad @-mention (triggers leader agent), Agent Task (enqueued by assign or @-mention), Assignment vs @-mention distinction, Agent Task Deduplication (per-comment scope), Issue Assignee (person or agent), Project Lead (member or agent), Agent Self-@ Mention Guard (prevents infinite loops) (+1 more)

### Community 547 - "Community 547"
Cohesion: 0.06
Nodes (35): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+27 more)

### Community 548 - "Community 548"
Cohesion: 0.17
Nodes (12): close_tooltip, closed, connecting, hint, no_runtime, reconnect_tooltip, retry, session_title (+4 more)

### Community 549 - "Community 549"
Cohesion: 0.17
Nodes (12): web, checking_index, disabled, hint, index_missing, mode_static, mode_tunnel, no_runtime (+4 more)

### Community 550 - "Community 550"
Cohesion: 0.12
Nodes (16): cancel, confirm, confirming, description_default, description_with_title, title, session_history, delete_dialog (+8 more)

### Community 551 - "Community 551"
Cohesion: 0.06
Nodes (34): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+26 more)

### Community 552 - "Community 552"
Cohesion: 0.06
Nodes (34): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+26 more)

### Community 553 - "Community 553"
Cohesion: 0.15
Nodes (12): picker, create_first, empty, new_project, no_results, open_studio, search_placeholder, subtitle (+4 more)

### Community 554 - "Community 554"
Cohesion: 0.16
Nodes (6): applyTheme(), buildPresenterHTML(), cycleTheme(), fromHash(), go(), openPresenterWindow()

### Community 555 - "Community 555"
Cohesion: 0.22
Nodes (9): cancel, confirm, deleting, description_no_agents, description_with_agents_one, description_with_agents_other, title, warning (+1 more)

### Community 556 - "Community 556"
Cohesion: 0.15
Nodes (13): create, back, back_aria, close, close_aria, file_tree, no_files, file_viewer (+5 more)

### Community 557 - "Community 557"
Cohesion: 0.23
Nodes (11): CreateProjectParams, GetProjectInWorkspaceParams, GetProjectIssueStatsRow, ListProjectsParams, UpdateProjectParams, Bool, Context, Queries (+3 more)

### Community 558 - "Community 558"
Cohesion: 0.06
Nodes (33): charts, forkTrend, starGrowth, contributors, active, total, growth, total (+25 more)

### Community 559 - "Community 559"
Cohesion: 0.06
Nodes (33): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+25 more)

### Community 560 - "Community 560"
Cohesion: 0.18
Nodes (12): DBTX, newTestHandler(), TestFindOrCreateUserGating(), TestSignupGating(), mockDB, mockRow, CommandTag, Config (+4 more)

### Community 561 - "Community 561"
Cohesion: 0.29
Nodes (10): ActiveDuplicateError, DuplicateMessage(), LockAndFindActiveDuplicate(), lockKey(), NewActiveDuplicateError(), NormalizeTitle(), Context, Issue (+2 more)

### Community 562 - "Community 562"
Cohesion: 0.22
Nodes (9): Context, Int32, Row, T, TestServerHealthReadinessCachesResult(), TestServerHealthReadyHandlerDBPingFailure(), TestServerHealthReadyHandlerMigrationOutOfDate(), stubReadinessDB (+1 more)

### Community 563 - "Community 563"
Cohesion: 0.22
Nodes (9): devDependencies, @aicortex/tsconfig, jsdom, react, react-dom, @testing-library/react, @types/react, typescript (+1 more)

### Community 564 - "Community 564"
Cohesion: 0.40
Nodes (12): newSquadCommentTriggerFixture(), TestCommentMentionsAnyone(), TestCreateComment_DualRoleAgentWorkerCommentWakesLeader(), TestCreateComment_SquadLeaderSkipOnlyInspectsCurrentMention(), TestCreateComment_SquadMentionPrivateLeaderBlocksPlainMember(), TestCreateComment_SquadMentionTriggersLeader(), TestCreateRetryTask_InheritsIsLeaderTask(), TestShouldEnqueueSquadLeaderOnComment_LeaderSelfTriggerByRole() (+4 more)

### Community 565 - "Community 565"
Cohesion: 0.56
Nodes (7): fakeKiroACPScript(), TestKiroBackendInvokesACPWithTrustAllTools(), TestKiroBackendSetModelFailureFailsTask(), TestKiroBackendUsesSessionLoadForResume(), TestKiroToolNameFromTitle(), TestNewReturnsKiroBackend(), T

### Community 566 - "Community 566"
Cohesion: 0.40
Nodes (5): DBPool, NewDBPool(), pqQuoteIdent(), Context, Pool

### Community 567 - "Community 567"
Cohesion: 0.15
Nodes (12): picker, create_first, empty, new_project, no_results, open_studio, search_placeholder, subtitle (+4 more)

### Community 568 - "Community 568"
Cohesion: 0.36
Nodes (8): data, jsonPath, lines, raw, repoRoot, seen, tsPath, wrapComment()

### Community 569 - "Community 569"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 570 - "Community 570"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 571 - "Community 571"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 572 - "Community 572"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 573 - "Community 573"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 574 - "Community 574"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 575 - "Community 575"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 576 - "Community 576"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 577 - "Community 577"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 578 - "Community 578"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 579 - "Community 579"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 580 - "Community 580"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 581 - "Community 581"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 582 - "Community 582"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 583 - "Community 583"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 584 - "Community 584"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 585 - "Community 585"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 586 - "Community 586"
Cohesion: 0.44
Nodes (9): TestEventFieldsPassedThrough(), TestPanicInHandlerDoesNotBreakOthers(), TestPublishDeliversToSubscribers(), TestPublishNoSubscribersIsNoop(), TestPublishOnlyMatchingType(), TestSubscribeAllCalledAfterTypeSpecific(), TestSubscribeAllPanicRecovery(), TestSubscribeAllReceivesAllEventTypes() (+1 more)

### Community 587 - "Community 587"
Cohesion: 0.26
Nodes (8): CreateProjectResourceParams, GetProjectResourceCountsRow, GetProjectResourceInWorkspaceParams, Context, Queries, ProjectResource, Text, UUID

### Community 588 - "Community 588"
Cohesion: 0.26
Nodes (10): CreateUserParams, JoinCloudWaitlistParams, PatchUserOnboardingParams, SetStarterContentStateParams, UpdateUserParams, Context, Queries, Text (+2 more)

### Community 589 - "Community 589"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 590 - "Community 590"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 591 - "Community 591"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 592 - "Community 592"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 593 - "Community 593"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 594 - "Community 594"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 595 - "Community 595"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 596 - "Community 596"
Cohesion: 0.36
Nodes (8): accent, category, description, icon, instructions, name, skills, slug

### Community 597 - "Community 597"
Cohesion: 0.24
Nodes (13): chatMessageToResponse(), ChatMessageResponse, ChatSessionResponse, CreateChatSessionRequest, PendingChatTaskItem, PendingChatTaskResponse, PendingChatTasksResponse, SendChatMessageRequest (+5 more)

### Community 598 - "Community 598"
Cohesion: 0.13
Nodes (17): name, peerDependencies, i18next, lucide-react, react, react-dom, react-i18next, @tanstack/react-query (+9 more)

### Community 599 - "Community 599"
Cohesion: 0.45
Nodes (13): pickFixtureAgent(), reloadAutopilotStatus(), seedAutopilot(), seedAutopilotRuns(), TestAutopilotFailureMonitor_AgentCreatorRoutesToOwner(), TestAutopilotFailureMonitor_BelowThresholdNoOp(), TestAutopilotFailureMonitor_LeavesAlreadyPausedAlone(), TestAutopilotFailureMonitor_PausesOffenderAndNotifiesCreator() (+5 more)

### Community 600 - "Community 600"
Cohesion: 0.06
Nodes (32): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+24 more)

### Community 601 - "Community 601"
Cohesion: 0.12
Nodes (16): cancel, confirm, confirming, description_default, description_with_title, title, session_history, delete_dialog (+8 more)

### Community 602 - "Community 602"
Cohesion: 0.06
Nodes (32): `agent_created`, `agent_task_failed` / `agent_task_cancelled`, `agent_task_queued` / `agent_task_dispatched` / `agent_task_started` / `agent_task_completed`, Architecture, `autopilot_run_started` / `autopilot_run_completed` / `autopilot_run_failed`, `chat_message_sent`, `cloud_waitlist_joined`, Configuration (+24 more)

### Community 603 - "Community 603"
Cohesion: 0.06
Nodes (32): 1. 数据库（会话持久化）, 2. 协议扩展（server/pkg/protocol）, 3. Daemon 端, 4.1 REST API, 4.2 WebSocket 中继, 4.3 权限, 4. Server 端, 5.1 路由 (+24 more)

### Community 604 - "Community 604"
Cohesion: 0.13
Nodes (15): sessions, cancel, closed_group, create, creating, empty_prompt, list_title, new_action (+7 more)

### Community 605 - "Community 605"
Cohesion: 0.06
Nodes (33): import_dialog, agents_count_one, agents_count_other, cancel, category_agents, category_skills, category_squads, change_file (+25 more)

### Community 606 - "Community 606"
Cohesion: 0.22
Nodes (9): cancel, confirm, deleting, description_no_agents, description_with_agents_one, description_with_agents_other, title, warning (+1 more)

### Community 607 - "Community 607"
Cohesion: 0.15
Nodes (13): create, back, back_aria, close, close_aria, file_tree, no_files, file_viewer (+5 more)

### Community 608 - "Community 608"
Cohesion: 0.25
Nodes (8): ALLOW_SIGNUP, ALLOWED_EMAIL_DOMAINS, ALLOWED_EMAILS, AICORTEX_DEV_VERIFICATION_CODE, Email + Verification Code, Resend, Signup Allowlist, SMTP Relay

### Community 610 - "Community 610"
Cohesion: 0.06
Nodes (32): a) Install the CLI and an AI agent, Architecture Overview, b) One-command setup, Caddy (Recommended), CLI / Daemon, Configuration, Cookies, Database Setup (+24 more)

### Community 611 - "Community 611"
Cohesion: 0.48
Nodes (10): TestReportUpdateResult_AbortsOnContextCancel(), TestReportUpdateResult_DoesNotRetryOn4xx(), TestReportUpdateResult_GivesUpAfterAllAttemptsFail(), TestReportUpdateResult_RetriesOn500AndEventuallySucceeds(), TestReportUpdateResult_SendsCorrectPath(), updateReportDaemon(), withFastUpdateReportBackoffs(), Daemon (+2 more)

### Community 612 - "Community 612"
Cohesion: 0.32
Nodes (8): contributing-check-pipeline, contributing-development-model, contributing-make-commands, contributing-prerequisites, contributing-worktree, dev-env-database-isolation, dev-env-port-isolation, doc-contributing-zh

### Community 613 - "Community 613"
Cohesion: 0.16
Nodes (19): DownloadClient(), DownloadPage(), metadata, emptyRelease(), fetchLatestRelease(), GitHubReleasePayload, isWithinFreshWindow(), LatestRelease (+11 more)

### Community 614 - "Community 614"
Cohesion: 0.06
Nodes (32): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+24 more)

### Community 615 - "Community 615"
Cohesion: 0.06
Nodes (32): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+24 more)

### Community 616 - "Community 616"
Cohesion: 0.06
Nodes (31): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+23 more)

### Community 617 - "Community 617"
Cohesion: 0.18
Nodes (11): empty_state, chat_with_name, first_time_actions, first_time_intro, first_time_pillars, first_time_pillars_suffix, first_time_title, returning_subtitle (+3 more)

### Community 618 - "Community 618"
Cohesion: 0.18
Nodes (12): buttons, dropHandlers, file, makeUpload(), onSend, { onUploadFile }, renderInput(), state (+4 more)

### Community 619 - "Community 619"
Cohesion: 0.06
Nodes (30): 0. 生成前必须通过的类名校验(最重要), 10. WebGL 背景的遮罩透明度, 11. Light hero 的 shader 不能有强中心点, 12. Dark hero 允许更多视觉冲击, 13. 左文右图的对齐, 14. 图片的微弱圆角, 15. 图片路径用相对路径, 16. 页码在 `.chrome` 里写死 (+22 more)

### Community 620 - "Community 620"
Cohesion: 0.14
Nodes (21): isTunnelHopHeader(), prepareTunnelEmbedResponse(), runtimeTunnelPreviewPath(), scanRuntimeTunnel(), tunnelUpstreamQuery(), writeTunnelError(), runtimeTunnelResponse, ProxyResponse (+13 more)

### Community 621 - "Community 621"
Cohesion: 0.25
Nodes (8): status, backlog, blocked, cancelled, done, in_progress, in_review, todo

### Community 622 - "Community 622"
Cohesion: 0.42
Nodes (5): Bus, New(), Event, Handler, RWMutex

### Community 623 - "Community 623"
Cohesion: 0.10
Nodes (21): cursorACPConnOptsFromExec(), CursorACPSupported(), OpenCursorACPConn(), chooseCursorInvocation(), TestChooseCursorInvocation_PassthroughForNonLauncher(), cursorACPBackend, CursorACPConn, CursorACPConnOpts (+13 more)

### Community 624 - "Community 624"
Cohesion: 0.56
Nodes (9): newWaitlistRequest(), newWaitlistTestUser(), TestJoinCloudWaitlistAllowsEmptyReason(), TestJoinCloudWaitlistMissingEmailReturns400(), TestJoinCloudWaitlistRecordsEmailAndReason(), TestJoinCloudWaitlistRejectsOverlongReason(), TestJoinCloudWaitlistSecondCallOverwrites(), Request (+1 more)

### Community 625 - "Community 625"
Cohesion: 0.38
Nodes (7): sanitizeCopiedCodexConfig(), stripSkillsConfigEntries(), TestSanitizeCopiedCodexConfig(), TestSanitizeCopiedCodexConfigMissingFile(), TestSanitizeCopiedCodexConfigNoop(), TestStripSkillsConfigEntries(), T

### Community 626 - "Community 626"
Cohesion: 0.32
Nodes (8): Agent Quick Create 3-Phase Plan, Phase 1: Agent Template, Phase 2: Skill Finder, Phase 3: AI Create Agent, Quick-create Issue Pattern, AI Skill Finder, Skill System, sqlc.yaml

### Community 628 - "Community 628"
Cohesion: 0.44
Nodes (5): checkQuickCreateCliVersion(), CliVersionCheck, CliVersionState, lessThan(), parseSemver()

### Community 629 - "Community 629"
Cohesion: 0.27
Nodes (10): isApiErrorWithStatus(), isMissingTaskError(), isTransientDaemonError(), TRANSIENT_DAEMON_RETRY_DELAYS, transientDaemonRetryDelay(), artifactKeys, useRuntimeArtifactSources(), buildArtifactRawURL() (+2 more)

### Community 630 - "Community 630"
Cohesion: 0.24
Nodes (10): CreateMemberParams, GetMemberByUserAndWorkspaceParams, ListMembersWithUserRow, UpdateMemberRoleParams, Context, Queries, Member, Text (+2 more)

### Community 631 - "Community 631"
Cohesion: 0.27
Nodes (7): CreateWorkspaceParams, UpdateWorkspaceParams, Context, Queries, Text, UUID, Workspace

### Community 632 - "Community 632"
Cohesion: 0.25
Nodes (8): agentHarnessId, contextTokens, model, promptTokens, provider, sessionFile, sessionId, agentMeta

### Community 634 - "Community 634"
Cohesion: 0.33
Nodes (7): name, private, scripts, lint, typecheck, type, version

### Community 635 - "Community 635"
Cohesion: 0.07
Nodes (30): add, save, unsaved_changes, input_placeholder, intro, launch_mode_args_placeholder, launch_mode_prefix, remove_aria (+22 more)

### Community 637 - "Community 637"
Cohesion: 0.25
Nodes (8): add_trigger_dialog, label_field, label_placeholder, submit, submitting, title, toast_add_failed, toast_added

### Community 638 - "Community 638"
Cohesion: 0.25
Nodes (8): friday, monday, saturday, sunday, thursday, tuesday, wednesday, days

### Community 640 - "Community 640"
Cohesion: 0.21
Nodes (10): Collector, CounterVec, Gauge, HistogramVec, isHealthProbePath(), NewHTTPMetrics(), routePattern(), HTTPMetrics (+2 more)

### Community 642 - "Community 642"
Cohesion: 0.25
Nodes (8): status, backlog, blocked, cancelled, done, in_progress, in_review, todo

### Community 643 - "Community 643"
Cohesion: 0.72
Nodes (7): stubPowerShell(), TestPlatformCursorInvocation_RewritesCmdLauncherToPowerShellFile(), TestPlatformCursorInvocation_SkipsWhenNotCmdOrBat(), TestPlatformCursorInvocation_SkipsWhenPowerShellMissing(), TestPlatformCursorInvocation_SkipsWhenPS1Missing(), writeFile(), T

### Community 644 - "Community 644"
Cohesion: 0.43
Nodes (6): AICortexSide(), ArchitectureDiagram(), Connector(), Pill(), SectionLabel(), YourSide()

### Community 645 - "Community 645"
Cohesion: 0.15
Nodes (13): welcome, continue_on_web, download_desktop, headline_emphasis, headline_line1, headline_line2, illustration_caption, lede (+5 more)

### Community 646 - "Community 646"
Cohesion: 0.29
Nodes (7): Agent Project Context (resources in working directory and meta-skill prompt), Meta-skill prompt Project Context section, Project Resource (typed pointer, e.g. GitHub repo), .aicortex/project/resources.json (structured resource list), Workspace Repos (fallback when project has no repo resources), Project Resources Documentation, Resource Type: github_repo (url + default_branch_hint)

### Community 647 - "Community 647"
Cohesion: 0.36
Nodes (6): compilerOptions, outDir, rootDir, exclude, extends, include

### Community 648 - "Community 648"
Cohesion: 0.33
Nodes (7): Desktop Smoke Build, AICortex Backend, AICortex Web, electron-builder.yml, GoReleaser, aicortex/homebrew-tap, Release

### Community 649 - "Community 649"
Cohesion: 0.14
Nodes (14): resources, add_button, attached_badge, empty, local_path_placeholder, popover_title, remove_tooltip, section_header (+6 more)

### Community 650 - "Community 650"
Cohesion: 0.14
Nodes (14): resources, add_button, attached_badge, empty, local_path_placeholder, popover_title, remove_tooltip, section_header (+6 more)

### Community 651 - "Community 651"
Cohesion: 0.21
Nodes (11): name, private, scripts, build, dev, lint, start, test (+3 more)

### Community 652 - "Community 652"
Cohesion: 0.18
Nodes (7): ConPty, commandLineFromCmd(), openPlatformPTY(), windowsQuoteArg(), winPTY, Cmd, platformPTY

### Community 653 - "Community 653"
Cohesion: 0.17
Nodes (12): cloud_waitlist, email_label, email_placeholder, failed_toast, intro_main, intro_warning, join, on_list (+4 more)

### Community 656 - "Community 656"
Cohesion: 0.38
Nodes (9): newRuntimeSetWatcher(), newTaskSlotSemaphore(), noopWriter, TestPollLoopShutdownWaitsForPollersBeforeTaskWG(), TestRunRuntimeHeartbeatIsolatesSlowRuntime(), TestRunRuntimePollerIsolatesSlowRuntime(), TestRunRuntimePollerSkipsClaimWhenAtCapacity(), TestRuntimeSetWatcherFanOut() (+1 more)

### Community 657 - "Community 657"
Cohesion: 0.12
Nodes (16): offline, preparing, queued, reconnecting, starting_up, thinking, typing, status_pill (+8 more)

### Community 658 - "Community 658"
Cohesion: 0.22
Nodes (11): charts, heatmap_less, heatmap_more, tooltip_total, detail_page, not_found_hint, not_found_title, queued_one (+3 more)

### Community 660 - "Community 660"
Cohesion: 0.07
Nodes (29): 10. Responsive Behavior, 11. Imagery, 12. Agent Prompt Guide, 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition (+21 more)

### Community 661 - "Community 661"
Cohesion: 0.05
Nodes (42): attach_file, copy_code, pagination_next, pagination_previous, plain_text, toggle_sidebar, RESOURCES, enKeys (+34 more)

### Community 662 - "Community 662"
Cohesion: 0.15
Nodes (13): description, label, health, about_to_gc, offline, online, recently_lost, description (+5 more)

### Community 663 - "Community 663"
Cohesion: 0.07
Nodes (29): 2.5D 等距视角布局, PRD: Agent 虚拟办公室动画, Sprite Sheet 资产规格, Sprite 状态机, Squad 会议室场景, 交互设计, 入口位置, 前端组件结构 (+21 more)

### Community 664 - "Community 664"
Cohesion: 0.15
Nodes (13): description, label, health, about_to_gc, offline, online, recently_lost, description (+5 more)

### Community 665 - "Community 665"
Cohesion: 0.07
Nodes (30): import_dialog, agents_count_other, cancel, category_agents, category_skills, category_squads, change_file, click_to_change (+22 more)

### Community 666 - "Community 666"
Cohesion: 0.40
Nodes (5): NewHealthChecker(), HealthChecker, HealthResult, Context, Duration

### Community 667 - "Community 667"
Cohesion: 0.38
Nodes (7): Workspace Invitation (email-based, 7-day validity), Private Agent (assignable by owner/admin only), Workspace Role (owner/admin/member), Members and Roles Documentation, Role: Admin, Role: Member, Role: Owner (highest workspace role)

### Community 668 - "Community 668"
Cohesion: 0.43
Nodes (6): electronBin, original, plistGet(), plistPath, plistSet(), require

### Community 669 - "Community 669"
Cohesion: 0.27
Nodes (7): reactionToResponse(), ReactionResponse, CommentReaction, Handler, Request, ResponseWriter, UUID

### Community 670 - "Community 670"
Cohesion: 0.22
Nodes (12): CountAssigneeChangesByActorParams, CountAssigneeChangesByActorRow, CreateActivityParams, HasSquadLeaderNoActionEvaluationForTaskParams, ListActivitiesForIssueParams, ListRecentActivitiesForWorkspaceParams, ActivityLog, Context (+4 more)

### Community 671 - "Community 671"
Cohesion: 0.12
Nodes (18): ACPChatConn, KiroACPSupported(), chatACPPoolEntry, agentIDFromTask(), chatACPPoolKey(), newChatACPPool(), chatACPPool, chatACPPoolEntry (+10 more)

### Community 672 - "Community 672"
Cohesion: 0.25
Nodes (10): CreatePinnedItemParams, DeletePinnedItemParams, DeletePinnedItemsByItemParams, GetMaxPinnedItemPositionParams, ListPinnedItemsParams, UpdatePinnedItemPositionParams, Context, Queries (+2 more)

### Community 673 - "Community 673"
Cohesion: 0.07
Nodes (28): 1. Code naming, 2. i18n translation glossary, 3. Chinese voice and style, Comments in code, Commit messages, Database (Go + sqlc), Don't translate — brands and acronyms, Entities — mixed rule (`issue` / `skill` / `task`) (+20 more)

### Community 674 - "Community 674"
Cohesion: 0.23
Nodes (6): CreateVerificationCodeParams, Context, Queries, Timestamptz, UUID, VerificationCode

### Community 675 - "Community 675"
Cohesion: 0.07
Nodes (28): 1. 代码命名, 2. i18n 翻译术语表, 3. 中文风格, Commit message, Go, Issue 编号, Key 命名约定, TypeScript (+20 more)

### Community 676 - "Community 676"
Cohesion: 0.07
Nodes (29): dismiss, no_targets, pick_target, preview, add_comment, comment_added, comment_mode_hint, comment_on (+21 more)

### Community 677 - "Community 677"
Cohesion: 0.29
Nodes (7): devDependencies, @aicortex/tsconfig, rehype-sanitize, @types/linkify-it, @types/react, @types/react-dom, typescript

### Community 678 - "Community 678"
Cohesion: 0.36
Nodes (6): compilerOptions, outDir, rootDir, exclude, extends, include

### Community 679 - "Community 679"
Cohesion: 0.29
Nodes (7): activity_tooltip, created_days_ago_other, created_today, failed_suffix, last_7_days, no_activity, runs_other

### Community 680 - "Community 680"
Cohesion: 0.14
Nodes (14): duplicate_keys_toast, empty_readonly, hide_value_aria, intro_prefix, intro_readonly, intro_separator, intro_suffix, key_placeholder (+6 more)

### Community 681 - "Community 681"
Cohesion: 0.15
Nodes (13): cancel, description, empty, field_cache_read, field_cache_write, field_input, field_output, open_button (+5 more)

### Community 682 - "Community 682"
Cohesion: 0.35
Nodes (7): boolFloat(), NewRealtimeCollector(), newRealtimeDesc(), RealtimeCollector, Desc, Metric, Metrics

### Community 683 - "Community 683"
Cohesion: 0.29
Nodes (7): row, actions_aria, archived, fallback_runtime_cloud, fallback_runtime_local, no_description, you

### Community 684 - "Community 684"
Cohesion: 0.29
Nodes (7): description, label, output_modes, create_issue, run_only, description, label

### Community 685 - "Community 685"
Cohesion: 0.07
Nodes (28): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+20 more)

### Community 686 - "Community 686"
Cohesion: 0.07
Nodes (29): preview, add_comment, comment_added, comment_mode_hint, comment_on, comment_placeholder, comment_saved, disconnect_port (+21 more)

### Community 687 - "Community 687"
Cohesion: 0.12
Nodes (16): offline, preparing, queued, reconnecting, starting_up, thinking, typing, status_pill (+8 more)

### Community 688 - "Community 688"
Cohesion: 0.09
Nodes (21): kiroACPConnOptsFromExec(), OpenKiroACPConn(), TestKiroACPBlockedArgs(), TestKiroACPConnOptsFromExec(), kiroToolNameFromTitle(), KiroACPConn, KiroACPConnOpts, kiroBackend (+13 more)

### Community 689 - "Community 689"
Cohesion: 0.15
Nodes (13): page, export, import, learn_more, list_load_failed, list_load_failed_default, new_agent, of_total (+5 more)

### Community 690 - "Community 690"
Cohesion: 0.29
Nodes (7): board, add_issue_tooltip, empty_column, empty_grouping, hidden_columns_label, hide_column, show_column

### Community 691 - "Community 691"
Cohesion: 0.31
Nodes (8): BaseMentionExtension, md, renderMarkdown, start, startFn, token, tokenize(), tokenizeFn

### Community 692 - "Community 692"
Cohesion: 0.35
Nodes (12): renderAutopilotContext(), renderIssueContext(), renderQuickCreateContext(), resolveSkillsDir(), sanitizeSkillName(), writeContextFiles(), writeProjectResources(), writeSkillFiles() (+4 more)

### Community 693 - "Community 693"
Cohesion: 0.07
Nodes (27): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Motion & Interaction, 7. Voice & Brand, 8. Anti-patterns (+19 more)

### Community 694 - "Community 694"
Cohesion: 0.06
Nodes (40): chip, fallback_label, delete_dialog, cancel, confirm, description, title, lead (+32 more)

### Community 695 - "Community 695"
Cohesion: 0.38
Nodes (5): compilerOptions, composite, types, extends, include

### Community 696 - "Community 696"
Cohesion: 0.32
Nodes (5): disconnect(), observe(), unobserve(), localStorageMock, store

### Community 697 - "Community 697"
Cohesion: 0.33
Nodes (6): Agent Task Execution Flow, Cloud Quickstart, Agent Creation Flow, Daemon Setup via CLI, CLI Installation Methods, First Task Assignment

### Community 698 - "Community 698"
Cohesion: 0.60
Nodes (6): docs/assets/banner.jpg, docs/assets/hero-screenshot.png, docs/assets/logo-dark.svg, docs/assets/logo-light.svg, AICortex Brand, docs/assets/ directory

### Community 699 - "Community 699"
Cohesion: 0.40
Nodes (6): concept-squad, concept-squad-archive, concept-squad-cli-commands, concept-squad-leader-flow, concept-squad-retrigger-rules, doc-squads

### Community 700 - "Community 700"
Cohesion: 0.07
Nodes (27): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Motion & Interaction, 7. Voice & Brand, 8. Anti-patterns (+19 more)

### Community 701 - "Community 701"
Cohesion: 0.07
Nodes (27): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+19 more)

### Community 702 - "Community 702"
Cohesion: 0.25
Nodes (23): listContainsAgent(), newRequestAs(), privateAgentTestFixture(), TestCreateChatSession_PrivateAgentForbidsPlainMember(), TestCreateIssue_AssignToPrivateAgentForbidsPlainMember(), TestGetAgent_PrivateAgentForbidsPlainMember(), TestGetAgent_RejectsForgedAgentIDHeader(), TestListAgents_FiltersPrivateForPlainMember() (+15 more)

### Community 703 - "Community 703"
Cohesion: 0.14
Nodes (20): parseRuntimeIDs(), DaemonAuth(), DaemonAuthPathFromContext(), DaemonIDFromContext(), DaemonWorkspaceIDFromContext(), TestDaemonAuth_DaemonTokenCacheHit(), TestDaemonAuth_InvalidMDT_NilQueries(), TestDaemonAuth_MissingAuth() (+12 more)

### Community 704 - "Community 704"
Cohesion: 0.44
Nodes (12): T, cleanupSweeperFixture(), setupSweeperTestFixture(), TestExpireStaleQueuedTasks(), TestExpireStaleQueuedTasksRespectsBatchLimit(), TestRefreshAgentStatusFromTasks(), TestSweepDispatchedStaleTask(), TestSweepDoesNotResetIssueAlreadyInReview() (+4 more)

### Community 705 - "Community 705"
Cohesion: 0.18
Nodes (11): runtime_aside, add_more_body, add_more_title, good_eyebrow, learn_more, swap_body, swap_title, what_eyebrow (+3 more)

### Community 706 - "Community 706"
Cohesion: 0.29
Nodes (6): subscriberToResponse(), SubscriberResponse, Handler, IssueSubscriber, Request, ResponseWriter

### Community 707 - "Community 707"
Cohesion: 0.08
Nodes (14): audioExamples, byChip, chips, counts, CURATED, CURATED_RANK, curatedPriority(), hasAnySlug() (+6 more)

### Community 708 - "Community 708"
Cohesion: 0.07
Nodes (26): Caddy (Recommended), CLI / Daemon, Cookies, Database Pool Tuning (Optional), Database Setup, Email (Required for Authentication), File Storage (Optional), Google OAuth (Optional) (+18 more)

### Community 709 - "Community 709"
Cohesion: 0.12
Nodes (27): upsertDevSessionInCache(), useUpdateDevSettings(), devKeys, projectDevSessionsOptions(), DevProjectSessionLayout, DevSessionLayoutByProject, emptyProjectSessionLayout(), mergeSessionLayout() (+19 more)

### Community 710 - "Community 710"
Cohesion: 0.08
Nodes (25): Agent: 崔敏俊(Developer), Agent: 张理(PM), Agent: 朴志勋(研发PM), Agent: 李智浩(Developer), Agent: 知识库管理员, Agent: 郑秀妍(QA), dev-workflow (ID: 722dcb8f...), E2E（Playwright） (+17 more)

### Community 711 - "Community 711"
Cohesion: 0.11
Nodes (26): Context, Queries, TaskService, Context, Issue, TaskService, UUID, Context (+18 more)

### Community 712 - "Community 712"
Cohesion: 0.33
Nodes (6): view, board, list, section, tooltip_board, tooltip_list

### Community 713 - "Community 713"
Cohesion: 0.14
Nodes (13): public, agent_default_name, disconnected, loading, new_session, reconnecting, send_tooltip, status_disabled (+5 more)

### Community 714 - "Community 714"
Cohesion: 0.27
Nodes (8): createSkillWithFilesInTx(), skillCreateInput, Context, CreateSkillFileRequest, Handler, Queries, UUID, SkillWithFilesResponse

### Community 715 - "Community 715"
Cohesion: 0.21
Nodes (9): IssueFilters, issues, labelBug, labeledIssues, labelFeat, labelP0, makeIssue(), NO_FILTER (+1 more)

### Community 716 - "Community 716"
Cohesion: 0.33
Nodes (6): origin_card, imported_clawhub, imported_github, imported_runtime, imported_skills_sh, provider

### Community 717 - "Community 717"
Cohesion: 0.20
Nodes (10): starter_content, description_prefix, description_suffix, description_term, dismiss_action, dismiss_failed, import_action, import_failed (+2 more)

### Community 718 - "Community 718"
Cohesion: 0.58
Nodes (7): newLanguageTestUser(), newPatchMeRequest(), TestUpdateMeAcceptsLanguage(), TestUpdateMePreservesLanguageWhenNotProvided(), TestUpdateMeRejectsUnsupportedLanguage(), Request, T

### Community 719 - "Community 719"
Cohesion: 0.14
Nodes (14): management, copied, copy_link, create, delete, delete_confirm, description, edit (+6 more)

### Community 720 - "Community 720"
Cohesion: 0.17
Nodes (12): devDependencies, jsdom, tailwindcss, @tailwindcss/postcss, @testing-library/jest-dom, @testing-library/react, @testing-library/user-event, @types/react (+4 more)

### Community 721 - "Community 721"
Cohesion: 0.08
Nodes (24): 0. 基础结构（所有 slide 都一样）, 8 页节奏模板(可直接套用), A. 类名必须来自 template.html, B. 图片比例规范（非常重要）, C. 图片定位准则（避免图片堆到页面最底部、被浏览器工具栏遮挡）, ⚠️ chrome 和 kicker 不要写同一句话, D. 主题色与主题节奏, Layout 10: 图文混排（Lead Image + Side Text） (+16 more)

### Community 722 - "Community 722"
Cohesion: 0.15
Nodes (13): session, back, files_empty, preview_unavailable, sessions, tabs_files, tabs_preview, tabs_terminal (+5 more)

### Community 723 - "Community 723"
Cohesion: 0.57
Nodes (5): TestClient_IdentityHeaders_GetJSON(), TestClient_IdentityHeaders_PostJSON(), TestClient_VersionOmittedWhenUnset(), TestNormalizeGOOS(), T

### Community 724 - "Community 724"
Cohesion: 0.43
Nodes (5): createMemoryStorage(), disconnect(), observe(), storage, unobserve()

### Community 725 - "Community 725"
Cohesion: 0.18
Nodes (17): BuildStepFuncs, DBPool, PortPool, BuildStepFuncs, Manager, NewManager(), WithBuildSteps(), WithConcurrencyLimits() (+9 more)

### Community 726 - "Community 726"
Cohesion: 0.08
Nodes (24): Animations / Transitions, Background, Bottom Bar (fixed, centered, glass pill), Card 1 — Empty / Create Room (glass), Card 2 — Subscription Growth Experiments (solid white), Card 3 — Weekly Insights (solid white), Card 4 — Product Strategy 2023 (glass, dark translucent), Card 5 — User Onboarding Team (solid white) (+16 more)

### Community 727 - "Community 727"
Cohesion: 0.13
Nodes (35): AutoChatterConfig, DefaultAutoChatterConfig(), LLMClient, Bus, Context, Event, ForumAutoState, Queries (+27 more)

### Community 728 - "Community 728"
Cohesion: 0.33
Nodes (6): lastCallUsage, cacheRead, cacheWrite, input, output, total

### Community 729 - "Community 729"
Cohesion: 0.33
Nodes (6): nearLimitFiles, totalNearLimit, truncatedFiles, warningMode, warningShown, bootstrapTruncation

### Community 730 - "Community 730"
Cohesion: 0.33
Nodes (6): attempts, fallbackUsed, runner, winnerModel, winnerProvider, executionTrace

### Community 731 - "Community 731"
Cohesion: 0.08
Nodes (24): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+16 more)

### Community 732 - "Community 732"
Cohesion: 0.62
Nodes (5): openPool(), setupResolverFixture(), TestResolveWorkspaceIDFromRequest(), Pool, T

### Community 733 - "Community 733"
Cohesion: 0.62
Nodes (11): T, cleanupRerunFixture(), setupRerunTestFixture(), TestEnqueueTaskForIssueDoesNotForceFreshSession(), TestGetLastTaskSessionExcludesAPIInvalidRequest(), TestGetLastTaskSessionExcludesLegacyAPI400(), TestGetLastTaskSessionExcludesPoisonedFailures(), TestGetLastTaskSessionFallbackPoisonedClassifier() (+3 more)

### Community 734 - "Community 734"
Cohesion: 0.08
Nodes (23): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+15 more)

### Community 735 - "Community 735"
Cohesion: 0.08
Nodes (23): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Border & Divider (+15 more)

### Community 736 - "Community 736"
Cohesion: 0.08
Nodes (24): binary_unsupported, empty, hint, load_error, loading_content, save, save_error, saving (+16 more)

### Community 737 - "Community 737"
Cohesion: 0.08
Nodes (23): 2.1 · 必改占位符（**容易漏**）, 2.2 · 选定主题色(5 套预设 · 不允许自定义), 3.0.5 · 规划主题节奏（**和类预检同等重要**), 3.0 · 预检:类名必须在 template.html 里有定义（**最重要**）, 3.1 · 挑布局, 3.2 · 图片比例规范, 6 问澄清清单, Magazine Web Ppt (+15 more)

### Community 738 - "Community 738"
Cohesion: 0.08
Nodes (23): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+15 more)

### Community 739 - "Community 739"
Cohesion: 0.33
Nodes (6): frequency_long, custom, daily, hourly, weekdays, weekly

### Community 740 - "Community 740"
Cohesion: 0.33
Nodes (6): run, issue_linked, skipped_group, view_log, label, summary

### Community 741 - "Community 741"
Cohesion: 0.33
Nodes (6): custom, daily, hourly, weekdays, weekly, describe

### Community 742 - "Community 742"
Cohesion: 0.33
Nodes (6): custom, daily, hourly, weekdays, weekly, frequencies

### Community 743 - "Community 743"
Cohesion: 0.22
Nodes (9): model_dropdown, clear_full, default_provider, discovery_failed, label, managed_by_runtime_hint, managed_by_runtime_title, runtime_offline_manual (+1 more)

### Community 744 - "Community 744"
Cohesion: 0.18
Nodes (11): tunnels, add_action, delete_aria, empty, hint, loading, open_external, port_placeholder (+3 more)

### Community 745 - "Community 745"
Cohesion: 0.08
Nodes (23): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+15 more)

### Community 746 - "Community 746"
Cohesion: 0.57
Nodes (6): init(), printLabelTable(), runIssueLabelAdd(), runIssueLabelList(), runIssueLabelRemove(), Command

### Community 747 - "Community 747"
Cohesion: 0.08
Nodes (23): Client, Client（Agent 执行节点）, Daemon 1: 连接默认 server（用 ~/.aicortex/config.json）, Daemon 健康检查, Make 命令对照, Production Build & Deployment, Server（开发）, Server：开发模式（简要） (+15 more)

### Community 748 - "Community 748"
Cohesion: 0.09
Nodes (22): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Border & Divider (+14 more)

### Community 749 - "Community 749"
Cohesion: 0.54
Nodes (6): TestGetRuntimeUsage_BucketsByUsageTime(), TestGetRuntimeUsageDailyRollupCutoffUsesRuntimeTimezone(), TestRuntimeHandlersRejectMalformedRuntimeID(), TestUpdateAgentRuntimeTimezoneValidatesPermissionAndValue(), TestUpsertAgentRuntimePreservesTimezoneOverride(), T

### Community 750 - "Community 750"
Cohesion: 0.09
Nodes (22): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Border (+14 more)

### Community 751 - "Community 751"
Cohesion: 0.11
Nodes (25): bridgeScript(), injectPreviewBridge(), PreviewBridgeHoverPayload, PreviewBridgeSelectPayload, syncPreviewTool(), canvasElementLooksBlank(), canvasToPngFile(), captureIframeDomSnapshot() (+17 more)

### Community 752 - "Community 752"
Cohesion: 0.19
Nodes (21): decodeMs, logClaimEndpointSlow(), logHeartbeatEndpointSlow(), normalizeRuntimeTimezone(), normalizeWorkspaceRepos(), parseWorkspaceRepos(), workspaceReposResponse(), workspaceReposVersion() (+13 more)

### Community 753 - "Community 753"
Cohesion: 0.17
Nodes (12): bubble_menu, bold, code, italic, link, list, list_dropdown, quote (+4 more)

### Community 755 - "Community 755"
Cohesion: 0.10
Nodes (20): FeatureArtifactBrowse(), FeatureArtifactServing(), FeatureIssuePreview(), envDefaultTrue(), FeatureDesignExport(), FeatureDesignJury(), FeatureDesignStudio(), PluginEntry (+12 more)

### Community 756 - "Community 756"
Cohesion: 0.09
Nodes (22): 7.1 视觉基础（Phase 1）, 7.2 Dark/Light（已就位）, 7.3 i18n（Phase 10）, 7.4 CI（Phase 0）, 7.5 dev:docs 快捷命令（已做）, AICortex Docs 站重写规划（v2）, 一、产品定位（文档的落脚点）, 七、技术基础设施 (+14 more)

### Community 757 - "Community 757"
Cohesion: 0.33
Nodes (6): origin_card, imported_clawhub, imported_github, imported_runtime, imported_skills_sh, provider

### Community 758 - "Community 758"
Cohesion: 0.40
Nodes (5): Agent Sprite State Machine, Agent Virtual Office, PixiJS, PresenceMap, Virtual Office Scene

### Community 759 - "Community 759"
Cohesion: 0.09
Nodes (22): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Border (+14 more)

### Community 760 - "Community 760"
Cohesion: 0.40
Nodes (5): concept-email-service, concept-selfhost, concept-selfhost-cli-setup, doc-selfhost-quickstart, doc-selfhost-quickstart-zh

### Community 761 - "Community 761"
Cohesion: 0.36
Nodes (5): Listener, Context, Daemon, HandlerFunc, Time

### Community 762 - "Community 762"
Cohesion: 0.36
Nodes (6): registerAutopilotListeners(), syncRunFromTaskEvent(), AutopilotService, Bus, Context, Event

### Community 763 - "Community 763"
Cohesion: 0.29
Nodes (13): buildWorkspaceUpdateBody(), init(), runWorkspaceGet(), runWorkspaceList(), runWorkspaceMembers(), runWorkspaceUpdate(), resetWorkspaceUpdateFlags(), setBoolFlag() (+5 more)

### Community 764 - "Community 764"
Cohesion: 0.09
Nodes (22): Avatar, Border Radius, Brand Colors, Brand Identity, Chat Bubble, Chat Bubble Colors, Color Palette, Components (+14 more)

### Community 765 - "Community 765"
Cohesion: 0.09
Nodes (23): view_render_tooltip, view_source_tooltip, files, terminal, web, close_tooltip, closed, connecting (+15 more)

### Community 766 - "Community 766"
Cohesion: 0.09
Nodes (21): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout, 6. Depth, 7. Do's and Don'ts, 8. Responsive Behavior (+13 more)

### Community 767 - "Community 767"
Cohesion: 0.09
Nodes (21): Presenter Mode Guide · 演讲者模式指南, 两种做法, 何时使用演讲者模式, 双屏演讲的标准流程, 常见错误, 必备 HTML 结构, ❌ 忘记引入 runtime.js, ❌ 把逐字稿写在 slide 可见位置 (+13 more)

### Community 768 - "Community 768"
Cohesion: 0.18
Nodes (11): listDir(), NewBrowser(), TestBrowserReadIndexHTML(), writeFile(), Browser, ArtifactListEntry, ArtifactRequestPayload, ArtifactResponsePayload (+3 more)

### Community 769 - "Community 769"
Cohesion: 0.15
Nodes (16): EnsureAgentQuestionFormSkill(), EnsureQuestionFormSkill(), findOrCreateQuestionFormSkill(), skillTeachesQuestionForm(), TestEnsureQuestionFormSkill_appendsWhenMissing(), TestEnsureQuestionFormSkill_skipsWhenPresent(), TemplateEntry, TemplateCatalog() (+8 more)

### Community 770 - "Community 770"
Cohesion: 0.44
Nodes (8): isReservedSlug(), currentActiveTab(), DesktopNavigationProvider(), extractWorkspaceSlug(), requireRuntimeAppUrl(), TabNavigationProvider(), tryRouteToOtherWorkspace(), tryRouteToOverlay()

### Community 771 - "Community 771"
Cohesion: 0.18
Nodes (11): tunnels, add_action, delete_aria, empty, hint, loading, open_external, port_placeholder (+3 more)

### Community 772 - "Community 772"
Cohesion: 0.40
Nodes (5): save_bar, discard, save, saving, unsaved

### Community 773 - "Community 773"
Cohesion: 0.09
Nodes (21): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Border (+13 more)

### Community 774 - "Community 774"
Cohesion: 0.28
Nodes (7): AddIssueSubscriberParams, IsIssueSubscriberParams, RemoveIssueSubscriberParams, Context, Queries, IssueSubscriber, UUID

### Community 775 - "Community 775"
Cohesion: 0.57
Nodes (5): TestDashboardEndpoints(), TestDashboardRollupClearsOnIssueDelete(), TestDashboardRollupReattributesOnLinkTaskToIssue(), TestDashboardRollupReattributesOnProjectChange(), T

### Community 776 - "Community 776"
Cohesion: 0.09
Nodes (21): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Border & Divider (+13 more)

### Community 777 - "Community 777"
Cohesion: 0.29
Nodes (6): body, calls, client, fetchMock, file, headers

### Community 778 - "Community 778"
Cohesion: 0.40
Nodes (4): compilerOptions, jsx, lib, extends

### Community 779 - "Community 779"
Cohesion: 0.57
Nodes (5): TestHubImplementsBroadcaster(), TestMetrics_RecordEvent(), TestMetrics_RecordEvent_Concurrent(), TestMetrics_Snapshot_IncludesCounters(), T

### Community 780 - "Community 780"
Cohesion: 0.31
Nodes (7): daemonSysProcAttr(), isAccessDeniedSpawnErr(), notifyShutdownContext(), tailLogFile(), CancelFunc, Context, SysProcAttr

### Community 781 - "Community 781"
Cohesion: 0.49
Nodes (8): ClaimDueScheduleTriggersRow, advanceNextRun(), recoverLostTriggers(), runAutopilotScheduler(), tickScheduledAutopilots(), AutopilotService, Context, Queries

### Community 782 - "Community 782"
Cohesion: 0.39
Nodes (5): New(), DBTX, Queries, Queries, Tx

### Community 783 - "Community 783"
Cohesion: 0.28
Nodes (7): CreateTaskMessageParams, ListTaskMessagesSinceParams, Context, Queries, Text, UUID, TaskMessage

### Community 784 - "Community 784"
Cohesion: 0.40
Nodes (5): usage, cacheWrite, input, output, total

### Community 785 - "Community 785"
Cohesion: 0.22
Nodes (12): artifactListResponse, artifactSourceResponse, prepareArtifactEmbedResponse(), writeArtifactError(), AgentTaskQueue, Context, Handler, ListEntry (+4 more)

### Community 786 - "Community 786"
Cohesion: 0.40
Nodes (5): peerDependencies, i18next, react, react-dom, react-i18next

### Community 787 - "Community 787"
Cohesion: 0.22
Nodes (7): Duration, Mutex, Time, T, NewProxyLimiter(), TestProxyLimiterAllow(), ProxyLimiter

### Community 788 - "Community 788"
Cohesion: 0.09
Nodes (21): Presenter Mode Guide · 演讲者模式指南, 两种做法, 何时使用演讲者模式, 双屏演讲的标准流程, 常见错误, 必备 HTML 结构, ❌ 忘记引入 runtime.js, ❌ 把逐字稿写在 slide 可见位置 (+13 more)

### Community 789 - "Community 789"
Cohesion: 0.40
Nodes (5): issue_picker, no_results, prompt_to_search, search_placeholder, searching

### Community 790 - "Community 790"
Cohesion: 0.18
Nodes (5): openPlatformPTY(), unixPTY, Cmd, File, platformPTY

### Community 791 - "Community 791"
Cohesion: 0.10
Nodes (20): App Lifecycle, Command Reference, Contributing Guide, Destructive Reset, Development Model, Environment Files, Explicit Setup (advanced), First-Time Setup (+12 more)

### Community 792 - "Community 792"
Cohesion: 0.31
Nodes (7): AppFeatureFlags, ConfigState, configStore, DEFAULT_TUNNEL_SCAN_PORTS, defaultFeatures, useConfigStore(), useTunnelScanPorts()

### Community 793 - "Community 793"
Cohesion: 0.21
Nodes (19): init(), resolveAgent(), runAutopilotCreate(), runAutopilotDelete(), runAutopilotGet(), runAutopilotList(), runAutopilotRuns(), runAutopilotTrigger() (+11 more)

### Community 794 - "Community 794"
Cohesion: 0.40
Nodes (5): cancel, confirm, description, title, revoke_dialog

### Community 795 - "Community 795"
Cohesion: 0.10
Nodes (20): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Spacing & Layout, 6. Motion, 7. Usage Guardrails, Brand Accent (+12 more)

### Community 796 - "Community 796"
Cohesion: 0.50
Nodes (4): repository, directory, type, url

### Community 797 - "Community 797"
Cohesion: 0.39
Nodes (6): main(), monthFloor(), stampWatermark(), Context, Pool, Time

### Community 798 - "Community 798"
Cohesion: 0.39
Nodes (6): main(), monthFloor(), stampWatermark(), Context, Pool, Time

### Community 799 - "Community 799"
Cohesion: 0.50
Nodes (4): NavigationAdapter, Platform Bridge (CoreProvider + NavigationAdapter), CoreProvider, React Context (Platform Plumbing)

### Community 801 - "Community 801"
Cohesion: 0.14
Nodes (16): AppLinkProps, adapter, callerFocus, callerMouseEnter, makeAdapter(), openInNewTab, order, prefetch (+8 more)

### Community 802 - "Community 802"
Cohesion: 0.32
Nodes (7): GetNotificationPreferenceParams, ListNotificationPreferencesByUsersParams, UpsertNotificationPreferenceParams, NotificationPreference, Context, Queries, UUID

### Community 803 - "Community 803"
Cohesion: 0.60
Nodes (3): reservedSlugsFile, isReservedSlug(), loadReservedSlugs()

### Community 804 - "Community 804"
Cohesion: 0.29
Nodes (3): T, TestUnescapeBackslashEscapes(), UnescapeBackslashEscapes()

### Community 805 - "Community 805"
Cohesion: 0.73
Nodes (4): clearInvitationsForTestWorkspace(), TestCreateInvitation_AllowsAfterExpiry(), TestCreateInvitation_BlocksWhilePending(), T

### Community 806 - "Community 806"
Cohesion: 0.25
Nodes (4): ContentSecurityPolicy(), TestContentSecurityPolicy(), Handler, T

### Community 807 - "Community 807"
Cohesion: 0.24
Nodes (10): name, peerDependencies, react, private, scripts, lint, test, typecheck (+2 more)

### Community 808 - "Community 808"
Cohesion: 0.10
Nodes (19): Callout 引用框, Chrome & Foot, Figure 图片框, Frame Caption 变体, Ghost 巨型背景字, Highlight 荧光标记, Icons 图标, Pillar 支柱卡 (+11 more)

### Community 809 - "Community 809"
Cohesion: 0.50
Nodes (4): Desktop Renderer App Shell, Desktop Renderer Entry HTML, Renderer Main Entry Point (/src/main.tsx), #root DOM Container

### Community 810 - "Community 810"
Cohesion: 0.53
Nodes (4): check.sh script, check.sh script, cleanup(), wait_for_port()

### Community 811 - "Community 811"
Cohesion: 0.47
Nodes (4): HealthResponse, healthWorkspace, repoCheckoutRequest, healthWorkspace

### Community 812 - "Community 812"
Cohesion: 0.38
Nodes (5): ForumReactionResponse, ForumReplyResponse, ForumPostResponse, ForumReactionResponse, ForumReplyResponse

### Community 813 - "Community 813"
Cohesion: 0.31
Nodes (6): AddIssueReactionParams, RemoveIssueReactionParams, Context, Queries, IssueReaction, UUID

### Community 814 - "Community 814"
Cohesion: 0.18
Nodes (11): tabs, chat-share, general, integrations, labs, members, notifications, preferences (+3 more)

### Community 815 - "Community 815"
Cohesion: 0.22
Nodes (11): CreateChatShareLinkParams, GetChatShareLinkInWorkspaceParams, UpdateChatShareLinkParams, Bool, ChatShareLink, Context, Queries, Int4 (+3 more)

### Community 816 - "Community 816"
Cohesion: 0.31
Nodes (6): AddReactionParams, RemoveReactionParams, CommentReaction, Context, Queries, UUID

### Community 817 - "Community 817"
Cohesion: 0.19
Nodes (13): CreateDesignChatSessionParams, GetDesignSystemResourceInProjectParams, ListDesignChatSessionsByProjectParams, ListDesignChatSessionsByProjectRow, ListDesignSystemResourcesByProjectParams, SetWorkspaceDefaultDesignAgentParams, ChatSession, Context (+5 more)

### Community 818 - "Community 818"
Cohesion: 0.50
Nodes (4): chars, nonProjectContextChars, projectContextChars, systemPrompt

### Community 819 - "Community 819"
Cohesion: 0.50
Nodes (4): tools, entries, listChars, schemaChars

### Community 820 - "Community 820"
Cohesion: 0.60
Nodes (3): name, private, version

### Community 821 - "Community 821"
Cohesion: 0.14
Nodes (24): getRecencyMap(), getStorage(), readRecencyMap(), recencyKey(), RecencyMap, recordMentionUsage(), sortUserItemsByRecency(), storageKey() (+16 more)

### Community 822 - "Community 822"
Cohesion: 0.29
Nodes (8): makeRuntime(), mocks, { onNext }, props, renderStep(), rt, setPicker(), TEST_RESOURCES

### Community 823 - "Community 823"
Cohesion: 0.50
Nodes (3): defaultPowerShellLookup(), platformCursorInvocation(), Logger

### Community 824 - "Community 824"
Cohesion: 0.60
Nodes (3): TestHideAgentWindowPreservesExistingSysProcAttr(), TestHideAgentWindowSetsCreateNewConsole(), T

### Community 825 - "Community 825"
Cohesion: 0.50
Nodes (3): init(), runVersion(), Command

### Community 833 - "Community 833"
Cohesion: 0.67
Nodes (3): CLI Release Workflow, GoReleaser Multi-Platform Build, Homebrew Tap Publishing

### Community 834 - "Community 834"
Cohesion: 0.67
Nodes (3): AICortex Design System, Neutral Color Tokens, Semantic Color Tokens

### Community 836 - "Community 836"
Cohesion: 0.67
Nodes (3): Docs Execution Outline (v1), Docs Rewrite Plan (v2), Product Overview

### Community 838 - "Community 838"
Cohesion: 0.10
Nodes (19): 1. `--accent` — Accent color, 2. `--scale` — Type scale (0.85 / 1.0 / 1.15), 3. `--density` — Layout density (Tight / Normal / Roomy), 4. `--mode` — Light / Dark, 5. `--motion` — Off / Subtle / Lively, Hard rules, Implementation primitives, Output contract (+11 more)

### Community 840 - "Community 840"
Cohesion: 0.67
Nodes (3): Electron Builder Configuration, AICortex Brain Icon, Desktop App Icon (PNG)

### Community 841 - "Community 841"
Cohesion: 0.21
Nodes (16): CreateDevSessionRequest, devSessionFromCreatorListRow(), devSessionFromListRow(), devSessionFromRow(), devSessionFromWorkspaceListRow(), DevSessionResponse, SyncDevAgentSessionRequest, ListDevChatSessionsByCreatorRow (+8 more)

### Community 842 - "Community 842"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 846 - "Community 846"
Cohesion: 0.40
Nodes (3): ensure-postgres.sh script, ensure-postgres.sh script, PGPASSWORD

### Community 847 - "Community 847"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 848 - "Community 848"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 850 - "Community 850"
Cohesion: 0.60
Nodes (3): T, TestMarkRuntimesOfflineByIDs_OfflinesGenuinelyStale(), TestMarkRuntimesOfflineByIDs_RespectsConcurrentHeartbeat()

### Community 851 - "Community 851"
Cohesion: 0.50
Nodes (3): Template, TemplateSkillRef, TemplateSkillRef

### Community 854 - "Community 854"
Cohesion: 0.28
Nodes (5): Feedback, CreateFeedbackParams, Context, Queries, UUID

### Community 855 - "Community 855"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 856 - "Community 856"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 857 - "Community 857"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 858 - "Community 858"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 860 - "Community 860"
Cohesion: 0.67
Nodes (3): description, title, add_subissue_picker

### Community 875 - "Community 875"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 877 - "Community 877"
Cohesion: 0.33
Nodes (5): rewriteGitHubTransport, RoundTripper, Request, Response, URL

### Community 879 - "Community 879"
Cohesion: 0.12
Nodes (28): normalizeTerminalScope(), scanTerminalSessionRow(), terminalSessionResponse, contextKey, buildMiddleware(), MemberFromContext(), RequireWorkspaceMember(), RequireWorkspaceMemberFromURL() (+20 more)

### Community 886 - "Community 886"
Cohesion: 0.40
Nodes (3): AppFeatureFlags, AppConfig, AppFeatureFlags

### Community 892 - "Community 892"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 893 - "Community 893"
Cohesion: 0.33
Nodes (4): AgentTaskQueue, Context, Queries, HasSquadLeaderNoActionEvaluationForTask()

### Community 927 - "Community 927"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 928 - "Community 928"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 929 - "Community 929"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 930 - "Community 930"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 931 - "Community 931"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 932 - "Community 932"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 933 - "Community 933"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 934 - "Community 934"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 935 - "Community 935"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 936 - "Community 936"
Cohesion: 0.11
Nodes (18): 1. Philosophy consistency · 哲学一致性, 2. Visual hierarchy · 视觉层级, 3. Detail execution · 细节执行, 4. Functionality · 功能性, 5. Innovation · 创新性, Critique Skill · 5 维度专家评审, Hard rules, Output contract (+10 more)

### Community 937 - "Community 937"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 938 - "Community 938"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 939 - "Community 939"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 940 - "Community 940"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 941 - "Community 941"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 942 - "Community 942"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 943 - "Community 943"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 944 - "Community 944"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 945 - "Community 945"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 946 - "Community 946"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 947 - "Community 947"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 948 - "Community 948"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 949 - "Community 949"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 950 - "Community 950"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 951 - "Community 951"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 952 - "Community 952"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 953 - "Community 953"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 954 - "Community 954"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 955 - "Community 955"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 956 - "Community 956"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 957 - "Community 957"
Cohesion: 0.11
Nodes (19): session, back, clear_comment, collapse_sessions, comment_mode, comment_prefix, comment_preview_hint, expand_sessions (+11 more)

### Community 958 - "Community 958"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 959 - "Community 959"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 960 - "Community 960"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 961 - "Community 961"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 962 - "Community 962"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 963 - "Community 963"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 964 - "Community 964"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 965 - "Community 965"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 966 - "Community 966"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 967 - "Community 967"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 968 - "Community 968"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 969 - "Community 969"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 970 - "Community 970"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 971 - "Community 971"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 972 - "Community 972"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 973 - "Community 973"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 974 - "Community 974"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 975 - "Community 975"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 976 - "Community 976"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 977 - "Community 977"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 978 - "Community 978"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 979 - "Community 979"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 980 - "Community 980"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 981 - "Community 981"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 982 - "Community 982"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 983 - "Community 983"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 984 - "Community 984"
Cohesion: 0.11
Nodes (33): useTaskArtifacts(), ChatArtifactPanel(), isTextArtifact(), TEXT_EXTENSIONS, buildArtifactRawURL(), IMAGE_EXTENSIONS, isHtmlArtifact(), isImageArtifact() (+25 more)

### Community 985 - "Community 985"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 986 - "Community 986"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 987 - "Community 987"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 988 - "Community 988"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 989 - "Community 989"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 990 - "Community 990"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 991 - "Community 991"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 992 - "Community 992"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 993 - "Community 993"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 994 - "Community 994"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 995 - "Community 995"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 996 - "Community 996"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 997 - "Community 997"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 998 - "Community 998"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 999 - "Community 999"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1000 - "Community 1000"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1001 - "Community 1001"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1002 - "Community 1002"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1003 - "Community 1003"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1004 - "Community 1004"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1005 - "Community 1005"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1006 - "Community 1006"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1007 - "Community 1007"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1008 - "Community 1008"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1009 - "Community 1009"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1010 - "Community 1010"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1011 - "Community 1011"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1012 - "Community 1012"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1013 - "Community 1013"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1014 - "Community 1014"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1015 - "Community 1015"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1016 - "Community 1016"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1017 - "Community 1017"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1018 - "Community 1018"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1019 - "Community 1019"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1020 - "Community 1020"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1021 - "Community 1021"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1022 - "Community 1022"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1023 - "Community 1023"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1024 - "Community 1024"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1025 - "Community 1025"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1026 - "Community 1026"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1027 - "Community 1027"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1028 - "Community 1028"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1029 - "Community 1029"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1030 - "Community 1030"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1031 - "Community 1031"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1032 - "Community 1032"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1033 - "Community 1033"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1034 - "Community 1034"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1035 - "Community 1035"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1036 - "Community 1036"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1037 - "Community 1037"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1038 - "Community 1038"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1039 - "Community 1039"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1040 - "Community 1040"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1041 - "Community 1041"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1042 - "Community 1042"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1043 - "Community 1043"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1044 - "Community 1044"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1045 - "Community 1045"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1046 - "Community 1046"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1047 - "Community 1047"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1048 - "Community 1048"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1049 - "Community 1049"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1050 - "Community 1050"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1051 - "Community 1051"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1052 - "Community 1052"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1053 - "Community 1053"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1054 - "Community 1054"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1055 - "Community 1055"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1056 - "Community 1056"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1057 - "Community 1057"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1058 - "Community 1058"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1059 - "Community 1059"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1060 - "Community 1060"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1061 - "Community 1061"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1062 - "Community 1062"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1063 - "Community 1063"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1064 - "Community 1064"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1065 - "Community 1065"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1066 - "Community 1066"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1067 - "Community 1067"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1068 - "Community 1068"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1069 - "Community 1069"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1070 - "Community 1070"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1071 - "Community 1071"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1072 - "Community 1072"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1073 - "Community 1073"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1074 - "Community 1074"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1075 - "Community 1075"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1076 - "Community 1076"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1077 - "Community 1077"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1078 - "Community 1078"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1079 - "Community 1079"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1080 - "Community 1080"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1081 - "Community 1081"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1082 - "Community 1082"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1083 - "Community 1083"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1084 - "Community 1084"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1085 - "Community 1085"
Cohesion: 0.11
Nodes (18): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+10 more)

### Community 1086 - "Community 1086"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1087 - "Community 1087"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1088 - "Community 1088"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1089 - "Community 1089"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1090 - "Community 1090"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1091 - "Community 1091"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1092 - "Community 1092"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1093 - "Community 1093"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1094 - "Community 1094"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1095 - "Community 1095"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1096 - "Community 1096"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1097 - "Community 1097"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1098 - "Community 1098"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1099 - "Community 1099"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1100 - "Community 1100"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1101 - "Community 1101"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1102 - "Community 1102"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1103 - "Community 1103"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1104 - "Community 1104"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1105 - "Community 1105"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1106 - "Community 1106"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1107 - "Community 1107"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1108 - "Community 1108"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1109 - "Community 1109"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1110 - "Community 1110"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1111 - "Community 1111"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1112 - "Community 1112"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1113 - "Community 1113"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1114 - "Community 1114"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1115 - "Community 1115"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1116 - "Community 1116"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1117 - "Community 1117"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1118 - "Community 1118"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1119 - "Community 1119"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1120 - "Community 1120"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1121 - "Community 1121"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1122 - "Community 1122"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1123 - "Community 1123"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1124 - "Community 1124"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1125 - "Community 1125"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1126 - "Community 1126"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1127 - "Community 1127"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1128 - "Community 1128"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1129 - "Community 1129"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1130 - "Community 1130"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1131 - "Community 1131"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1132 - "Community 1132"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1133 - "Community 1133"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1134 - "Community 1134"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1135 - "Community 1135"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1136 - "Community 1136"
Cohesion: 0.09
Nodes (14): ManagedRelay, MirroredRelay, recordingManagedRelay, NewMirroredRelay(), TestMirroredRelayDoesNotMirrorDaemonRuntimeEvents(), TestMirroredRelayPublishesSameEventIDToBothBackends(), TestMirroredRelayRecordsDivergenceWhenOneBackendFails(), relayPublishCall (+6 more)

### Community 1137 - "Community 1137"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1138 - "Community 1138"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1139 - "Community 1139"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1140 - "Community 1140"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1141 - "Community 1141"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1142 - "Community 1142"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1143 - "Community 1143"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1144 - "Community 1144"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1145 - "Community 1145"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1146 - "Community 1146"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1147 - "Community 1147"
Cohesion: 0.11
Nodes (18): computedHash, source, sourceType, computedHash, source, sourceType, skills, frontend-design (+10 more)

### Community 1148 - "Community 1148"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1149 - "Community 1149"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1150 - "Community 1150"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1151 - "Community 1151"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1152 - "Community 1152"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1153 - "Community 1153"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1154 - "Community 1154"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1155 - "Community 1155"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1156 - "Community 1156"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1157 - "Community 1157"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1158 - "Community 1158"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1159 - "Community 1159"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1160 - "Community 1160"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1161 - "Community 1161"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1162 - "Community 1162"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1163 - "Community 1163"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1164 - "Community 1164"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1165 - "Community 1165"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1166 - "Community 1166"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1167 - "Community 1167"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1168 - "Community 1168"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1169 - "Community 1169"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1170 - "Community 1170"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1171 - "Community 1171"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1172 - "Community 1172"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1173 - "Community 1173"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1174 - "Community 1174"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1175 - "Community 1175"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1176 - "Community 1176"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1177 - "Community 1177"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1178 - "Community 1178"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1179 - "Community 1179"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1180 - "Community 1180"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1181 - "Community 1181"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1182 - "Community 1182"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1183 - "Community 1183"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1184 - "Community 1184"
Cohesion: 0.11
Nodes (18): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout, 6. Depth: 5-layer cascading shadow system, 7. Do's and Don'ts, 8. Responsive: 479px, 768px, 992px (+10 more)

### Community 1185 - "Community 1185"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1186 - "Community 1186"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1187 - "Community 1187"
Cohesion: 0.11
Nodes (19): description_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1188 - "Community 1188"
Cohesion: 0.11
Nodes (19): title_i18n, ar, de, en, es, fr, id, it (+11 more)

### Community 1189 - "Community 1189"
Cohesion: 0.11
Nodes (18): select_file, modes, deck, design_system, hyperframes, prototype, template, export (+10 more)

### Community 1190 - "Community 1190"
Cohesion: 0.11
Nodes (17): 1. Capture the source PNG, 2. Extract the dominant colors, 3. Map to the 6-token palette, 4. Add the theme block, 5. Verify, atlas — Museum chapter, bevel — Y2K editorial, bluehouse — Consumer card (+9 more)

### Community 1191 - "Community 1191"
Cohesion: 0.23
Nodes (13): daemonTokenCacheKey(), NewDaemonTokenCache(), TestDaemonTokenCache_NilSafe(), TestDaemonTokenCache_Set_RespectsClampedTTL(), TestDaemonTokenCache_SetGetInvalidate(), TestDaemonTokenCache_TTL(), TestNewDaemonTokenCache_NilRedisReturnsNil(), DaemonTokenCache (+5 more)

### Community 1192 - "Community 1192"
Cohesion: 0.24
Nodes (11): BuildMinimalPPTX(), BuildZipArchive(), PreparePrintHTML(), xmlEscape(), sanitizeExportName(), AgentTaskQueue, ChatSession, Context (+3 more)

### Community 1193 - "Community 1193"
Cohesion: 0.11
Nodes (17): CloudFront (optional), Cookie domain, Core server variables, Daemon tuning parameters, Database connection pool, Email configuration, File storage configuration, Frontend access control (+9 more)

### Community 1194 - "Community 1194"
Cohesion: 0.11
Nodes (17): CloudFront（可选）, Cookie 域, GitHub 集成, Resend, S3 / S3 兼容存储, SMTP relay, 下一步, 前端访问控制 (+9 more)

### Community 1195 - "Community 1195"
Cohesion: 0.11
Nodes (17): Capability matrix, Claude Code, Codex, Copilot, Cursor, Gemini, Hermes, Kimi (+9 more)

### Community 1196 - "Community 1196"
Cohesion: 0.11
Nodes (17): Claude Code, Codex, Copilot, Cursor, Gemini, Hermes, Kimi, Kiro CLI (+9 more)

### Community 1197 - "Community 1197"
Cohesion: 0.09
Nodes (34): memberAllowedForPrivateAgent(), AgentRuntimeResponse, parseProjectIDParam(), DashboardAgentRunTimeResponse, DashboardRunTimeDailyResponse, DashboardUsageByAgentResponse, DashboardUsageDailyResponse, roleAllowed() (+26 more)

### Community 1198 - "Community 1198"
Cohesion: 0.11
Nodes (17): 1. Visual Theme & Atmosphere, 2. Color Palette & Roles, 3. Typography Rules, 4. Component Stylings, 5. Layout Principles, 6. Depth & Elevation, 7. Do's and Don'ts, 8. Responsive Behavior (+9 more)

### Community 1199 - "Community 1199"
Cohesion: 0.18
Nodes (11): skills_section, collapse, label, list_empty_default, list_empty_multi, list_loading, list_no_match, placeholder (+3 more)

### Community 1200 - "Community 1200"
Cohesion: 0.12
Nodes (16): Check Which Database a Checkout Uses, Commands, Daily Workflow, Destructive Reset, Development Model, Feature Worktree, First-Time Setup, List All Local Databases (+8 more)

### Community 1201 - "Community 1201"
Cohesion: 0.12
Nodes (17): assets, skills, od, capabilities, context, featured, kind, mode (+9 more)

### Community 1202 - "Community 1202"
Cohesion: 0.12
Nodes (17): select_file, export, files, preview, questions, terminal, tools, download_html (+9 more)

### Community 1203 - "Community 1203"
Cohesion: 0.12
Nodes (17): property_editor, box, cancel, close, delete, fill, font_weight, height (+9 more)

### Community 1204 - "Community 1204"
Cohesion: 0.12
Nodes (16): checkbox — multiple choice, Example 1: Gathering user info, Example 2: Project requirements, Example 3: Simple survey, Examples, Format, How answers come back, Interactive Question Forms (+8 more)

### Community 1205 - "Community 1205"
Cohesion: 0.12
Nodes (16): 1. Initialize your runtime, 2. Verify connectivity, 3. Bring an agent online, 4. Route your first task, AICortex, Architecture, CLI Reference, Core Capabilities (+8 more)

### Community 1206 - "Community 1206"
Cohesion: 0.12
Nodes (16): 1. 初始化运行时, 2. 确认连接, 3. 上线一个 Agent, 4. 路由你的第一个任务, AICortex, AICortex 是什么？, CLI 参考, macOS / Linux（安装脚本） (+8 more)

### Community 1207 - "Community 1207"
Cohesion: 0.12
Nodes (16): a) Install the CLI and an AI agent, Architecture, b) One-command setup, Manual CLI Configuration, Manual Docker Compose Setup, Advanced Configuration, Quick Install (Recommended), Self-Hosting Guide (+8 more)

### Community 1208 - "Community 1208"
Cohesion: 0.12
Nodes (17): property_editor, box, cancel, close, delete, fill, font_weight, height (+9 more)

### Community 1209 - "Community 1209"
Cohesion: 0.12
Nodes (17): session, back, clear_comment, collapse_sessions, comment_mode, comment_prefix, comment_preview_hint, expand_sessions (+9 more)

### Community 1210 - "Community 1210"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1211 - "Community 1211"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1212 - "Community 1212"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1213 - "Community 1213"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1214 - "Community 1214"
Cohesion: 0.18
Nodes (11): tabs, activity, custom_args, discard_confirm, discard_dialog_description, discard_dialog_title, discard_keep, environment (+3 more)

### Community 1215 - "Community 1215"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1216 - "Community 1216"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1217 - "Community 1217"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1218 - "Community 1218"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1219 - "Community 1219"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1220 - "Community 1220"
Cohesion: 0.12
Nodes (15): modes, deck, design_system, hyperframes, prototype, template, picker, create_first (+7 more)

### Community 1221 - "Community 1221"
Cohesion: 0.15
Nodes (13): session, back, files_empty, preview_unavailable, sessions, tabs_files, tabs_preview, tabs_terminal (+5 more)

### Community 1222 - "Community 1222"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1223 - "Community 1223"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1224 - "Community 1224"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1225 - "Community 1225"
Cohesion: 0.28
Nodes (4): fakeLivenessStore, Context, Duration, Mutex

### Community 1226 - "Community 1226"
Cohesion: 0.17
Nodes (12): hub, agent_offline, agent_online, back_to_project, brand_name, composer_placeholder, hero_subtitle, hero_title (+4 more)

### Community 1227 - "Community 1227"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1228 - "Community 1228"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, inputs, kind, mode (+8 more)

### Community 1229 - "Community 1229"
Cohesion: 0.12
Nodes (16): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+8 more)

### Community 1230 - "Community 1230"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1231 - "Community 1231"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1232 - "Community 1232"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1233 - "Community 1233"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1234 - "Community 1234"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1235 - "Community 1235"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1236 - "Community 1236"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1237 - "Community 1237"
Cohesion: 0.12
Nodes (15): Browser / runtime support, Hard rules, Replit Deck Skill, Resource map, Scope & provenance, Step 0 — Pre-flight (mandatory reads), Step 1 — Commit to one theme, Step 2 — Plan slide rhythm before writing HTML (+7 more)

### Community 1238 - "Community 1238"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1239 - "Community 1239"
Cohesion: 0.23
Nodes (11): T, GenerateUsageSkill(), setTestHome(), TestGenerateUsageSkill(), TestGenerateUsageSkill_OverwritesExisting(), TestNormalizeProvider(), LocalSkillRoot(), NormalizeProvider() (+3 more)

### Community 1240 - "Community 1240"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1241 - "Community 1241"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1242 - "Community 1242"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1243 - "Community 1243"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1244 - "Community 1244"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1245 - "Community 1245"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, featured, kind, mode (+8 more)

### Community 1246 - "Community 1246"
Cohesion: 0.12
Nodes (16): assets, skills, od, capabilities, context, kind, mode, pipeline (+8 more)

### Community 1247 - "Community 1247"
Cohesion: 0.17
Nodes (12): bubble_menu, bold, code, italic, link, list, list_dropdown, quote (+4 more)

### Community 1248 - "Community 1248"
Cohesion: 0.17
Nodes (12): hub, agent_offline, agent_online, back_to_project, brand_name, composer_placeholder, hero_subtitle, hero_title (+4 more)

### Community 1249 - "Community 1249"
Cohesion: 0.36
Nodes (6): NewDaemonWSCollector(), newDaemonWSDesc(), DaemonWSCollector, Desc, Metric, Metrics

### Community 1250 - "Community 1250"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1251 - "Community 1251"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1252 - "Community 1252"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1253 - "Community 1253"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1254 - "Community 1254"
Cohesion: 0.13
Nodes (14): Authoring rules (important), Before you author anything — ALWAYS ask or recommend, Catalogs (load when needed), File structure, html-ppt — HTML PPT Studio, Install, Keyboard cheat sheet, License & author (+6 more)

### Community 1255 - "Community 1255"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1256 - "Community 1256"
Cohesion: 0.13
Nodes (14): 1. Style anchor (always prepend), 2. Variable slots (substitute per render), 3. Per-slot composition templates, 4. Chinese project input template, 5. Calling convention, `about.png` — 1:1 (1024×1024), Atelier Zero — Image Generation Prompt Pack, `capabilities.png` — 1:1 (1024×1024) (+6 more)

### Community 1257 - "Community 1257"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1258 - "Community 1258"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1259 - "Community 1259"
Cohesion: 0.13
Nodes (15): Daemon Profiles (Multi-Instance Isolation), 1. Start backend, frontend, and database, 2. Create a test user and token (automated auth), 3. Create a workspace, 4. Compute profile name and write CLI config, 5. Start the daemon from source, Database-Level Isolation Per Worktree, Desktop App Local Testing (+7 more)

### Community 1260 - "Community 1260"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1261 - "Community 1261"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1262 - "Community 1262"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1263 - "Community 1263"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1264 - "Community 1264"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1265 - "Community 1265"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1266 - "Community 1266"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1267 - "Community 1267"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1268 - "Community 1268"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1269 - "Community 1269"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1270 - "Community 1270"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1271 - "Community 1271"
Cohesion: 0.13
Nodes (14): 1. Create a GitHub App, 2. Set environment variables, 3. Run migrations, 4. Connect from the UI, 5. Verify with a curl probe, Disconnecting, How identifiers are matched, Limitations (+6 more)

### Community 1272 - "Community 1272"
Cohesion: 0.13
Nodes (14): 1. 创建一个 GitHub App, 2. 配置环境变量, 3. 执行 migration, 4. 在 UI 里连接, 5. 用 curl 自检, Merge 自动转 Done 的规则, Self-Host 配置, 下一步 (+6 more)

### Community 1273 - "Community 1273"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1274 - "Community 1274"
Cohesion: 0.20
Nodes (9): AICortexIcon(), AICortexIconProps, borderedSizes, AICortexLanding(), HomepagePage(), metadata, LandingPage(), metadata (+1 more)

### Community 1275 - "Community 1275"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1276 - "Community 1276"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1277 - "Community 1277"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1278 - "Community 1278"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1279 - "Community 1279"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1280 - "Community 1280"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1281 - "Community 1281"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1282 - "Community 1282"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1283 - "Community 1283"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1284 - "Community 1284"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1285 - "Community 1285"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1286 - "Community 1286"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1287 - "Community 1287"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1288 - "Community 1288"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1289 - "Community 1289"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1290 - "Community 1290"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1291 - "Community 1291"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1292 - "Community 1292"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1293 - "Community 1293"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1294 - "Community 1294"
Cohesion: 0.53
Nodes (5): isWindowsBash(), isWindowsCmdShell(), isWindowsPowerShell(), newTerminalCommand(), Cmd

### Community 1295 - "Community 1295"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1296 - "Community 1296"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1297 - "Community 1297"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1298 - "Community 1298"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1299 - "Community 1299"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1300 - "Community 1300"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1301 - "Community 1301"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1302 - "Community 1302"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1303 - "Community 1303"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1304 - "Community 1304"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1305 - "Community 1305"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1306 - "Community 1306"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1307 - "Community 1307"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1308 - "Community 1308"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1309 - "Community 1309"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1310 - "Community 1310"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1311 - "Community 1311"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1312 - "Community 1312"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1313 - "Community 1313"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1314 - "Community 1314"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1315 - "Community 1315"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1316 - "Community 1316"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1317 - "Community 1317"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1318 - "Community 1318"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1319 - "Community 1319"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1320 - "Community 1320"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1321 - "Community 1321"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1322 - "Community 1322"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1323 - "Community 1323"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1324 - "Community 1324"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1325 - "Community 1325"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1326 - "Community 1326"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1327 - "Community 1327"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1328 - "Community 1328"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1329 - "Community 1329"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1330 - "Community 1330"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1331 - "Community 1331"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1332 - "Community 1332"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1333 - "Community 1333"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1334 - "Community 1334"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1335 - "Community 1335"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1336 - "Community 1336"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1337 - "Community 1337"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1338 - "Community 1338"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1339 - "Community 1339"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1340 - "Community 1340"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1341 - "Community 1341"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1342 - "Community 1342"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1343 - "Community 1343"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1344 - "Community 1344"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1345 - "Community 1345"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1346 - "Community 1346"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1347 - "Community 1347"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1348 - "Community 1348"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1349 - "Community 1349"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1350 - "Community 1350"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1351 - "Community 1351"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1352 - "Community 1352"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1353 - "Community 1353"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1354 - "Community 1354"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1355 - "Community 1355"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1356 - "Community 1356"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1357 - "Community 1357"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1358 - "Community 1358"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1359 - "Community 1359"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1360 - "Community 1360"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1361 - "Community 1361"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1362 - "Community 1362"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1363 - "Community 1363"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1364 - "Community 1364"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1365 - "Community 1365"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1366 - "Community 1366"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1367 - "Community 1367"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1368 - "Community 1368"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1369 - "Community 1369"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1370 - "Community 1370"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1371 - "Community 1371"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1372 - "Community 1372"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1373 - "Community 1373"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1374 - "Community 1374"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1375 - "Community 1375"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1376 - "Community 1376"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1377 - "Community 1377"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1378 - "Community 1378"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1379 - "Community 1379"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1380 - "Community 1380"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1381 - "Community 1381"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1382 - "Community 1382"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1383 - "Community 1383"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1384 - "Community 1384"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1385 - "Community 1385"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1386 - "Community 1386"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1387 - "Community 1387"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1388 - "Community 1388"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1389 - "Community 1389"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1390 - "Community 1390"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1391 - "Community 1391"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1392 - "Community 1392"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1393 - "Community 1393"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1394 - "Community 1394"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1395 - "Community 1395"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1396 - "Community 1396"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1397 - "Community 1397"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1398 - "Community 1398"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1399 - "Community 1399"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1400 - "Community 1400"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1401 - "Community 1401"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1402 - "Community 1402"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1403 - "Community 1403"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1404 - "Community 1404"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1405 - "Community 1405"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1406 - "Community 1406"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1407 - "Community 1407"
Cohesion: 0.13
Nodes (14): 1. Gather brand inputs, 2. Decide the image strategy, 3. Compose the artifact, 4. (Optional) Mirror the deployable Astro site, Boundaries, `bring-your-own`, Files in this skill, `generate` — gpt-image-2 mode (+6 more)

### Community 1408 - "Community 1408"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1409 - "Community 1409"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1410 - "Community 1410"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1411 - "Community 1411"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1412 - "Community 1412"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1413 - "Community 1413"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1414 - "Community 1414"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1415 - "Community 1415"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1416 - "Community 1416"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1417 - "Community 1417"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1418 - "Community 1418"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1419 - "Community 1419"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1420 - "Community 1420"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1421 - "Community 1421"
Cohesion: 0.33
Nodes (4): PortPool, Context, Mutex, Querier

### Community 1422 - "Community 1422"
Cohesion: 0.13
Nodes (14): Authoring rules (important), Before you author anything — ALWAYS ask or recommend, Catalogs (load when needed), File structure, html-ppt — HTML PPT Studio, Install, Keyboard cheat sheet, License & author (+6 more)

### Community 1423 - "Community 1423"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1424 - "Community 1424"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1425 - "Community 1425"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1426 - "Community 1426"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1427 - "Community 1427"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1428 - "Community 1428"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1429 - "Community 1429"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1430 - "Community 1430"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1431 - "Community 1431"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1432 - "Community 1432"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1433 - "Community 1433"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1434 - "Community 1434"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1435 - "Community 1435"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1436 - "Community 1436"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1437 - "Community 1437"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1438 - "Community 1438"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1439 - "Community 1439"
Cohesion: 0.13
Nodes (15): assets, skills, od, capabilities, context, kind, mode, pipeline (+7 more)

### Community 1440 - "Community 1440"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1441 - "Community 1441"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1442 - "Community 1442"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1443 - "Community 1443"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1444 - "Community 1444"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1445 - "Community 1445"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1446 - "Community 1446"
Cohesion: 0.13
Nodes (14): author, name, url, compat, agentSkills, description, homepage, license (+6 more)

### Community 1447 - "Community 1447"
Cohesion: 0.13
Nodes (15): agent_label, agent_placeholder, allow_new_sessions_description, allow_new_sessions_label, create_success, expires_label, guide_message_label, guide_message_placeholder (+7 more)

### Community 1448 - "Community 1448"
Cohesion: 0.14
Nodes (13): P2 — polish, Structural, Accent restraint, Emission contract, Five-dimension critique (silent, before emit), Honesty, Layout rhythm, P0 — hard gates (non-negotiable) (+5 more)

### Community 1449 - "Community 1449"
Cohesion: 0.14
Nodes (13): Class inventory, Pre-flight, Default arcs, Layout 1 — Cover (slide 1), Layout 2 — Body slide (eyebrow + headline + lead), Layout 3 — Big stat (data billboard), Layout 4 — Three-point row, Layout 5 — Pipeline (numbered steps) (+5 more)

### Community 1450 - "Community 1450"
Cohesion: 0.14
Nodes (13): 2. 核心概念词典, 5. 产品地图（全部路由）, 6. 跨平台差异：Web vs 桌面, AICortex 产品全景文档, Web 特有, 为什么两端要做差异, 公共 / 认证, 共享（绝大部分功能） (+5 more)

### Community 1451 - "Community 1451"
Cohesion: 0.14
Nodes (14): subcategories, all, apps, brand_design, course_training, creative_decks, dashboards, devtools (+6 more)

### Community 1452 - "Community 1452"
Cohesion: 0.14
Nodes (13): 15 Full-deck templates, 27 CSS animations + 20 Canvas FX, 31 Single-page layouts, 36 Themes, html-ppt — HTML PPT Studio, Install (one command), Keyboard cheat sheet, License (+5 more)

### Community 1453 - "Community 1453"
Cohesion: 0.14
Nodes (13): 15 Templates de deck completo, 27 animações CSS + 20 Canvas FX, 31 Layouts de página única, 36 Temas, Atalhos de teclado, Estrutura do projeto, Filosofia, html-ppt — HTML PPT Studio (+5 more)

### Community 1454 - "Community 1454"
Cohesion: 0.14
Nodes (13): 15 套完整 deck 模板, 27 个 CSS 动画 + 20 个 Canvas FX, 31 种单页布局, 36 套主题, html-ppt · HTML PPT 工作室, Skill 内容一览, 一行命令安装, 协议 (+5 more)

### Community 1455 - "Community 1455"
Cohesion: 0.14
Nodes (14): od, capabilities, inputs, kind, mode, pipeline, platform, preview (+6 more)

### Community 1456 - "Community 1456"
Cohesion: 0.14
Nodes (14): od, capabilities, featured, kind, mode, pipeline, platform, preview (+6 more)

### Community 1457 - "Community 1457"
Cohesion: 0.14
Nodes (14): od, capabilities, inputs, kind, mode, pipeline, scenario, surface (+6 more)

### Community 1458 - "Community 1458"
Cohesion: 0.14
Nodes (14): od, capabilities, inputs, kind, mode, pipeline, platform, preview (+6 more)

### Community 1459 - "Community 1459"
Cohesion: 0.14
Nodes (14): od, capabilities, inputs, kind, mode, pipeline, platform, preview (+6 more)

### Community 1460 - "Community 1460"
Cohesion: 0.14
Nodes (14): od, capabilities, inputs, kind, mode, pipeline, platform, preview (+6 more)

### Community 1461 - "Community 1461"
Cohesion: 0.67
Nodes (3): isAgentActivityTask(), isDesignStudioTask(), isDevStudioTask()

### Community 1462 - "Community 1462"
Cohesion: 0.20
Nodes (10): context_anchor, aria_start, aria_stop, card_tooltip_issue, card_tooltip_issue_with_subtitle, card_tooltip_project, tooltip_disabled, tooltip_off (+2 more)

### Community 1463 - "Community 1463"
Cohesion: 0.14
Nodes (14): subcategories, all, apps, brand_design, course_training, creative_decks, dashboards, devtools (+6 more)

### Community 1464 - "Community 1464"
Cohesion: 0.33
Nodes (6): 3.6 Skill 技能, CLI 对应命令, 产品里的位置, 它怎么工作, 对应表, 数据形态

### Community 1465 - "Community 1465"
Cohesion: 0.20
Nodes (10): labels, assigned_to, created_with_agent, failed_with_detail, reacted_to_comment, removed_assignee, removed_due_date, set_due_date_to (+2 more)

### Community 1466 - "Community 1466"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1467 - "Community 1467"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1468 - "Community 1468"
Cohesion: 0.15
Nodes (12): 10. What to NOT do, 1. Understand the deck, 2. Pick a theme, 3. Outline the deck, 4. Scaffold the deck, 5. Author each slide, 6. Add animations sparingly, 7. Chinese + English decks (+4 more)

### Community 1469 - "Community 1469"
Cohesion: 0.15
Nodes (12): Bold & editorial, Bold & statement, Cool & dark, Effect-heavy, Effect-heavy / dramatic, How to apply, How to extend, Light & calm (+4 more)

### Community 1470 - "Community 1470"
Cohesion: 0.15
Nodes (12): L01 · cover-hero  (all themes), L02 · kpi-row-6  (helix · atlas · world-*), L03 · split-hero-metric  (helix), L04 · memo-hero-statement  (holm), L05 · two-column-ask  (holm), L06 · gallery-plate  (vance), L07 · campaign-cover  (bevel), L08 · finance-hero-grid  (world-dark / world-mint) (+4 more)

### Community 1471 - "Community 1471"
Cohesion: 0.15
Nodes (12): Class inventory (must exist in `template.html`), Layout 1 — Hero, centered, Layout 2 — Hero, split (text + visual), Layout 3 — Feature triplet, Layout 4 — Stat row (data billboard), Layout 5 — Pull quote (testimonial), Layout 6 — CTA strip (closing), Layout 7 — Log list (changelog / blog index / posts) (+4 more)

### Community 1472 - "Community 1472"
Cohesion: 0.28
Nodes (7): buildWorkloadIndex(), activeAgent, archivedAgent, makeAgent(), makeTask(), tasks, workload

### Community 1473 - "Community 1473"
Cohesion: 0.23
Nodes (13): Desktop Route Categories, Desktop Workspace Destructive Operations Order, Internal Packages Pattern, No-Duplication Rule (Shared Extraction), Optimistic Mutations Pattern, Package Boundary Hard Rules, Packages Core (Headless Business Logic), Packages UI (Atomic Components) (+5 more)

### Community 1474 - "Community 1474"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1475 - "Community 1475"
Cohesion: 0.15
Nodes (12): Clinical Case Report Skill, Important — Prescribing Safety, Step 0 — Load reference files, Step 1 — Parse the brief, Step 2 — Build the clinical narrative, Step 3 — Generate physiologically consistent clinical data, Step 4 — Write the assessment, Step 5 — Write the management plan (+4 more)

### Community 1476 - "Community 1476"
Cohesion: 0.33
Nodes (6): page, breadcrumb_title, breadcrumb_workspace_fallback, empty_hint, empty_title, move_failed

### Community 1477 - "Community 1477"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1478 - "Community 1478"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1479 - "Community 1479"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1480 - "Community 1480"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1481 - "Community 1481"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1482 - "Community 1482"
Cohesion: 0.15
Nodes (13): question_form, answered, answered_summary_fallback, choose, hint, locked_prev, locked_submitted, required (+5 more)

### Community 1483 - "Community 1483"
Cohesion: 0.15
Nodes (13): add_to_queue, attach, bg, close, color, comment_placeholder, edit_properties, font (+5 more)

### Community 1484 - "Community 1484"
Cohesion: 0.15
Nodes (13): add_to_input, attach_image, box_select, close, note_placeholder, pen, queue, redo (+5 more)

### Community 1485 - "Community 1485"
Cohesion: 0.25
Nodes (7): list, archive_tooltip, empty, mark_done_tooltip, page, back, title

### Community 1486 - "Community 1486"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1487 - "Community 1487"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1488 - "Community 1488"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1489 - "Community 1489"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1491 - "Community 1491"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1492 - "Community 1492"
Cohesion: 0.32
Nodes (5): TestDefaultTerminalShellWindowsIgnoresMissingSHELL(), TestDefaultTerminalShellWindowsPrefersCOMSPEC(), defaultTerminalShell(), shellExists(), T

### Community 1493 - "Community 1493"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1494 - "Community 1494"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1495 - "Community 1495"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1496 - "Community 1496"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1497 - "Community 1497"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1498 - "Community 1498"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1499 - "Community 1499"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1500 - "Community 1500"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1501 - "Community 1501"
Cohesion: 0.25
Nodes (8): friday, monday, saturday, sunday, thursday, tuesday, wednesday, days

### Community 1502 - "Community 1502"
Cohesion: 0.22
Nodes (9): activity_tooltip, created_days_ago_one, created_days_ago_other, created_today, failed_suffix, last_7_days, no_activity, runs_one (+1 more)

### Community 1503 - "Community 1503"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1504 - "Community 1504"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1505 - "Community 1505"
Cohesion: 0.29
Nodes (9): HandlerFunc, Request, T, hasBearerToken(), hasForwardingHeader(), isDirectLoopbackRequest(), realtimeMetricsHandler(), TestRealtimeMetricsHandler_NoToken_LoopbackOnly() (+1 more)

### Community 1506 - "Community 1506"
Cohesion: 0.24
Nodes (10): Client, Header, Logger, Message, Proxy, isHopByHopHeader(), NewProxy(), sanitizeResponseHeaders() (+2 more)

### Community 1507 - "Community 1507"
Cohesion: 0.15
Nodes (12): 10. What to NOT do, 1. Understand the deck, 2. Pick a theme, 3. Outline the deck, 4. Scaffold the deck, 5. Author each slide, 6. Add animations sparingly, 7. Chinese + English decks (+4 more)

### Community 1508 - "Community 1508"
Cohesion: 0.15
Nodes (12): Bold & editorial, Bold & statement, Cool & dark, Effect-heavy, Effect-heavy / dramatic, How to apply, How to extend, Light & calm (+4 more)

### Community 1509 - "Community 1509"
Cohesion: 0.15
Nodes (13): od, capabilities, inputs, kind, mode, pipeline, preview, scenario (+5 more)

### Community 1510 - "Community 1510"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1511 - "Community 1511"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1512 - "Community 1512"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1513 - "Community 1513"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1514 - "Community 1514"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1515 - "Community 1515"
Cohesion: 0.15
Nodes (13): od, capabilities, kind, mode, pipeline, platform, preview, scenario (+5 more)

### Community 1516 - "Community 1516"
Cohesion: 0.15
Nodes (13): od, capabilities, featured, kind, mode, pipeline, preview, scenario (+5 more)

### Community 1517 - "Community 1517"
Cohesion: 0.15
Nodes (13): binary_unsupported, empty, hint, load_error, loading_content, save, save_error, saving (+5 more)

### Community 1518 - "Community 1518"
Cohesion: 0.15
Nodes (13): question_form, answered, answered_summary_fallback, choose, hint, locked_prev, locked_submitted, required (+5 more)

### Community 1519 - "Community 1519"
Cohesion: 0.15
Nodes (13): add_to_queue, attach, bg, close, color, comment_placeholder, edit_properties, font (+5 more)

### Community 1520 - "Community 1520"
Cohesion: 0.15
Nodes (13): add_to_input, attach_image, box_select, close, note_placeholder, pen, queue, redo (+5 more)

### Community 1521 - "Community 1521"
Cohesion: 0.67
Nodes (3): Feature Worktree, Main Checkout, Recommended Daily Workflow

### Community 1522 - "Community 1522"
Cohesion: 0.17
Nodes (11): 3D & perspective, Ambient / continuous, Animations catalog, Directional fades, Dramatic entries, FX (canvas), Lists & numbers, Respecting motion preferences (+3 more)

### Community 1523 - "Community 1523"
Cohesion: 0.17
Nodes (11): 1. xhs-white-editorial — 白底杂志风, 2. graphify-dark-graph — 暗底知识图谱, 3. knowledge-arch-blueprint — 奶油蓝图架构, 4. hermes-cyber-terminal — 暗终端 honest-review, 5. obsidian-claude-gradient — GitHub 暗紫渐变, 6. testing-safety-alert — 红琥珀警示, 7. xhs-pastel-card — 柔和马卡龙慢生活, 8. dir-key-nav-minimal — 方向键 8 色极简 (+3 more)

### Community 1524 - "Community 1524"
Cohesion: 0.17
Nodes (11): Closers, Code & terminal, Diagrams & flows, Layouts catalog, Naming / structure conventions, Numbers & data, Openers & transitions, Picking a layout (+3 more)

### Community 1525 - "Community 1525"
Cohesion: 0.17
Nodes (11): activity-feed row, auto-toggle (page header, ghost), callout (Notion idiom), Components, kpi card (one cell of the KPI grid), linked-database row, live-pill (top-right of the topbar), refresh button (page header, primary) (+3 more)

### Community 1526 - "Community 1526"
Cohesion: 0.17
Nodes (11): atlas-dot  (atlas only), bevel-frame  (bevel only), bh-card  (bluehouse only), Components · replit-deck, Content-type cross-reference, eyebrow, meta-bar, numeric display  (`.num`, `.num-label`, `.num-delta`) (+3 more)

### Community 1527 - "Community 1527"
Cohesion: 0.24
Nodes (9): ListEntry, NewPendingStore(), pendingEntry, PendingStore, Response, Duration, ListEntry, Mutex (+1 more)

### Community 1528 - "Community 1528"
Cohesion: 0.17
Nodes (11): AICortex CLI — Installation Guide for AI Agents, One-Line Instruction for Users, Option A: Homebrew (preferred — macOS/Linux), Option B: Download from GitHub Releases (macOS/Linux, no Homebrew), Option C: Windows (PowerShell), Step 1: Check if AICortex CLI is already installed, Step 2: Install the AICortex CLI, Step 3: Log in (+3 more)

### Community 1529 - "Community 1529"
Cohesion: 0.17
Nodes (11): Archive and restore, Attaching domain expertise: Skills, Concurrency limit, Create an agent, Custom CLI arguments (custom_args), Custom environment variables (custom_env), Next steps, Pick an AI coding tool (+3 more)

### Community 1530 - "Community 1530"
Cohesion: 0.17
Nodes (11): 下一步, 写系统指令, 创建一个智能体, 可见性, 并发上限, 归档和恢复, 挂专业知识：Skill, 自定义命令行参数（custom_args） (+3 more)

### Community 1531 - "Community 1531"
Cohesion: 0.17
Nodes (11): Agents and skills, Auth and setup, Autopilots, Daemon and runtimes, Getting authenticated, Getting full flags, Issues and projects, Miscellaneous (+3 more)

### Community 1532 - "Community 1532"
Cohesion: 0.17
Nodes (11): Autopilots, Issue 和 Project, 下一步, 守护进程和运行时, 小队, 工作区和成员, 智能体和 Skill, 杂项 (+3 more)

### Community 1533 - "Community 1533"
Cohesion: 0.17
Nodes (11): Create a squad, How a squad-assigned issue runs, `@`-mention a squad in a comment, Next, Permissions, Reassign or archive a squad, Squad operations from the CLI, What a squad is, in mechanics (+3 more)

### Community 1534 - "Community 1534"
Cohesion: 0.17
Nodes (11): CLI 命令, 下一步, 什么时候用小队，什么时候用单个智能体, 分配给小队的 issue 是怎么跑的, 创建小队, 在评论里 `@` 一个小队, 小队的运转机制, 权限 (+3 more)

### Community 1535 - "Community 1535"
Cohesion: 0.17
Nodes (12): queue_bar, cancel_edit_aria, delete_aria, edit_aria, more_queued_one, more_queued_other, move_up_aria, queued_count_one (+4 more)

### Community 1536 - "Community 1536"
Cohesion: 0.50
Nodes (7): TestRollupTaskUsageDaily_AggregatesAndIsIdempotent(), TestRollupTaskUsageDaily_InvalidationOnIssueDelete(), TestRollupTaskUsageDaily_InvalidationOnReassign(), TestRollupTaskUsageDaily_UsesRuntimeTimezone(), TestRollupTaskUsageDaily_WatermarkAdvances(), TestRollupTaskUsageDaily_WorkspaceMismatch(), T

### Community 1537 - "Community 1537"
Cohesion: 0.33
Nodes (6): time_range, time_24h, time_30d, time_3d, time_7d, time_all

### Community 1538 - "Community 1538"
Cohesion: 0.36
Nodes (8): executeFakeCodex(), TestCodexExecuteSemanticInactivityAllowsContinuousDeltaProgress(), TestCodexExecuteSemanticInactivityAllowsContinuousMessages(), TestCodexExecuteSemanticInactivityDoesNotAffectNormalTurnCompletion(), TestCodexExecuteTimesOutWhenTurnStopsAfterToolResult(), writeFakeCodexAppServer(), ExecOptions, Result

### Community 1539 - "Community 1539"
Cohesion: 0.26
Nodes (8): PageviewTracker(), popDirectionHints, useTabHistory(), useTabRouterSync(), useActiveTabHistory(), useActiveTabIdentity(), useActiveTabRouter(), useTabStore

### Community 1540 - "Community 1540"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1541 - "Community 1541"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1542 - "Community 1542"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1543 - "Community 1543"
Cohesion: 0.17
Nodes (11): Data / evidence rules, IB Pitch Book, Provenance, Relationship to Open Design financial skills, Resource map, Step 0 — Pre-flight, Step 1 — Structure, Step 2 — Build (+3 more)

### Community 1544 - "Community 1544"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1545 - "Community 1545"
Cohesion: 0.17
Nodes (11): 1. Gather brand brief, 2. Pick the language stack, 3. Write `index.html`, 4. Self-check before delivering, Boundaries, Files in this skill, kami-landing, Page structure (+3 more)

### Community 1546 - "Community 1546"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1547 - "Community 1547"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1548 - "Community 1548"
Cohesion: 0.17
Nodes (12): scripts, build, clean, dev:desktop, dev:desktop:staging, dev:docs, dev:web, generate:reserved-slugs (+4 more)

### Community 1549 - "Community 1549"
Cohesion: 0.35
Nodes (7): BridgeEventBus, formatProgressMessage(), NewBridgeEventBus(), Context, PreviewEnvironment, ProvisionState, Queries

### Community 1550 - "Community 1550"
Cohesion: 0.17
Nodes (11): document, dataPath, format, generatedPreviewPath, templatePath, preview, entry, type (+3 more)

### Community 1551 - "Community 1551"
Cohesion: 0.17
Nodes (11): 1. Monocle Editorial · 国际杂志风 ✦ 默认推荐, 2. WIRED Tech · 数据 + 工程, 3. Kinfolk Slow · 慢生活 / 人文, 4. Domus Architectural · 建筑 / 空间感, 5. Lab / Reference · 学术 + 工艺手册, ❌ 不要做的事, 何时用这份文档, 决策记录（生成前必做） (+3 more)

### Community 1552 - "Community 1552"
Cohesion: 0.17
Nodes (11): 3D & perspective, Ambient / continuous, Animations catalog, Directional fades, Dramatic entries, FX (canvas), Lists & numbers, Respecting motion preferences (+3 more)

### Community 1553 - "Community 1553"
Cohesion: 0.17
Nodes (11): 1. xhs-white-editorial — 白底杂志风, 2. graphify-dark-graph — 暗底知识图谱, 3. knowledge-arch-blueprint — 奶油蓝图架构, 4. hermes-cyber-terminal — 暗终端 honest-review, 5. obsidian-claude-gradient — GitHub 暗紫渐变, 6. testing-safety-alert — 红琥珀警示, 7. xhs-pastel-card — 柔和马卡龙慢生活, 8. dir-key-nav-minimal — 方向键 8 色极简 (+3 more)

### Community 1554 - "Community 1554"
Cohesion: 0.17
Nodes (11): Closers, Code & terminal, Diagrams & flows, Layouts catalog, Naming / structure conventions, Numbers & data, Openers & transitions, Picking a layout (+3 more)

### Community 1555 - "Community 1555"
Cohesion: 0.17
Nodes (11): Hard rules (the seed protects most of these — don't fight it), Output contract, Resource map, Step 0 — Pre-flight (do this once before writing anything), Step 1 — Prepare the artifact from the seed, Step 2 — Plan the section list, Step 3 — Paste and fill, Step 4 — Self-check (+3 more)

### Community 1556 - "Community 1556"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1557 - "Community 1557"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1558 - "Community 1558"
Cohesion: 0.17
Nodes (12): od, capabilities, kind, mode, pipeline, preview, scenario, surface (+4 more)

### Community 1559 - "Community 1559"
Cohesion: 0.17
Nodes (12): queue_bar, cancel_edit_aria, delete_aria, edit_aria, more_queued_one, more_queued_other, move_up_aria, queued_count_one (+4 more)

### Community 1560 - "Community 1560"
Cohesion: 0.17
Nodes (12): web, checking_index, disabled, hint, index_missing, mode_static, mode_tunnel, no_runtime (+4 more)

### Community 1561 - "Community 1561"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1562 - "Community 1562"
Cohesion: 0.22
Nodes (11): AICortex Platform, Multi-Tenancy (Workspace-ID Header), Agent CLI Auto-Detection, Daemon Workspace Garbage Collection, Daemon Lifecycle (Poll-Detect-Execute-Stream), Agent Daemon (Local Runtime), Compound Skills (Reusable Patterns), AI Engineering Team Orchestration Layer (+3 more)

### Community 1563 - "Community 1563"
Cohesion: 0.38
Nodes (8): Client, EnvironmentFromEnv(), isDisabled(), NewFromEnv(), normalizeEnvironment(), Event, NoopClient, Time

### Community 1564 - "Community 1564"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1565 - "Community 1565"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1566 - "Community 1566"
Cohesion: 0.18
Nodes (10): ❌ 不要做的事, 主题色预设（Themes）, 使用方法, 切换原则, 🖋 墨水经典 (Monocle 默认), 推荐选择参考, 🌿 森林墨 (Forest Ink), 🌙 沙丘 (Dune) (+2 more)

### Community 1567 - "Community 1567"
Cohesion: 0.18
Nodes (10): Class inventory, Pre-flight, Archetype A — Feed (home / for-you / inbox), Archetype B — Detail (single item), Archetype C — Onboarding (1 of N), Archetype D — Profile (someone's page), Archetype E — Checkout / form, Archetype F — Focus / hero card (timer, map, single tool) (+2 more)

### Community 1568 - "Community 1568"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1569 - "Community 1569"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1570 - "Community 1570"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1571 - "Community 1571"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1572 - "Community 1572"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1573 - "Community 1573"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1574 - "Community 1574"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1575 - "Community 1575"
Cohesion: 0.18
Nodes (11): Agent Daemon, Configuration, How It Works, Logs, Profiles, Self-Hosted Server, Start, Status (+3 more)

### Community 1576 - "Community 1576"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1577 - "Community 1577"
Cohesion: 0.40
Nodes (5): 4. 系统架构全景, AI / LLM 在哪里, 分层职责, 后台任务, 实时层（WebSocket）

### Community 1578 - "Community 1578"
Cohesion: 0.29
Nodes (7): description, label, output_modes, create_issue, run_only, description, label

### Community 1579 - "Community 1579"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1580 - "Community 1580"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1581 - "Community 1581"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1582 - "Community 1582"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1583 - "Community 1583"
Cohesion: 0.35
Nodes (6): fakeBackend, Context, ExecOptions, Int32, Result, Session

### Community 1584 - "Community 1584"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1585 - "Community 1585"
Cohesion: 0.18
Nodes (10): Agent Prompt Guide, Color Palette & Roles, Component Stylings, Depth & Elevation, Do's and Don'ts, Layout Principles, Neutral Modern, Responsive Behavior (+2 more)

### Community 1586 - "Community 1586"
Cohesion: 0.33
Nodes (6): task_failure, agent_error, manual, runtime_offline, runtime_recovery, timeout

### Community 1587 - "Community 1587"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1588 - "Community 1588"
Cohesion: 0.18
Nodes (10): 1. `.aicortex/project/resources.json`, 2. A "Project Context" section in the meta-skill prompt, Adding a new resource type, Attaching repos at project creation, Failure mode, Mental model, Today: `github_repo`, What's intentionally **not** in scope here (+2 more)

### Community 1589 - "Community 1589"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1590 - "Community 1590"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1591 - "Community 1591"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1592 - "Community 1592"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1593 - "Community 1593"
Cohesion: 0.25
Nodes (8): fri, mon, sat, sun, thu, tue, wed, days_short

### Community 1594 - "Community 1594"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1595 - "Community 1595"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1596 - "Community 1596"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1597 - "Community 1597"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1598 - "Community 1598"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1599 - "Community 1599"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1600 - "Community 1600"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1601 - "Community 1601"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1602 - "Community 1602"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1603 - "Community 1603"
Cohesion: 0.25
Nodes (8): attachment, close, download_failed, preview, preview_failed, preview_loading, preview_too_large, preview_unsupported

### Community 1604 - "Community 1604"
Cohesion: 0.25
Nodes (15): InboxDetailLabel(), shortDate(), useTypeLabels(), escapeRegExp(), getInboxDisplayTitle(), getQuickCreateFailureDetail(), singleLine(), stripQuickCreatePrefix() (+7 more)

### Community 1605 - "Community 1605"
Cohesion: 0.45
Nodes (9): createOtherTestWorkspace(), TestAttachLabelCrossWorkspaceLabel(), TestColorCaseNormalization(), TestIssueLabelAttachDetach(), TestLabelCRUD(), TestLabelNameTooLong(), TestLabelNotFoundAcrossWorkspaces(), TestUpdateLabelCrossWorkspace() (+1 more)

### Community 1606 - "Community 1606"
Cohesion: 0.18
Nodes (10): 1. Gather the brief, 2. Pick the language stack, 3. Write `index.html`, 4. Self-check, Boundaries, kami-deck, See also, Slide types (+2 more)

### Community 1607 - "Community 1607"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1608 - "Community 1608"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1609 - "Community 1609"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1610 - "Community 1610"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1611 - "Community 1611"
Cohesion: 0.25
Nodes (8): artifacts, empty_dir, empty_sources, hint, loading_files, loading_sources, preview_frame_title, title

### Community 1612 - "Community 1612"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1613 - "Community 1613"
Cohesion: 0.18
Nodes (10): Hard rules, Mobile App Skill, Resource map, Step 0 — Pre-flight, Step 1 — Copy the seed, Step 2 — Pick exactly one archetype, Step 3 — Paste and fill, Step 4 — Self-check (+2 more)

### Community 1614 - "Community 1614"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1615 - "Community 1615"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1616 - "Community 1616"
Cohesion: 0.33
Nodes (6): cancel, confirm, deleting, description, title, delete_dialog

### Community 1617 - "Community 1617"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1618 - "Community 1618"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1619 - "Community 1619"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1620 - "Community 1620"
Cohesion: 0.18
Nodes (10): 1. Author `inputs.json`, 2. (Optional) generate or stub imagery, 3. Compose the deck, 4. Self-check, Boundaries, open-design-landing-deck, See also, Slide types (+2 more)

### Community 1621 - "Community 1621"
Cohesion: 0.18
Nodes (10): name, @types/react, @types/react-dom, packageManager, pnpm, onlyBuiltDependencies, overrides, private (+2 more)

### Community 1622 - "Community 1622"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1623 - "Community 1623"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1624 - "Community 1624"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1625 - "Community 1625"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1626 - "Community 1626"
Cohesion: 0.18
Nodes (10): presenter-mode-reveal · 演讲者模式模板, 主题切换, 使用场景, 修改 / 扩展, 写逐字稿的规范, 快速开始, 文件结构, 注意事项 (+2 more)

### Community 1627 - "Community 1627"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1628 - "Community 1628"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1629 - "Community 1629"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1630 - "Community 1630"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1631 - "Community 1631"
Cohesion: 0.83
Nodes (3): setTestHome(), TestRunSkillGenerate(), T

### Community 1632 - "Community 1632"
Cohesion: 0.27
Nodes (7): Duration, Mutex, Time, NewPendingStore(), pendingEntry, PendingStore, ProxyResponse

### Community 1633 - "Community 1633"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1634 - "Community 1634"
Cohesion: 0.18
Nodes (10): Hard rules, Resource map, Simple Deck Skill, Step 0 — Pre-flight, Step 1 — Copy the seed, Step 2 — Decide slide count + theme rhythm BEFORE writing any slide, Step 3 — Paste and fill, Step 4 — Self-check (+2 more)

### Community 1635 - "Community 1635"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1636 - "Community 1636"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1637 - "Community 1637"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1638 - "Community 1638"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1639 - "Community 1639"
Cohesion: 0.18
Nodes (10): 1. Visual Theme & Atmosphere, 2. Color, 3. Typography, 4. Spacing & Grid, 5. Layout & Composition, 6. Components, 7. Motion & Interaction, 8. Voice & Brand (+2 more)

### Community 1640 - "Community 1640"
Cohesion: 0.18
Nodes (10): Agent Prompt Guide, Color Palette & Roles, Component Stylings, Depth & Elevation, Do's and Don'ts, Layout Principles, Responsive Behavior, Typography Rules (+2 more)

### Community 1641 - "Community 1641"
Cohesion: 0.20
Nodes (10): Assign Issue, Change Status, Comments, Create Issue, Execution History, Get Issue, Issues, List Issues (+2 more)

### Community 1642 - "Community 1642"
Cohesion: 0.20
Nodes (9): Backend Structure, Data Flow, Frontend Architecture, Internal Packages Pattern, Multi-tenancy, Overview, Package Boundaries, Project Structure (+1 more)

### Community 1643 - "Community 1643"
Cohesion: 0.20
Nodes (10): 2. task-scheduler — PM 任务调度, 3. qa-workflow — QA 测试流程, AICortex 开发团队 Skill 定义文档, 任务流转示例, 关键原则, 团队协作, 当前团队, 核心规则（防断链） (+2 more)

### Community 1644 - "Community 1644"
Cohesion: 0.20
Nodes (10): 7. 附录：关键数据表速查, Agent / Runtime / Skill, Issue / 协作, 任务执行, 对话, 工作区 / 成员, 自动化, 身份 / 认证 (+2 more)

### Community 1645 - "Community 1645"
Cohesion: 0.20
Nodes (9): 1. Pull the project and start the backend, 2. Important: keep production safety on, 3. Configure the email service (optional but recommended), 4. First login + create a workspace, 5. Point the CLI at your own server, 6. Create an agent + assign your first task, Common issues, Next steps (+1 more)

### Community 1646 - "Community 1646"
Cohesion: 0.20
Nodes (9): 1. 拉取项目 + 一键启动后端, 2. 重要：保持生产安全配置, 3. 配置邮件服务（可选但推荐）, 4. 首次登录 + 创建工作区, 5. 连接命令行工具到你自己的 server, 6. 创建智能体 + 分配第一个任务, 下一步, 前置要求 (+1 more)

### Community 1647 - "Community 1647"
Cohesion: 0.20
Nodes (10): audio, deck, design_system_chip, from_template, hyperframes, image, live_artifact, prototype (+2 more)

### Community 1648 - "Community 1648"
Cohesion: 0.20
Nodes (3): FS, data, lines

### Community 1649 - "Community 1649"
Cohesion: 0.47
Nodes (8): TestCreateProjectAttachesResources(), TestCreateProjectRollsBackOnInvalidResource(), TestCreateProjectWithResourcesEchoesCount(), TestIsValidGitRepoURL(), TestProjectResourceAcceptsSSHRepoURLs(), TestProjectResourceCountBreadcrumb(), TestProjectResourceLifecycle(), T

### Community 1650 - "Community 1650"
Cohesion: 0.54
Nodes (6): createFileUploadExtension(), dedupFiles(), finalizeFileCard(), removeImageBySrc(), removeUploadingFileCard(), uploadAndInsertFile()

### Community 1651 - "Community 1651"
Cohesion: 0.25
Nodes (7): a, b, c, ids, items, { recordVisit }, { recordVisit, pruneWorkspaces }

### Community 1652 - "Community 1652"
Cohesion: 0.20
Nodes (9): devDependencies, pg, @playwright/test, turbo, @types/node, @types/pg, typescript, globalEnv (+1 more)

### Community 1653 - "Community 1653"
Cohesion: 0.29
Nodes (7): columns, activity_7d, agent, runs, runtime, status, workload

### Community 1654 - "Community 1654"
Cohesion: 0.20
Nodes (9): Connector execution, Connector listing, Connector Policy Reference, Connector safety model, Credential handling constraints, Execution boundaries, Output protection, Persistence rules (+1 more)

### Community 1655 - "Community 1655"
Cohesion: 0.25
Nodes (8): table, actions, agent, expires, status, title, token, uses

### Community 1656 - "Community 1656"
Cohesion: 0.20
Nodes (10): cache, persistent, dependsOn, tasks, dev:staging, lint, test, typecheck (+2 more)

### Community 1657 - "Community 1657"
Cohesion: 0.20
Nodes (9): Hard rules, Resource map, Step 0 — Read the project metadata, Step 1 — Plan the shot, Step 2 — Compose the prompt, Step 3 — Dispatch via the media contract, Step 4 — Hand off, Video Shortform Skill (+1 more)

### Community 1658 - "Community 1658"
Cohesion: 0.20
Nodes (10): audio, deck, design_system_chip, from_template, hyperframes, image, live_artifact, prototype (+2 more)

### Community 1660 - "Community 1660"
Cohesion: 0.25
Nodes (8): errors, archive_all_failed, archive_all_read_failed, archive_completed_failed, archive_failed, mark_all_read_failed, mark_done_failed, mark_read_failed

### Community 1661 - "Community 1661"
Cohesion: 0.22
Nodes (8): CLI and Agent Daemon Guide, Configuration, Other Commands, Output Formats, Quick Start, Set Values, Setup, View Config

### Community 1662 - "Community 1662"
Cohesion: 0.22
Nodes (9): 1. dev-workflow — 开发工作流, Git 配置, Pipeline 协作规则, 启动服务前检测端口, 团队角色, 开发流程, 端口分配, 约束 (+1 more)

### Community 1663 - "Community 1663"
Cohesion: 0.22
Nodes (8): Can you still invite people when signup is disabled?, Fixed local testing codes, Google OAuth configuration, How email + verification code sign-in works, Next, Option A: Resend (recommended for cloud / public-internet deployments), Option B: SMTP relay (for self-hosted / on-premise deployments), Restricting who can sign up

### Community 1664 - "Community 1664"
Cohesion: 0.22
Nodes (8): Email + 验证码登录怎么工作, Option A：Resend（公网/云端部署推荐）, Option B：SMTP relay（内网/自部署）, 下一步, 关了注册还能邀请人进来吗, 固定本地测试验证码, 怎么配 Google OAuth, 怎么限制谁能注册

### Community 1665 - "Community 1665"
Cohesion: 0.22
Nodes (8): Configure an autopilot, Next, Pick an execution mode, Run it on a schedule, Trigger once manually, View run history, What happens when an autopilot fails, What's not yet available

### Community 1666 - "Community 1666"
Cohesion: 0.22
Nodes (8): Autopilot 失败会怎样, 下一步, 手动触发一次, 暂不可用的能力, 看运行历史, 让它按时间跑, 选择执行模式, 配置一个 Autopilot

### Community 1667 - "Community 1667"
Cohesion: 0.22
Nodes (9): 1.1 写作守则（硬约束）, 1.2 目标读者分级, 1.3 状态码, 1.4 Flag（只在需要决策时填）, 1.5 分工流程, 1.6 每篇目标字数, 1.7 Review Checklist（提 review 前自查）, 1.8 MDX 样例模板 (+1 more)

### Community 1668 - "Community 1668"
Cohesion: 0.22
Nodes (8): Assigning an issue to an agent, Comments, Deleting an issue, Issue numbers, Next, Priority, Projects, Status

### Community 1669 - "Community 1669"
Cohesion: 0.22
Nodes (8): Issue 编号, Project, 下一步, 优先级, 删除 issue, 把 issue 分配给智能体, 状态, 评论

### Community 1670 - "Community 1670"
Cohesion: 0.22
Nodes (9): 3.7 Autopilot 自动驾驶, 三种触发方式, 两种执行模式, 产品里的位置, 内置模板, 对应表, 并发策略, 数据形态 (+1 more)

### Community 1671 - "Community 1671"
Cohesion: 0.39
Nodes (5): IssueDependency, Context, Queries, Issue, UUID

### Community 1672 - "Community 1672"
Cohesion: 0.22
Nodes (8): 1. Sign up, 2. Install the CLI and start the daemon, 3. Verify your runtime, 4. Create an agent, 5. Assign your first task, macOS / Linux (Homebrew - recommended), macOS / Linux (install script), Windows (PowerShell)

### Community 1673 - "Community 1673"
Cohesion: 0.22
Nodes (8): AI Disclosure, Changes Made, Checklist, How to Test, Related Issue, Screenshots (optional), Type of Change, What does this PR do?

### Community 1674 - "Community 1674"
Cohesion: 0.31
Nodes (6): workspaceExploreEnabled(), Context, Handler, Member, Request, ResponseWriter

### Community 1675 - "Community 1675"
Cohesion: 0.22
Nodes (8): Hard rules, Image Poster Skill, Resource map, Step 0 — Read the project metadata, Step 1 — Compose the prompt, Step 2 — Dispatch via the media contract, Step 3 — Hand off, Workflow

### Community 1676 - "Community 1676"
Cohesion: 0.22
Nodes (8): Canvas tokens (use these exact values), Connector → UI mapping (pick the matching family), ⚠️ Design system policy, Fallback heuristics (for unknown connectors), Implementation constraints (paired do / don't), Orbit General Briefing, Page sections (top to bottom), ⚠️ Source-of-truth protocol (read this first)

### Community 1677 - "Community 1677"
Cohesion: 0.29
Nodes (7): image, copy_link, copy_link_failed, delete, download, link_copied, view

### Community 1678 - "Community 1678"
Cohesion: 0.22
Nodes (8): fields, notes, refreshPolicy, minInterval, note, recommended, sources, transformations

### Community 1679 - "Community 1679"
Cohesion: 0.22
Nodes (8): 1. Read context, 2. Plan sections, 3. Apply design system, 4. Write the file, 5. Self-check, 6. Done, For skill authors reading this as a reference, SaaS Landing Skill

### Community 1680 - "Community 1680"
Cohesion: 0.22
Nodes (8): Alternative: Manual Setup, Custom Ports, Install, Prerequisites, Self-Hosting Setup (for AI Agents), Stopping, Troubleshooting, Verification

### Community 1681 - "Community 1681"
Cohesion: 0.29
Nodes (7): table, created, lead, name, priority, progress, status

### Community 1682 - "Community 1682"
Cohesion: 0.22
Nodes (8): Output, Output, Preflight: Load hardened template and brand foundation, Quality gates, Steps: Token replacement with validation and escaping, Validation: Run hardened quality gates before emitting, Waitlist Page Skill, Workflow

### Community 1683 - "Community 1683"
Cohesion: 0.22
Nodes (9): picker, create_first, empty, new_project, no_results, open_studio, search_placeholder, subtitle (+1 more)

### Community 1684 - "Community 1684"
Cohesion: 0.57
Nodes (6): fakeKimiACPScript(), TestKimiBackendInvokesACPSubcommand(), TestKimiBackendSetModelFailureFailsTask(), TestKimiToolNameFromTitle(), TestNewReturnsKimiBackend(), T

### Community 1685 - "Community 1685"
Cohesion: 0.25
Nodes (7): Architecture, Commands, Package Boundaries (hard rules), Project Overview, Quick Reference, Repository Guidelines, State Management (critical)

### Community 1686 - "Community 1686"
Cohesion: 0.13
Nodes (15): 3.1 Workspace 工作区, 3.3 Project 项目, 3.4 Agent 智能体, 3. 功能全景（按模块）, 交互, 产品里的位置, 产品里的位置, 产品里的位置 (+7 more)

### Community 1687 - "Community 1687"
Cohesion: 0.25
Nodes (7): Structural, Connector wiring (only when connector !== mock), Content, Honesty (the most-violated rule), Polish (P1), Pre-emit checklist (P0 must pass; do not write index.html until green), Visual integrity

### Community 1688 - "Community 1688"
Cohesion: 0.25
Nodes (8): Associating Issues with Projects, Change Status, Create Project, Delete Project, Get Project, List Projects, Projects, Update Project

### Community 1689 - "Community 1689"
Cohesion: 0.25
Nodes (7): Build from Source, Download from GitHub Releases, Homebrew (macOS/Linux), Installation, Quick Start, Update, Verify

### Community 1691 - "Community 1691"
Cohesion: 0.25
Nodes (7): 1. Create an account, 2. Install the AICortex CLI, 3. Log in + start the daemon, 4. Verify the runtime is online, 5. Create an agent, 6. Assign your first task, Next steps

### Community 1692 - "Community 1692"
Cohesion: 0.25
Nodes (7): 1. 注册账号, 2. 装 AICortex 命令行工具, 3. 登录 + 启动守护进程, 4. 验证 Runtime 在线, 5. 创建智能体, 6. 分配第一个任务, 下一步

### Community 1693 - "Community 1693"
Cohesion: 0.25
Nodes (7): Codex sandbox troubleshooting (macOS `no such host`), If you need to hand-verify, Options and trade-offs, Quick self-check commands, Root cause, Symptom fingerprint, What the daemon does now

### Community 1694 - "Community 1694"
Cohesion: 0.25
Nodes (7): `@all` notifies the entire workspace, Editing and deleting a comment, `@` mentions, Next, Posting a comment, Reactions, Replying to a comment

### Community 1695 - "Community 1695"
Cohesion: 0.25
Nodes (7): `@all` 会通知整个工作区, 下一步, 发评论, 回复某条评论, `@` 提及, 编辑和删除评论, 表情反应

### Community 1696 - "Community 1696"
Cohesion: 0.25
Nodes (7): How many tasks can run in parallel, Next, Starting the daemon, Troubleshooting agents that aren't working, What happens to in-flight tasks after a daemon crash, When a runtime is marked offline, Why one machine has multiple runtimes

### Community 1697 - "Community 1697"
Cohesion: 0.25
Nodes (7): Agent 不工作怎么排查, 一次能并发跑多少任务, 下一步, 为什么一台机器会有多个运行时, 启动守护进程, 守护进程崩溃后，没跑完的任务会怎样, 运行时什么时候被判定为离线

### Community 1698 - "Community 1698"
Cohesion: 0.25
Nodes (7): 6.1 Inbox & Subscriptions — ⬜ Not started [v1], AICortex Docs 执行大纲, 三、大纲概览（v1 导航）, 五、进度汇总（v1 25 篇）, 六、决策点记录（append-only）, 四、V2 Pending 清单（24 篇，v1 ship 后再动）, 板块 6：Staying Informed

### Community 1699 - "Community 1699"
Cohesion: 0.25
Nodes (8): 3.12 认证、登录与 Onboarding, Onboarding 流程（正在重设计中）, Signup 限流, 产品里的位置, 对应表, 登录方式, 认证后的跳转规则, 邀请接受（Zero-workspace）

### Community 1700 - "Community 1700"
Cohesion: 0.25
Nodes (8): 3.2 Issue 议题管理, 交互, 产品里的位置, 对应表, 核心字段, 视图, 评论与讨论, 附件

### Community 1701 - "Community 1701"
Cohesion: 0.25
Nodes (8): 3.5 Runtime 运行时 & Daemon 守护进程, CLI 与 Daemon 的关系, Daemon 是什么, Runtime 展示, Runtime 的生命周期, 产品里的位置, 安全边界, 对应表

### Community 1702 - "Community 1702"
Cohesion: 0.25
Nodes (7): Can a task continue from the previous context, How a failed task affects issue status, Manual rerun vs. automatic retry, Next, The states a task goes through, What happens when a task times out, Which failures retry automatically, which don't

### Community 1703 - "Community 1703"
Cohesion: 0.25
Nodes (7): Daemon can't connect to the server, Emails not received, Fixed local test code doesn't work, Port conflicts, Tasks stuck in `queued`, WebSocket can't connect, Where to find logs

### Community 1704 - "Community 1704"
Cohesion: 0.25
Nodes (8): awaiting_lead, awaiting_title, failed_title, step_generate, step_prepare, step_understand, title, generation

### Community 1705 - "Community 1705"
Cohesion: 0.33
Nodes (5): { container }, found, iframe, lightboxFrame, memoTypeSymbol

### Community 1707 - "Community 1707"
Cohesion: 0.33
Nodes (6): custom, daily, hourly, weekdays, weekly, describe

### Community 1708 - "Community 1708"
Cohesion: 0.25
Nodes (7): Banned, Hard rules, HTML PPT — Tactical Telemetry / CRT Terminal, Motion, Pre-flight, Required slide archetypes (10–14 total), Source

### Community 1709 - "Community 1709"
Cohesion: 0.25
Nodes (7): Banned, Hard rules, HTML PPT — Editorial Minimalism, Motion, Pre-flight, Required slide archetypes (10–12 total recommended), Source

### Community 1710 - "Community 1710"
Cohesion: 0.25
Nodes (7): 8-Bit Orbit, At a glance, Avoid for, Best for, Output contract, Source & license, Workflow

### Community 1711 - "Community 1711"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Biennale Yellow, Output contract, Source & license, Workflow

### Community 1712 - "Community 1712"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, BlockFrame, Output contract, Source & license, Workflow

### Community 1713 - "Community 1713"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Blue Professional, Output contract, Source & license, Workflow

### Community 1714 - "Community 1714"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Bold Poster, Output contract, Source & license, Workflow

### Community 1715 - "Community 1715"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Broadside, Output contract, Source & license, Workflow

### Community 1716 - "Community 1716"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Capsule, Output contract, Source & license, Workflow

### Community 1717 - "Community 1717"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Cartesian, Output contract, Source & license, Workflow

### Community 1718 - "Community 1718"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Cobalt Grid, Output contract, Source & license, Workflow

### Community 1719 - "Community 1719"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Coral, Output contract, Source & license, Workflow

### Community 1720 - "Community 1720"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Creative Mode, Output contract, Source & license, Workflow

### Community 1721 - "Community 1721"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Daisy Days, Output contract, Source & license, Workflow

### Community 1722 - "Community 1722"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Editorial Tri-Tone, Output contract, Source & license, Workflow

### Community 1723 - "Community 1723"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Grove, Output contract, Source & license, Workflow

### Community 1724 - "Community 1724"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Long Table, Output contract, Source & license, Workflow

### Community 1725 - "Community 1725"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Mat, Output contract, Source & license, Workflow

### Community 1726 - "Community 1726"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Monochrome, Output contract, Source & license, Workflow

### Community 1727 - "Community 1727"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Neo-Grid Bold, Output contract, Source & license, Workflow

### Community 1728 - "Community 1728"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, People's Platform (Block & Bold), Source & license, Workflow

### Community 1729 - "Community 1729"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Pin & Paper, Source & license, Workflow

### Community 1730 - "Community 1730"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Pink Script — After Hours, Source & license, Workflow

### Community 1731 - "Community 1731"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Playful, Source & license, Workflow

### Community 1732 - "Community 1732"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Raw Grid, Source & license, Workflow

### Community 1733 - "Community 1733"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Retro Windows, Source & license, Workflow

### Community 1734 - "Community 1734"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Retro Zine, Source & license, Workflow

### Community 1735 - "Community 1735"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Sakura Chroma, Source & license, Workflow

### Community 1736 - "Community 1736"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Scatterbrain, Source & license, Workflow

### Community 1737 - "Community 1737"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Signal, Source & license, Workflow

### Community 1738 - "Community 1738"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Soft Editorial, Source & license, Workflow

### Community 1739 - "Community 1739"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Source & license, Stencil & Tablet, Workflow

### Community 1740 - "Community 1740"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Source & license, Studio, Workflow

### Community 1741 - "Community 1741"
Cohesion: 0.25
Nodes (7): At a glance, Avoid for, Best for, Output contract, Source & license, Vellum, Workflow

### Community 1742 - "Community 1742"
Cohesion: 0.33
Nodes (6): frequency_long, custom, daily, hourly, weekdays, weekly

### Community 1743 - "Community 1743"
Cohesion: 0.25
Nodes (7): Build order, Hard nos (anti-AI-slop), Live behavior (the part that earns the "Live" in Live Artifact), Live Dashboard, Output contract, Pre-flight (must complete before emitting any HTML), Self-critique (must run before emitting)

### Community 1744 - "Community 1744"
Cohesion: 0.25
Nodes (7): Canvas tokens (use these exact values), ⚠️ Design system policy, Implementation constraints (paired do / don't), Orbit · GitHub Briefing, Page sections, Pill / chip rules, ⚠️ Source-of-truth protocol (read this first)

### Community 1745 - "Community 1745"
Cohesion: 0.25
Nodes (7): Canvas tokens (use these exact values), ⚠️ Design system policy, Implementation constraints (paired do / don't), Orbit · Gmail Briefing, Page sections (top to bottom — the page is one column, not a 3-pane app), Pill / icon rules, ⚠️ Source-of-truth protocol (read this first)

### Community 1746 - "Community 1746"
Cohesion: 0.25
Nodes (7): Canvas tokens — light theme (default to ship), ⚠️ Design system policy, Identifier / chip rules, Implementation constraints (paired do / don't), Orbit · Linear Briefing, Page sections, ⚠️ Source-of-truth protocol (read this first)

### Community 1747 - "Community 1747"
Cohesion: 0.25
Nodes (7): Block formatting rules, Canvas tokens (use these exact values), ⚠️ Design system policy, Implementation constraints (paired do / don't), Orbit · Notion Briefing, Page sections (top to bottom), ⚠️ Source-of-truth protocol (read this first)

### Community 1748 - "Community 1748"
Cohesion: 0.25
Nodes (7): Binding rules, Bounded JSON limits, Create/update input, HTML document contract, Live Artifact Schema Reference, Minimal static artifact input, Source files

### Community 1749 - "Community 1749"
Cohesion: 0.25
Nodes (7): DCF sensitivity, Disclaimer slide, Every slide, Football field, Investment-banking pitch book — layout conventions, Strategic alternatives matrix, Tables

### Community 1750 - "Community 1750"
Cohesion: 0.29
Nodes (7): workdir, cli_all_permissions_description, cli_all_permissions_label, pinned_description, pinned_label, section_header, toast_update_failed

### Community 1751 - "Community 1751"
Cohesion: 0.25
Nodes (7): Banned, Hard rules, Motion, Pre-flight, Required components, Source, Web Prototype — Industrial Brutalist (Swiss Print)

### Community 1752 - "Community 1752"
Cohesion: 0.25
Nodes (7): Banned, Hard rules, Motion (subtle only), Pre-flight, Required components, Source, Web Prototype — Editorial Minimalism

### Community 1753 - "Community 1753"
Cohesion: 0.25
Nodes (7): Banned, Hard rules, Motion, Pre-flight, Required components, Source, Web Prototype — Soft Premium

### Community 1754 - "Community 1754"
Cohesion: 0.57
Nodes (5): classifyPaste(), isJsonDocumentText(), isStructuredPlainText(), PasteClassificationInput, PasteMode

### Community 1755 - "Community 1755"
Cohesion: 0.25
Nodes (8): awaiting_lead, awaiting_title, failed_title, step_generate, step_prepare, step_understand, title, generation

### Community 1756 - "Community 1756"
Cohesion: 0.52
Nodes (5): checkPathOnOS(), installOnPosix(), installOnWindows(), runInstall(), Command

### Community 1757 - "Community 1757"
Cohesion: 0.57
Nodes (5): BlockMathExtension, BlockMathView(), InlineMathExtension, InlineMathView(), renderMath()

### Community 1758 - "Community 1758"
Cohesion: 0.25
Nodes (8): attachment, close, download_failed, preview, preview_failed, preview_loading, preview_too_large, preview_unsupported

### Community 1759 - "Community 1759"
Cohesion: 0.29
Nodes (7): Autopilot Commands, Create / Update / Delete, Get Autopilot Details, List Autopilots, Manual Trigger, Run History, Schedule Triggers

### Community 1760 - "Community 1760"
Cohesion: 0.29
Nodes (6): Codex Interactive Capability Map, Interaction Requirements, Output Contract, Use When, Visual Direction, Workflow

### Community 1761 - "Community 1761"
Cohesion: 0.43
Nodes (4): pdc_push.sh script, clean_local_docker(), compose_cmd(), setup_compose_env()

### Community 1762 - "Community 1762"
Cohesion: 0.33
Nodes (6): custom, daily, hourly, weekdays, weekly, frequencies

### Community 1763 - "Community 1763"
Cohesion: 0.52
Nodes (5): CraftForMode(), CraftForRequires(), loadPromptFile(), BuildPrompt(), PromptInput

### Community 1764 - "Community 1764"
Cohesion: 0.29
Nodes (6): Assign from the CLI, Assign from the UI, Next, Reassign or unassign, What happens after assignment, Why only one active `task` per agent per issue

### Community 1765 - "Community 1765"
Cohesion: 0.29
Nodes (6): 下一步, 为什么同一 issue 同时只能一个活跃 `task`, 分配之后会发生什么, 在界面里分配, 换分配人或取消分配, 用 CLI 分配

### Community 1766 - "Community 1766"
Cohesion: 0.29
Nodes (6): Archive a conversation, How multi-turn context is preserved, Next, Start a conversation, What an agent can and cannot do in chat, Why not just @-mention the agent?

### Community 1767 - "Community 1767"
Cohesion: 0.29
Nodes (6): 下一步, 为什么不用 @ 智能体就够, 多轮对话怎么保留上下文, 开始一次对话, 归档对话, 智能体在对话里能做什么、不能做什么

### Community 1768 - "Community 1768"
Cohesion: 0.29
Nodes (6): Desktop or web — which to pick, Do I still need the standalone CLI and daemon?, Downloading and installing, How Desktop auto-updates, Multiple tabs: what happens when you switch workspaces, Next steps

### Community 1769 - "Community 1769"
Cohesion: 0.29
Nodes (6): Desktop 和 Web 该用哪个, Desktop 怎么自动更新, 下一步, 多 tab：工作区之间切换怎么表现, 怎么下载安装, 还要单独装 CLI 和守护进程吗

### Community 1770 - "Community 1770"
Cohesion: 0.29
Nodes (6): Agents never receive notifications, `@all` notifies the entire workspace, Next, Sub-issue status changes bubble up to the parent, Subscription rules, What shows up in your inbox

### Community 1771 - "Community 1771"
Cohesion: 0.29
Nodes (6): `@all` 会通知整个工作区, 下一步, 子 issue 状态变化会冒泡到父 issue, 收件箱里会收到什么, 智能体永远不会收到通知, 订阅规则

### Community 1772 - "Community 1772"
Cohesion: 0.29
Nodes (6): Agents @-mentioning themselves does not loop, `@all` does not trigger any agent, How it differs from assignment, Mention an agent in a comment, Next, What happens when you @ multiple agents

### Community 1773 - "Community 1773"
Cohesion: 0.29
Nodes (6): @all 不会触发任何智能体, 下一步, 和分配的差别, 在评论里 @ 一个智能体, @ 多个智能体会怎样, 智能体自己 @ 自己不会死循环

### Community 1774 - "Community 1774"
Cohesion: 0.29
Nodes (7): 3.10 成员、邀请与权限, UI, 产品里的位置, 对应表, 角色体系, 邀请接受的桌面特殊处理, 邀请流程

### Community 1775 - "Community 1775"
Cohesion: 0.29
Nodes (7): 3.9 Inbox 收件箱与通知, UI, 产品里的位置, 对应表, 数据形态, 订阅机制（自动）, 通知触发场景

### Community 1776 - "Community 1776"
Cohesion: 0.29
Nodes (6): Attaching resources, Deleting a project, How projects relate to issues, Next, Pinning to the sidebar, Project lead

### Community 1777 - "Community 1777"
Cohesion: 0.29
Nodes (6): pin 到侧边栏, 下一步, 关联 resources, 删除项目, 项目和 issue 的关系, 项目负责人

### Community 1778 - "Community 1778"
Cohesion: 0.29
Nodes (6): Attaching to an agent, Importing a skill, Next steps, Safety of third-party skills, Skills vs. MCP, Workspace skills vs. local skills

### Community 1779 - "Community 1779"
Cohesion: 0.29
Nodes (6): Skill 和 MCP 的区别, 下一步, 导入 Skill, 工作区 Skill 和本机 Skill, 挂到智能体, 第三方 Skill 的安全

### Community 1780 - "Community 1780"
Cohesion: 0.29
Nodes (7): toolbar, comment, edit, mark, screenshot, zoom_in, zoom_out

### Community 1781 - "Community 1781"
Cohesion: 0.29
Nodes (7): issue_preview, continue_in_design_studio, empty, frame_title, loading, open_new_tab, section

### Community 1782 - "Community 1782"
Cohesion: 0.29
Nodes (7): columns, activity_7d, agent, runs, runtime, status, workload

### Community 1783 - "Community 1783"
Cohesion: 0.29
Nodes (6): GitHub Dashboard Skill, Resource map, Self-check, Visual rules, When to use this skill, Workflow

### Community 1784 - "Community 1784"
Cohesion: 0.67
Nodes (5): clearFeedbackForTestUser(), TestCreateFeedbackEmptyMessage(), TestCreateFeedbackHappyPath(), TestCreateFeedbackRateLimit(), T

### Community 1785 - "Community 1785"
Cohesion: 0.76
Nodes (5): createTestIssue(), deleteTestIssue(), TestBatchUpdateNoMutationReturnsZero(), TestBatchUpdateValidUpdatesPersistAndCount(), T

### Community 1786 - "Community 1786"
Cohesion: 0.71
Nodes (5): insertHandlerTestSkill(), TestGetSkill_IncludesContent(), TestListAgentSkills_OmitsContent(), TestListSkills_OmitsContent(), T

### Community 1787 - "Community 1787"
Cohesion: 0.29
Nodes (6): Current status, Live Artifact Skill, Required files, Resource map, When to use this skill, Workflow

### Community 1788 - "Community 1788"
Cohesion: 0.29
Nodes (6): Connectors, `connectors.json` schema, Fallback behavior, Provider-specific cheat sheet, Resolution order (what the daemon does), Wiring inside `index.html`

### Community 1789 - "Community 1789"
Cohesion: 0.29
Nodes (6): Audit storage, Commit behavior, Connector-backed refresh, Refresh Contract Reference, Refreshable source metadata, Source execution model

### Community 1790 - "Community 1790"
Cohesion: 0.43
Nodes (5): T, ParseScanPorts(), ScanPortsFromEnv(), TestParseScanPorts(), TestScanPortsFromEnvDefault()

### Community 1791 - "Community 1791"
Cohesion: 0.33
Nodes (7): Task Cancelled State — User Manually Cancelled, Task Completed State — Finished Successfully, Output Written Back, Task Dispatched State — Daemon Claimed and Starting AI Tool, Task Failed State — Error or Timeout, May Auto-retry, Task Queued State — Waiting for Daemon Pickup, Task Running State — AI Coding Tool Doing Work, Task State Machine — Queued, Dispatched, Running, Completed, Failed, Cancelled

### Community 1792 - "Community 1792"
Cohesion: 0.29
Nodes (7): no_matches, no_archived, no_filter_match, search_active, search_active_filtered, search_archived, title

### Community 1793 - "Community 1793"
Cohesion: 0.29
Nodes (7): custom, daily, hourly, no_days, weekdays, weekly, summary

### Community 1794 - "Community 1794"
Cohesion: 0.29
Nodes (7): toolbar, comment, edit, mark, screenshot, zoom_in, zoom_out

### Community 1795 - "Community 1795"
Cohesion: 0.29
Nodes (7): issue_preview, continue_in_design_studio, empty, frame_title, loading, open_new_tab, section

### Community 1796 - "Community 1796"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1797 - "Community 1797"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1798 - "Community 1798"
Cohesion: 0.33
Nodes (5): A · Classic dashboard (default), B · Kanban-flavored, C · KPI-only hero, Density & responsive notes (apply to all three), Layouts

### Community 1799 - "Community 1799"
Cohesion: 0.33
Nodes (5): P0 — must pass, P1 — should pass, P2 — nice to have, Anti-fake-device checklist, Mobile app checklist

### Community 1800 - "Community 1800"
Cohesion: 0.33
Nodes (5): P0 — must pass, P1 — should pass, P2 — nice to have, Simple deck checklist, Theme rhythm spot-check

### Community 1801 - "Community 1801"
Cohesion: 0.33
Nodes (5): P0 — must pass, P1 — should pass, P2 — nice to have, Anti-slop spot-check, Web prototype checklist

### Community 1802 - "Community 1802"
Cohesion: 0.33
Nodes (5): default_quality, design_system, $schema, skill, slots

### Community 1803 - "Community 1803"
Cohesion: 0.33
Nodes (6): App Stops but PostgreSQL Keeps Running, Check Which Database a Checkout Uses, List All Local Databases in Shared PostgreSQL, Missing Env File, Troubleshooting, Worktree Is Accidentally Using the Main Database

### Community 1804 - "Community 1804"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1805 - "Community 1805"
Cohesion: 0.60
Nodes (5): DesignParameter, odMeta, parseFrontmatter(), ParseOpenDesignMetadata(), parseYAMLStringList()

### Community 1806 - "Community 1806"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1807 - "Community 1807"
Cohesion: 0.33
Nodes (6): Bug：[BUG-001] <标题>, 回归测试, 复现步骤, 实际结果, 附件, 预期结果

### Community 1808 - "Community 1808"
Cohesion: 0.33
Nodes (6): Pipeline 多步骤任务, 一句话任务 → 创建 issue → 指派, 全部完成, 创建步骤, 工作流程, 推进下一步（被 auto-dispatch 时）

### Community 1809 - "Community 1809"
Cohesion: 0.33
Nodes (5): Four ways to get an agent working, Next steps, Runtimes: where it runs, and how many tools, The lifecycle of a task, The three core components

### Community 1810 - "Community 1810"
Cohesion: 0.33
Nodes (5): 一个任务从创建到完成会经历什么, 下一步, 系统的三个核心组件, 让智能体开工的四种方式, 运行时：在哪里跑，跑几家工具

### Community 1811 - "Community 1811"
Cohesion: 0.33
Nodes (5): Always at least one owner, Inviting a new member, Next, Permissions at a glance, Removing a member

### Community 1812 - "Community 1812"
Cohesion: 0.33
Nodes (5): 下一步, 权限概览, 移除成员, 至少保留一名 owner, 邀请新成员

### Community 1813 - "Community 1813"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1814 - "Community 1814"
Cohesion: 0.29
Nodes (7): label, tooltip, visibility, private, workspace, label, tooltip

### Community 1815 - "Community 1815"
Cohesion: 0.33
Nodes (6): 3.14 CLI 命令行工具, Agent / Skill / Autopilot / Project / Repo, Runtime, 产品里的位置, 工作区 / 议题, 配置 / 更新

### Community 1816 - "Community 1816"
Cohesion: 0.29
Nodes (7): profile_card, detail_link, owner_label, runtime_label, skills_label, unavailable, unknown_runtime

### Community 1817 - "Community 1817"
Cohesion: 0.33
Nodes (6): run, issue_linked, skipped_group, view_log, label, summary

### Community 1818 - "Community 1818"
Cohesion: 0.33
Nodes (6): queue, clear_all, remove, send_to_chat, sending, title

### Community 1819 - "Community 1819"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1820 - "Community 1820"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1821 - "Community 1821"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1822 - "Community 1822"
Cohesion: 0.33
Nodes (5): Agent Execution Model, Agents as Teammates, Multi-Workspace Support, Reusable Skills, Supported Agent Providers

### Community 1823 - "Community 1823"
Cohesion: 0.36
Nodes (6): NewDBCollector(), newDBDesc(), DBCollector, Desc, Metric, Pool

### Community 1824 - "Community 1824"
Cohesion: 0.60
Nodes (3): Handler, Request, ResponseWriter

### Community 1825 - "Community 1825"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1826 - "Community 1826"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1827 - "Community 1827"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1828 - "Community 1828"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1829 - "Community 1829"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1830 - "Community 1830"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1831 - "Community 1831"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1833 - "Community 1833"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1834 - "Community 1834"
Cohesion: 0.33
Nodes (5): Case Presentation Formats, Format 1: SOAP (Default), Format 2: Conference / Grand Rounds, Format 3: Brief Ward Rounds, Format Selection Guide

### Community 1835 - "Community 1835"
Cohesion: 0.33
Nodes (6): assets, craft, designSystem, skills, primary, context

### Community 1836 - "Community 1836"
Cohesion: 0.33
Nodes (5): Adapting the sample data safely, Output contract, Resource map, Social Media Matrix Tracker Template, Workflow

### Community 1837 - "Community 1837"
Cohesion: 0.29
Nodes (7): row, actions_aria, archived, fallback_runtime_cloud, fallback_runtime_local, no_description, you

### Community 1838 - "Community 1838"
Cohesion: 0.33
Nodes (6): queue, clear_all, remove, send_to_chat, sending, title

### Community 1839 - "Community 1839"
Cohesion: 0.29
Nodes (7): failed_title, open_button, opening_description, opening_title, prepare_failed, preparing, desktop_handoff

### Community 1840 - "Community 1840"
Cohesion: 0.33
Nodes (6): 1. AICortex 是什么, 一句话定位, 定位一句话版本, 支持的 Coding Agent, 解决的问题, 部署形态

### Community 1841 - "Community 1841"
Cohesion: 0.50
Nodes (3): init(), runRepoCheckout(), Command

### Community 1842 - "Community 1842"
Cohesion: 0.60
Nodes (3): initFxIn(), reinitFxIn(), stopFxIn()

### Community 1843 - "Community 1843"
Cohesion: 0.40
Nodes (4): How it differs from a human, Next steps, What an agent can do, Who can assign an agent

### Community 1844 - "Community 1844"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1845 - "Community 1845"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1846 - "Community 1846"
Cohesion: 0.40
Nodes (4): P0 — Must Pass Before Emitting Artifact, P1 — Should Pass, P2 — Nice to Have, Clinical Case Report — Quality Checklist

### Community 1847 - "Community 1847"
Cohesion: 0.40
Nodes (4): Checklist, P0, P1, P2

### Community 1848 - "Community 1848"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1849 - "Community 1849"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Presenter Mode (演讲者模式), When this card is picked

### Community 1850 - "Community 1850"
Cohesion: 0.40
Nodes (4): P0 — must pass, P1 — should pass, P2 — nice to have, Self-review checklist (IB pitch book)

### Community 1851 - "Community 1851"
Cohesion: 0.40
Nodes (4): Checklist, P0, P1, P2

### Community 1852 - "Community 1852"
Cohesion: 0.40
Nodes (4): P0 (must pass before emitting `<artifact>`), P2 (polish), P1 (quality bar), Trading Analysis Dashboard Template Checklist

### Community 1853 - "Community 1853"
Cohesion: 0.40
Nodes (4): P0 — Must pass before emitting `<artifact>`, Checklist — Waitlist Page, P1 — Should pass for quality submission, P2 — Nice-to-have polish

### Community 1854 - "Community 1854"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1855 - "Community 1855"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1856 - "Community 1856"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1857 - "Community 1857"
Cohesion: 0.40
Nodes (5): Authentication, Browser Login, Check Status, Logout, Token Login

### Community 1858 - "Community 1858"
Cohesion: 0.40
Nodes (5): Get Details, List Members, List Workspaces, Watch / Unwatch, Workspaces

### Community 1859 - "Community 1859"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1860 - "Community 1860"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1861 - "Community 1861"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1862 - "Community 1862"
Cohesion: 0.40
Nodes (5): Feature Worktree, Return to a Previously Configured Worktree, Stable Main Environment, Typical Flows, Validate Before Pushing

### Community 1863 - "Community 1863"
Cohesion: 0.40
Nodes (4): Assets, Motion & layout, Output rules, Structure

### Community 1864 - "Community 1864"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1865 - "Community 1865"
Cohesion: 0.70
Nodes (4): dev-win.sh script, _check_tool(), _find_in_windows_path(), _windows_path_try()

### Community 1868 - "Community 1868"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1869 - "Community 1869"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1870 - "Community 1870"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1871 - "Community 1871"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1872 - "Community 1872"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1873 - "Community 1873"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1874 - "Community 1874"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1875 - "Community 1875"
Cohesion: 0.40
Nodes (4): html-ppt-presenter-mode-reveal, bootstrap_deck, bundles, work_dir_hint

### Community 1876 - "Community 1876"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1877 - "Community 1877"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1878 - "Community 1878"
Cohesion: 0.40
Nodes (4): 下一步, 它和人不一样在哪, 智能体能做什么, 谁能把智能体分配出去

### Community 1879 - "Community 1879"
Cohesion: 0.40
Nodes (5): 1.1 Welcome — 👀 In review [v1], 1.2 How AICortex Works — ⬜ Not started [v1], 1.3 Quickstart (Cloud) — ⬜ Not started [v1], 1.4 Quickstart (Self-Host) — ⬜ Not started [v1], 板块 1：Welcome & Quickstart

### Community 1880 - "Community 1880"
Cohesion: 0.40
Nodes (5): 2.1 Workspaces — ⬜ Not started [v1], 2.2 Members & Roles — ⬜ Not started [v1], 2.3 Issues & Projects — ⬜ Not started [v1], 2.4 Comments — ⬜ Not started [v1], 板块 2：Workspace & Team

### Community 1881 - "Community 1881"
Cohesion: 0.40
Nodes (5): 5.1 Assigning Issues to Agents — ⬜ Not started [v1], 5.2 Mentioning Agents in Comments — ⬜ Not started [v1], 5.3 Chat — ⬜ Not started [v1], 5.4 Routines — ⬜ Not started [v1], 板块 5：Working with Agents

### Community 1882 - "Community 1882"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1883 - "Community 1883"
Cohesion: 0.40
Nodes (5): 3.11 搜索与命令面板, 产品里的位置, 全文搜索, 列表过滤, 命令面板（Cmd+K）

### Community 1884 - "Community 1884"
Cohesion: 0.40
Nodes (5): 3.8 Chat 对话, 产品里的位置, 功能, 和 Issue 评论的区别, 对应表

### Community 1885 - "Community 1885"
Cohesion: 0.33
Nodes (6): heading_dropdown, heading_1, heading_2, heading_3, normal_text, text

### Community 1886 - "Community 1886"
Cohesion: 0.40
Nodes (4): Creating a workspace, Deleting a workspace, Issue numbers, Next

### Community 1887 - "Community 1887"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1888 - "Community 1888"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1889 - "Community 1889"
Cohesion: 0.40
Nodes (5): address, more, refresh, viewport_label, chrome

### Community 1890 - "Community 1890"
Cohesion: 0.33
Nodes (6): cancel, confirm, deleting, description, title, delete_dialog

### Community 1891 - "Community 1891"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1892 - "Community 1892"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1893 - "Community 1893"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1894 - "Community 1894"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1895 - "Community 1895"
Cohesion: 0.40
Nodes (4): FlowAI Dashboard Skill, Output contract, Resource map, Workflow

### Community 1896 - "Community 1896"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1897 - "Community 1897"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1898 - "Community 1898"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1899 - "Community 1899"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1900 - "Community 1900"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1901 - "Community 1901"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1902 - "Community 1902"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1903 - "Community 1903"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1904 - "Community 1904"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1905 - "Community 1905"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1906 - "Community 1906"
Cohesion: 0.40
Nodes (4): 1. Set up and start the daemon, 2. Verify your runtime, 3. Create an agent, 4. Assign your first task

### Community 1907 - "Community 1907"
Cohesion: 0.60
Nodes (3): TestCreateAgent_RejectsDuplicateName(), TestListWorkspaceAgentTaskSnapshot(), T

### Community 1908 - "Community 1908"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1909 - "Community 1909"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1910 - "Community 1910"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Course Module, When this card is picked

### Community 1911 - "Community 1911"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1912 - "Community 1912"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 8 色极简方向键, When this card is picked

### Community 1913 - "Community 1913"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1914 - "Community 1914"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 暗底知识图谱, When this card is picked

### Community 1915 - "Community 1915"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1916 - "Community 1916"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 暗终端测评, When this card is picked

### Community 1917 - "Community 1917"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1918 - "Community 1918"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 奶油蓝图架构, When this card is picked

### Community 1919 - "Community 1919"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1920 - "Community 1920"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · GitHub 暗紫渐变, When this card is picked

### Community 1921 - "Community 1921"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1922 - "Community 1922"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Pitch Deck, When this card is picked

### Community 1923 - "Community 1923"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1924 - "Community 1924"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Product Launch, When this card is picked

### Community 1925 - "Community 1925"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1926 - "Community 1926"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1927 - "Community 1927"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1928 - "Community 1928"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1929 - "Community 1929"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1930 - "Community 1930"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Tech Sharing, When this card is picked

### Community 1931 - "Community 1931"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1932 - "Community 1932"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 红琥珀警示, When this card is picked

### Community 1933 - "Community 1933"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1934 - "Community 1934"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Weekly Report, When this card is picked

### Community 1935 - "Community 1935"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1936 - "Community 1936"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 柔和马卡龙慢生活, When this card is picked

### Community 1937 - "Community 1937"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1938 - "Community 1938"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 小红书 图文, When this card is picked

### Community 1939 - "Community 1939"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1940 - "Community 1940"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · 白底杂志风, When this card is picked

### Community 1941 - "Community 1941"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1942 - "Community 1942"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1943 - "Community 1943"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1944 - "Community 1944"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1945 - "Community 1945"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1946 - "Community 1946"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1947 - "Community 1947"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1948 - "Community 1948"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1949 - "Community 1949"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1950 - "Community 1950"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1951 - "Community 1951"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1952 - "Community 1952"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1953 - "Community 1953"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1954 - "Community 1954"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1955 - "Community 1955"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1956 - "Community 1956"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1957 - "Community 1957"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1958 - "Community 1958"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1959 - "Community 1959"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1960 - "Community 1960"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1961 - "Community 1961"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1962 - "Community 1962"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1963 - "Community 1963"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1964 - "Community 1964"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1965 - "Community 1965"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1966 - "Community 1966"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1967 - "Community 1967"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1968 - "Community 1968"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1969 - "Community 1969"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1970 - "Community 1970"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1971 - "Community 1971"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1972 - "Community 1972"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1973 - "Community 1973"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1974 - "Community 1974"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1975 - "Community 1975"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1976 - "Community 1976"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1977 - "Community 1977"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1978 - "Community 1978"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1979 - "Community 1979"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1980 - "Community 1980"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1981 - "Community 1981"
Cohesion: 0.29
Nodes (7): label, tooltip, visibility, private, workspace, label, tooltip

### Community 1982 - "Community 1982"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1983 - "Community 1983"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1984 - "Community 1984"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1985 - "Community 1985"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1986 - "Community 1986"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1987 - "Community 1987"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1988 - "Community 1988"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1989 - "Community 1989"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1990 - "Community 1990"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1991 - "Community 1991"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 1992 - "Community 1992"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1993 - "Community 1993"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1994 - "Community 1994"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1995 - "Community 1995"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1996 - "Community 1996"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1997 - "Community 1997"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1998 - "Community 1998"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 1999 - "Community 1999"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2000 - "Community 2000"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2001 - "Community 2001"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2002 - "Community 2002"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2003 - "Community 2003"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2004 - "Community 2004"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2005 - "Community 2005"
Cohesion: 0.40
Nodes (4): Apache-2.0 notice handling, Attribution, What is adapted, What is original to Open Design

### Community 2006 - "Community 2006"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2007 - "Community 2007"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2008 - "Community 2008"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2009 - "Community 2009"
Cohesion: 0.40
Nodes (4): Attribution, How to author the deck, HTML PPT · Presenter Mode (演讲者模式), When this card is picked

### Community 2010 - "Community 2010"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2011 - "Community 2011"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2012 - "Community 2012"
Cohesion: 0.40
Nodes (4): Deck / presenter mode (brief mentions 演讲主题、时长、听众), Discovery, Example (brief: "app"), Format (required)

### Community 2013 - "Community 2013"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2014 - "Community 2014"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2015 - "Community 2015"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2016 - "Community 2016"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2017 - "Community 2017"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2018 - "Community 2018"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2019 - "Community 2019"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2020 - "Community 2020"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2021 - "Community 2021"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2022 - "Community 2022"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2023 - "Community 2023"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2024 - "Community 2024"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2025 - "Community 2025"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2026 - "Community 2026"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2027 - "Community 2027"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2028 - "Community 2028"
Cohesion: 0.40
Nodes (4): Output contract, Resource map, Trading Analysis Dashboard Template, Workflow

### Community 2029 - "Community 2029"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2030 - "Community 2030"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2031 - "Community 2031"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2032 - "Community 2032"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2033 - "Community 2033"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2034 - "Community 2034"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2035 - "Community 2035"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2036 - "Community 2036"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2037 - "Community 2037"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2038 - "Community 2038"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2039 - "Community 2039"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2040 - "Community 2040"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2041 - "Community 2041"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2042 - "Community 2042"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2043 - "Community 2043"
Cohesion: 0.40
Nodes (5): assets, designSystem, skills, primary, context

### Community 2044 - "Community 2044"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2045 - "Community 2045"
Cohesion: 0.40
Nodes (5): useCase, en, zh-CN, exampleOutputs, query

### Community 2046 - "Community 2046"
Cohesion: 0.40
Nodes (5): address, more, refresh, viewport_label, chrome

### Community 2047 - "Community 2047"
Cohesion: 0.29
Nodes (7): image, copy_link, copy_link_failed, delete, download, link_copied, view

### Community 2048 - "Community 2048"
Cohesion: 0.40
Nodes (5): add_child, description, title, toast_failed, toast_success

### Community 2049 - "Community 2049"
Cohesion: 0.33
Nodes (6): mention, all_members, group_issues, group_users, no_results, searching

### Community 2050 - "Community 2050"
Cohesion: 0.33
Nodes (6): priority, high, low, medium, none, urgent

### Community 2051 - "Community 2051"
Cohesion: 0.50
Nodes (3): Blog Post Skill, Output contract, Workflow

### Community 2052 - "Community 2052"
Cohesion: 0.50
Nodes (3): Blog Post Skill, Output contract, Workflow

### Community 2053 - "Community 2053"
Cohesion: 0.50
Nodes (4): Build from Source, Homebrew (macOS/Linux), Installation, Update

### Community 2054 - "Community 2054"
Cohesion: 0.50
Nodes (3): Dashboard Skill, Output contract, Workflow

### Community 2055 - "Community 2055"
Cohesion: 0.50
Nodes (3): Dating Web Skill, Output contract, Workflow

### Community 2056 - "Community 2056"
Cohesion: 0.33
Nodes (6): detail, archive, edit_advanced, empty, original_input, select_prompt

### Community 2057 - "Community 2057"
Cohesion: 0.50
Nodes (3): Digital E-Guide Skill, Output contract, Workflow

### Community 2058 - "Community 2058"
Cohesion: 0.50
Nodes (4): 3.1 What is an Agent — ⬜ Not started [v1], 3.2 Creating & Configuring Agents — ⬜ Not started [v1], 3.3 Skills — ⬜ Not started [v1], 板块 3：Agents

### Community 2059 - "Community 2059"
Cohesion: 0.50
Nodes (4): 4.1 Daemon & Runtimes — ⬜ Not started [v1], 4.2 Tasks — ⬜ Not started [v1], 4.3 Providers Matrix — ⬜ Not started [v1], 板块 4：How Agents Run

### Community 2060 - "Community 2060"
Cohesion: 0.50
Nodes (4): 7.1 Environment Variables — ⬜ Not started [v1], 7.2 Authentication Setup — ⬜ Not started [v1], 7.3 Troubleshooting — ⬜ Not started [v1], 板块 7：Administration

### Community 2061 - "Community 2061"
Cohesion: 0.50
Nodes (4): 8.1 CLI Cheatsheet — ⬜ Not started [v1], 8.2 Authentication & Tokens — ⬜ Not started [v1], 8.3 Desktop App — ⬜ Not started [v1], 板块 8：Reference

### Community 2062 - "Community 2062"
Cohesion: 0.50
Nodes (4): V1（25 篇，第一次 ship）, V1 篇目清单, V2（24 篇 Pending）, 二、版本规划

### Community 2063 - "Community 2063"
Cohesion: 0.50
Nodes (3): Next steps, Three ways to use AICortex, Where agents run

### Community 2064 - "Community 2064"
Cohesion: 0.50
Nodes (3): 三种使用方式, 下一步, 智能体在哪里运行

### Community 2065 - "Community 2065"
Cohesion: 0.50
Nodes (3): Docs Page Skill, Output contract, Workflow

### Community 2066 - "Community 2066"
Cohesion: 0.39
Nodes (4): decideVersionAction(), base, VersionAction, VersionCheckHealth

### Community 2067 - "Community 2067"
Cohesion: 0.50
Nodes (3): Email Marketing Skill, Output contract, Workflow

### Community 2068 - "Community 2068"
Cohesion: 0.24
Nodes (16): CodeRange, collectLinkifyMatches(), DetectedLink, detectLinks(), findCodeRanges(), findInlineLinkEnd(), findMarkdownLinkRanges(), findMatchingBracket() (+8 more)

### Community 2069 - "Community 2069"
Cohesion: 0.50
Nodes (3): Engineering Runbook Skill, Output contract, Workflow

### Community 2070 - "Community 2070"
Cohesion: 0.50
Nodes (3): Finance Report Skill, Output contract, Workflow

### Community 2071 - "Community 2071"
Cohesion: 0.50
Nodes (3): Gamified App Skill, Output contract, Workflow

### Community 2073 - "Community 2073"
Cohesion: 0.67
Nodes (3): issueArtifactReportItem, issueArtifactReportRequest, issueArtifactResponse

### Community 2075 - "Community 2075"
Cohesion: 0.40
Nodes (5): time, days, hours, just_now, minutes

### Community 2077 - "Community 2077"
Cohesion: 0.50
Nodes (3): HR Onboarding Skill, Output contract, Workflow

### Community 2078 - "Community 2078"
Cohesion: 0.50
Nodes (3): Invoice Skill, Output contract, Workflow

### Community 2079 - "Community 2079"
Cohesion: 0.50
Nodes (4): assets, craft, skills, context

### Community 2080 - "Community 2080"
Cohesion: 0.50
Nodes (4): assets, craft, skills, context

### Community 2081 - "Community 2081"
Cohesion: 0.50
Nodes (3): Kanban Board Skill, Output contract, Workflow

### Community 2082 - "Community 2082"
Cohesion: 0.50
Nodes (3): Magazine Poster Skill, Output contract, Workflow

### Community 2083 - "Community 2083"
Cohesion: 0.50
Nodes (3): Meeting Notes Skill, Output contract, Workflow

### Community 2084 - "Community 2084"
Cohesion: 0.50
Nodes (3): Mobile Onboarding Skill, Output contract, Workflow

### Community 2085 - "Community 2085"
Cohesion: 0.50
Nodes (3): Motion Frames Skill, Output contract, Workflow

### Community 2086 - "Community 2086"
Cohesion: 0.50
Nodes (3): Output contract, Product Spec Skill, Workflow

### Community 2087 - "Community 2087"
Cohesion: 0.50
Nodes (3): Output contract, Pricing Page Skill, Workflow

### Community 2088 - "Community 2088"
Cohesion: 0.50
Nodes (3): Compliance note (discussion materials), External-distribution gate, Required disclosure blocks

### Community 2089 - "Community 2089"
Cohesion: 0.40
Nodes (5): menu, archive_all, archive_all_read, archive_completed, mark_all_read

### Community 2090 - "Community 2090"
Cohesion: 0.50
Nodes (3): Output contract, Social Carousel Skill, Workflow

### Community 2091 - "Community 2091"
Cohesion: 0.50
Nodes (3): Output contract, Social Media Dashboard Skill, Workflow

### Community 2092 - "Community 2092"
Cohesion: 0.50
Nodes (3): Output contract, Sprite Animation Skill, Workflow

### Community 2093 - "Community 2093"
Cohesion: 0.50
Nodes (3): Output contract, Team OKRs Skill, Workflow

### Community 2094 - "Community 2094"
Cohesion: 0.50
Nodes (4): dependsOn, inputs, outputs, build

### Community 2095 - "Community 2095"
Cohesion: 0.50
Nodes (3): Output contract, Weekly Update Deck Skill, Workflow

### Community 2096 - "Community 2096"
Cohesion: 0.50
Nodes (3): Output contract, Wireframe Sketch Skill, Workflow

### Community 2097 - "Community 2097"
Cohesion: 0.50
Nodes (4): dismiss, no_targets, pick_target, hint

### Community 2098 - "Community 2098"
Cohesion: 0.33
Nodes (6): heading_dropdown, heading_1, heading_2, heading_3, normal_text, text

### Community 2099 - "Community 2099"
Cohesion: 0.67
Nodes (4): extractThreadID(), TestExtractThreadID(), TestExtractThreadIDMissing(), RawMessage

### Community 2100 - "Community 2100"
Cohesion: 0.50
Nodes (4): 3.13 设置与个人资料, My Account 标签, Workspace 标签, 产品里的位置

### Community 2104 - "Community 2104"
Cohesion: 0.67
Nodes (3): preview, entry, type

### Community 2107 - "Community 2107"
Cohesion: 0.67
Nodes (3): preview, entry, type

### Community 2108 - "Community 2108"
Cohesion: 0.67
Nodes (3): Local Skills, Two-Layer Skill System, Workspace Skills

### Community 2109 - "Community 2109"
Cohesion: 0.67
Nodes (3): assets, skills, context

### Community 2110 - "Community 2110"
Cohesion: 0.67
Nodes (3): preview, entry, type

### Community 2111 - "Community 2111"
Cohesion: 0.33
Nodes (6): mention, all_members, group_issues, group_users, no_results, searching

### Community 2113 - "Community 2113"
Cohesion: 0.50
Nodes (4): sub_issue, create_failed, created, tooltip

### Community 2114 - "Community 2114"
Cohesion: 0.33
Nodes (6): status, cancelled, completed, in_progress, paused, planned

### Community 2115 - "Community 2115"
Cohesion: 0.67
Nodes (3): cache, persistent, dev

### Community 2223 - "Community 2223"
Cohesion: 0.67
Nodes (3): description, title, add_subissue_picker

### Community 2224 - "Community 2224"
Cohesion: 0.67
Nodes (3): set_parent_picker, description, title

### Community 2225 - "Community 2225"
Cohesion: 0.40
Nodes (5): save_bar, discard, save, saving, unsaved

### Community 2226 - "Community 2226"
Cohesion: 0.67
Nodes (3): entries, promptChars, skills

### Community 2227 - "Community 2227"
Cohesion: 0.33
Nodes (6): T, Mention, HasMentionAll(), ParseMentions(), TestHasMentionAll(), TestParseMentions()

### Community 2228 - "Community 2228"
Cohesion: 0.67
Nodes (3): TestDefaultTerminalShellUnixFallsBackToSh(), TestDefaultTerminalShellUnixUsesSHELL(), T

### Community 2229 - "Community 2229"
Cohesion: 0.34
Nodes (11): LandingBackdrop(), LandingHero(), ProductImage(), ClaudeCodeLogo(), CodexLogo(), GeminiCliLogo(), heroButtonClassName(), ImageIcon() (+3 more)

### Community 2230 - "Community 2230"
Cohesion: 0.22
Nodes (9): deleteBtn, I18nWrapper(), input, onConfirm, onOpenChange, render(), { rerender }, TEST_RESOURCES (+1 more)

### Community 2231 - "Community 2231"
Cohesion: 0.42
Nodes (8): testCmd(), TestLoginTokenFlagParsing(), TestLoginTokenFlagWiring(), TestNormalizeAPIBaseURL(), TestResolveAppURL(), TestResolveCallbackBinding(), Command, T

### Community 2233 - "Community 2233"
Cohesion: 0.67
Nodes (5): ExtractVersion(), Files(), LatestVersion(), ResolveDir(), searchRoots()

### Community 2234 - "Community 2234"
Cohesion: 0.67
Nodes (3): clawhubLatestVersion, clawhubSkill, clawhubGetSkillResponse

### Community 2238 - "Community 2238"
Cohesion: 0.29
Nodes (7): workdir, cli_all_permissions_description, cli_all_permissions_label, pinned_description, pinned_label, section_header, toast_update_failed

### Community 2240 - "Community 2240"
Cohesion: 0.50
Nodes (3): Agent Skills, Quick Start, Task Assignment

### Community 2241 - "Community 2241"
Cohesion: 0.50
Nodes (4): export, failed, loading, success

### Community 2242 - "Community 2242"
Cohesion: 0.50
Nodes (3): project_id, project_title, resources

### Community 2245 - "Community 2245"
Cohesion: 0.50
Nodes (4): viewport, desktop, mobile, tablet

### Community 2246 - "Community 2246"
Cohesion: 0.67
Nodes (3): requestShaping, authMode, thinking

### Community 2249 - "Community 2249"
Cohesion: 0.50
Nodes (4): sub_issue, create_failed, created, tooltip

### Community 2250 - "Community 2250"
Cohesion: 0.67
Nodes (3): Handler, Request, ResponseWriter

### Community 2254 - "Community 2254"
Cohesion: 0.29
Nodes (7): scope, agents_description, agents_label, all_description, all_label, members_description, members_label

## Knowledge Gaps
- **20857 isolated node(s):** `project_id`, `project_title`, `resources`, `type`, `url` (+20852 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **113 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `FS` connect `Community 1648` to `Community 707`, `Community 68`, `Community 335`, `Community 404`, `Community 52`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `readFile()` connect `OpenClaw Runtime Args` to `Community 768`, `Community 522`, `Community 523`, `Comments & Timeline`, `Agent Query Key Factories`, `Community 404`, `Community 1684`, `Community 149`, `Community 150`, `Community 28`, `Community 43`, `Community 304`, `Community 565`, `Community 64`, `Community 66`, `Community 67`, `Community 470`, `Community 214`, `Community 1239`, `Community 1756`, `Community 478`, `Community 1631`, `Community 224`, `Community 97`, `Community 492`, `Community 625`, `Community 508`, `Community 125`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `loadFromFS()` connect `Community 404` to `Community 1648`, `OpenClaw Runtime Args`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Are the 154 inferred relationships involving `newRequest()` (e.g. with `requestDaemonShutdown()` and `FetchLatestRelease()`) actually correct?**
  _`newRequest()` has 154 INFERRED edges - model-reasoned connections that need verification._
- **What connects `project_id`, `project_title`, `resources` to the rest of the system?**
  _20864 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `API Error Handling & Client Setup` be split into smaller, more focused modules?**
  _Cohesion score 0.044444444444444446 - nodes in this community are weakly interconnected._
- **Should `Virtual Office Presence System` be split into smaller, more focused modules?**
  _Cohesion score 0.04264960682393709 - nodes in this community are weakly interconnected._