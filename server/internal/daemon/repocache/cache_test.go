...
// for the AICortex Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: aicortex-agent <github@aicortex.ai>"
...
// path: a bare cache that already carries the AICortex prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: aicortex-agent") {
...
// prepare-commit-msg hook that does NOT include the aicortexHookMarker
// sentinel — only the older `# Installed by the AICortex daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no aicortexHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: aicortex-agent") {
...
// without the AICortex marker (e.g. one a user added manually) must be left
