...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/aicortex_workspaces/.repos)
	logger *slog.Logger
...
// aicortexHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const aicortexHookMarker = "# aicortex:prepare-commit-msg:co-authored-by"
...
// treats a hook as AICortex-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before aicortexHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	aicortexHookMarker,
	"# Installed by the AICortex daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the AICortex Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# aicortex:prepare-commit-msg:co-authored-by
# AICortex: add Co-authored-by trailer for the AICortex Agent.
# Installed by the AICortex daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: aicortex-agent <github@aicortex.ai>"
...
// a Co-authored-by trailer for the AICortex Agent. The hook is installed in the
...
// installed by the AICortex daemon (current or any previously released
