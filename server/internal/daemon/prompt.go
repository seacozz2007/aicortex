...
func BuildPrompt(task Task, provider string) string {
	...
	b.WriteString("You are running as a local coding agent for a AICortex workspace.\n\n")
	fmt.Fprintf(&b, "Your assigned issue ID is: %s\n\n", task.IssueID)
	fmt.Fprintf(&b, "Start by running `aicortex issue get %s --output json` to understand your task, then complete it.\n", task.IssueID)
	fmt.Fprintf(&b, "If you need comment history, `aicortex issue comment list %s --output json` returns all comments for the issue (server caps at 2000). Pass `--since <RFC3339>` to fetch only comments newer than a known cursor.\n", task.IssueID)
	return b.String()
}

// buildQuickCreatePrompt constructs a prompt for quick-create tasks. The
// user typed a single natural-language sentence in the create-issue modal;
// the agent's job is to translate it into one `aicortex issue create` CLI
...
// call `aicortex issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a AICortex workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `aicortex issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `aicortex workspace members --output json`, `aicortex agent list --output json`, and `aicortex squad list --output json` and find the matching entity by display name. Squads are first-class assignees too — a squad name (e.g. \"Super Human\") routes work to the squad leader, who then delegates. On a clean unambiguous match, prefer `--assignee-id <uuid>` using the `user_id` (member) or `id` (agent or squad) from that JSON — UUID matching is exact and robust to name collisions in workspaces with overlapping names. `--assignee <name>` (fuzzy) is acceptable as a fallback when names are unambiguous. On no match or ambiguous match, do NOT pass either flag — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	...
	b.WriteString("- Run exactly one `aicortex issue create --output json` invocation. Do not retry for any reason — even on non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	...
	b.WriteString("- Do NOT call `aicortex issue get` or `aicortex issue comment add` — there is no issue to query or comment on.\n")
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if task.Agent != nil && strings.Contains(task.Agent.Instructions, "## Squad Operating Protocol") {
			fmt.Fprintf(&b, "⚠️ **Squad leader no_action rule:** If you decide no action is needed, call `aicortex squad activity %s no_action --reason \"...\"` and EXIT. DO NOT post any comment — not even one that says \"no action needed\" or \"exiting silently\". The squad activity call records your decision; a comment is redundant noise.\n\n", task.IssueID)
		}
	}
	fmt.Fprintf(&b, "Start by running `aicortex issue get %s --output json` to understand your task, then decide how to proceed.\n\n", task.IssueID)
	fmt.Fprintf(&b, "If you need comment history, `aicortex issue comment list %s --output json` returns all comments for the issue (server caps at 2000). Pass `--since <RFC3339>` to fetch only comments newer than a known cursor.\n\n", task.IssueID)
...
func buildChatPrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a chat assistant for a AICortex workspace.\n")
	...
	// have expired. `aicortex attachment download <id>` re-signs at click
	...
	if len(task.ChatMessageAttachments) > 0 {
		...
		b.WriteString("Use `aicortex attachment download <id>` to fetch each file locally before referring to it.\n")
	}
	return b.String()
}
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `aicortex autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
		b.WriteString("Complete the instructions above.\n")
	}
	b.WriteString("Do not run `aicortex issue get`; this run does not have an issue ID.\n")
	return b.String()
}
