...
func TestPatternsFromEnv_DefaultsWhenUnset(t *testing.T) {
	t.Setenv("AICORTEX_GC_ARTIFACT_PATTERNS", "")
	defaults := []string{"node_modules", ".next", ".turbo"}
	got := patternsFromEnv("AICORTEX_GC_ARTIFACT_PATTERNS", defaults)
...
func TestPatternsFromEnv_DropsSeparatorBearingEntries(t *testing.T) {
	t.Setenv("AICORTEX_GC_ARTIFACT_PATTERNS", "node_modules, .next ,foo/bar, ../etc, ,target")
	got := patternsFromEnv("AICORTEX_GC_ARTIFACT_PATTERNS", nil)
...
// resolvable via the daemon's PATH (or pinned to an explicit AICORTEX_*_PATH),
...
func TestLoadConfig_SkipsLoginShellWhenLookPathSucceeds(t *testing.T) {
	...
	t.Setenv("AICORTEX_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
