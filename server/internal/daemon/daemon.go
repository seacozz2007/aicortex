...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'aicortex login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'aicortex login --profile %s'", d.cfg.Profile)
		}
...
// `aicortex repo checkout` accepts project-only URLs without an extra round
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by AICortex Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/aicortex)
...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/aicortex symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "aicortex")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "aicortex")
		} else {
...
// release. Used by pollLoop to expose AICORTEX_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// here would make AICORTEX_WORKSPACE_ID empty in the agent env, and the
	...
	// `aicortex repo checkout` would reject project-only URLs that aren't also
	...
	// via `aicortex repo checkout <url>`.
	...
	// can call the AICortex API and the local daemon (e.g. `aicortex repo checkout`).
	// AICORTEX_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	agentEnv := map[string]string{
		"AICORTEX_TOKEN":        d.client.Token(),
		"AICORTEX_SERVER_URL":   d.cfg.ServerBaseURL,
		"AICORTEX_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"AICORTEX_WORKSPACE_ID": task.WorkspaceID,
		"AICORTEX_AGENT_NAME":   agentName,
		"AICORTEX_AGENT_ID":     task.AgentID,
		"AICORTEX_TASK_ID":      task.ID,
		"AICORTEX_TASK_SLOT":    strconv.Itoa(slot),
	}
	if task.AutopilotRunID != "" {
		agentEnv["AICORTEX_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["AICORTEX_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the aicortex CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["AICORTEX_QUICK_CREATE_TASK_ID"] = task.ID
	}
	// Ensure the aicortex CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// aicortex binary so that `aicortex` commands in the agent always resolve.
	...
	// then the daemon-wide AICORTEX_<PROVIDER>_MODEL env var. If
	...
	// `aicortex issue status` / `aicortex issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "AICORTEX_") {
		return true
	}
	switch upper {
