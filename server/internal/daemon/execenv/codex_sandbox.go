...
// `no such host` errors when calling out (for example, `aicortex issue get`
// hitting the AICortex API). See upstream issue openai/codex#10390.
//
...
// actually reach the AICortex API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the AICortex CLI can reach the API.
...
// aicortexManagedBeginMarker / aicortexManagedEndMarker delimit the block the
...
const (
	aicortexManagedBeginMarker = "# BEGIN aicortex-managed (do not edit; regenerated by daemon)"
	aicortexManagedEndMarker   = "# END aicortex-managed"
)

// renderAICortexManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.aicortex]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertAICortexManagedBlock), avoids both traps.
func renderAICortexManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(aicortexManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(aicortexManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(aicortexManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(aicortexManagedEndMarker) + `\n*`)

// upsertAICortexManagedBlock returns the config content with the aicortex-managed
...
// `[permissions.aicortex]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderAICortexManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertAICortexManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderAICortexManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the aicortex-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertAICortexManagedBlock(existing, policy)
