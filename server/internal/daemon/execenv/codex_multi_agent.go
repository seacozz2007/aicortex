...
// subagents. The AICortex daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the AICortex runtime
...
//
// Users who explicitly want Codex native subagents inside a AICortex task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `AICORTEX_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// AICortexCodexMultiAgentEnv is the env var users can set to keep Codex
...
const AICortexCodexMultiAgentEnv = "AICORTEX_CODEX_MULTI_AGENT"

// aicortexMultiAgentBeginMarker / aicortexMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	aicortexMultiAgentBeginMarker = "# BEGIN aicortex-managed multi-agent (do not edit; regenerated by daemon)"
	aicortexMultiAgentEndMarker   = "# END aicortex-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(aicortexMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(aicortexMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(AICortexCodexMultiAgentEnv))
...
// renderAICortexMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderAICortexMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(aicortexMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(aicortexMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderAICortexMultiAgentBlock(true)
...
//
// When AICORTEX_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per AICORTEX_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderAICortexMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
