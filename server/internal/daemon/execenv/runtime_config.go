...
// about the AICortex runtime environment and available CLI tools.
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	var b strings.Builder

	b.WriteString("# AICortex Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the AICortex platform. Use the `aicortex` CLI to interact with the platform.\n\n")
	...
	b.WriteString("- `aicortex issue comment list <issue-id> [--since <RFC3339>] --output json` — List all comments on an issue (server caps at 2000 rows). Use `--since` for incremental polling.\n")
	b.WriteString("- `aicortex issue label list <issue-id> --output json` — List labels currently attached to an issue\n")
	b.WriteString("- `aicortex issue subscriber list <issue-id> --output json` — List members/agents subscribed to an issue\n")
	b.WriteString("- `aicortex label list --output json` — List all labels defined in the workspace (returns id + name + color)\n")
	b.WriteString("- `aicortex workspace get --output json` — Get workspace details and context\n")
	b.WriteString("- `aicortex workspace members [workspace-id] --output json` — List workspace members (user IDs, names, roles)\n")
	b.WriteString("- `aicortex agent list --output json` — List agents in workspace\n")
	b.WriteString("- `aicortex squad list --output json` — List squads in workspace (squads are first-class assignees — assigning an issue to a squad routes it to the squad leader, who then delegates)\n")
	b.WriteString("- `aicortex repo checkout <url> [--ref <branch-or-sha>]` — Check out a repository into the working directory (creates a git worktree with a dedicated branch; use `--ref` for review/QA on a specific branch, tag, or commit)\n")
	b.WriteString("- `aicortex issue runs <issue-id> [--full-id] --output json` — List all execution runs for an issue (status, timestamps, errors); table task IDs are short prefixes unless `--full-id` is set\n")
	b.WriteString("- `aicortex issue run-messages <task-id> [--issue <issue-id>] [--since <seq>] --output json` — List messages for a specific execution run; full task UUIDs work directly, copied short task prefixes must be scoped with `--issue <issue-id>`\n")
	b.WriteString("- `aicortex attachment download <id> [-o <dir>]` — Download an attachment file locally by ID\n")
	b.WriteString("- `aicortex autopilot list [--status X] [--full-id] [--output json]` — List autopilots (scheduled/triggered agent automations) in the workspace; copied short IDs are accepted by autopilot subcommands when unique\n")
	b.WriteString("- `aicortex autopilot get <id> --output json` — Get autopilot details including triggers\n")
	b.WriteString("- `aicortex autopilot runs <id> [--limit N] --output json` — List execution history for an autopilot\n")
	...
	b.WriteString("- `aicortex issue label add <issue-id> <label-id>` — Attach a label to an issue (look up the label id via `aicortex label list`)\n")
	b.WriteString("- `aicortex issue label remove <issue-id> <label-id>` — Detach a label from an issue\n")
	b.WriteString("- `aicortex issue subscriber add <issue-id> [--user <name>|--user-id <uuid>]` — Subscribe a member or agent to issue updates (defaults to the caller when neither flag is set; the two flags are mutually exclusive)\n")
	b.WriteString("- `aicortex issue subscriber remove <issue-id> [--user <name>|--user-id <uuid>]` — Unsubscribe a member or agent\n")
	// Available Commands lists `aicortex issue comment add` and the
	...
	b.WriteString("- `aicortex issue comment add <issue-id> [--content \"...\" | --content-stdin | --content-file <path>] [--parent <comment-id>] [--attachment <path>]` — Post a comment. Three input modes, pick whichever fits the content:\n")
	...
	b.WriteString("- `aicortex issue create` / `aicortex issue update` accept the same three modes for `--description`: `--description \"...\"`, `--description-stdin`, or `--description-file <path>`.\n")
	b.WriteString("- `aicortex issue comment delete <comment-id>` — Delete a comment\n")
	b.WriteString("- `aicortex label create --name \"...\" --color \"#hex\"` — Define a new workspace label (use this only when the label you need does not exist yet; reuse existing labels via `aicortex label list` first)\n")
	b.WriteString("- `aicortex autopilot create --title \"...\" --agent <name> --mode create_issue|run_only [--description \"...\"]` — Create an autopilot\n")
	...
	if provider == "codex" {
		...
		if runtimeGOOS == "windows" {
			b.WriteString("Codex often follows the per-turn reply command literally. On Windows, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`** — do NOT pipe via `--content-stdin`. PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to a native command, silently dropping non-ASCII characters as `?` before they reach `aicortex.exe`. Never use inline `--content` for agent-authored comments. ")
	...
	if len(ctx.Repos) > 0 {
		...
		b.WriteString("Use `aicortex repo checkout <url>` to check out a repository into your working directory. Add `--ref <branch-or-sha>` when you need an exact branch, tag, or commit.\n\n")
	...
	// The full structured payload is also available at .aicortex/project/resources.json
	...
	if ctx.ProjectID != "" || len(ctx.ProjectResources) > 0 {
		...
		if len(ctx.ProjectResources) > 0 {
			b.WriteString("Project resources (also written to `.aicortex/project/resources.json`):\n\n")
			for _, r := range ctx.ProjectResources {
				fmt.Fprintf(&b, "- %s\n", formatProjectResource(r))
			}
			b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
			b.WriteString("For `github_repo` resources, use `aicortex repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
		} else {
	...
	if ctx.ChatSessionID != "" {
		...
		b.WriteString("- You have full access to the `aicortex` CLI to look up issues, workspace info, members, agents, etc.\n")
		b.WriteString("- If asked about issues, use `aicortex issue list --output json` or `aicortex issue get <id> --output json`\n")
		b.WriteString("- If asked about the workspace, use `aicortex workspace get --output json`\n")
		b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
		b.WriteString("- If the task requires code changes, use `aicortex repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
		...
		b.WriteString("**This task was triggered by quick-create.** There is NO existing AICortex issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
		b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
		b.WriteString("- Run exactly one `aicortex issue create` invocation, then exit.\n")
		b.WriteString("- Do NOT call `aicortex issue get`, `aicortex issue status`, or `aicortex issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `aicortex issue create` succeeded.\n")
		...
		b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned AICortex issue for this run.\n\n")
		...
		if ctx.AutopilotID != "" {
			fmt.Fprintf(&b, "- Run `aicortex autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
		}
		b.WriteString("- Complete the autopilot instructions directly\n")
		b.WriteString("- Do not run `aicortex issue get`, `aicortex issue comment add`, or `aicortex issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue\n\n")
	} else if ctx.TriggerCommentID != "" {
		// Comment-triggered: focus on reading and replying
		b.WriteString("**This task was triggered by a NEW comment.** Your primary job is to respond to THIS specific comment, even if you have handled similar requests before in this session.\n\n")
		fmt.Fprintf(&b, "1. Run `aicortex issue get %s --output json` to understand the issue context\n", ctx.IssueID)
		fmt.Fprintf(&b, "2. Run `aicortex issue comment list %s --output json` to read the conversation (returns all comments, capped server-side at 2000)\n", ctx.IssueID)
		...
		fmt.Fprintf(&b, "1. Run `aicortex issue get %s --output json` to understand your task\n", ctx.IssueID)
		fmt.Fprintf(&b, "2. Run `aicortex issue comment list %s --output json` to read the full comment history (returns all comments, capped server-side at 2000) — this is mandatory, not optional. Earlier comments often carry context the issue body lacks (e.g. which repo to work in, the prior agent's findings, the reason the issue was reassigned to you). Skipping this step is the most common cause of agents acting on stale or incomplete instructions.\n", ctx.IssueID)
		fmt.Fprintf(&b, "3. Run `aicortex issue status %s in_progress`\n", ctx.IssueID)
		b.WriteString("4. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)\n")
		if ctx.IsSquadLeader {
			fmt.Fprintf(&b, "5. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `aicortex squad activity %s no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): `aicortex issue comment add %s --content \"...\"`. Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID, ctx.IssueID)
		} else {
			fmt.Fprintf(&b, "5. **Post your final results as a comment — this step is mandatory**: `aicortex issue comment add %s --content \"...\"`. Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID)
		}
		fmt.Fprintf(&b, "6. When done, run `aicortex issue status %s in_review`\n", ctx.IssueID)
		fmt.Fprintf(&b, "7. If blocked, run `aicortex issue status %s blocked` and post a comment explaining why\n\n", ctx.IssueID)
	}
	...
	b.WriteString("Use `aicortex issue list --output json` to look up issue IDs, and `aicortex workspace members --output json` for member IDs.\n\n")
	...
	b.WriteString("## Important: Always Use the `aicortex` CLI\n\n")
	b.WriteString("All interactions with AICortex platform resources — including issues, comments, attachments, images, files, and any other platform data — **must** go through the `aicortex` CLI. ")
	b.WriteString("Do NOT use `curl`, `wget`, or any other HTTP client to access AICortex URLs or APIs directly. ")
	b.WriteString("AICortex resource URLs require authenticated access that only the `aicortex` CLI can provide.\n\n")
	b.WriteString("If you need to perform an operation that is not covered by any existing `aicortex` command, ")
	...
	case ctx.QuickCreatePrompt != "":
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `aicortex issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `aicortex issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `aicortex issue create`. Use the created issue's `identifier` from JSON output when available; otherwise use its `id`. Do not assume any workspace issue prefix such as `MUL-`; workspaces can use custom prefixes.\n")
	...
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `aicortex issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `aicortex squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `aicortex issue comment add`.** The user does NOT see your terminal output, assistant chat text, or run logs — only comments on the issue. A task that finishes without a result comment is invisible to the user, even if the work itself was correct.\n\n")
		}
