...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/aicortex/aicortex","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".aicortex", "project", "resources.json")
	...
	for _, want := range []string{
		...
		"Agent UX 2026",
		"GitHub repo",
		"https://github.com/aicortex/aicortex",
		"default branch: `main`",
		".aicortex/project/resources.json",
	} {
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".aicortex", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"aicortex autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `aicortex issue get") {
...
func TestInjectRuntimeConfigAutopilotAdvertisesBothModes(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"aicortex autopilot create --title \"...\" --agent <name> --mode create_issue|run_only",
		"aicortex autopilot update <id>",
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "AICortex Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "aicortex issue get") {
		t.Error("should reference aicortex CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `aicortex issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"aicortex issue comment add issue-1",
				"mandatory",
			}
...
// as `?` before reaching `aicortex.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, aicortexManagedBeginMarker) || !strings.Contains(s, aicortexManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertAICortexManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), aicortexManagedBeginMarker); n != 1 {
...
`
	...
	beginIdx := strings.Index(s, aicortexManagedBeginMarker)
	endIdx := strings.Index(s, aicortexManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.aicortex]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.aicortex]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.aicortex content")
	}
	...
	if n := strings.Count(string(data2), aicortexManagedBeginMarker); n != 1 {
...
` + aicortexManagedBeginMarker + `
sandbox_mode = "workspace-write"

[sandbox_workspace_write]
network_access = true
` + aicortexManagedEndMarker + `
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.aicortex], got:\n%s", s)
	}
	if strings.Count(s, aicortexManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), aicortexManagedBeginMarker) {
		t.Error("reused config.toml missing aicortex-managed block")
	}
}
...
// inside a AICortex task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
