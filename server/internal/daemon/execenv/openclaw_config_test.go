...
// MUL-2219 DoD calls out: the directory AICortex writes skills into MUST be
...
func TestPrepareOpenclawSkillWriteMatchesScanPath(t *testing.T) {
	...
	for _, s := range skills {
		...
		if _, err := os.Stat(want); err != nil {
			t.Errorf("openclaw scan target %s missing — AICortex's write path and the openclaw scanner are out of sync: %v", want, err)
		}
	}
}
