...
// checked out on demand by the agent via `aicortex repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/aicortex_workspaces)
...
// on demand via `aicortex repo checkout <url>`.
