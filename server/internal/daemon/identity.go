...
// same machine shares one UUID stored at `~/.aicortex/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.aicortex/profiles/*/daemon.id` and returns every
...
// explicitly pins AICORTEX_DAEMON_ID to the hostname itself, there's nothing
