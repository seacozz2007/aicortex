...
const (
	...
	// stuck runs out of the operator's hair. Set AICORTEX_AGENT_IDLE_WATCHDOG=0
	// to disable.
...
// AICORTEX_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // aicortex CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/aicortex_workspaces)
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("AICORTEX_SERVER_URL", DefaultServerURL)
	...
	// pinned to an explicit AICORTEX_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned AICORTEX_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("AICORTEX_CLAUDE_PATH", "claude", "AICORTEX_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("AICORTEX_CODEX_PATH", "codex", "AICORTEX_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("AICORTEX_OPENCODE_PATH", "opencode", "AICORTEX_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("AICORTEX_OPENCLAW_PATH", "openclaw", "AICORTEX_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("AICORTEX_HERMES_PATH", "hermes", "AICORTEX_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("AICORTEX_GEMINI_PATH", "gemini", "AICORTEX_GEMINI_MODEL"); ok {
		agents["gemini"] = e
	}
	if e, ok := probe("AICORTEX_PI_PATH", "pi", "AICORTEX_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("AICORTEX_CURSOR_PATH", "cursor-agent", "AICORTEX_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("AICORTEX_COPILOT_PATH", "copilot", "AICORTEX_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("AICORTEX_KIMI_PATH", "kimi", "AICORTEX_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("AICORTEX_KIRO_PATH", "kiro-cli", "AICORTEX_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	if len(agents) == 0 {
	...
	claudeArgs, err := shellArgsFromEnv("AICORTEX_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("AICORTEX_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("AICORTEX_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("AICORTEX_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("AICORTEX_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("AICORTEX_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// AICORTEX_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("AICORTEX_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("AICORTEX_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via AICORTEX_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("AICORTEX_DAEMON_ID"))
	...
	// — e.g. ~/.aicortex/daemon.id (default) plus ~/.aicortex/profiles/<x>/
	...
	// the user explicitly pins AICORTEX_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("AICORTEX_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("AICORTEX_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/aicortex_workspaces or ~/aicortex_workspaces_<profile>)
	...
	gcInterval, err := durationFromEnv("AICORTEX_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("AICORTEX_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("AICORTEX_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("AICORTEX_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("AICORTEX_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: env > defaults > CLI override.
	autoUpdateEnabled := true
	if v := strings.TrimSpace(os.Getenv("AICORTEX_DAEMON_AUTO_UPDATE")); v == "false" || v == "0" {
		autoUpdateEnabled = false
	}
	if overrides.DisableAutoUpdate {
		autoUpdateEnabled = false
	}
	autoUpdateInterval, err := durationFromEnv("AICORTEX_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid AICORTEX_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("AICORTEX_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// AICORTEX_WORKSPACES_ROOT env > default ($HOME/aicortex_workspaces, or
// $HOME/aicortex_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `aicortex daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("AICORTEX_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set AICORTEX_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "aicortex_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "aicortex_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("AICORTEX_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// before any AICORTEX_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom AICORTEX_*_PATH values never reach this
