...
`, wsID, testUserID); err != nil {
	...
	targetEmail := fmt.Sprintf("revocation-%s@aicortex.ai", slug)
...
VALUES ($1, $2, 'Target Runtime', 'local', 'aicortex_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
VALUES ($1, $2, 'Other Runtime', 'local', 'aicortex_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
`, wsID, testUserID); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id
`, "Revocation No Runtimes Target", "revocation-no-runtimes@aicortex.ai").Scan(&targetUserID); err != nil {
		t.Fatalf("create target user: %v", err)
	}
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workspace WHERE id = $1`, wsID)
