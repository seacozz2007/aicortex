...
func privateAgentTestFixture(t *testing.T) (agentID, ownerID, memberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Private Agent Owner', 'private-agent-owner@aicortex.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'private-agent-owner@aicortex.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Member', 'plain-member@aicortex.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-member@aicortex.test'`)
	})
...
func TestMentionAgent_RejectsCrossWorkspaceAgentUUID(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Foreign Owner', 'cross-ws-foreign@aicortex.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'cross-ws-foreign@aicortex.test'`)
	})
	...
	// AICortex's mention format is markdown-linked: [@Name](mention://agent/<uuid>).
