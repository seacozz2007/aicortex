...
import (
	...
	"github.com/aicortex/aicortex/server/pkg/redact"
)
...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // aicortex CLI version
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					...
					for _, row := range rows {
						...
						// so `aicortex repo checkout` and the meta-skill render
	...
	if task.ChatSessionID.Valid {
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			// If the chat session is linked to a project, populate
			// ProjectResources so the daemon can resolve the work_dir from
			// the project's local_path or github_repo resources.
			...
			// the agent wants to `aicortex attachment download <id>` (URL is
	...
	if task.Context != nil && !task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid {
		...
		if json.Unmarshal(task.Context, &qc) == nil && qc.Type == service.QuickCreateContextType {
			...
			// the project, and `aicortex repo checkout` sees the project's
	...
	// as the only authority for AICORTEX_WORKSPACE_ID in the agent env. An
...
func (h *Handler) ReportTaskMessages(w http.ResponseWriter, r *http.Request) {
	...
	for _, msg := range req.Messages {
		...
		if workspaceID != "" {
			...
			if task.ChatSessionID.Valid {
				h.publishChat(protocol.EventTaskMessage, workspaceID, "system", "", uuidToString(task.ChatSessionID), payload)
			} else {
