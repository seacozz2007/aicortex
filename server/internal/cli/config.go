...
const defaultCLIConfigPath = ".aicortex/config.json"
...
// An empty profile returns the default path (~/.aicortex/config.json).
// A named profile returns ~/.aicortex/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".aicortex", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.aicortex/. A named profile returns ~/.aicortex/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".aicortex"), nil
	}
	return filepath.Join(home, ".aicortex", "profiles", profile), nil
}
