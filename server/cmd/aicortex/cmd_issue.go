...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	// Quick-create stamp: when the daemon sets AICORTEX_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `aicortex issue create` call
	...
	if taskID := os.Getenv("AICORTEX_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set AICORTEX_WORKSPACE_ID")
	}
...
// from `aicortex workspace members --output json`) should use this instead of
