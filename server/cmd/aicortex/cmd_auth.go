...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate aicortex with AICortex",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("AICORTEX_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'aicortex setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'aicortex login' to authenticate.")
		return nil
	}
...
<title>AICortex — Authenticated</title>
<style>
