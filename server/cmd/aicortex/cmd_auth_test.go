...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers AICORTEX_APP_URL", func(t *testing.T) {
		t.Setenv("AICORTEX_APP_URL", "http://localhost:14000")
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `aicortex login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
