...
import (
	...
	"github.com/aicortex/aicortex/server/internal/daemon"
	logger_pkg "github.com/aicortex/aicortex/server/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"overridable via AICORTEX_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: AICORTEX_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: AICORTEX_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: AICORTEX_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: AICORTEX_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: AICORTEX_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Per-task timeout (env: AICORTEX_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: AICORTEX_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: AICORTEX_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: AICORTEX_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: AICORTEX_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: AICORTEX_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: AICORTEX_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: AICORTEX_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: AICORTEX_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: AICORTEX_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Per-task timeout (env: AICORTEX_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: AICORTEX_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: AICORTEX_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: AICORTEX_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: AICORTEX_DAEMON_AUTO_UPDATE_INTERVAL)")
...
// Empty profile → ~/.aicortex/, named profile → ~/.aicortex/profiles/<name>/.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "AICORTEX_SERVER_URL", "")
	if serverURL == "" {
	...
	cfg.LaunchedBy = os.Getenv("AICORTEX_LAUNCHED_BY")
