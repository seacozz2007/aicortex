...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("AICORTEX_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("AICORTEX_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("AICORTEX_WORKSPACE_ID")
	agentName := os.Getenv("AICORTEX_AGENT_NAME")
	taskID := os.Getenv("AICORTEX_TASK_ID")
