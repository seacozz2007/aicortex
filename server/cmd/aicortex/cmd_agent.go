...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, AICORTEX_SERVER_URL env, or 'aicortex config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("AICORTEX_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("AICORTEX_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "AICORTEX_SERVER_URL", "")
	if val != "" {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'aicortex setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
// inside a daemon-managed agent task (daemon sets AICORTEX_AGENT_ID and
// AICORTEX_TASK_ID in the agent env). In that context the workspace must be
// provided explicitly by the daemon — falling back to user-global
// ~/.aicortex/config.json would let the agent act on whatever workspace the
...
func inAgentExecutionContext() bool {
	return os.Getenv("AICORTEX_AGENT_ID") != "" || os.Getenv("AICORTEX_TASK_ID") != ""
}

func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "AICORTEX_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inAgentExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: AICORTEX_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set AICORTEX_WORKSPACE_ID env, or run 'aicortex config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentCreateFromTemplate(cmd *cobra.Command, client *cli.APIClient, name, runtimeID, slug string) error {
	...
	// Matches the timeout used by `aicortex skill import` (cmd_skill.go).
