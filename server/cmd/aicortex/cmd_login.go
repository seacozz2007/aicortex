...
func tryResolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"AICORTEX_APP_URL", "FRONTEND_ORIGIN"} {
...
var loginCmd = &cobra.Command{
	...
	Long:  "Log in to AICortex, then automatically discover and watch all your workspaces.",
...
// "prompt me interactively", preserving the legacy `aicortex login --token`
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if err := autoWatchWorkspaces(cmd); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'aicortex workspace list' and 'aicortex workspace watch <id>' to set up manually.\n")
		return nil
	}

	fmt.Fprintf(os.Stderr, "\n→ Run 'aicortex daemon start' to start your local agent runtime.\n")
	return nil
}
...
func waitForWorkspaceCreation(cmd *cobra.Command, client *cli.APIClient) ([]struct {
	...
	if appURL == "" {
		...
		fmt.Fprintln(os.Stderr, "\nNo workspaces found.")
		fmt.Fprintln(os.Stderr, "Create a workspace in the web dashboard, then run 'aicortex login' again.")
		return nil, nil
	}
