...
func triggerContinueConversation(ctx context.Context, bus *events.Bus, queries *db.Queries, llmClient llm.LLMClient, forumState *forum.ForumAutoState, e events.Event) {
	...
	lastReplierID := forumState.PostAuthor(postID)
	// PostAuthor returns the original author; for thread context we need the last replier
	// Exclude the last replier to prevent self-loop
	excludeIDs := []string{lastReplierID}
	// Also get the last agent from state (which is tracked by LastAgentID after RegisterReply)
	// The NextReplyAgent will exclude these
	...
	if !found {
		// No eligible agent found among thread participants; try any agent not in the thread yet
		for i := range agents {
			...
			if !candidateExcluded && forumState.CanReply(postID, candidateID) {
