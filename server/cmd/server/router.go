...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) chi.Router {
	...
	// GitHub App webhook (no AICortex auth — requests are authenticated via
	...
	r.Route("/api/daemon", func(r chi.Router) {
		...
		r.Post("/tasks/{taskId}/artifacts", h.ReportTaskArtifacts)
	...
	r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/api/recent-activities", h.ListRecentActivities)
			...
			r.Route("/api/issues", func(r chi.Router) {
				...
				r.Get("/blocked-by", h.ListIssuesBlockedBy)
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetIssue)
					r.Put("/", h.UpdateIssue)
