...
// the aicortex CLI version reported by the daemon at registration time. The
...
// `aicortex issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("aicortex CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("aicortex CLI version is below required minimum")
)
