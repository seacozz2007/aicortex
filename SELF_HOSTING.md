...
Deploy AICortex on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`aicortex` CLI** and runs the **agent daemon** on their own machine.
...
This installs the `aicortex` CLI, checks out the latest self-host assets, pulls the official AICortex images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `aicortex-backend:dev` / `aicortex-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `AICORTEX_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `AICORTEX_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
```bash
curl -fsSL https://raw.githubusercontent.com/aicortex/aicortex/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to AICortex Cloud

If you've been self-hosting and want to switch your CLI to [AICortex Cloud](https://aicortex.ai):

```bash
aicortex setup
```

This reconfigures the CLI for aicortex.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `AICORTEX_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `aicortex setup`:
