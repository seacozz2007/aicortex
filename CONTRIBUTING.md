...
This guide documents the local development workflow for contributors working on the AICortex codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=aicortex`
...
```bash
POSTGRES_DB=aicortex_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://aicortex:aicortex@localhost:5432/aicortex_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../aicortex-feature -b feat/my-change main
cd ../aicortex-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `aicortex_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`aicortex login`).
...
whatever server is configured in `~/.aicortex/config.json`. That's fine for
...
Example: worktree at `../aicortex-feat-auth` produces profile
`dev-aicortex_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `AICORTEX_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.aicortex/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `aicortex` commands automatically use the same
...
rm -rf "$HOME/.aicortex/profiles/$PROFILE"
```
...
1. Compiles the `aicortex` CLI from `server/cmd/aicortex` into
   `apps/desktop/resources/bin/aicortex`
...
backend logs. If you set `AICORTEX_DEV_VERIFICATION_CODE=888888` before starting
...
Nothing in this flow touches the system-installed `aicortex` or the default
`~/.aicortex/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.aicortex/config.json` | `~/.aicortex/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.aicortex/daemon.pid` | `~/.aicortex/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/aicortex_workspaces/` | `~/aicortex_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `aicortex_<slug>_<hash>` |
| Desktop profile | `desktop-api.aicortex.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U aicortex -d postgres -At -c "select datname from pg_database order by datname;"
```
